...
gtStripContig(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
	...
	if (img->row_offset > (uint32)TIFF_SSIZE_T_MAX || h > (uint32)TIFF_SSIZE_T_MAX)
		return (0);
	...
	for (row = 0; row < h; row += nrow)
	{
		...
		if (row > (uint32)TIFF_SSIZE_T_MAX - img->row_offset) {
			ret = 0;
			break;
		}
		actual_row = row + img->row_offset;
		actual_row %= rowsperstrip;
		if (nrowsub > (uint32)TIFF_SSIZE_T_MAX || actual_row > (uint32)TIFF_SSIZE_T_MAX - nrowsub) {
			ret = 0;
			break;
		}
		actual_row += nrowsub;
