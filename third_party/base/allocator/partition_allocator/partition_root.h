...
#ifndef THIRD_PARTY_BASE_ALLOCATOR_PARTITION_ALLOCATOR_PARTITION_ROOT_H_
#define THIRD_PARTY_BASE_ALLOCATOR_PARTITION_ALLOCATOR_PARTITION_ROOT_H_
...
#include "third_party/base/allocator/partition_allocator/address_pool_manager_types.h"
#include "third_party/base/allocator/partition_allocator/allocation_guard.h"
...
#include "third_party/base/allocator/partition_allocator/partition_alloc-inl.h"
...
#include "third_party/base/allocator/partition_allocator/partition_alloc_base/time/time.h"
...
#include "third_party/base/allocator/partition_allocator/partition_alloc_forward.h"
#include "third_party/base/allocator/partition_allocator/partition_alloc_hooks.h"
#include "third_party/base/allocator/partition_allocator/partition_alloc_notreached.h"
#include "third_party/base/allocator/partition_allocator/partition_bucket_lookup.h"
...
#include "third_party/base/allocator/partition_allocator/partition_tag.h"
#include "third_party/base/allocator/partition_allocator/reservation_offset_table.h"
#include "third_party/base/allocator/partition_allocator/starscan/pcscan.h"
#include "third_party/base/allocator/partition_allocator/starscan/state_bitmap.h"
#include "third_party/base/allocator/partition_allocator/tagging.h"
#include "third_party/base/allocator/partition_allocator/thread_cache.h"

// We use this to make MEMORY_TOOL_REPLACES_ALLOCATOR behave the same for max
// size as other alloc code.
#define CHECK_MAX_SIZE_OR_RETURN_NULLPTR(size, flags)     \
  if (size > pdfium::base::internal::MaxDirectMapped()) { \
...
struct PA_ALIGNAS(64) PA_COMPONENT_EXPORT(PARTITION_ALLOC) PartitionRoot {
  ...
  pdfium::base::PartitionTag current_partition_tag = 0;
  ...
  void EnableLargeEmptySlotSpanRing() {
    ::pdfium::base::internal::ScopedGuard locker{lock_};
  ...
  GetDirectMapSlotSize(size_t raw_size) {
    ...
    return pdfium::base::internal::base::bits::AlignUp(
...
#if defined(PA_USE_MTE_CHECKED_PTR_WITH_64_BITS_POINTERS)
  PA_ALWAYS_INLINE pdfium::base::PartitionTag GetNewPartitionTag() {
...
PartitionAllocGetSlotStartInBRPPool(uintptr_t address) {
  address = ::pdfium::base::internal::UnmaskPtr(address);
  ...
  PA_DCHECK(slot_span_start ==
            ::pdfium::base::internal::UnmaskPtr(slot_span_start));
  size_t offset_in_slot_span = address - slot_span_start;

  auto* bucket = slot_span->bucket;
  return ::pdfium::base::internal::RemaskPtr(
      slot_span_start +
...
PartitionRoot<thread_safe>::AllocFromBucket(Bucket* bucket,
  ...
  PA_DCHECK(
      ...
      pdfium::base::internal::base::bits::IsPowerOfTwo(slot_span_alignment));
...
PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::FreeNoHooks(void* object) {
  ...
  if (PA_LIKELY(slot_size <= internal::kMaxMemoryTaggingSize)) {
    ...
    slot_start = ::pdfium::base::internal::TagMemoryRangeIncrement(slot_start,
                                                                   slot_size);
    // Incrementing the MTE-tag in the memory range invalidates the |object|'s
    // tag, so it must be retagged.
    object = ::pdfium::base::internal::RemaskPtr(object);
  }
#else
  ...
  if (!root->IsDirectMappedBucket(slot_span->bucket)) {
    ...
    pdfium::base::internal::PartitionTagIncrementValue(object,
                                                       slot_size_less_extras);
  }
#endif  // defined(PA_USE_MTE_CHECKED_PTR_WITH_64_BITS_POINTERS)
  ...
  if (PA_UNLIKELY(IsQuarantineEnabled())) {
    if (PA_LIKELY(internal::IsManagedByNormalBuckets(slot_start))) {
      uintptr_t unmasked_slot_start =
          ::pdfium::base::internal::UnmaskPtr(slot_start);
...
PA_ALWAYS_INLINE bool PartitionRoot<thread_safe>::IsValidSlotSpan(
    SlotSpan* slot_span) {
  slot_span = ::pdfium::base::internal::UnmaskPtr(slot_span);
...
#if BUILDFLAG(RECORD_ALLOC_INFO)
  pdfium::base::internal::RecordAllocOrFree(addr | 0x01, raw_size);
#endif  // BUILDFLAG(RECORD_ALLOC_INFO)
}
...
PartitionRoot<thread_safe>::DecreaseTotalSizeOfAllocatedBytes(uintptr_t addr,
  ...
  total_size_of_allocated_bytes -= len;
#if BUILDFLAG(RECORD_ALLOC_INFO)
  pdfium::base::internal::RecordAllocOrFree(addr | 0x00, len);
#endif  // BUILDFLAG(RECORD_ALLOC_INFO)
}
...
PA_ALWAYS_INLINE void* PartitionRoot<thread_safe>::AlignedAllocWithFlags(
  ...
  PA_CHECK(pdfium::base::internal::base::bits::IsPowerOfTwo(alignment));
  ...
  if (alignment <= internal::PartitionPageSize()) {
    ...
    if (PA_UNLIKELY(raw_size < alignment)) {
      ...
      raw_size = static_cast<size_t>(1)
                 << (int{sizeof(size_t) * 8} -
                     pdfium::base::internal::base::bits::CountLeadingZeroBits(
                         raw_size - 1));
    }
    PA_DCHECK(pdfium::base::internal::base::bits::IsPowerOfTwo(raw_size));
...
using ::pdfium::base::internal::PartitionAllocGetSlotStartInBRPPool;
#endif  // BUILDFLAG(USE_BACKUP_REF_PTR)

}  // namespace pdfium::base

#endif  // THIRD_PARTY_BASE_ALLOCATOR_PARTITION_ALLOCATOR_PARTITION_ROOT_H_
