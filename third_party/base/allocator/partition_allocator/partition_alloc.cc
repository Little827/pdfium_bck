...
subtle::SpinLock xlock;
subtle::SpinLock& GetSpinLock() {
  return xlock;
}

bool g_initialized = false;

void PartitionAllocBaseInit(internal::PartitionRootBase* root) {
  DCHECK(!root->initialized);
  {
    subtle::SpinLock::Guard guard(GetSpinLock());
    if (!g_initialized) {
      g_initialized = true;
...
void PartitionRootGeneric::Init() {
  ...
  DCHECK_EQ(current_size, 1 << kGenericMaxBucketedOrder);
  DCHECK_EQ(bucket, &this->buckets[0] + kGenericNumBuckets);
  ...
  DCHECK_EQ(bucket, &this->buckets[0] + kGenericNumBuckets);
  DCHECK_EQ(bucket_ptr, &this->bucket_lookups[0] +
...
bool PartitionReallocDirectMappedInPlace(PartitionRootGeneric* root,
  ...
  DCHECK_EQ(page->get_raw_size(), raw_size);
  ...
  if (truncated_slots) {
    ...
    if (unprovisioned_bytes && discard) {
      ...
      *entry_ptr = nullptr;
      ...
      DCHECK_EQ(num_new_entries, num_slots - page->num_allocated_slots);
      // Discard the memory.
