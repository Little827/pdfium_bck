...
#include "third_party/base/allocator/partition_allocator/partition_alloc_base/check.h"
...
#define PA_PCHECK(condition)                              \
  if (!(condition)) {                                     \
    int error = errno;                                    \
    ::pdfium::base::internal::base::debug::Alias(&error); \
    PA_IMMEDIATE_CRASH();                                 \
  }
...
#define PA_DEBUG_DATA_ON_STACK(name, value)                            \
  ::pdfium::base::internal::DebugKv PA_DEBUG_UNIQUE_NAME{name, value}; \
  ::pdfium::base::internal::base::debug::Alias(&PA_DEBUG_UNIQUE_NAME);
