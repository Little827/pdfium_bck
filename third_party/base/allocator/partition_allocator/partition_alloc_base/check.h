...
#ifndef THIRD_PARTY_BASE_ALLOCATOR_PARTITION_ALLOCATOR_PARTITION_ALLOC_BASE_CHECK_H_
#define THIRD_PARTY_BASE_ALLOCATOR_PARTITION_ALLOCATOR_PARTITION_ALLOC_BASE_CHECK_H_
...
#define PA_BASE_PCHECK(condition)                                             \
  PA_LAZY_CHECK_STREAM(::pdfium::base::internal::logging::CheckError::PCheck( \
                           __FILE__, __LINE__)                                \
                           .stream(),                                         \
...
#define PA_BASE_CHECK(condition)                                             \
  PA_LAZY_CHECK_STREAM(::pdfium::base::internal::logging::CheckError::Check( \
...
#define PA_BASE_DCHECK(condition)                                             \
  PA_LAZY_CHECK_STREAM(::pdfium::base::internal::logging::CheckError::DCheck( \
                           __FILE__, __LINE__, #condition)                    \
                           .stream(),                                         \
                       !PA_ANALYZER_ASSUME_TRUE(condition))

#define PA_BASE_DPCHECK(condition)                                             \
  PA_LAZY_CHECK_STREAM(::pdfium::base::internal::logging::CheckError::DPCheck( \
                           __FILE__, __LINE__, #condition)                     \
                           .stream(),                                          \
                       !PA_ANALYZER_ASSUME_TRUE(condition))
...
#define PA_RAW_CHECK(condition)                                               \
  do {                                                                        \
    if (!(condition))                                                         \
      ::pdfium::base::internal::logging::RawCheck("Check failed: " #condition \
                                                  "\n");                      \
  } while (0)

}  // namespace pdfium::base::internal::logging

#endif  // THIRD_PARTY_BASE_ALLOCATOR_PARTITION_ALLOCATOR_PARTITION_ALLOC_BASE_CHECK_H_
