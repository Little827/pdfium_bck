...
#include "third_party/base/allocator/partition_allocator/starscan/pcscan_internal.h"
...
#include "third_party/base/allocator/partition_allocator/address_pool_manager.h"
#include "third_party/base/allocator/partition_allocator/address_pool_manager_bitmap.h"
...
#include "third_party/base/allocator/partition_allocator/partition_alloc_base/immediate_crash.h"
#include "third_party/base/allocator/partition_allocator/partition_alloc_base/memory/ref_counted.h"
#include "third_party/base/allocator/partition_allocator/partition_alloc_base/memory/scoped_refptr.h"
#include "third_party/base/allocator/partition_allocator/partition_alloc_base/no_destructor.h"
#include "third_party/base/allocator/partition_allocator/partition_alloc_base/threading/platform_thread.h"
...
#include "third_party/base/allocator/partition_allocator/starscan/metadata_allocator.h"
#include "third_party/base/allocator/partition_allocator/starscan/pcscan_scheduling.h"
#include "third_party/base/allocator/partition_allocator/starscan/raceful_worklist.h"
#include "third_party/base/allocator/partition_allocator/starscan/scan_loop.h"
#include "third_party/base/allocator/partition_allocator/starscan/snapshot.h"
#include "third_party/base/allocator/partition_allocator/starscan/stack/stack.h"
#include "third_party/base/allocator/partition_allocator/starscan/stats_collector.h"
#include "third_party/base/allocator/partition_allocator/starscan/stats_reporter.h"
...
struct DisableMTEScope final {
  DisableMTEScope() {
    ::pdfium::base::ChangeMemoryTaggingModeForCurrentThread(
        ::pdfium::base::TagViolationReportingMode::kDisabled);
  }
  ~DisableMTEScope() {
    ::pdfium::base::ChangeMemoryTaggingModeForCurrentThread(
        parent_tagging_mode);
 ...
 private:
  ::pdfium::base::TagViolationReportingMode parent_tagging_mode =
      ::pdfium::base::internal::GetMemoryTaggingModeForCurrentThread();
};
...
SuperPageSnapshot::SuperPageSnapshot(uintptr_t super_page) {
  ...
  ::pdfium::base::internal::ScopedGuard lock(extent_entry->root->lock_);
  ...
  view.VisitConcurrently([clear_type](uintptr_t super_page) {
    ...
    bitmap->IterateQuarantined([root, clear_type](uintptr_t slot_start) {
      ...
      if (clear_type == PCScan::ClearType::kLazy) {
        void* object = ::pdfium::base::internal::RemaskPtr(
 ...
 private:
  ...
  PA_SCAN_INLINE void CheckPointer(uintptr_t maybe_ptr) {
    quarantine_size_ += task_.TryMarkSlotInNormalBuckets(
        ::pdfium::base::internal::UnmaskPtr(maybe_ptr));
  }
...
[[maybe_unused]] void SweepSuperPageWithBatchedFree(
  ...
  const auto bitmap_iterator = [&](uintptr_t slot_start) {
    ...
    auto* entry = PartitionFreelistEntry::EmplaceAndInitNull(
        ::pdfium::base::internal::RemaskPtr(slot_start));
...
void PCScanInternal::Initialize(PCScan::InitConfig config) {
  ...
  static pdfium::base::StatsReporter s_no_op_reporter;
...
void PCScanInternal::RegisterScannableRoot(Root* root) {
  ...
  {
    ::pdfium::base::internal::ScopedGuard guard(root->lock_);
...
void PCScanInternal::ProtectPages(uintptr_t begin, size_t size) {
  ...
  write_protector_->ProtectPages(
      begin,
      pdfium::base::internal::base::bits::AlignUp(size, SystemPageSize()));
}
...
void PCScanInternal::RegisterStatsReporter(
    pdfium::base::StatsReporter* reporter) {
  PA_DCHECK(reporter);
  stats_reporter_ = reporter;
}

pdfium::base::StatsReporter& PCScanInternal::GetReporter() {
