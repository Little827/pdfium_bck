...
class PartitionAllocPCScanTestBase : public testing::Test {
 ...
 private:
  ...
  pdfium::base::PartitionAllocator allocator_;
};
...
bool IsInFreeList(uintptr_t slot_start) {
  ...
  uintptr_t slot_start_tagged = ::pdfium::base::internal::RemaskPtr(slot_start);
...
TEST_F(PartitionAllocPCScanTest, DontScanUnusedRawSize) {
  ...
  uintptr_t source_end =
      ::pdfium::base::internal::RemaskPtr(slot_start + slot_span->GetRawSize());
  // Write the pointer.
...
TEST_F(PartitionAllocPCScanWithMTETest, QuarantineOnlyOnTagOverflow) {
...

  {
    ...
    if (!HasOverflowTag(::pdfium::base::internal::RemaskPtr(obj1))) {
      // Assert that the pointer is the same.
      ASSERT_EQ(::pdfium::base::internal::UnmaskPtr(obj1),
                ::pdfium::base::internal::UnmaskPtr(obj2));
  ...
  for (size_t i = 0; i < 16; ++i) {
    ...
    obj = ::pdfium::base::internal::RemaskPtr(obj);
