...
ALWAYS_INLINE void PartitionRegisterEmptyPage(PartitionPage* page) {
  ...
  if (page->empty_cache_index != -1) {
    ...
    DCHECK_EQ(root->global_empty_page_ring[page->empty_cache_index], page);
...
void PartitionPage::FreeSlowPath() {
  ...
  if (LIKELY(this->num_allocated_slots == 0)) {
    ...
    DCHECK_EQ(this->num_allocated_slots, bucket->get_slots_per_span() - 1);
...
void PartitionPage::DecommitIfPossible(PartitionRootBase* root) {
  ...
  DCHECK_EQ(this, root->global_empty_page_ring[empty_cache_index]);
  empty_cache_index = -1;
  if (is_empty())
    Decommit(root);
}
