...
#include "third_party/base/allocator/partition_allocator/chromecast_buildflags.h"
#include "third_party/base/allocator/partition_allocator/dangling_raw_ptr_checks.h"
#include "third_party/base/allocator/partition_allocator/page_allocator_constants.h"
#include "third_party/base/allocator/partition_allocator/partition_address_space.h"
#include "third_party/base/allocator/partition_allocator/partition_alloc_base/bits.h"
...
#include "third_party/base/allocator/partition_allocator/partition_alloc_base/debug/debugging_buildflags.h"
#include "third_party/base/allocator/partition_allocator/partition_alloc_base/logging.h"
#include "third_party/base/allocator/partition_allocator/partition_alloc_base/numerics/checked_math.h"
#include "third_party/base/allocator/partition_allocator/partition_alloc_base/rand_util.h"
#include "third_party/base/allocator/partition_allocator/partition_alloc_base/strings/stringprintf.h"
#include "third_party/base/allocator/partition_allocator/partition_alloc_base/thread_annotations.h"
#include "third_party/base/allocator/partition_allocator/partition_alloc_base/threading/platform_thread_for_testing.h"
#include "third_party/base/allocator/partition_allocator/partition_alloc_buildflags.h"
...
#include "third_party/base/allocator/partition_allocator/partition_ref_count.h"
#include "third_party/base/allocator/partition_allocator/partition_root.h"
#include "third_party/base/allocator/partition_allocator/partition_tag_bitmap.h"
#include "third_party/base/allocator/partition_allocator/reservation_offset_table.h"
...
#define PA_EXPECT_PTR_EQ(ptr1, ptr2)                      \
  {                                                       \
    EXPECT_EQ(::pdfium::base::internal::UnmaskPtr(ptr1),  \
              ::pdfium::base::internal::UnmaskPtr(ptr2)); \
  }
#define PA_EXPECT_PTR_NE(ptr1, ptr2)                      \
  {                                                       \
    EXPECT_NE(::pdfium::base::internal::UnmaskPtr(ptr1),  \
              ::pdfium::base::internal::UnmaskPtr(ptr2)); \
  }
#define PA_EXPECT_ADDR_EQ(addr1, addr2)                    \
  {                                                        \
    EXPECT_EQ(::pdfium::base::internal::UnmaskPtr(addr1),  \
              ::pdfium::base::internal::UnmaskPtr(addr2)); \
  }
#define PA_EXPECT_ADDR_NE(addr1, addr2)                    \
  {                                                        \
    EXPECT_NE(::pdfium::base::internal::UnmaskPtr(addr1),  \
              ::pdfium::base::internal::UnmaskPtr(addr2)); \
  }
...
const size_t kTestSizes[] = {
    ...
    17,
    100,
    pdfium::base::internal::SystemPageSize(),
    pdfium::base::internal::SystemPageSize() + 1,
    pdfium::base::PartitionRoot<
        pdfium::base::internal::ThreadSafe>::GetDirectMapSlotSize(100),
    1 << 20,
    1 << 21,
};
constexpr size_t kTestSizesCount = std::size(kTestSizes);

void AllocateRandomly(pdfium::base::PartitionRoot
                              << pdfium::base::internal::ThreadSafe >
                          *root,
                      size_t count,
                      unsigned int flags) {
  std::vector<void*> allocations(count, nullptr);
  for (size_t i = 0; i < count; ++i) {
    const size_t size = kTestSizes[pdfium::base::internal::base::RandGenerator(
        kTestSizesCount)];
...
class CountDanglingRawPtr {
 public:
  CountDanglingRawPtr() {
    ...
    old_detected_fn_ = pdfium::base::GetDanglingRawPtrDetectedFn();
    old_released_fn_ = pdfium::base::GetDanglingRawPtrReleasedFn();

    pdfium::base::SetDanglingRawPtrDetectedFn(
        CountDanglingRawPtr::DanglingRawPtrDetected);
    pdfium::base::SetDanglingRawPtrReleasedFn(
        CountDanglingRawPtr::DanglingRawPtrReleased);
  }
  ~CountDanglingRawPtr() {
    pdfium::base::SetDanglingRawPtrDetectedFn(old_detected_fn_);
    pdfium::base::SetDanglingRawPtrReleasedFn(old_released_fn_);
  }
 ...
 private:
  ...
  pdfium::base::DanglingRawPtrDetectedFn* old_detected_fn_;
  pdfium::base::DanglingRawPtrReleasedFn* old_released_fn_;
};

}  // namespace

// Note: This test exercises interfaces inside the `partition_alloc`
// namespace, but inspects objects inside `<pdfium::base::internal`.
...
#endif
const size_t kRealAllocSize = pdfium::base::internal::base::bits::AlignUp(
...
class PartitionAllocTest : public testing::TestWithParam<bool> {
 protected:
  ...
  SlotSpan* GetFullSlotSpan(size_t size) {
    ...
    EXPECT_EQ(
        SlotSpan::FromSlotStart(::pdfium::base::internal::UnmaskPtr(first)),
        SlotSpan::FromSlotStart(::pdfium::base::internal::UnmaskPtr(last)));
    if (bucket->num_system_pages_per_slot_span ==
        NumSystemPagesPerPartitionPage())
      EXPECT_EQ(
          ::pdfium::base::internal::UnmaskPtr(first) & PartitionPageBaseMask(),
          ::pdfium::base::internal::UnmaskPtr(last) & PartitionPageBaseMask());
...
void FreeFullSlotSpan(PartitionRoot<internal::ThreadSafe>* root,
  ...
  for (i = 0; i < num_slots; ++i) {
    ...
    root->Free(reinterpret_cast<void*>(
        ::pdfium::base::internal::RemaskPtr(address + kPointerOffset)));
    address += size;
  }
...
TEST_P(PartitionAllocTest, Basic) {
  ...
  EXPECT_EQ(PartitionPageSize() +
                pdfium::base::internal::ReservedTagBitmapSize() +
                kPointerOffset,
...
TEST_P(PartitionAllocTest, MultiAlloc) {
  ...
  EXPECT_TRUE(ptr2);
  ptrdiff_t diff =
      reinterpret_cast<char*>(::pdfium::base::internal::UnmaskPtr(ptr2)) -
      reinterpret_cast<char*>(::pdfium::base::internal::UnmaskPtr(ptr1));
  ...
  ptr2 = reinterpret_cast<char*>(
      allocator.root()->Alloc(kTestAllocSize, type_name));
  EXPECT_TRUE(ptr2);
  diff = ::pdfium::base::internal::UnmaskPtr(ptr2) -
         ::pdfium::base::internal::UnmaskPtr(ptr1);
  EXPECT_EQ(static_cast<ptrdiff_t>(kRealAllocSize), diff);
  ...
  char* ptr3 = reinterpret_cast<char*>(
      allocator.root()->Alloc(kTestAllocSize, type_name));
  EXPECT_TRUE(ptr3);
  diff = ::pdfium::base::internal::UnmaskPtr(ptr3) -
         ::pdfium::base::internal::UnmaskPtr(ptr1);
...
TEST_P(PartitionAllocTest, SlotSpanTransitions) {
  ...
  uintptr_t address = ::pdfium::base::internal::RemaskPtr(
  ...
  address = ::pdfium::base::internal::RemaskPtr(
...
TEST_P(PartitionAllocTest, MultiPageAllocs) {
  ...
  size_t num_slot_spans_needed =
      (NumPartitionPagesPerSuperPage() - 2 -
       pdfium::base::internal::NumPartitionPagesPerTagBitmap()) /
  ...
  for (i = 0; i < num_slot_spans_needed; ++i) {
    ...
    if (i == num_slot_spans_needed - 1) {
      ...
      // the second page.
      EXPECT_EQ(
          PartitionPageSize() + pdfium::base::internal::ReservedTagBitmapSize(),
...
TEST_P(PartitionAllocTest, Alloc) {
  ...
  // the same bucket, pdfium::base::internal::base::bits::AlignUp(1 + x +
  // kExtraAllocSize, kAlignment)
  // == pdfium::base::internal::base::bits::AlignUp(kSmallestBucket + x +
  // kExtraAllocSize, kAlignment), because slot_size is multiples of
  // kAlignment. So (x + kExtraAllocSize) must be multiples of
  // kAlignment. x =
  // pdfium::base::internal::base::bits::AlignUp(kExtraAllocSize,
  // kAlignment) - kExtraAllocSize;
  size_t base_size =
      pdfium::base::internal::base::bits::AlignUp(kExtraAllocSize, kAlignment) -
      kExtraAllocSize;
...
TEST_P(PartitionAllocTest, IsValidPtrDelta) {
  const size_t kMinReasonableTestSize =
      pdfium::base::internal::base::bits::AlignUp(kExtraAllocSize + 1,
                                                  kAlignment);
...
TEST_P(PartitionAllocDeathTest, DirectMapGuardPages) {
  const size_t kSizes[] = {
      ...
      kMaxBucketed + PartitionPageSize(),
      pdfium::base::internal::base::bits::AlignUp(kMaxBucketed + kSuperPageSize,
                                                  kSuperPageSize) -
  ...
  for (size_t size : kSizes) {
    ...
    EXPECT_DEATH(*(char_ptr + pdfium::base::internal::base::bits::AlignUp(
...
TEST_P(PartitionAllocTest, Bookkeeping) {
  ...
  for (size_t huge_size : huge_sizes) {
    ...
    for (size_t alignment : alignments) {
      ...
      size_t aligned_size = pdfium::base::internal::base::bits::AlignUp(
      ...
      size_t expected_direct_map_size =
          pdfium::base::internal::base::bits::AlignUp(
...
TEST_P(PartitionAllocTest, RefCountBasic) {
  ...
  *ptr1 = kCookie;
  ...
  ptr1 = ::pdfium::base::internal::RemaskPtr(ptr1);
  ...
  ref_count = ::pdfium::base::internal::RemaskPtr(ref_count);
...
void PartitionAllocTest::RunRefCountReallocSubtest(size_t orig_size,
  ...
  ref_count1 = ::pdfium::base::internal::RemaskPtr(ref_count1);
  ...
  if (::pdfium::base::internal::UnmaskPtr(ptr1) ==
      ::pdfium::base::internal::UnmaskPtr(ptr2)) {
...
TEST_P(PartitionAllocTest, CheckReservationType) {
  ...
  EXPECT_TRUE(IsManagedByNormalBucketsOrDirectMap(address_to_check));
  address_to_check =
      pdfium::base::internal::base::bits::AlignDown(address, kSuperPageSize);
  ...
  EXPECT_TRUE(IsManagedByNormalBucketsOrDirectMap(address_to_check));
  address_to_check =
      pdfium::base::internal::base::bits::AlignUp(address, kSuperPageSize);
  EXPECT_FALSE(IsReservationStart(address_to_check));
  EXPECT_FALSE(IsManagedByNormalBuckets(address_to_check));
  EXPECT_TRUE(IsManagedByDirectMap(address_to_check));
  EXPECT_TRUE(IsManagedByNormalBucketsOrDirectMap(address_to_check));
...
TEST_P(PartitionAllocTest, ConfigurablePool) {
  ...
  for (size_t pool_size = max_pool_size; pool_size >= min_pool_size;
       pool_size /= 2) {
    PA_DCHECK(pdfium::base::internal::base::bits::IsPowerOfTwo(pool_size));
    EXPECT_FALSE(IsConfigurablePoolAvailable());
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      uintptr_t allocation_base = ::pdfium::base::internal::UnmaskPtr(
...
TEST_P(PartitionAllocTest, PartitionTagBasic) {
  ...
  constexpr pdfium::base::PartitionTag kTag1 =
      static_cast << pdfium::base::PartitionTag > (0xBADA);
  constexpr pdfium::base::PartitionTag kTag2 =
      static_cast << pdfium::base::PartitionTag > (0xDB8A);
  constexpr pdfium::base::PartitionTag kTag3 =
      static_cast << pdfium::base::PartitionTag > (0xA3C4);

  pdfium::base::internal::PartitionTagSetValue(
  ...
  EXPECT_EQ(kTag1, pdfium::base::internal::PartitionTagGetValue(ptr1));
  EXPECT_EQ(kTag2, pdfium::base::internal::PartitionTagGetValue(ptr2));
  EXPECT_EQ(kTag3, pdfium::base::internal::PartitionTagGetValue(ptr3));
...
TEST_P(PartitionAllocTest, SortFreelist) {
  ...
  for (size_t i = 1; i < allocations.size(); i++) {
    EXPECT_LT(reinterpret_cast<uintptr_t>(
                  ::pdfium::base::internal::UnmaskPtr(allocations[i - 1])),
              reinterpret_cast<uintptr_t>(
                  ::pdfium::base::internal::UnmaskPtr(allocations[i])));
  }
...
TEST_P(PartitionAllocTest, SmallSlotSpanWaste) {
  for (PartitionRoot<ThreadSafe>::Bucket& bucket : allocator.root()->buckets) {
    ...
    size_t small_system_page_count =
        pdfium::base::internal::ComputeSystemPagesPerSlotSpan(bucket.slot_size,
                                                              true);
    size_t small_waste =
...
TEST_P(PartitionAllocTest, SortActiveSlotSpans) {
  auto run_test = [](size_t count) {
    ...
    for (size_t i = 0; i < count; i++) {
      ...
      slot_span.num_unprovisioned_slots =
          pdfium::base::internal::base::RandGenerator(
    ...
    EXPECT_TRUE(std::is_sorted(sorted_slot_spans.begin(),
                               ...
                               pdfium::base::internal::CompareSlotSpans));
