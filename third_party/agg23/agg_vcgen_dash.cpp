...
void vcgen_dash::calc_dash_start(float ds)
{
  ...
  // loop to stuck. Scale the float variables by 1000 and round them to
  // integers so that we can keep 3 decimal digits of precision.
  long long scaled_ds = std::llround(ds * 1000.0);
  long long scaled_dashes[max_dashes];
  for (int i = 0; i < m_num_dashes; i++) {
    scaled_dashes[i] = std::llround(m_dashes[i] * 1000.0);
  }
  ...
  m_curr_dash_start = static_cast<float>(scaled_curr_dash_start) / 1000.0;
}
