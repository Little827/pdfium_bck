    ...
    while (p < len) {
      ...
      if (n >= 13) {
        ...
        if (t >= 5 || n == len) {
          ...
          Optional<size_t> b =
              determineConsecutiveBinaryCount(msg, &byteArr, p, e);
          if (!b.has_value() || e != BCExceptionNO)
            return L" ";
          size_t unwrapped = b.value();
          if (unwrapped == 0) {
            unwrapped = 1;
          }
          if (unwrapped == 1 && encodingMode == TEXT_COMPACTION) {
            encodeBinary(&byteArr, p, 1, TEXT_COMPACTION, sb);
          } else {
            encodeBinary(&byteArr, p, unwrapped, encodingMode, sb);
...
void CBC_PDF417HighLevelEncoder::encodeBinary(std::vector<uint8_t>* bytes,
  ...
  if (count >= 6) {
    ...
    while ((startpos + count - idx) >= 6) {
      ...
      for (i = 0; i < 6; i++) {
        t <<= 8;
        t += (*bytes)[idx + static_cast<size_t>(i)] & 0xff;
      }
      for (i = 0; i < 5; i++) {
        chars[i] = (t % 900);
        t /= 900;
  ...
  for (i = static_cast<int64_t>(idx);
       i < static_cast<int64_t>(startpos + count); i++) {
    int32_t ch = (*bytes)[static_cast<size_t>(i)] & 0xff;
    sb += ch;
  }
}
...
void CBC_PDF417HighLevelEncoder::encodeNumeric(WideString msg,
  ...
  while (idx < count) {
    ...
    for (int64_t i = static_cast<int64_t>(tmp.GetLength()) - 1; i >= 0; i--) {
      sb += tmp[static_cast<size_t>(i)];
    }
    idx += len;
  }
}
