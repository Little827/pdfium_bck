...
CBC_HighLevelEncoder::Encoding CBC_HighLevelEncoder::LookAheadTest(
  ...
  while (true) {
    ...
    if (ascii_count < base256_count && ascii_count < c40_count &&
        ascii_count < text_count && ascii_count < x12_count &&
        ascii_count < editfact_count) {
      return Encoding::ASCII;
    }
    ...
    if (minCount == 1 && mins[EncoderIndex(Encoding::EDIFACT)] > 0)
      return Encoding::EDIFACT;
    if (minCount == 1 && mins[EncoderIndex(Encoding::TEXT)] > 0)
      return Encoding::TEXT;
    if (minCount == 1 && mins[EncoderIndex(Encoding::X12)] > 0)
      return Encoding::X12;
    if (c40_count + 1 < ascii_count && c40_count + 1 < base256_count &&
        c40_count + 1 < editfact_count && c40_count + 1 < text_count) {
