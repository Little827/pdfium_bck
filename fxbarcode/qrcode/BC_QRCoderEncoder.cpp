...
CBC_ReedSolomonGF256* s_QRCodeField = nullptr;
...
std::vector<uint8_t> GenerateECBytes(const std::vector<uint8_t>& dataBytes,
  ...
  CBC_ReedSolomonEncoder encoder(s_QRCodeField);
...
void CBC_QRCoderEncoder::Initialize() {
  s_QRCodeField = new CBC_ReedSolomonGF256(0x011D);
  s_QRCodeField->Init();
}

// static
void CBC_QRCoderEncoder::Finalize() {
  delete s_QRCodeField;
  s_QRCodeField = nullptr;
}
