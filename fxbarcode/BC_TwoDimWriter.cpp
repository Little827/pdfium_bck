...
void CBC_TwoDimWriter::RenderDeviceResult(CFX_RenderDevice* device,
  ...
  for (int32_t x = 0; x < m_inputWidth; x++) {
    for (int32_t y = 0; y < m_inputHeight; y++) {
      ...
      if (m_output->Get(x, y)) {
        // std::cerr << "(" << (leftPos + x * m_multiX) << ", " << (topPos + y *
        // m_multiY) << ", " << (leftPos + (x + 1) * m_multiX) << ", " <<
        // (topPos + (y + 1) * m_multiY) << ")," << std::endl;
...
bool CBC_TwoDimWriter::RenderResult(uint8_t* code,
  ...
  std::cerr << "2 m_outputWidth " << m_outputWidth << ", m_Width " << m_Width
            << std::endl;
  std::cerr << "2 m_outputHeight " << m_outputHeight << ", m_Height "
            << m_Height << std::endl;
  if (m_bFixedSize) {
    if (m_Width < m_outputWidth || m_Height < m_outputHeight) {
      return false;
    ...
    if (m_Width > m_outputWidth || m_Height > m_outputHeight) {
      int width_factor = floor(static_cast<float>(m_Width) / m_outputWidth);
      int height_factor = floor(static_cast<float>(m_Height) / m_outputHeight);
      width_factor = std::max(width_factor, 1);
      height_factor = std::max(height_factor, 1);

      m_outputWidth = static_cast<int32_t>(m_outputWidth * width_factor);
      m_outputHeight = static_cast<int32_t>(m_outputHeight * height_factor);
    }
  }
  std::cerr << "3 m_outputWidth " << m_outputWidth << ", tempWidth "
            << tempWidth << std::endl;
  std::cerr << "3 m_outputHeight " << m_outputHeight << ", tempHeight "
            << tempHeight << std::endl;
  ...
  std::cerr << "4 m_multiX " << m_multiX << std::endl;
  std::cerr << "4 m_multiY " << m_multiY << std::endl;

  m_leftPadding = std::max((m_Width - m_outputWidth) / 2, 0);
  m_topPadding = std::max((m_Height - m_outputHeight) / 2, 0);
  std::cerr << "5 m_Height " << m_Height << ", m_outputHeight "
            << m_outputHeight << std::endl;
  std::cerr << "5 m_topPadding " << m_topPadding << std::endl;
