...
#include "core/fxcrt/fixed_data_vector.h"
...
TEST(FixedDataVector, NoData) {
  FixedDataVector<int> vec;
...
TEST(FixedDataVector, WithData) {
  FixedDataVector<int> vec(4);
...
TEST(FixedDataVector, Move) {
  FixedDataVector<int> vec(4);
  constexpr int kData[] = {1, 2, 3, 4};
  ASSERT_EQ(4u, vec.writable_span().size());
  fxcrt::spancpy(vec.writable_span(), pdfium::make_span(kData));

  FixedDataVector<int> vec2(std::move(vec));
