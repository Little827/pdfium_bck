...
TEST(FixedUniniDataVector, NoData) {
  FixedUniniDataVector<int> vec;
...
TEST(FixedUniniDataVector, WithData) {
  FixedUniniDataVector<int> vec(4);
...
TEST(FixedUniniDataVector, Move) {
  FixedUniniDataVector<int> vec(4);
  constexpr int kData[] = {1, 2, 3, 4};
  ASSERT_EQ(4u, vec.writable_span().size());
  fxcrt::spancpy(vec.writable_span(), pdfium::make_span(kData));

  FixedUniniDataVector<int> vec2(std::move(vec));
