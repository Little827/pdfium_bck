...
std::optional<size_t> ByteString::Find(ByteStringView subStr,
  ...
  std::optional<size_t> result =
      fxcrt::spanpos(m_pData->span().subspan(start), subStr.span());
  if (!result.has_value()) {
    return std::nullopt;
  }
...
size_t ByteString::Replace(ByteStringView pOld, ByteStringView pNew) {
  ...
  size_t nCount = 0;
  {
    ...
    while (true) {
      std::optional<size_t> found = fxcrt::spanpos(search_span, pOld.span());
      if (!found.has_value()) {
        break;
      }
      nCount++;
  ...
  RetainPtr<StringData> pNewData = StringData::Create(nNewLength);
  {
    ...
    for (size_t i = 0; i < nCount; i++) {
      size_t found = fxcrt::spanpos(search_span, pOld.span()).value();
