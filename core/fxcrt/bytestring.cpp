...
absl::optional<size_t> ByteString::Find(ByteStringView subStr,
  ...
  auto result = FX_strpos(m_pData->span().subspan(start), subStr.span());
  if (!result.has_value()) {
    return absl::nullopt;
  }
...
size_t ByteString::Replace(ByteStringView pOld, ByteStringView pNew) {
  ...
  {
    // Limit span lifetime.
    pdfium::span<char> pStart = m_pData->span();
    while (true) {
      absl::optional<size_t> found = FX_strpos(pStart, pOld.span());
      if (!found.has_value()) {
        break;
      }
      nCount++;
      pStart = pStart.subspan(found.value() + pOld.GetLength());
    }
  }
  if (nCount == 0)
    return 0;
  ...
  RetainPtr<StringData> pNewData(StringData::Create(nNewLength));
  {
    ...
    pdfium::span<const char> pStart = m_pData->span();
    pdfium::span<char> pDest = pNewData->span();
    for (size_t i = 0; i < nCount; i++) {
      size_t found = FX_strpos(pStart, pOld.span()).value();
      pDest = fxcrt::spancpy(pDest, pStart.first(found));
      pDest = fxcrt::spancpy(pDest, pNew.span());
      pStart = pStart.subspan(found + pOld.GetLength());
    }
    pDest = fxcrt::spancpy(pDest, pStart);
    DCHECK(pDest.empty());
  }
