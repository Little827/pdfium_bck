...
template <typename T>
struct IsDataPartitionException : std::false_type {};

// Use with caution. No further checks are made to see if `T` is appropriate
// for the Data Partition (e.g. no pointers, strings, vtables, etc.)
#define FX_DATA_PARTITION_EXCEPTION(T) \
  template <>                          \
  struct IsDataPartitionException<T> : std::true_type {}
...
struct FxPartitionAllocAllocator {
 public:
  ...
  static_assert(std::is_arithmetic<T>::value ||
                    IsDataPartitionException<T>::value,
