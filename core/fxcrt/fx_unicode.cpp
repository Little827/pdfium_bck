...
constexpr uint32_t kMirrorBitPos = 5;
...
constexpr uint32_t kCharTypeBitPos = 6;
...
const uint32_t kExtendedTextLayoutCodeProperties[] = {
#include "core/fxcrt/fx_ucddata.inc"
};
#undef CHARPROP____
...
wchar_t FX_GetMirrorChar(wchar_t wch) {
  uint16_t dwProps = GetUnicodeProperties(wch);
  uint16_t dwTemp = (dwProps & kMirrorBitMask);
