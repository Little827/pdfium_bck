...
#include "core/fxcrt/string_base_template.h"
...
template <typename T>
void StringBaseTemplate<T>::ReallocBeforeWrite(size_t nNewLength) {
...
template <typename T>
void StringBaseTemplate<T>::AllocBeforeWrite(size_t nNewLength) {
  if (m_pData && m_pData->CanOperateInPlace(nNewLength)) {
    return;
  }
  if (nNewLength == 0) {
...
template <typename T>
void StringBaseTemplate<T>::AssignCopy(const T* pSrcData, size_t nSrcLen) {
...
template <typename T>
void StringBaseTemplate<T>::Concat(const T* pSrcData, size_t nSrcLen) {
...
template <typename T>
void StringBaseTemplate<T>::clear() {
...
// Instantiate.
template class StringBaseTemplate<char>;
template class StringBaseTemplate<wchar_t>;
