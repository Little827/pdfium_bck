...
CFX_UnicodeStreamProxy::CFX_UnicodeStreamProxy(
...
CFX_UnicodeStreamProxy::CFX_UnicodeStreamProxy(uint8_t* data, size_t size)
    : CFX_UnicodeStreamProxy(
          pdfium::MakeRetain<CFX_MemoryStream>(data, size, false),
          false) {}

CFX_UnicodeStreamProxy::~CFX_UnicodeStreamProxy() {}

void CFX_UnicodeStreamProxy::Seek(From eSeek, FX_FILESIZE iOffset) {
  switch (eSeek) {
    case From::Begin:
      m_iPosition = iOffset;
      break;
...
void CFX_UnicodeStreamProxy::SetCodePage(uint16_t wCodePage) {
  if (m_wBOMLength > 0)
    return;
  m_wCodePage = wCodePage;
}

size_t CFX_UnicodeStreamProxy::ReadData(uint8_t* pBuffer, size_t iBufferSize) {
...
size_t CFX_UnicodeStreamProxy::ReadString(wchar_t* pStr,
                                          size_t iMaxLength,
                                          bool* bEOS) {
...
void CFX_UnicodeStreamProxy::WriteString(const WideStringView& str) {
