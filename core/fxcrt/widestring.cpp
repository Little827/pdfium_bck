...
std::optional<size_t> GuessSizeForVSWPrintf(WCString pFormat, va_list argList) {
  size_t nMaxLen = 0;
  for (WCString pStr = pFormat; *pStr != 0; ++pStr) {
    if (*pStr != '%') {
...
WideString WideString::FormatV(const wchar_t* format, va_list argList) {
  ...
  auto guess = GuessSizeForVSWPrintf(UNSAFE_BUFFERS(WCString::Create(format)),
                                     argListCopy);
  va_end(argListCopy);
...
WideString::WideString(WCString ptr)
