...
std::optional<size_t> GuessSizeForVSWPrintf(const wchar_t* pFormat,
                                            va_list argList) {
  size_t nMaxLen = 0;
  for (const wchar_t* pStr = pFormat; *pStr != 0;
       pStr = FXSYS_CStringAdvance(pStr)) {
    if (*pStr != '%') {
    ...
    int iWidth = 0;
    for (; *pStr != 0; pStr = FXSYS_CStringAdvance(pStr)) {
      if (*pStr == '#') {
        nMaxLen += 2;
      } else if (*pStr == '*') {
    ...
    int nModifier = 0;
    // SAFETY: expression short-circuits if NUL encountered.
    if UNSAFE_BUFFERS ((*pStr == L'I' && *(pStr + 1) == L'6' &&
                        *(pStr + 2) == L'4')) {
      // SAFETY: pStr[0..3] have been examined above and are non-NUL. Since
      // pStr is a NUL-terminated string, advancing past them will remain
      // inside the string's allocation.
