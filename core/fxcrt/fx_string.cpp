...
void AppendCodePoint(char32_t code_point, ByteString& buffer) {
...
void AppendCodePoint(char32_t code_point, WideString& buffer) {
  if (code_point > pdfium::kMaximumSupplementaryCodePoint) {
    // Invalid code point above U+10FFFF.
    return;
  }
...
ByteString FX_UTF8Encode(WideStringView wsStr) {
  ...
  for (char32_t code_point : pdfium::CodePointView(wsStr)) {
    AppendCodePoint(code_point, buffer);
  }
  return buffer;
}
...
WideString FX_UTF8Decode(ByteStringView bsStr) {
  ...
  for (char byte : bsStr) {
    ...
    if (code_unit < 0x80) {
      remaining = 0;
      AppendCodePoint(code_unit, buffer);
