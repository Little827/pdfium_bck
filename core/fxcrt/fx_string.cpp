...
const float kFractionScalesFloat[] = {
...
T StringTo(ByteStringView strc,
  ...
  if (cc < len && strc[cc] == '.') {
    cc++;
    while (cc < len) {
      value += frac_scale[scale] * FXSYS_DecimalCharToInt(strc.CharAt(cc));
      scale++;
...
float StringToFloat(ByteStringView strc) {
  return StringTo<float>(strc, FX_ArraySize(kFractionScalesFloat),
                         kFractionScalesFloat);
}
...
double StringToDouble(ByteStringView strc) {
  return StringTo<double>(strc, FX_ArraySize(kFractionScalesDouble),
                          kFractionScalesDouble);
}
