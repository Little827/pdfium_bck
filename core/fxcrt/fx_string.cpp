...
const float kFractionScalesFlt[] = {0.1f,          0.01f,         0.001f,
                                    0.0001f,       0.00001f,      0.000001f,
                                    0.0000001f,    0.00000001f,   0.000000001f,
                                    0.0000000001f, 0.00000000001f};

float FractionalScaleFlt(size_t scale_factor, int value) {
  return kFractionScalesFlt[scale_factor] * value;
}

const double kFractionScalesDbl[] = {
    0.1,       0.01,       0.001,       0.0001,       0.00001,      0.000001,
    0.0000001, 0.00000001, 0.000000001, 0.0000000001, 0.00000000001};

double FractionalScaleDbl(size_t scale_factor, int value) {
  return kFractionScalesDbl[scale_factor] * value;
}

template <class T>
T StringTo(const ByteStringView strc,
           const T empty_value,
           const int fractional_scales_size,
           T (*fractional_scale)(size_t scale_factor, int value)) {
  if (strc.IsEmpty())
    return empty_value;
  ...
  if (cc < len && strc[cc] == '.') {
    cc++;
    while (cc < len) {
      value += fractional_scale(scale, FXSYS_DecimalCharToInt(strc.CharAt(cc)));
      scale++;
...
template <class T>
size_t ToString(T value, const T zero_val, int (*rounder)(T), char* buf) {
  buf[0] = '0';
  buf[1] = '\0';
  if (value == zero_val) {
    return 1;
  ...
  int scale = 1;
  int scaled = rounder(value);
  while (scaled < 100000) {
    if (scale == 1000000) {
      break;
    }
    scale *= 10;
    scaled = rounder(value * scale);
  }
  if (scaled == 0) {
    return 1;
  }
...
float StringToFloat(ByteStringView strc) {
  return StringTo<float>(strc, 0.0f, FX_ArraySize(kFractionScalesFlt),
                         FractionalScaleFlt);
}
...
size_t FloatToString(float f, char* buf) {
  return ToString<float>(f, 0.0f, FXSYS_roundf, buf);
}

double StringToDouble(ByteStringView strc) {
  return StringTo<double>(strc, 0.0, FX_ArraySize(kFractionScalesDbl),
                          FractionalScaleDbl);
}
...
size_t DoubleToString(double d, char* buf) {
  return ToString<double>(d, 0.0, FXSYS_round, buf);
}
