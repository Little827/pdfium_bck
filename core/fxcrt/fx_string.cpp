...
template <class T>
size_t ToString(T value, int (*rounder)(T), char* buf) {
  buf[0] = '0';
  buf[1] = '\0';
  if (value == 0) {
    return 1;
...
float StringToFloat(ByteStringView strc) {
  return StringTo<float>(strc, FX_ArraySize(kFractionScalesFlt),
                         FractionalScaleFlt);
}
...
double StringToDouble(ByteStringView strc) {
  return StringTo<double>(strc, FX_ArraySize(kFractionScalesDbl),
                          FractionalScaleDbl);
}
