...
TEST(CFX_CSSValueListParserTest, number_parsing) {
  ...
  EXPECT_EQ(L"1234", maybe_next.value());
  ...
  ASSERT_TRUE(maybe_next.has_value());
  EXPECT_EQ(CFX_CSSValue::PrimitiveType::kNumber, type);
  EXPECT_EQ(L"4321.1234", maybe_next.value());
  ...
  ASSERT_TRUE(maybe_next.has_value());
  EXPECT_EQ(CFX_CSSValue::PrimitiveType::kNumber, type);
  EXPECT_EQ(L"4321.12.34", maybe_next.value());

  parser = std::make_unique<CFX_CSSValueListParser>(L"43a1.12.34", L' ');
  maybe_next = parser->NextValue(&type);
  ASSERT_TRUE(maybe_next.has_value());
  EXPECT_EQ(CFX_CSSValue::PrimitiveType::kNumber, type);
  EXPECT_EQ(L"43a1.12.34", maybe_next.value());
}
...
TEST(CFX_CSSValueListParserTest, string_parsing) {
  ...
  EXPECT_EQ(L"string", maybe_next.value());
  ...
  ASSERT_TRUE(maybe_next.has_value());
  EXPECT_EQ(CFX_CSSValue::PrimitiveType::kString, type);
  EXPECT_EQ(L"another string", maybe_next.value());

  parser = std::make_unique<CFX_CSSValueListParser>(L"standalone", L' ');
  maybe_next = parser->NextValue(&type);
  ASSERT_TRUE(maybe_next.has_value());
  EXPECT_EQ(CFX_CSSValue::PrimitiveType::kString, type);
  EXPECT_EQ(L"standalone", maybe_next.value());
}
...
TEST(CFX_CSSValueListParserTest, multiparsing) {
  ...
  ASSERT_TRUE(maybe_next.has_value());
  EXPECT_EQ(CFX_CSSValue::PrimitiveType::kNumber, type);
  EXPECT_EQ(L"1", maybe_next.value());

  maybe_next = parser->NextValue(&type);
  ASSERT_TRUE(maybe_next.has_value());
  EXPECT_EQ(CFX_CSSValue::PrimitiveType::kNumber, type);
  EXPECT_EQ(L"2", maybe_next.value());

  maybe_next = parser->NextValue(&type);
  ASSERT_TRUE(maybe_next.has_value());
  EXPECT_EQ(CFX_CSSValue::PrimitiveType::kNumber, type);
  EXPECT_EQ(L"3", maybe_next.value());
  ...
  ASSERT_TRUE(maybe_next.has_value());
  EXPECT_EQ(CFX_CSSValue::PrimitiveType::kString, type);
  EXPECT_EQ(L"str", maybe_next.value());

  maybe_next = parser->NextValue(&type);
  ASSERT_TRUE(maybe_next.has_value());
  EXPECT_EQ(CFX_CSSValue::PrimitiveType::kRGB, type);
  EXPECT_EQ(L"rgb(1, 2, 3)", maybe_next.value());

  maybe_next = parser->NextValue(&type);
  ASSERT_TRUE(maybe_next.has_value());
  EXPECT_EQ(CFX_CSSValue::PrimitiveType::kNumber, type);
  EXPECT_EQ(L"4", maybe_next.value());
}
