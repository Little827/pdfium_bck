...
class CFX_CSSStyleSheetTest : public testing::Test {
 public:
  ...
  static bool FindSelector(CFX_CSSStyleRule* style, WideStringView selector) {
...
TEST_F(CFX_CSSStyleSheetTest, ParseMultipleSelectors) {
  ...
  EXPECT_TRUE(FindSelector(style, L"a"));
  ...
  ASSERT_TRUE(style);
  EXPECT_EQ(1u, style->CountSelectorLists());
  EXPECT_TRUE(FindSelector(style, L"bcdef"));
  ...
  ASSERT_TRUE(style);
  EXPECT_EQ(1u, style->CountSelectorLists());
  EXPECT_TRUE(FindSelector(style, L"*"));
