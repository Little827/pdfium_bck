    ...
    switch (m_eMode) {
        ...
        switch (wch) {
          ...
          case '{':
            if (!m_Output.IsEmpty())
              return CFX_CSSSyntaxStatus::kSelector;
            m_Input.MoveNext();
            PushMode(SyntaxMode::kRuleSet);  // Back to validate ruleset again.
            m_eMode = SyntaxMode::kPropertyName;
            return CFX_CSSSyntaxStatus::kDeclOpen;
          case '/':
            if (m_Input.GetNextChar() == '*') {
              PushMode(SyntaxMode::kSelector);
              m_eMode = SyntaxMode::kComment;
              if (!m_Output.IsEmpty())
                return CFX_CSSSyntaxStatus::kSelector;
              break;
            ...
            FALLTHROUGH;
          default:
            m_Output.AppendCharIfNotLeadingSpace(wch);
            m_Input.MoveNext();
            break;
        }
        break;
      case SyntaxMode::kPropertyName:
        switch (wch) {
          ...
          case '/':
            if (m_Input.GetNextChar() == '*') {
              PushMode(SyntaxMode::kPropertyName);
              m_eMode = SyntaxMode::kComment;
              if (!m_Output.IsEmpty())
                return CFX_CSSSyntaxStatus::kPropertyName;
              break;
      ...
      case SyntaxMode::kPropertyValue:
        switch (wch) {
          ...
          case '/':
            if (m_Input.GetNextChar() == '*') {
              PushMode(SyntaxMode::kPropertyValue);
              m_eMode = SyntaxMode::kComment;
              if (!m_Output.IsEmpty())
                return CFX_CSSSyntaxStatus::kPropertyValue;
              break;
...
void CFX_CSSSyntaxParser::PushMode(SyntaxMode eMode) {
...
WideStringView CFX_CSSSyntaxParser::GetCurrentString() const {
  return m_Output.GetTrailingSpaceTrimmedString();
}
