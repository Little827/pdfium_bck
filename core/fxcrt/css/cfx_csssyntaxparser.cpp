    ...
    switch (m_eMode) {
        ...
        switch (wch) {
          ...
          default:
            if (wch <= ' ') {
              ...
              return CFX_CSSSyntaxParser::Status::kStyleRule;
            } else {
              m_bHasError = true;
              return CFX_CSSSyntaxParser::Status::kError;
            }
      ...
      case Mode::kSelector:
        switch (wch) {
          case ',':
            m_Input.MoveNext();
            if (!m_Output.IsEmpty())
              return CFX_CSSSyntaxParser::Status::kSelector;
            break;
          case '{':
            if (!m_Output.IsEmpty())
              return CFX_CSSSyntaxParser::Status::kSelector;
            ...
            m_eMode = Mode::kPropertyName;
            return CFX_CSSSyntaxParser::Status::kDeclOpen;
          case '/':
            if (m_Input.GetNextChar() == '*') {
      ...
      case Mode::kPropertyName:
        switch (wch) {
          case ':':
            ...
            return CFX_CSSSyntaxParser::Status::kPropertyName;
          case '}':
            m_Input.MoveNext();
            if (!RestoreMode())
              return CFX_CSSSyntaxParser::Status::kError;

            return CFX_CSSSyntaxParser::Status::kDeclClose;
          case '/':
            if (m_Input.GetNextChar() == '*') {
      ...
      case Mode::kPropertyValue:
        switch (wch) {
          ...
          case '}':
            m_eMode = Mode::kPropertyName;
            return CFX_CSSSyntaxParser::Status::kPropertyValue;
          case '/':
            if (m_Input.GetNextChar() == '*') {
