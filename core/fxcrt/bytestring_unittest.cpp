...
TEST(ByteString, ElementAccess) {
  ...
  // Test ptr equality.
  EXPECT_EQ(abc.c_str().get(), mutable_abc.c_str().get());
...
TEST(CString, Normal) {
  ...
  CString blank = CString::Create("");
  EXPECT_NE(nullptr, blank.get());
  EXPECT_DEATH((++blank), "");

  blank = CString::Create("blerf");
  EXPECT_NE(nullptr, blank.get());
  EXPECT_DEATH((++blank), "");

  CString boo = CString::Create("boo");
  EXPECT_EQ('b', *boo++);
  EXPECT_EQ('o', *boo++);
  EXPECT_EQ('o', *boo++);
  EXPECT_EQ('\0', *boo);
