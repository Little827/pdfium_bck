...
template <typename T>
class FixedUniniDataVector {
 public:
  FixedUniniDataVector() : FixedUniniDataVector(0) {}
  explicit FixedUniniDataVector(size_t size)
      : data_(size ? FX_AllocUninit(T, size) : nullptr), size_(size) {}
  FixedUniniDataVector(const FixedUniniDataVector&) = delete;
  FixedUniniDataVector& operator=(const FixedUniniDataVector&) = delete;
  FixedUniniDataVector(FixedUniniDataVector&& that) noexcept {
    data_ = std::move(that.data_);
    size_ = that.size_;
    that.size_ = 0;
  }
  FixedUniniDataVector& operator=(FixedUniniDataVector&& that) noexcept {
    data_ = std::move(that.data_);
    size_ = that.size_;
    that.size_ = 0;
    return *this;
  }
  ~FixedUniniDataVector() = default;
...
using fxcrt::FixedUniniDataVector;
