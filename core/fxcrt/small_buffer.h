...
class SmallBuffer {
 public:
  explicit SmallBuffer(size_t actual_size) : m_pSize(actual_size) {
    ...
    if (actual_size)
      memset(fixed(), 0, sizeof(T) * actual_size);
  }
  size_t size() const { return m_pSize; }
  bool is_dynamic() const { return !!m_pDynamicData; }
  T* data() { return is_dynamic() ? dynamic() : fixed(); }
  T* begin() { return data(); }
  T* end() { return begin() + size(); }
  T* fixed() { return m_FixedData.data(); }
  T* dynamic() { return m_pDynamicData.get(); }
