...
bool CFX_XMLParser::DoSyntaxParse(CFX_XMLDocument* doc) {
  FX_FILESIZE buffer_current_idx = 0;
  ...
  wchar_t current_character_to_skip_too = 0;

  while (true) {
    if (buffer_current_idx >= buffer_size) {
      if (m_pStream->IsEOF())
        return true;
      ...
      buffer_current_idx = 0;
      buffer_size = buffer_chars;
    }

    while (buffer_current_idx < buffer_size) {
      wchar_t ch = buffer[buffer_current_idx];
      switch (current_parser_state) {
        ...
        case FDE_XmlSyntaxState::SkipCommentOrDecl:
          if (FXSYS_wcsnicmp(buffer.data() + buffer_current_idx, L"--", 2) ==
              0) {
            buffer_current_idx += 2;
            current_parser_state = FDE_XmlSyntaxState::SkipComment;
          } else if (FXSYS_wcsnicmp(buffer.data() + buffer_current_idx,
                                    L"[CDATA[", 7) == 0) {
            buffer_current_idx += 7;
            current_parser_state = FDE_XmlSyntaxState::SkipCData;
          } else {
            current_parser_state = FDE_XmlSyntaxState::SkipDeclNode;
            current_character_to_skip_too = L'>';
            character_to_skip_too_stack.push(L'>');
          }
          break;
        case FDE_XmlSyntaxState::SkipCData: {
          if (FXSYS_wcsnicmp(buffer.data() + buffer_current_idx, L"]]>", 3) ==
              0) {
            buffer_current_idx += 3;
            current_parser_state = FDE_XmlSyntaxState::Text;
        ...
        case FDE_XmlSyntaxState::SkipDeclNode:
          if (current_character_to_skip_too == L'\'' ||
              current_character_to_skip_too == L'\"') {
            buffer_current_idx++;
            if (ch != current_character_to_skip_too)
              break;
            ...
            if (character_to_skip_too_stack.empty())
              ...
              current_character_to_skip_too = character_to_skip_too_stack.top();
          } else {
            switch (ch) {
              case L'<':
                current_character_to_skip_too = L'>';
