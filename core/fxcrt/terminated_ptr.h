...
class TerminatedPtr {
 public:
  UNSAFE_BUFFER_USAGE static TerminatedPtr Create(const T* ptr, size_t nulpos) {
    return TerminatedPtr(ptr, nulpos);
  }
 ...
 private:
  ...
  // Seee https://github.com/llvm/llvm-project/issues/80482
  // This is explicit so as to bother the caller for ordinary pointers.
  UNSAFE_BUFFER_USAGE explicit TerminatedPtr(const T* ptr,
                                             size_t nulpos) noexcept
      : ptr_(ptr) {
    DCHECK(!ptr_[nullpos]);
  }
