...
// explicitly blocks non-sequential advancement operators, such as `+` and
// `+=` since these might skip over a terminator. It also blocks decrementing
// the pointer in any manner, since it is impossible to know whether it is
// safe to do so.
//
// TerminatedPtr implicitly converts back to T* for other usage.
//
...
class TerminatedPtr {
 public:
  ...
  template <size_t M>
  constexpr TerminatedPtr(const T (&literal)[M])  // NOLINT(runtime/explicit)
      noexcept
      ...
      __attribute__((enable_if(literal[M - 1u] == T{0}, "not terminated")))
#endif
      : ptr_(&literal[0u]) {
  }
  ...
  // No decrement, since there is no way to tell if this would back up before
