...
// writing over the terminator can occur, leading to OOB access.
//
template <typename T>
class TerminatedPtr {
 public:
  ...
  constexpr TerminatedPtr(std::nullptr_t ptr) {}
  ...
  TerminatedPtr operator++(int) {
    ...
    return TerminatedPtr(old);
  }

  UNSAFE_BUFFER_USAGE TerminatedPtr& operator+=(size_t count) {
    UNSAFE_BUFFERS(ptr_ += count);
    return *this;
  }

  UNSAFE_BUFFER_USAGE TerminatedPtr operator+(size_t count) {
    return TerminatedPtr(UNSAFE_BUFFERS(ptr_ + count));
  }
 ...
 private:
  ...
  UNSAFE_BUFFER_USAGE explicit TerminatedPtr(const T* ptr) noexcept
      : ptr_(ptr) {}
