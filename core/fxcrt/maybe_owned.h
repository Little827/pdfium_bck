...
class MaybeOwned {
 public:
  ...
  MaybeOwned() = default;
  explicit MaybeOwned(T* ptr) : m_Obj(UnownedType(ptr)) {}
  explicit MaybeOwned(const UnownedType& ptr) : m_Obj(ptr) {}
  explicit MaybeOwned(OwnedType ptr) : m_Obj(std::move(ptr)) {}
  ...
  void Reset(T* ptr = nullptr) { m_Obj = UnownedType(ptr); }
  void Reset(OwnedType ptr) { m_Obj = std::move(ptr); }

  bool IsOwned() const { return absl::holds_alternative<OwnedType>(m_Obj); }
  ...
  T* Get() const& {
    return absl::visit([](const auto& obj) { return obj.get(); }, m_Obj);
  }
  T* Get() && {
    auto local_variable_preventing_move_elision = std::move(m_Obj);
  ...
  OwnedType Release() {
    auto result = std::move(absl::get<OwnedType>(m_Obj));
    m_Obj = UnownedType(result.get());
    return result;
  }
 ...
 private:
  absl::variant<UnownedType, OwnedType> m_Obj;
};
