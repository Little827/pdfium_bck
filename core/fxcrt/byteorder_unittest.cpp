...
TEST(ByteOrder, ByteSwapToLE16) {
  ...
  for (uint32_t v = 0; v < 0x10000; ++v) {
    ...
    uint16_t expected =
        FXWORD_GetLsbFirst(reinterpret_cast<const uint8_t*>(&v16));
...
TEST(ByteOrder, ByteSwapToLE32) {
  for (uint32_t v : kTestValues32) {
    uint32_t expected =
        FXDWORD_GetLsbFirst(reinterpret_cast<const uint8_t*>(&v));
...
TEST(ByteOrder, ByteSwapToBE16) {
  ...
  for (uint32_t v = 0; v < 0x10000; ++v) {
    const uint16_t v16 = v;
    uint16_t expected =
        FXWORD_GetMsbFirst(reinterpret_cast<const uint8_t*>(&v16));
...
TEST(ByteOrder, ByteSwapToBE32) {
  for (uint32_t v : kTestValues32) {
    uint32_t expected =
        FXDWORD_GetMsbFirst(reinterpret_cast<const uint8_t*>(&v));
