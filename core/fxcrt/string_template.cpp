...
size_t StringTemplate<T>::Insert(size_t index, T ch) {
  ...
  fxcrt::spanmove(m_pData->capacity().subspan(index + 1),
                  m_pData->capacity().subspan(index, new_length - index));
...
size_t StringTemplate<T>::Delete(size_t index, size_t count) {
  ...
  fxcrt::spanmove(m_pData->capacity().subspan(index),
                  m_pData->capacity().subspan(removal_length, chars_to_copy));
...
template <typename T>
std::optional<size_t> StringTemplate<T>::Find(StringView subStr,
                                              size_t start) const {
  if (!m_pData) {
    return std::nullopt;
  }
