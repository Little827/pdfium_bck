...
TEST(fxcrt, FXSYS_dround) {
  EXPECT_EQ(0, FXSYS_dround(0.0));
  EXPECT_EQ(0, FXSYS_dround(-0.0));
  EXPECT_EQ(0, FXSYS_dround(0.00001));
  EXPECT_EQ(0, FXSYS_dround(-0.00001));
  EXPECT_EQ(3, FXSYS_dround(3.14159));
  EXPECT_EQ(4, FXSYS_dround(3.5));

  // Check for smallest non-zero double values.
  EXPECT_EQ(0, FXSYS_dround(std::numeric_limits<double>::min()));
  EXPECT_EQ(0, FXSYS_dround(-std::numeric_limits<double>::min()));
  ...
  EXPECT_EQ(2147483647, FXSYS_dround(2.147483647e+9));

  // Using a slightly larger value, expect to see it be capped at MAX_INT.
  EXPECT_EQ(2147483647, FXSYS_dround(2.14748365e+9));

  EXPECT_EQ(2147483647, FXSYS_dround(2.14748365e+10));
  EXPECT_EQ(2147483647, FXSYS_dround(std::numeric_limits<double>::max()));
  ...
  EXPECT_EQ(-2147483648, FXSYS_dround(-2.1474836479999998e+9));
  EXPECT_EQ(-2147483648, FXSYS_dround(-2.147483648e+9));
  EXPECT_EQ(-2147483648, FXSYS_dround(-2.147483648e+10));
  EXPECT_EQ(-2147483648, FXSYS_dround(-std::numeric_limits<double>::max()));

  // NaN should give zero.
  EXPECT_EQ(0, FXSYS_dround(NAN));
}
