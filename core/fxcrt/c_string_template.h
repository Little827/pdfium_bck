...
class CStringTemplate {
 public:
  UNSAFE_BUFFER_USAGE static CStringTemplate Create(const T* arg) {
    return CStringTemplate(arg);
  }

  constexpr CStringTemplate() = default;
  constexpr CStringTemplate(std::nullptr_t ptr) {}

  // Implicity convert literals, say, without bothering the caller.
  template <size_t N>
  constexpr CStringTemplate(const T (&array)[N]) : ptr_(array) {}

  // Not const enough.
  template <size_t N>
  constexpr CStringTemplate(T (&array)[N]) = delete;
 ...
 private:
  // TODO(tsepez): make public once complier catches these case.
  // Bother the caller for ordinary pointers.
...
static_assert(sizeof(CString) == sizeof(char*),
              "CString can not be more expensive than char* pointers");

static_assert(sizeof(WCString) == sizeof(wchar_t*),
              "WCString can not be more expensive than wchar_t* pointers");
