...
class CStringTemplate {
 public:
  CStringTemplate() = default;

  // Implicity convert literals, say, without bothering the caller.
  template <size_t N>
  CStringTemplate(const T (&array)[N]) : ptr_(array) {
    CHECK(array[N - 1] == 0);
  }
  ...
  // Assign literals, say, without bothering the caller.
  template <size_t N>
  CStringTemplate& operator=(const T (&array)[N]) {
    CHECK(array[N - 1] == 0);
    ptr_ = array;
    return *this;
  }
