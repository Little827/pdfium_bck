...
CFX_FloatRect CFX_Matrix::TransformRect(const CFX_FloatRect& rect) const {
  ...
  for (size_t i = 1; i < std::size(points); i++) {
    // SAFETY: loop bounds.
    new_right = std::max(new_right, UNSAFE_BUFFERS(points[i].x));
    new_left = std::min(new_left, UNSAFE_BUFFERS(points[i].x));
    new_top = std::max(new_top, UNSAFE_BUFFERS(points[i].y));
    new_bottom = std::min(new_bottom, UNSAFE_BUFFERS(points[i].y));
  }
