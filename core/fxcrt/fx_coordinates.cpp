...
void MatchFloatRange(float f1, float f2, int* i1, int* i2) {
  ...
  float d1_floor = floorf(f1);
  float d1_ceil = ceilf(f1);
  float error1 = f1 - d1_floor + fabsf(f2 - d1_floor - length);
  float error2 = d1_ceil - f1 + fabsf(f2 - d1_ceil - length);
  float start = error1 > error2 ? d1_ceil : d1_floor;
