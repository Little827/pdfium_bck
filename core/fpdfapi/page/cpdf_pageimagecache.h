 ...
 private:
  ...
  class Entry {
   ...
   private:
    ...
    RetainPtr<CachedImage> m_pCachedBitmap;
    RetainPtr<CachedImage> m_pCachedMask;
    bool m_bCachedSetMaxSizeRequired = false;
  };

  // Makes a `CachedImage` backed by `image`. `realize_hint` indicates whether
  // it would be beneficial to realize `image` before caching.
  static RetainPtr<CachedImage> MakeCachedImage(RetainPtr<CFX_DIBBase> image,
                                                bool realize_hint);
