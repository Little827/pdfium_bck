...
bool CPDF_TextObject::GetFillColor(int* r, int* g, int* b) const {
  CPDF_Color* fill_color = m_ColorState.GetFillColor();
  if (!fill_color)
    return false;

  return fill_color->GetRGB(r, g, b);
}

bool CPDF_TextObject::GetStrokeColor(int* r, int* g, int* b) const {
  CPDF_Color* stroke_color = m_ColorState.GetStrokeColor();
  if (!stroke_color)
    return false;

  return stroke_color->GetRGB(r, g, b);
}
