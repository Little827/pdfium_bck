...
pdfium::span<const uint8_t> CPDF_PageImageCache::CachedImage::GetBuffer()
  ...
  // should call `RealizeSkImage()` instead. Consider removing this, or at least
  // making it `NOTREACHED_NORETURN()`.
  NOTREACHED();
...
RetainPtr<CFX_DIBBase> CPDF_PageImageCache::MakeCachedImage(
    RetainPtr<CFX_DIBBase> image,
    bool realize_hint) {
#if defined(_SKIA_SUPPORT_)
