...
RetainPtr<CPDF_Font> CPDF_DocPageData::AddWindowsFont(LOGFONTA* pLogFont) {
  ...
  const bool bCJK = FX_CharSetIsCJK(FX_GetCharsetFromInt(pLogFont->lfCharSet));
  ByteString basefont;
  if (bCJK)
  ...
  CPDF_Dictionary* pFontDict = pBaseDict;
  if (!bCJK) {
    if (pLogFont->lfCharSet == static_cast<int>(FX_CharSet::kANSI) ||
        pLogFont->lfCharSet == static_cast<int>(FX_CharSet::kDefault) ||
        pLogFont->lfCharSet == static_cast<int>(FX_CharSet::kSymbol)) {
      pBaseDict->SetNewFor<CPDF_Name>("Encoding", "WinAnsiEncoding");
    } else {
      CalculateEncodingDict(FX_GetCharsetFromInt(pLogFont->lfCharSet),
                            pBaseDict);
    }
    int char_widths[224];
    ...
    pFontDict = ProcessbCJK(
        pBaseDict, FX_GetCharsetFromInt(pLogFont->lfCharSet), basefont,
