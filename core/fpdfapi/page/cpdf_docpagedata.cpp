...
RetainPtr<CPDF_Font> CPDF_DocPageData::AddWindowsFont(LOGFONTA* pLogFont) {
  ...
  const bool bCJK =
      FX_CharSetIsCJK(static_cast<FX_CharSet>(pLogFont->lfCharSet));
  ByteString basefont;
  if (bCJK)
  ...
  if (!bCJK) {
    if (pLogFont->lfCharSet == static_cast<int>(FX_CharSet::kANSI) ||
      ...
      CalculateEncodingDict(static_cast<FX_CharSet>(pLogFont->lfCharSet),
                            pBaseDict);
    }
    int char_widths[224];
    ...
    pFontDict = ProcessbCJK(
        pBaseDict, static_cast<FX_CharSet>(pLogFont->lfCharSet), basefont,
