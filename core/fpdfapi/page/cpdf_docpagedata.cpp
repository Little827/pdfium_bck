...
CPDF_Font* CPDF_DocPageData::LoadFont(CPDF_Dictionary* pFontDict) {
  ASSERT(pFontDict);
  return GetFont(pFontDict);
}

RetainPtr<CPDF_ColorSpace> CPDF_DocPageData::LoadColorSpace(
    const CPDF_Object* pCSObj,
    const CPDF_Dictionary* pResources) {
  return GetColorSpace(pCSObj, pResources);
}

CPDF_Pattern* CPDF_DocPageData::LoadPattern(CPDF_Object* pPatternObj,
                                            bool bShading,
                                            const CFX_Matrix& matrix) {
  return GetPattern(pPatternObj, bShading, matrix);
}
...
CPDF_Font* CPDF_DocPageData::AddFont(CFX_Font* pFont, int charset) {
  ...
  if (!bCJK) {
    ...
    if (charset == FX_CHARSET_ANSI || charset == FX_CHARSET_Default ||
      ...
      size_t i = m_pPDFDoc->CalculateEncodingDict(charset, pBaseDict);
    ...
    pFontDict = m_pPDFDoc->ProcessbCJK(
...
CPDF_Font* CPDF_DocPageData::AddWindowsFont(LOGFONTA* pLogFont) {
  ...
  pFontDict->SetFor(
      "FontDescriptor",
      AddIndirectObject(std::move(pFontDesc))->MakeReference(m_pPDFDoc.Get()));
  hFont = SelectObject(hDC, hFont);
  DeleteObject(hFont);
  DeleteDC(hDC);
  return LoadFont(pBaseDict);
