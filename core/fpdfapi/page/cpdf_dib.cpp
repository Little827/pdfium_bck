...
JpxDecodeAction GetJpxDecodeActionFromColorSpaces(
  ...
  if (pdf_colorspace->CountComponents() == 3 &&
  ...
  if (pdf_colorspace->CountComponents() == 1 &&
      jpx_info.colorspace == OPJ_CLRSPC_GRAY) {
    return JpxDecodeAction::kUseGray;
  }

  if (pdf_colorspace->CountComponents() == 4 &&
      jpx_info.colorspace == OPJ_CLRSPC_CMYK) {
...
int GetColorChannelNumberFromOpjColorSpace(OPJ_COLOR_SPACE colorspace) {
  switch (colorspace) {
    case OPJ_CLRSPC_GRAY:
      return 1;
...
RetainPtr<CFX_DIBitmap> CPDF_DIB::LoadJpxBitmap(
  ...
  if (original_colorspace) {
    ...
    m_nComponents =
        GetColorChannelNumberFromOpjColorSpace(image_info.colorspace);
  }
