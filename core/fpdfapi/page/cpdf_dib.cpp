...
bool CPDF_DIB::Load(CPDF_Document* pDoc, const CPDF_Stream* pStream) {
  ...
  if (m_bColorKey) {
    ...
    m_MaskedLine =
        std::vector<uint8_t, FxAllocAllocator<uint8_t>>(pitch.value());
  }
  m_Pitch = pitch.value();
  return true;
}
...
const uint8_t* CPDF_DIB::GetScanline(int line) const {
  ...
  if (!pSrcLine) {
    pdfium::span<uint8_t> pLineBuf =
        !m_MaskedLine.empty() ? m_MaskedLine : m_LineBuf;
  ...
  if (m_bpc * m_nComponents == 1) {
    ...
    uint32_t* dest_scan = reinterpret_cast<uint32_t*>(m_MaskedLine.data());
    for (int col = 0; col < m_Width; col++) {
      *dest_scan = GetBitValue(pSrcLine, col) ? set_argb : reset_argb;
      dest_scan++;
    }
    return m_MaskedLine.data();
  }
  if (m_bpc * m_nComponents <= 8) {
    ...
    uint8_t* pDestPixel = m_MaskedLine.data();
  ...
  if (m_bColorKey) {
    if (m_nComponents == 3 && m_bpc == 8) {
      uint8_t* alpha_channel = m_MaskedLine.data() + 3;
      for (int col = 0; col < m_Width; col++) {
      ...
      fxcrt::spanset(pdfium::make_span(m_MaskedLine), 0xFF);
    }
  }
  if (m_pColorSpace) {
