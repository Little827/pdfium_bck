...
JpxDecodeAction GetJpxDecodeAction(const CJPX_Decoder::JpxImageInfo& jpx_info,
  ...
  printf("\njpx_info.components = %d\n", jpx_info.components);
   switch (jpx_info.colorspace) {
    case OPJ_CLRSPC_UNKNOWN:
      printf("colorspace unknown\n");
      break;
    case OPJ_CLRSPC_UNSPECIFIED:
      printf("colorspace unspecified\n");
      break;
    case OPJ_CLRSPC_SRGB:
      printf("colorspace SRGB\n");
      break;
    case OPJ_CLRSPC_GRAY:
      printf("colorspace gray\n");
      break;
    case OPJ_CLRSPC_SYCC:
      printf("colorspace sycc\n");
      break;
    case OPJ_CLRSPC_EYCC:
      printf("colorspace eycc\n");
      break;
    case OPJ_CLRSPC_CMYK:
      printf("colorspace CMYK\n");
      break;
    default:
      break;
  }
  ...
  if (pdf_colorspace) {
    printf("Has pdf colorspace\n");
    printf("pdf colorspace component count = %d\n", pdf_colorspace->CountComponents());
    // There is no guarantee that the component number has to match.
    // Many PDFs generated by iOS meets this condition. See
    // https://crbug.com/1012369 for example.
      if (pdf_colorspace->CountComponents() == 3 && jpx_info.colorspace == OPJ_CLRSPC_SRGB) {
        if (jpx_info.components == 4)
          return JpxDecodeAction::kConvertArgbToRgb;
        else
          return JpxDecodeAction::kUseRgb;
      }

      if (pdf_colorspace->CountComponents() == 1 /*&& jpx_info.components == 2*/ &&
          jpx_info.colorspace == OPJ_CLRSPC_GRAY) {
        return JpxDecodeAction::kUseGray;
      }

      if (pdf_colorspace->CountComponents() == 4 /*&& jpx_info.components == 5*/ &&
          jpx_info.colorspace == OPJ_CLRSPC_CMYK) {
        return JpxDecodeAction::kUseCmyk;
      }

    // TODO: what if jpx_info.colorspace and pdf_colorspace are a missmatch?
    if (pdf_colorspace ==
...
bool CPDF_DIB::CreateDCTDecoder(pdfium::span<const uint8_t> src_span,
  ...
  printf("Set m_nComponents from info = %d\n", m_nComponents);
  m_CompData.clear();
  if (m_pColorSpace) {
...
RetainPtr<CFX_DIBitmap> CPDF_DIB::LoadJpxBitmap(
  ...
  switch (action) {
    ...
    case JpxDecodeAction::kUseGray:
      m_pColorSpace = CPDF_ColorSpace::GetStockCS(CPDF_ColorSpace::Family::kDeviceGray);
      break;
  ...
  if (original_colorspace) {
    ...
    m_nComponents = GetColorChannelNumberFromOpjColorSpace(image_info.colorspace);
    //m_nComponents = image_info.components;
    printf("SEt m_nComponents from image_info %d\n", m_nComponents);
  }

  FXDIB_Format format;
  if (action == JpxDecodeAction::kUseGray || action == JpxDecodeAction::kDoNothing) {
    format = FXDIB_Format::k8bppRgb;
    printf("Use k8bppRgb\n");
  } else if (action == JpxDecodeAction::kUseRgb && image_info.components == 3) {
    format = FXDIB_Format::kRgb;
    printf("Use kRgb\n");
  } else if (action == JpxDecodeAction::kConvertArgbToRgb && image_info.components == 4) {
    format = FXDIB_Format::kRgb32;
    printf("Use kRgb32\n");
  } else if (action == JpxDecodeAction::kUseRgb && image_info.components == 4) {
    format = FXDIB_Format::kRgb32;
    printf("Use kRgb32\n");
  } else {
  ...
  if (convert_argb_to_rgb) {
    printf("convert_argb_to_rgb - m_nComponents = %d\n", m_nComponents);
...
void CPDF_DIB::SetMaskProperties() {
  printf("SetMaskProperties() - m_nComponents = 1\n");
  m_bpc = 1;
  m_nComponents = 1;
