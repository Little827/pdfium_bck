...
CPDF_DIB::LoadState CPDF_DIB::StartLoadMask() {
  ...
  if (!mask) {
    ...
    return mask ? StartLoadMaskDIB(mask.Get()) : LoadState::kSuccess;
  }
  ...
  return StartLoadMaskDIB(mask.Get());
}
...
CPDF_DIB::LoadState CPDF_DIB::StartLoadMaskDIB(const CPDF_Stream* mask) {
  m_pMask = pdfium::MakeRetain<CPDF_DIB>();
  LoadState ret = m_pMask->StartLoadDIBBase(m_pDocument.Get(), mask, false,
