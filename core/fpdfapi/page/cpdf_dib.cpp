...
JpxDecodeAction GetJpxDecodeActionWithPdfColorSpace(
    const CJPX_Decoder::JpxImageInfo& jpx_info,
    const CPDF_ColorSpace* pdf_colorspace) {
  // The color space component number is the number of color channels, but the
  // component number from the JPX image is not. It could have extra information
  // stored in these components, such as transparency. Therefore these two
  // component numbers can be different. Many PDFs generated by iOS meets this
  // condition. See https://crbug.com/1012369dd for example.
...
JpxDecodeAction GetJpxDecodeAction(const CJPX_Decoder::JpxImageInfo& jpx_info,
                                   const CPDF_ColorSpace* pdf_colorspace) {
  if (pdf_colorspace) {
    return GetJpxDecodeActionWithPdfColorSpace(jpx_info, pdf_colorspace);
      }
...
RetainPtr<CFX_DIBitmap> CPDF_DIB::LoadJpxBitmap(
  ...
  if (action == JpxDecodeAction::kUseGray ||
      action == JpxDecodeAction::kDoNothing) {
  ...
  } else if (action == JpxDecodeAction::kUseGray &&
             image_info.components == 2) {
    if (m_pDict->GetIntegerFor("SMaskInData") == 1) {
      // TODO(nigi): handle SMaskInData
    }
  } else if (action == JpxDecodeAction::kUseCmyk &&
             image_info.components == 5) {
    if (m_pDict->GetIntegerFor("SMaskInData") == 1) {
      // TODO(nigi): handle SMaskInData
    }
  }
