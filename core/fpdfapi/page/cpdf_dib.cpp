...
RetainPtr<CFX_DIBitmap> CPDF_DIB::LoadJpxBitmap() {
  ...
  if (convert_argb_to_rgb) {
    ...
    RetainPtr<CFX_DIBitmap> original_bitmap = std::move(pCachedBitmap);
    pCachedBitmap = pdfium::MakeRetain<CFX_DIBitmap>();
    if (!pCachedBitmap->Create(image_info.width, image_info.height, FXDIB_Rgb))
      return nullptr;

    // TODO(thestig): Is there existing code that does this already?
    // Does the alpha channel from |src| need to be handled in someway?
    for (uint32_t row = 0; row < image_info.height; ++row) {
      const uint8_t* src = original_bitmap->GetScanline(row);
      uint8_t* dest = pCachedBitmap->GetWritableScanline(row);
