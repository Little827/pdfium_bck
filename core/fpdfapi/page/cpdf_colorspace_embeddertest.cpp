...
TEST_F(CPDF_ColorspaceEmbedderTest, Bug_981256) {
  ASSERT_TRUE(OpenDocument("bug_981256.pdf"));
...
// Test passes if it does not crash.
TEST_F(CPDF_ColorspaceEmbedderTest, Bug_981412) {
  ASSERT_TRUE(OpenDocument("bug_981412.pdf"));
  FPDF_PAGE page = LoadPage(0);
  ASSERT_TRUE(page);
  UnloadPage(page);
}
