...
TEST(CPDF_PSEngine, Round) {
  ...
  EXPECT_FLOAT_EQ(0.0f, DoOperator1(&engine, 0.0f, PSOP_ROUND));
  EXPECT_FLOAT_EQ(0.0f,
                  DoOperator1(&engine, 1.17549e-38f, PSOP_ROUND));  // Min float
  EXPECT_FLOAT_EQ(3.40282e+38f,
                  DoOperator1(&engine, 3.40282e+38f, PSOP_ROUND));  // Max float
  EXPECT_FLOAT_EQ(0.0f, DoOperator1(&engine, -1.17549e-38f,
                                    PSOP_ROUND));  // Negative Min float
  EXPECT_FLOAT_EQ(
      -3.40282e+38f,
      DoOperator1(&engine, -3.40282e+38f, PSOP_ROUND));  // Negative Max float
