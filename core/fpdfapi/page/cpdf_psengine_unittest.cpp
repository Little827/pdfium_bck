...
TEST(CPDF_PSEngine, Round) {
  ...
  EXPECT_FLOAT_EQ(0.0f, DoOperator1(&engine, 0.0f, PSOP_ROUND));
  float min_float =
      std::numeric_limits<float>::min();  // Smallest postitive float value.
...
TEST(CPDF_PSEngine, Truncate) {
  ...
  EXPECT_FLOAT_EQ(0.0f,
                  DoOperator1(&engine, min_float, PSOP_TRUNCATE));  // Min float
  EXPECT_FLOAT_EQ(0.0f, DoOperator1(&engine, -min_float,
