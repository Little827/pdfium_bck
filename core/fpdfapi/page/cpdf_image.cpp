...
void CPDF_Image::SetImage(const RetainPtr<CFX_DIBitmap>& pBitmap) {
  ...
  pdfium::span<uint8_t> src_span = pBitmap->GetBuffer();
  ...
  if (bCopyWithoutAlpha) {
    ...
    for (int32_t row = 0; row < BitmapHeight; row++) {
      ...
      for (int32_t column = 0; column < BitmapWidth; column++) {
        dest_span[dest_offset] = src_span[src_offset + 2];
        dest_span[dest_offset + 1] = src_span[src_offset + 1];
        dest_span[dest_offset + 2] = src_span[src_offset];
        dest_offset += 3;
        src_offset += src_step;
      }
      dest_span = dest_span.subspan(dest_pitch);
