...
uint32_t CPDF_ColorState::GetFillBGR() const {
  return m_Ref.GetObject()->m_FillBGR;
}

void CPDF_ColorState::SetFillBGR(uint32_t bgr) {
  m_Ref.GetPrivateCopy()->m_FillBGR = bgr;
}

uint32_t CPDF_ColorState::GetStrokeBGR() const {
  return m_Ref.GetObject()->m_StrokeBGR;
}

void CPDF_ColorState::SetStrokeBGR(uint32_t bgr) {
  m_Ref.GetPrivateCopy()->m_StrokeBGR = bgr;
}
...
void CPDF_ColorState::SetFillColor(CPDF_ColorSpace* pCS,
  ...
  SetColor(pData->m_FillColor, &pData->m_FillBGR, pCS, pValue, nValues);
}
...
void CPDF_ColorState::SetStrokeColor(CPDF_ColorSpace* pCS,
                                     ...
                                     uint32_t nValues) {
  ColorData* pData = m_Ref.GetPrivateCopy();
  SetColor(pData->m_StrokeColor, &pData->m_StrokeBGR, pCS, pValue, nValues);
}
...
void CPDF_ColorState::SetFillPattern(CPDF_Pattern* pPattern,
  ...
  if (CPDF_TilingPattern* pTilingPattern = pPattern->AsTilingPattern()) {
    if (!ret && pTilingPattern->colored()) {
      pData->m_FillBGR = 0x00BFBFBF;
      return;
    }
  }
  pData->m_FillBGR = ret ? FXSYS_BGR(B, G, R) : 0xFFFFFFFF;
}
...
void CPDF_ColorState::SetColor(CPDF_Color& color,
  ...
  int G;
  int B;
  *bgr = color.GetRGB(&R, &G, &B) ? FXSYS_BGR(B, G, R) : 0xFFFFFFFF;
}

CPDF_ColorState::ColorData::ColorData() : m_FillBGR(0), m_StrokeBGR(0) {}

CPDF_ColorState::ColorData::ColorData(const ColorData& src)
    : m_FillBGR(src.m_FillBGR), m_StrokeBGR(src.m_StrokeBGR) {
