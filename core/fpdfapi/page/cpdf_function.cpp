...
bool CPDF_Function::Init(const CPDF_Object* pObj,
  ...
  m_Domains.resize(nInputs);
  ReadArrayElementsToVector(pDomains, &m_Domains, nInputs);
  ...
  if (m_nOutputs > 0) {
    ...
    m_Ranges.resize(nOutputs);
    ReadArrayElementsToVector(pRanges, &m_Ranges, nOutputs);
  }
