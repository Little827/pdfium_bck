...
CPDF_PSOP::CPDF_PSOP() : m_proc(std::make_unique<CPDF_PSProc>()) {}

CPDF_PSOP::CPDF_PSOP(PDF_PSOP op) : m_op(op) {
...
bool CPDF_PSOP::Execute(CPDF_PSEngine* pEngine) {
  if (m_op != PSOP_PROC) {
    NOTREACHED();
    return false;
  }
  return m_proc->Execute(pEngine);
}
