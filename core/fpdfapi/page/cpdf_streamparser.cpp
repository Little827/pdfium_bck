...
uint32_t DecodeInlineStream(pdfium::span<const uint8_t> src_span,
  ...
  if (decoder == "LZWDecode") {
    DataAndBytesConsumed result = FlateOrLZWDecode(
  ...
  if (decoder == "ASCII85Decode") {
    DataAndBytesConsumed result = A85Decode(src_span);
    return result.bytes_consumed;
  }
  if (decoder == "ASCIIHexDecode") {
    DataAndBytesConsumed result = HexDecode(src_span);
    return result.bytes_consumed;
  }
  if (decoder == "RunLengthDecode") {
    DataAndBytesConsumed result = RunLengthDecode(src_span);
    return result.bytes_consumed;
  }
