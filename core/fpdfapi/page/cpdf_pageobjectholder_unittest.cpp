...
TEST(CPDFPageObjectHolder, GraphicsDataAsKey) {
  ...
  EXPECT_FALSE(GraphicsData({fNan, 1.0f, 1}) < GraphicsData({fInf, 1.0f, 1}));
  EXPECT_FALSE(GraphicsData({fInf, 1.0f, 1}) < GraphicsData({fMax, 1.0f, 1}));
  EXPECT_FALSE(GraphicsData({fMax, 1.0f, 1}) < GraphicsData({fMin, 1.0f, 1}));

  EXPECT_TRUE(GraphicsData({fInf, 1.0f, 1}) < GraphicsData({fNan, 1.0f, 1}));
  EXPECT_TRUE(GraphicsData({fMax, 1.0f, 1}) < GraphicsData({fInf, 1.0f, 1}));
  EXPECT_TRUE(GraphicsData({fMin, 1.0f, 1}) < GraphicsData({fMax, 1.0f, 1}));
  ...
  const int expected[72] = {
      71, 35, 65, 29, 59, 23, 53, 17, 47, 11, 41, 5,  70, 34, 64, 28, 58, 22,
      52, 16, 46, 10, 40, 4,  69, 33, 63, 27, 57, 21, 51, 15, 45, 9,  39, 3,
      68, 32, 62, 26, 56, 20, 50, 14, 44, 8,  38, 2,  67, 31, 61, 25, 55, 19,
      49, 13, 43, 7,  37, 1,  66, 30, 60, 24, 54, 18, 48, 12, 42, 6,  36, 0};
