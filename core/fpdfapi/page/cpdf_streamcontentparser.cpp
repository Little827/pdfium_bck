...
CPDF_StreamContentParser::CPDF_StreamContentParser(
  ...
  if (pStates) {
    *m_pCurStates = *pStates;
    ...
    m_pCurStates->m_GraphicStates.m_GeneralState.Emplace();
    m_pCurStates->m_GraphicStates.m_GraphState.Emplace();
    m_pCurStates->m_GraphicStates.m_TextState.Emplace();
    m_pCurStates->m_GraphicStates.m_ColorState.Emplace();
  }
...
void CPDF_StreamContentParser::Handle_SetColorSpace_Fill() {
  ...
  m_pCurStates->m_GraphicStates.m_ColorState.GetMutableFillColor()
      ->SetColorSpace(std::move(pCS));
}
...
void CPDF_StreamContentParser::Handle_SetColorSpace_Stroke() {
  ...
  m_pCurStates->m_GraphicStates.m_ColorState.GetMutableStrokeColor()
      ->SetColorSpace(std::move(pCS));
}
...
void CPDF_StreamContentParser::AddForm(RetainPtr<CPDF_Stream> pStream,
  ...
  status.m_GraphicStates.m_GeneralState =
      m_pCurStates->m_GraphicStates.m_GeneralState;
  status.m_GraphicStates.m_GraphState =
      m_pCurStates->m_GraphicStates.m_GraphState;
  status.m_GraphicStates.m_ColorState =
      m_pCurStates->m_GraphicStates.m_ColorState;
  status.m_GraphicStates.m_TextState =
      m_pCurStates->m_GraphicStates.m_TextState;
...
void CPDF_StreamContentParser::Handle_EndText() {
  ...
  if (TextRenderingModeIsClipMode(
          m_pCurStates->m_GraphicStates.m_TextState.GetTextMode())) {
    m_pCurStates->m_GraphicStates.m_ClipPath.AppendTexts(&m_ClipTextList);
  }
...
void CPDF_StreamContentParser::Handle_SetGray_Fill() {
  m_pCurStates->m_GraphicStates.m_ColorState.SetFillColor(
      CPDF_ColorSpace::GetStockCS(CPDF_ColorSpace::Family::kDeviceGray),
      GetNumbers(1));
}

void CPDF_StreamContentParser::Handle_SetGray_Stroke() {
  m_pCurStates->m_GraphicStates.m_ColorState.SetStrokeColor(
      CPDF_ColorSpace::GetStockCS(CPDF_ColorSpace::Family::kDeviceGray),
      GetNumbers(1));
}
...
void CPDF_StreamContentParser::Handle_SetExtendGraphState() {
  ...
  CHECK(!name.IsEmpty());
  m_pCurStates->m_GraphicStates.m_GeneralState.AppendGraphicsResourceName(
      std::move(name));
...
void CPDF_StreamContentParser::Handle_SetMiterLimit() {
  m_pCurStates->m_GraphicStates.m_GraphState.SetMiterLimit(GetNumber(0));
}
...
void CPDF_StreamContentParser::Handle_SetColor_Fill() {
  ...
  m_pCurStates->m_GraphicStates.m_ColorState.SetFillColor(nullptr,
                                                          GetNumbers(nargs));
}

void CPDF_StreamContentParser::Handle_SetColor_Stroke() {
  int nargs = std::min(m_ParamCount, 4U);
  m_pCurStates->m_GraphicStates.m_ColorState.SetStrokeColor(nullptr,
                                                            GetNumbers(nargs));
}
...
void CPDF_StreamContentParser::Handle_SetColorPS_Fill() {
  ...
  m_pCurStates->m_GraphicStates.m_ColorState.SetFillPattern(std::move(pPattern),
                                                            values);
}
...
void CPDF_StreamContentParser::Handle_SetColorPS_Stroke() {
  ...
  std::vector<float> values = GetNamedColors();
  m_pCurStates->m_GraphicStates.m_ColorState.SetStrokePattern(
      std::move(pPattern), values);
}
...
void CPDF_StreamContentParser::Handle_SetCharSpace() {
  m_pCurStates->m_GraphicStates.m_TextState.SetCharSpace(GetNumber(0));
}
...
void CPDF_StreamContentParser::Handle_SetFont() {
  m_pCurStates->m_GraphicStates.m_TextState.SetFontSize(GetNumber(0));
  RetainPtr<CPDF_Font> pFont = FindFont(GetString(1));
  if (pFont)
    m_pCurStates->m_GraphicStates.m_TextState.SetFont(std::move(pFont));
}
...
void CPDF_StreamContentParser::AddTextObject(const ByteString* pStrs,
  ...
  RetainPtr<CPDF_Font> pFont =
      m_pCurStates->m_GraphicStates.m_TextState.GetFont();
  if (!pFont)
    return;
...
float CPDF_StreamContentParser::GetVerticalTextSize(float fKerning) const {
  return fKerning * m_pCurStates->m_GraphicStates.m_TextState.GetFontSize() /
         1000;
}
...
void CPDF_StreamContentParser::OnChangeTextMatrix() {
  ...
  pdfium::span<float> pTextMatrix =
      m_pCurStates->m_GraphicStates.m_TextState.GetMutableMatrix();
...
void CPDF_StreamContentParser::Handle_SetTextRenderMode() {
  ...
  if (SetTextRenderingModeFromInt(GetInteger(0), &mode))
    m_pCurStates->m_GraphicStates.m_TextState.SetTextMode(mode);
}
...
void CPDF_StreamContentParser::Handle_SetWordSpace() {
  m_pCurStates->m_GraphicStates.m_TextState.SetWordSpace(GetNumber(0));
}
...
void CPDF_StreamContentParser::Handle_SetLineWidth() {
  m_pCurStates->m_GraphicStates.m_GraphState.SetLineWidth(GetNumber(0));
}
...
void CPDF_StreamContentParser::Handle_NextLineShowText_Space() {
  m_pCurStates->m_GraphicStates.m_TextState.SetWordSpace(GetNumber(2));
  m_pCurStates->m_GraphicStates.m_TextState.SetCharSpace(GetNumber(1));
...
void CPDF_StreamContentParser::AddPathObject(
  ...
  if (path_points.size() == 1) {
    if (path_clip_type != CFX_FillRenderOptions::FillType::kNoFill) {
      ...
      m_pCurStates->m_GraphicStates.m_ClipPath.AppendPathWithAutoMerge(
    ...
    if (point.m_Type != CFX_Path::Point::Type::kMove || !point.m_CloseFigure ||
        m_pCurStates->m_GraphicStates.m_GraphState.GetLineCap() !=
