...
void CPDF_StreamContentParser::AddForm(CPDF_Stream* pStream) {
  ...
  auto pFormObj = pdfium::MakeUnique<CPDF_FormObject>(m_nContentStreamIndex,
...
CPDF_ImageObject* CPDF_StreamContentParser::AddImage(
  ...
  auto pImageObj = pdfium::MakeUnique<CPDF_ImageObject>(m_nContentStreamIndex);
  pImageObj->SetImage(
...
void CPDF_StreamContentParser::Handle_ShadeFill() {
  ...
  CFX_Matrix matrix = m_pCurStates->m_CTM;
  matrix.Concat(m_mtContentToUser);
  auto pObj = pdfium::MakeUnique<CPDF_ShadingObject>(m_nContentStreamIndex,
                                                     pShading, matrix);
...
void CPDF_StreamContentParser::AddTextObject(ByteString* pStrs,
  ...
  {
    auto pText = pdfium::MakeUnique<CPDF_TextObject>(m_nContentStreamIndex);
...
void CPDF_StreamContentParser::AddPathObject(int FillType, bool bStroke) {
  ...
  matrix.Concat(m_mtContentToUser);
  if (bStroke || FillType) {
    auto pPathObj = pdfium::MakeUnique<CPDF_PathObject>(m_nContentStreamIndex);
...
uint32_t CPDF_StreamContentParser::Parse(const uint8_t* pData,
                                         uint32_t dwSize,
                                         uint32_t max_cost,
                                         int content_stream_index) {
  m_nContentStreamIndex = content_stream_index;
