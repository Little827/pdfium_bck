...
void CPDF_StreamContentParser::AddForm(CPDF_Stream* pStream) {
  ...
  auto pFormObj = pdfium::MakeUnique<CPDF_FormObject>(GetCurrentStream(),
...
CPDF_ImageObject* CPDF_StreamContentParser::AddImage(
  ...
  auto pImageObj = pdfium::MakeUnique<CPDF_ImageObject>(GetCurrentStream());
  pImageObj->SetImage(
...
void CPDF_StreamContentParser::Handle_ShadeFill() {
  ...
  CFX_Matrix matrix = m_pCurStates->m_CTM;
  matrix.Concat(m_mtContentToUser);
  auto pObj = pdfium::MakeUnique<CPDF_ShadingObject>(GetCurrentStream(),
                                                     pShading, matrix);
...
void CPDF_StreamContentParser::AddTextObject(ByteString* pStrs,
  ...
  {
    auto pText = pdfium::MakeUnique<CPDF_TextObject>(GetCurrentStream());
...
int32_t CPDF_StreamContentParser::GetCurrentStream() {
  // std::cerr << m_pSyntax->GetPos() << std::endl;
  auto it = std::upper_bound(m_StreamStartOffsets.begin(),
                             m_StreamStartOffsets.end(), m_pSyntax->GetPos());
  int32_t content_stream = (it - m_StreamStartOffsets.begin()) - 1;
  // std::cerr << "stream is " << content_stream << std::endl;
...
void CPDF_StreamContentParser::AddPathObject(int FillType, bool bStroke) {
  ...
  matrix.Concat(m_mtContentToUser);
  if (bStroke || FillType) {
    auto pPathObj = pdfium::MakeUnique<CPDF_PathObject>(GetCurrentStream());
