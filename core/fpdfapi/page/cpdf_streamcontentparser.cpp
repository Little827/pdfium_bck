...
int32_t CPDF_StreamContentParser::GetCurrentStreamIndex() {
  auto it = std::upper_bound(m_StreamStartOffsets.begin(),
                             m_StreamStartOffsets.end(), m_pSyntax->GetPos() + m_StartParseOffset);
  int32_t res = (it - m_StreamStartOffsets.begin()) - 1;
  // std::cerr << "m_pSyntax->GetPos() " << (m_pSyntax->GetPos() + m_StartParseOffset) << ", result " << res << std::endl;
  return res;
}
...
uint32_t CPDF_StreamContentParser::Parse(
  ...
  std::cerr << "CPDF_StreamContentParser::Parse" << std::endl;
