...
CPDF_Creator::Stage CPDF_Creator::WriteDoc_Stage1() {
  ASSERT(m_iStage > Stage::kInvalid || m_iStage < Stage::kTwenty20);
  if (m_iStage == Stage::kZero0) {
    ...
    m_iStage = Stage::kTen10;
  }
  if (m_iStage == Stage::kTen10) {
    if (!m_IsIncremental) {
      ...
      m_iStage = Stage::kTwenty20;
    } else {
      m_SavedOffset = m_pParser->GetFileAccess()->GetSize();
      m_iStage = Stage::kFifteen15;
    }
  }
  if (m_iStage == Stage::kFifteen15) {
...
CPDF_Creator::Stage CPDF_Creator::WriteDoc_Stage2() {
  ASSERT(m_iStage >= Stage::kTwenty20 || m_iStage < Stage::kEighty80);
  if (m_iStage == Stage::kTwenty20) {
    if (!m_IsIncremental && m_pParser) {
      m_CurObjNum = 0;
      m_iStage = Stage::kTwentyOne21;
    } else {
      m_iStage = Stage::kTwentyFive25;
    }
  }
  if (m_iStage == Stage::kTwentyOne21) {
    if (!WriteOldObjs())
      return Stage::kInvalid;

    m_iStage = Stage::kTwentyFive25;
  }
  if (m_iStage == Stage::kTwentyFive25) {
    m_CurObjNum = 0;
    m_iStage = Stage::kTwentySix26;
  }
  if (m_iStage == Stage::kTwentySix26) {
    if (!WriteNewObjs())
      return Stage::kInvalid;

    m_iStage = Stage::kTwentySeven27;
  }
  if (m_iStage == Stage::kTwentySeven27) {
    ...
    m_iStage = Stage::kEighty80;
  }
  return m_iStage;
}

CPDF_Creator::Stage CPDF_Creator::WriteDoc_Stage3() {
  ASSERT(m_iStage >= Stage::kEighty80 || m_iStage < Stage::kNinety90);

  uint32_t dwLastObjNum = m_dwLastObjNum;
  if (m_iStage == Stage::kEighty80) {
    ...
    if (!m_IsIncremental || !m_pParser->IsXRefStream()) {
      if (!m_IsIncremental || m_pParser->GetLastXRefOffset() == 0) {
        ...
        m_CurObjNum = 1;
        m_iStage = Stage::kEightyOne81;
      } else {
        if (!m_Archive->WriteString("xref\r\n"))
          return Stage::kInvalid;

        m_CurObjNum = 0;
        m_iStage = Stage::kEightyTwo82;
      }
    } else {
      m_iStage = Stage::kNinety90;
    }
  }
  if (m_iStage == Stage::kEightyOne81) {
    ByteString str;
  ...
  if (m_iStage == Stage::kEightyTwo82) {
    ByteString str;
...
CPDF_Creator::Stage CPDF_Creator::WriteDoc_Stage4() {
  ASSERT(m_iStage >= Stage::kNinety90);
...
bool CPDF_Creator::Create(uint32_t flags) {
  ...
  m_iStage = Stage::kZero0;
  m_dwLastObjNum = m_pDocument->GetLastObjNum();
  m_ObjectOffsets.clear();
  m_NewObjNumArray.clear();

  InitID();
  return Continue() > Stage::kInvalid;
}
...
CPDF_Creator::Stage CPDF_Creator::Continue() {
  if (m_iStage < Stage::kZero0)
    return m_iStage;

  Stage iRet = Stage::kZero0;
  while (m_iStage < Stage::kHundred100) {
    if (m_iStage < Stage::kTwenty20)
      iRet = WriteDoc_Stage1();
    else if (m_iStage < Stage::kEighty80)
      iRet = WriteDoc_Stage2();
    else if (m_iStage < Stage::kNinety90)
  ...
  if (iRet <= Stage::kZero0 || m_iStage == Stage::kHundred100) {
    m_iStage = Stage::kInvalid;
    if (iRet == Stage::kHundred100)
      return Stage::kZero0;
    if (iRet <= Stage::kZero0)
      return Stage::kInvalid;
    return iRet;
  }
