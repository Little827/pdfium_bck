...
CFX_Matrix GetCTMAtEndOfStream(const std::map<int32_t, CFX_Matrix>& all_ctms,
                               int32_t stream) {
...
CFX_Matrix GetCTMAtBeginningOfStream(
    const std::map<int32_t, CFX_Matrix>& all_ctms,
    int32_t stream) {
...
CPDF_PageContentGenerator::GenerateModifiedStreams() {
  ...
  const std::map<int32_t, CFX_Matrix>& all_ctms = m_pObjHolder->GetAllCTMs();
