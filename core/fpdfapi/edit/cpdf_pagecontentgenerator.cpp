...
void CPDF_PageContentGenerator::UpdateContentStreams(
  ...
  for (auto& pair : *new_stream_data) {
    ...
    if (stream_index == -1) {
      ...
      std::cerr << "  -> old_stream" << (void*)old_stream << std::endl;
      if (!old_stream)
        continue;
      ...
      std::cerr << "  -> old_stream->SetData(buf[" << old_stream->GetRawSize()
                << "]) objnum " << old_stream->GetObjNum() << std::endl;
...
void CPDF_PageContentGenerator::UpdateStreamlessPageObjects(
  ...
  for (auto& pPageObj : m_pageObjects) {
    if (pPageObj->GetContentStream() == -1)
