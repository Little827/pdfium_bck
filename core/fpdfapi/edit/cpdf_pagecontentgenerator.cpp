...
CPDF_PageContentGenerator::GenerateModifiedStreams() {
  ...
  std::set<int32_t> stream_is_empty;
  ...
  for (int32_t dirty_stream : all_dirty_streams) {
    ...
    *buf << "q\n";
    ...
    stream_is_empty.insert(dirty_stream);
  }
  ...
  for (auto& pPageObj : m_pageObjects) {
    ...
    stream_is_empty.erase(stream_index);
  ...
  for (int32_t dirty_stream : all_dirty_streams) {
    ...
    if (pdfium::ContainsKey(stream_is_empty, dirty_stream)) {
