...
CPDF_PageContentGenerator::GenerateModifiedStreams() {
  ...
  std::map<int32_t, bool> stream_has_any_object;
  ...
  for (int32_t dirty_stream : all_dirty_streams) {
    ...
    *buf << "q\n";
    ...
    stream_has_any_object[dirty_stream] = false;
  }
  ...
  for (auto& pPageObj : m_pageObjects) {
    ...
    stream_has_any_object[stream_index] = true;
  ...
  for (int32_t dirty_stream : all_dirty_streams) {
    ...
    if (stream_has_any_object[dirty_stream]) {
