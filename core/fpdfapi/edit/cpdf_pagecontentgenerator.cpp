...
void CPDF_PageContentGenerator::ProcessGraphics(std::ostringstream* buf,
  ...
  *buf << "q ";
  ...
  CPDF_ClipPath clipPath = pPageObj->m_ClipPath;
  for (size_t i = 0; clipPath.HasRef() && i < clipPath.GetPathCount(); ++i) {
    CPDF_Path path = clipPath.GetPath(i);
    ProcessPathPoints(buf, &path);
    switch (clipPath.GetClipType(i)) {
      case CFX_FillRenderOptions::FillType::kNoFill:
        // Clipping paths should always have a fill type, but fall back to
        // |kWinding| just in case.
        NOTREACHED();
        FALLTHROUGH;
    ...
    // Use a no-op path-painting operator to terminate the path without causing
    // any marks to be placed on the page.
    *buf << "n 0 g ";
  }
