...
TEST_F(CPDF_PageContentGeneratorTest, ProcessRect) {
  ...
  pPathObj->path().AppendPoint(CFX_PointF(0, 0), FX_PathToType::kMove);
  pPathObj->path().AppendPoint(CFX_PointF(5.2f, 0), FX_PathToType::kLine);
  pPathObj->path().AppendPoint(CFX_PointF(5.2f, 3.78f), FX_PathToType::kLine);
...
TEST_F(CPDF_PageContentGeneratorTest, ProcessPath) {
  auto pPathObj = std::make_unique<CPDF_PathObject>();
  pPathObj->set_filltype(CFX_FillRenderOptions::FillType::kWinding);
  pPathObj->path().AppendPoint(CFX_PointF(3.102f, 4.67f), FX_PathToType::kMove);
  pPathObj->path().AppendPoint(CFX_PointF(5.45f, 0.29f), FX_PathToType::kLine);
  ...
  pPathObj->path().AppendPoint(CFX_PointF(3.456f, 0.24f),
                               FX_PathToType::kBezier);
  pPathObj->path().AppendPoint(CFX_PointF(10.6f, 11.15f), FX_PathToType::kLine);
  pPathObj->path().AppendPoint(CFX_PointF(11, 12.5f), FX_PathToType::kLine);
...
TEST_F(CPDF_PageContentGeneratorTest, ProcessGraphics) {
  ...
  pPathObj->set_stroke(true);
  pPathObj->set_filltype(CFX_FillRenderOptions::FillType::kWinding);
  pPathObj->path().AppendPoint(CFX_PointF(1, 2), FX_PathToType::kMove);
  pPathObj->path().AppendPoint(CFX_PointF(3, 4), FX_PathToType::kLine);
  pPathObj->path().AppendPointAndClose(CFX_PointF(5, 6), FX_PathToType::kLine);
...
TEST_F(CPDF_PageContentGeneratorTest, ProcessText) {
  ...
  std::ostringstream buf;
  {
    ...
    pPath->AppendPoint(CFX_PointF(0, 0), FX_PathToType::kMove);
    pPath->AppendPoint(CFX_PointF(5, 0), FX_PathToType::kLine);
    pPath->AppendPoint(CFX_PointF(5, 4), FX_PathToType::kLine);
    pPath->AppendPointAndClose(CFX_PointF(0, 4), FX_PathToType::kLine);
