...
TEST_F(CPDF_PageContentGeneratorTest, BUG_937) {
      ...
      CPDF_ColorSpace::GetStockCS(CPDF_ColorSpace::Family::kDeviceRGB);
  {
    ...
    pPathObj->m_GraphicStates.m_ColorState.SetFillColor(pCS, rgb);
    pPathObj->m_GraphicStates.m_ColorState.SetStrokeColor(pCS, rgb);
    pPathObj->m_GraphicStates.m_GraphState.SetLineWidth(
        200000000000000000001.0);
...
TEST_F(CPDF_PageContentGeneratorTest, ProcessGraphics) {
  ...
  pPathObj->m_GraphicStates.m_ColorState.SetStrokeColor(pCS, rgb2);
  pPathObj->m_GraphicStates.m_GeneralState.SetFillAlpha(0.5f);
  pPathObj->m_GraphicStates.m_GeneralState.SetStrokeAlpha(0.8f);
  ...
  pPathObj->m_GraphicStates.m_GraphState.SetLineWidth(10.5f);
  buf.str("");
  TestProcessPath(&generator, &buf, pPathObj.get());
...
TEST_F(CPDF_PageContentGeneratorTest, ProcessStandardText) {
  ...
  pTextObj->m_GraphicStates.m_TextState.SetFontSize(10.0f);

  static const std::vector<float> rgb = {0.5f, 0.7f, 0.35f};
  RetainPtr<CPDF_ColorSpace> pCS =
      CPDF_ColorSpace::GetStockCS(CPDF_ColorSpace::Family::kDeviceRGB);
  pTextObj->m_GraphicStates.m_ColorState.SetFillColor(pCS, rgb);

  static const std::vector<float> rgb2 = {1, 0.9f, 0};
  pTextObj->m_GraphicStates.m_ColorState.SetStrokeColor(pCS, rgb2);
  pTextObj->m_GraphicStates.m_GeneralState.SetFillAlpha(0.5f);
  pTextObj->m_GraphicStates.m_GeneralState.SetStrokeAlpha(0.8f);
...
TEST_F(CPDF_PageContentGeneratorTest, ProcessText) {
  ...
  fxcrt::ostringstream buf;
  {
    ...
    pTextObj->m_GraphicStates.m_TextState.SetFontSize(15.5f);
    ...
    pTextObj->m_GraphicStates.m_ClipPath.Emplace();
    pTextObj->m_GraphicStates.m_ClipPath.AppendPath(
        *pPath, CFX_FillRenderOptions::FillType::kEvenOdd);
