...
CPDF_GeneratedStreams::CPDF_GeneratedStreams(
  ...
  std::cerr << "CPDF_GeneratedStreams::CPDF_GeneratedStreams("
            << page_objects.size() << ")" << std::endl;
  for (auto& page_object : page_objects) {
    int32_t content_stream = page_object->GetContentStream();
    if (!GetBuffer(content_stream)) {
      std::cerr << "  -> create for content_stream=" << content_stream
                << std::endl;
      buffers_.emplace(content_stream,
  ...
  std::cerr << "CPDF_GeneratedStreams::CPDF_GeneratedStreams("
            << page_objects.size() << ") END" << std::endl;
}

std::ostringstream* CPDF_GeneratedStreams::GetBuffer(int32_t content_stream) {
  std::cerr << "CPDF_GeneratedStreams::GetBuffer(" << content_stream << ")"
            << std::endl;
  ...
  std::cerr << "CPDF_GeneratedStreams::GetBuffer(" << content_stream << ") END"
            << std::endl;
