...
void CPDF_RenderContext::Render(CFX_RenderDevice* pDevice,
  ...
  for (auto& layer : m_Layers) {
    ...
    if (pLastMatrix) {
      ...
      status.Initialize(pLastMatrix, pStopObj, nullptr, nullptr, pOptions,
                        layer.m_pObjectHolder->GetTransparency(), false,
                        nullptr);
      status.RenderObjectList(layer.m_pObjectHolder.Get(), &FinalMatrix);
    } else {
      status.Initialize(nullptr, pStopObj, nullptr, nullptr, pOptions,
                        layer.m_pObjectHolder->GetTransparency(), false,
                        nullptr);
