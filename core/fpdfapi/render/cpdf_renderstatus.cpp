...
CFX_FillRenderOptions GetFillRenderOptionsForDrawTextPath(
  ...
  if (options.bNoTextSmooth)
    fill_options.is_path_aliased = true;

  return fill_options;
}

CFX_FillRenderOptions GetFillRenderOptionsForProcessPath(
    const CPDF_RenderOptions::Options& options,
...
bool CPDF_RenderStatus::ProcessPath(CPDF_PathObject* pPathObj,
  ...
  CFX_FillRenderOptions fill_options = GetFillRenderOptionsForProcessPath(
...
void CPDF_RenderStatus::ProcessClipPath(const CPDF_ClipPath& ClipPath,
  ...
  for (size_t i = 0; i < ClipPath.GetTextCount(); ++i) {
    ...
    CFX_FillRenderOptions fill_options(FXFILL_WINDING);
...
bool CPDF_RenderStatus::SelectClipPath(const CPDF_PathObject* pPathObj,
  ...
  CFX_FillRenderOptions fill_options(pPathObj->filltype());
...
bool CPDF_RenderStatus::ProcessText(CPDF_TextObject* textobj,
  ...
  if (bClip || bStroke) {
    ...
    CFX_FillRenderOptions fill_options(GetFillRenderOptionsForDrawTextPath(
        m_Options.GetOptions(), textobj, bStroke, bFill));
