...
#endif
  CompositeDIBitmap(bitmap_device.GetBitmap(), rect.left, rect.top, 0,
...
void CPDF_RenderStatus::DrawTilingPattern(CPDF_TilingPattern* pattern,
  ...
  CompositeDIBitmap(pScreen, clip_box.left, clip_box.top, 0, /*alpha=*/1.0f,
    ...
    if (!pDIBitmap->IsMaskFormat()) {
      if (alpha != 1.0f) {
        if (CFX_DefaultRenderDevice::UseSkiaRenderer()) {
          ...
          m_pDevice->StartDIBits(pDIBitmap, alpha, 0, m,
