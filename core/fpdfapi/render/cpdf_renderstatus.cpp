...
RetainPtr<CFX_DIBitmap> CPDF_RenderStatus::LoadSMask(
  ...
  int nCSFamily = 0;
  if (bLuminosity) {
    Optional<FX_ARGB> back_color =
        GetBackColor(pSMaskDict, pGroup->GetDict(), &nCSFamily);
    if (!back_color)
      return nullptr;
    bitmap.Clear(back_color.value());
  } else {
    bitmap.Clear(0);
  }
...
Optional<FX_ARGB> CPDF_RenderStatus::GetBackColor(
  ...
  *pCSFamily = pCS->GetFamily();
  ...
  uint32_t comps = std::max<uint32_t>(pCS->CountComponents(), 8);
  ...
  size_t count = std::min<size_t>(pBC->GetCount(), 8);
