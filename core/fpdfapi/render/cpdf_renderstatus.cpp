...
#endif
  CompositeToDevice(bitmap_device, rect.left, rect.top, 0, 255, blend_type,
                    transparency);
  return true;
}
...
void CPDF_RenderStatus::DrawTilingPattern(CPDF_TilingPattern* pattern,
  ...
  CompositeToDevice(bitmap_device, clip_box.left, clip_box.top, 0, 255,
  ...
  if (blend_mode == BlendMode::kNormal) {
    if (!pDIBitmap->IsMaskFormat()) {
      if (bitmap_alpha < 255) {
        ...
        device.MultiplyAlpha(bitmap_alpha);
      }
