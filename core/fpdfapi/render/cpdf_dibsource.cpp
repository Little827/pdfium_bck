...
std::unique_ptr<DIB_COMP_DATA, FxFreeDeleter>
CPDF_DIBSource::GetDecodeAndMaskArray(bool* bDefaultDecode, bool* bColorKey) {
  if (!m_pColorSpace)
    return nullptr;

  std::unique_ptr<DIB_COMP_DATA, FxFreeDeleter> pCompData(
  ...
  if (pDecode) {
    for (uint32_t i = 0; i < m_nComponents; i++) {
      pCompData.get()[i].m_DecodeMin = pDecode->GetNumberAt(i * 2);
      float max = pDecode->GetNumberAt(i * 2 + 1);
      pCompData.get()[i].m_DecodeStep =
          (max - pCompData.get()[i].m_DecodeMin) / max_data;
      float def_value;
      float def_min;
      float def_max;
      ...
      if (m_Family == PDFCS_INDEXED)
        def_max = max_data;
      if (def_min != pCompData.get()[i].m_DecodeMin || def_max != max)
        *bDefaultDecode = false;
    }
  } else {
    for (uint32_t i = 0; i < m_nComponents; i++) {
      float def_value;
      m_pColorSpace->GetDefaultValue(i, &def_value,
                                     &pCompData.get()[i].m_DecodeMin,
                                     &pCompData.get()[i].m_DecodeStep);
      if (m_Family == PDFCS_INDEXED)
        pCompData.get()[i].m_DecodeStep = max_data;
      pCompData.get()[i].m_DecodeStep =
          (pCompData.get()[i].m_DecodeStep - pCompData.get()[i].m_DecodeMin) /
          max_data;
    }
  }
  ...
  if (CPDF_Array* pArray = pMask->AsArray()) {
    if (pArray->GetCount() >= m_nComponents * 2) {
      for (uint32_t i = 0; i < m_nComponents; i++) {
        ...
        pCompData.get()[i].m_ColorKeyMin = std::max(min_num, 0);
        pCompData.get()[i].m_ColorKeyMax = std::min(max_num, max_data);
      }
    }
    *bColorKey = true;
  }
...
void CPDF_DIBSource::LoadPalette() {
  ...
  if (m_bpc * m_nComponents == 1) {
    ...
    color_values[0] = m_pCompData.get()[0].m_DecodeMin;
    ...
    FX_ARGB argb0 = ArgbEncode(255, FXSYS_round(R * 255), FXSYS_round(G * 255),
                               FXSYS_round(B * 255));
    color_values[0] += m_pCompData.get()[0].m_DecodeStep;
    color_values[1] += m_pCompData.get()[0].m_DecodeStep;
    color_values[2] += m_pCompData.get()[0].m_DecodeStep;
    m_pColorSpace->GetRGB(color_values, &R, &G, &B);
  ...
  for (int i = 0; i < palette_count; i++) {
    ...
    for (uint32_t j = 0; j < m_nComponents; j++) {
      ...
      color_data /= 1 << m_bpc;
      color_value[j] = m_pCompData.get()[j].m_DecodeMin +
                       m_pCompData.get()[j].m_DecodeStep * encoded_component;
    }
    float R = 0;
    float G = 0;
    float B = 0;
  ...
  for (int column = 0; column < m_Width; column++) {
    for (uint32_t color = 0; color < m_nComponents; color++) {
      if (bpp8) {
        ...
        color_values[color] = m_pCompData.get()[color].m_DecodeMin +
                              m_pCompData.get()[color].m_DecodeStep * data;
      } else {
...
const uint8_t* CPDF_DIBSource::GetScanline(int line) const {
  ...
  if (m_bpc * m_nComponents == 1) {
    ...
    if (m_pCompData.get()[0].m_ColorKeyMin == 0)
      reset_argb = 0;
    if (m_pCompData.get()[0].m_ColorKeyMax == 1)
      set_argb = 0;
  ...
  if (m_bpc * m_nComponents <= 8) {
    ...
    for (int col = 0; col < m_Width; col++) {
      ...
      *pDestPixel =
          IsColorIndexOutOfBounds(index, m_pCompData.get()[0]) ? 0xFF : 0;
      pDestPixel++;
    }
    return m_pMaskedLine.get();
  }
  if (m_bColorKey) {
    if (m_nComponents == 3 && m_bpc == 8) {
      ...
      for (int col = 0; col < m_Width; col++) {
        ...
        alpha_channel[col * 4] =
            AreColorIndicesOutOfBounds(pPixel, m_pCompData.get(), 3) ? 0xFF : 0;
      }
    } else {
...
void CPDF_DIBSource::DownSampleScanline8Bit(int orig_Bpp,
  ...
  if (m_bColorKey) {
    for (int i = 0; i < clip_width; i++) {
      ...
      *pDestPixel = (index < m_pCompData.get()[0].m_ColorKeyMin ||
                     index > m_pCompData.get()[0].m_ColorKeyMax)
                        ? 0xFF
                        : 0;
    }
    return;
  ...
  for (int i = 0; i < clip_width; i++) {
    ...
    if (src_x == last_src_x) {
        ...
        if (!m_bDefaultDecode) {
          for (uint32_t j = 0; j < m_nComponents; ++j) {
            ...
            int color_value = static_cast<int>(
                (m_pCompData.get()[j].m_DecodeMin +
                 m_pCompData.get()[j].m_DecodeStep * component_value) *
                    255.0f +
                0.5f);
      ...
      if (m_bColorKey) {
        int alpha = 0xFF;
        if (m_nComponents == 3 && m_bpc == 8) {
          alpha = (pSrcPixel[0] < m_pCompData.get()[0].m_ColorKeyMin ||
                   pSrcPixel[0] > m_pCompData.get()[0].m_ColorKeyMax ||
                   pSrcPixel[1] < m_pCompData.get()[1].m_ColorKeyMin ||
                   pSrcPixel[1] > m_pCompData.get()[1].m_ColorKeyMax ||
                   pSrcPixel[2] < m_pCompData.get()[2].m_ColorKeyMin ||
                   pSrcPixel[2] > m_pCompData.get()[2].m_ColorKeyMax)
                      ? 0xFF
                      : 0;
        }
        argb &= 0xFFFFFF;
