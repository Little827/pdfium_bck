...
int DetectFirstScan(const RetainPtr<CFX_DIBitmap>& pBitmap) {
  ...
  for (int line = 0; line < height; ++line) {
    const uint8_t* pBuf = pBitmap->GetWritableScanline(line).data();
...
int DetectLastScan(const RetainPtr<CFX_DIBitmap>& pBitmap) {
  ...
  for (int line = height - 1; line >= 0; --line) {
    pdfium::span<const uint8_t> pBuf = pBitmap->GetScanline(line);
    if (IsScanLineBpp(bpp, pBuf.data(), pBuf.size()))
      return line;
  }
  return -1;
}
