...
TEST_F(FPDFProgressiveRenderEmbedderTest, RenderHighlightWithColorScheme) {
  // TODO(crbug.com/pdfium/1500): When Skia is enabled, the highlighted area is
...
TEST_F(FPDFProgressiveRenderEmbedderTest,
  ...
  if (CFX_DefaultRenderDevice::SkiaIsDefaultRenderer())
    ...
    //
    // Note: The stroke color rendered for highlight is different from the
    // normal path since highlights have Multiply blend mode, while the other
    // path has Normal blend mode.
#if BUILDFLAG(IS_APPLE)
