...
bool CPDF_ImageRenderer::StartRenderDIBBase() {
  ...
  m_RenderOptions = FXDIB_RenderOptions();
...
void CPDF_ImageRenderer::CalculateDrawImage(
  ...
  if (image_render.Start(&bitmap_render, pDIBBase, 0xffffffff, 255, pNewMatrix,
                         m_RenderOptions, true, BlendMode::kNormal)) {
...
bool CPDF_ImageRenderer::StartDIBBase() {
  if (!m_RenderOptions.bInterpolateDownsample && m_pDIBBase->GetBPP() > 1) {
  ...
  if (m_pRenderStatus->GetRenderDevice()->StartDIBitsWithBlend(
          ...
          m_RenderOptions, &m_DeviceHandle, m_BlendType)) {
    if (m_DeviceHandle) {
      m_Status = 3;
      return true;
    }
...
#else
  if (m_pRenderStatus->GetRenderDevice()->StartDIBitsWithBlend(
          m_pDIBBase, m_BitmapAlpha, m_FillArgb, m_ImageMatrix, m_RenderOptions,
  ...
  if ((fabs(m_ImageMatrix.b) >= 0.5f || m_ImageMatrix.a == 0) ||
    ...
    m_pTransformer = pdfium::MakeUnique<CFX_ImageTransformer>(
        m_pDIBBase, m_ImageMatrix, m_RenderOptions, &clip_box);
    return true;
  }
  ...
  RetainPtr<CFX_DIBitmap> pStretched = m_pDIBBase->StretchTo(
      dest_width, dest_height, m_RenderOptions, &dest_clip);
  if (pStretched) {
