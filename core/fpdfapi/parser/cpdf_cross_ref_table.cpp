...
void CPDF_CrossRefTable::AddNormal(uint32_t obj_num,
  ...
  if (absl::holds_alternative<ObjectInfo::Compressed>(info.vary) &&
      gen_num == 0) {
    return;
  }
  info.gennum = gen_num;
...
void CPDF_CrossRefTable::UpdateInfo(
  ...
  while (cur_it != objects_info_.end() && new_it != new_objects_info.end()) {
    if (cur_it->first == new_it->first) {
      if (absl::holds_alternative<ObjectInfo::Normal>(cur_it->second.vary) &&
          absl::holds_alternative<ObjectInfo::Normal>(new_it->second.vary) &&
