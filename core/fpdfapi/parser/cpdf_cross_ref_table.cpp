...
void CPDF_CrossRefTable::AddCompressed(uint32_t obj_num,
  ...
  auto& archive_info = objects_info_[archive_obj_num];
  if (archive_info.type != ObjectType::kFree)
    archive_info.type = ObjectType::kObjStream;
}
...
void CPDF_CrossRefTable::UpdateTrailer(
  ...
  if (auto xref_stream = trailer_->RemoveFor("XRefStm")) {
    new_trailer->SetFor("XRefStm", std::move(xref_stream));
  }
  if (auto prev_trailer = trailer_->RemoveFor("Prev")) {
    new_trailer->SetFor("Prev", std::move(prev_trailer));
  }

  for (auto it = trailer_->begin(); it != trailer_->end();) {
    const auto key = it->first;
    ++it;
    new_trailer->SetFor(key, trailer_->RemoveFor(key));
  }
  trailer_ = std::move(new_trailer);
}
