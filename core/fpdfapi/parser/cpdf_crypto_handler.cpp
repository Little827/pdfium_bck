  ...
  if (m_Cipher == FXCIPHER_AES) {
    ...
    if (bEncrypt) {
      ...
      CRYPT_AESEncrypt(m_pAESContext.get(), dest_buf + 16, source.data(),
                       nblocks * 16);
      uint8_t padding[16];
      memcpy(padding, &source[nblocks * 16], source.size() % 16);
      ...
      CRYPT_AESDecrypt(m_pAESContext.get(), dest_buf, &source[16],
                       source.size() - 16);
...
bool CPDF_CryptoHandler::CryptStream(void* context,
  ...
  while (1) {
    ...
    memcpy(pContext->m_Block + pContext->m_BlockOffset, &source[src_off],
           copy_size);
    src_off += copy_size;
    src_left -= copy_size;
