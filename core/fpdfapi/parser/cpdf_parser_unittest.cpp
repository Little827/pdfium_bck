...
TEST(cpdf_parser, XrefHasInvalidArchiveObjectNumber) {
  ...
  // Found by following the startxref offset. No other objects besides this and
  // the placeholder, because CPDF_Parser currently stops parsing altogether
  // when it encounters an invalid archive object number.
  auto first_object_it = objects_info.find(7);
  ASSERT_NE(first_object_it, objects_info.end());
  EXPECT_EQ(CPDF_Parser::ObjectType::kNormal, first_object_it->second.type);
