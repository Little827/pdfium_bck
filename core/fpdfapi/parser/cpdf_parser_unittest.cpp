...
TEST(ParserTest, LoadCrossRefTable) {
  {
    ...
    static constexpr auto kExpecteds = fxcrt::ToArray<OffsetAndType>({
    ...
    for (size_t i = 0; i < std::size(kExpecteds); ++i) {
      EXPECT_EQ(kExpecteds[i].offset, GetObjInfo(parser, i).pos);
      EXPECT_EQ(kExpecteds[i].type, GetObjInfo(parser, i).type);
    }
  }
  {
    static const unsigned char kXrefTable[] =
    ...
    ASSERT_TRUE(parser.LoadCrossRefTable(/*pos=*/0, /*skip=*/false));
    static constexpr std::array<FX_FILESIZE, 13> kOffsets = {
        {0, 0, 0, 25325, 0, 0, 0, 0, 25518, 25635, 0, 0, 25777}};
    static constexpr std::array<CPDF_CrossRefTable::ObjectType, 13> kTypes = {{
