...
bool CPDF_Document::IsObjValidPage(uint32_t objnum) {
  if (!objnum)
    return false;

  CPDF_Dictionary* dict = ToDictionary(GetOrParseIndirectObject(objnum));
  if (!dict)
    return false;

  const CPDF_Object* type = dict->GetObjectFor("Type");
  if (type && type->IsName() && type->GetString() == "Page")
    return true;
