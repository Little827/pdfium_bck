...
bool CPDF_SecurityHandler::CheckUserPassword(const ByteString& password,
  ...
  if (m_Revision == 2) {
    ...
    CRYPT_ArcFourCryptBlock(ukeybuf, pdfium::make_span(m_EncryptKey, m_KeyLen));
  ...
  for (int32_t i = 19; i >= 0; i--) {
    ...
    CRYPT_ArcFourCryptBlock(test, pdfium::make_span(tmpkey, m_KeyLen));
  }
  CRYPT_md5_context md5;
  CRYPT_MD5Start(&md5);
...
ByteString CPDF_SecurityHandler::GetUserPassword(
  ...
  pdfium::span<uint8_t> okey_span = pdfium::make_span(okeybuf, okeylen);
  if (m_Revision == 2) {
    CRYPT_ArcFourCryptBlock(okey_span, pdfium::make_span(enckey, m_KeyLen));
  } else {
    for (int32_t i = 19; i >= 0; i--) {
      uint8_t tempkey[32] = {};
      for (int j = 0; j < m_KeyLen; j++)
        tempkey[j] = enckey[j] ^ static_cast<uint8_t>(i);
      CRYPT_ArcFourCryptBlock(okey_span, pdfium::make_span(tempkey, m_KeyLen));
    }
  }
  size_t len = 32;
...
void CPDF_SecurityHandler::OnCreateInternal(CPDF_Dictionary* pEncryptDict,
  ...
  if (bDefault) {
    ...
    CRYPT_ArcFourCryptBlock(passcode, pdfium::make_span(enckey, key_len));
    uint8_t tempkey[32];
    if (m_Revision >= 3) {
      for (uint8_t i = 1; i <= 19; i++) {
        for (int j = 0; j < key_len; j++)
          tempkey[j] = enckey[j] ^ i;
        CRYPT_ArcFourCryptBlock(passcode, pdfium::make_span(tempkey, key_len));
      }
    }
  ...
  if (m_Revision < 3) {
    ...
    CRYPT_ArcFourCryptBlock(tempbuf, pdfium::make_span(m_EncryptKey, key_len));
    ...
    uint8_t digest[32];
    CRYPT_MD5Finish(&md5, digest);
    pdfium::span<uint8_t> partial_digest_span = pdfium::make_span(digest, 16);
    CRYPT_ArcFourCryptBlock(partial_digest_span,
                            pdfium::make_span(m_EncryptKey, key_len));
    uint8_t tempkey[32];
    for (uint8_t i = 1; i <= 19; i++) {
      for (int j = 0; j < key_len; j++)
        tempkey[j] = m_EncryptKey[j] ^ i;
      CRYPT_ArcFourCryptBlock(partial_digest_span,
                              pdfium::make_span(tempkey, key_len));
    }
