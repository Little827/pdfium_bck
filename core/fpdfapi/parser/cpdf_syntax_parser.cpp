...
class ReadableSubStream final : public IFX_SeekableReadStream {
 public:
  ...
  bool ReadBlockWithOffset(void* buffer,
                           FX_FILESIZE offset,
    ...
    return m_pFileRead->ReadBlockWithOffset(buffer, m_PartOffset + offset,
                                            size);
  }
...
bool CPDF_SyntaxParser::ReadBlockAt(FX_FILESIZE read_pos) {
  ...
  if (!m_pFileAccess->ReadBlockWithOffset(m_pFileBuf.data(), read_pos,
                                          read_size)) {
    m_pFileBuf.clear();
    return false;
  }
...
bool CPDF_SyntaxParser::ReadBlock(uint8_t* pBuf, uint32_t size) {
  if (!m_pFileAccess->ReadBlockWithOffset(pBuf, m_Pos + m_HeaderOffset, size))
    return false;
  m_Pos += size;
  return true;
}
