...
bool CPDF_SyntaxParser::GetNextWordInternal() {
  m_WordSize = 0;
  bool bIsNumber = true;
...
CPDF_SyntaxParser::WordResult CPDF_SyntaxParser::GetNextWord() {
  ...
  bool is_number = GetNextWordInternal();
  ByteString word;
  if (!GetValidator()->has_read_problems())
    word = ByteString(m_WordBuffer, m_WordSize);
  return {word, is_number};
}
...
RetainPtr<CPDF_Object> CPDF_SyntaxParser::GetIndirectObject(
  ...
  uint32_t parser_objnum;
  {
    WordResult word_result = GetNextWord();
    const ByteString& word = word_result.word;
    if (!word_result.is_number || word.IsEmpty()) {
      SetPos(saved_pos);
      return nullptr;
    }
    parser_objnum = FXSYS_atoui(word.c_str());
  }

  uint32_t parser_gennum;
  {
    WordResult word_result = GetNextWord();
    const ByteString& word = word_result.word;
    if (!word_result.is_number || word.IsEmpty()) {
    ...
    parser_gennum = FXSYS_atoui(word.c_str());
  }
...
uint32_t CPDF_SyntaxParser::GetDirectNum() {
  bool bIsNumber = GetNextWordInternal();
  if (!bIsNumber)
    return 0;
