...
bool CPDF_SyntaxParser::GetCharAt(FX_FILESIZE pos, uint8_t& ch) {
  ...
  if (doLog) std::cerr << "01 m_Pos will be restored to " << m_Pos << std::endl;
  m_Pos = pos;
  if (doLog) std::cerr << "02 m_Pos was set to " << m_Pos << std::endl;
...
bool CPDF_SyntaxParser::GetNextChar(uint8_t& ch) {
  ...
  m_Pos++;
  if (doLog) std::cerr << "03 m_Pos was set to " << m_Pos << std::endl;
  return true;
}
...
bool CPDF_SyntaxParser::ReadBlock(uint8_t* pBuf, uint32_t size) {
  if (!m_pFileAccess->ReadBlock(pBuf, m_Pos + m_HeaderOffset, size))
    return false;
  m_Pos += size;
  if (doLog) std::cerr << "04 m_Pos was set to " << m_Pos << std::endl;
  return true;
}
...
void CPDF_SyntaxParser::GetNextWordInternal(bool* bIsNumber) {
    ...
    if (ch == '/') {
      while (1) {
        ...
        if (!PDFCharIsOther(ch) && !PDFCharIsNumeric(ch)) {
          m_Pos--;
  if (doLog) std::cerr << "05 m_Pos was set to " << m_Pos << std::endl;
          return;
        }
      ...
      if (ch == '<')
        ...
        m_Pos--;
  if (doLog) std::cerr << "06 m_Pos was set to " << m_Pos << std::endl;
      }
    } else if (ch == '>') {
      if (!GetNextChar(ch))
        return;
      ...
      if (ch == '>')
        ...
        m_Pos--;
  if (doLog) std::cerr << "07 m_Pos was set to " << m_Pos << std::endl;
      }
    }
    return;
  }
  ...
  while (1) {
    ...
    if (PDFCharIsDelimiter(ch) || PDFCharIsWhitespace(ch)) {
      m_Pos--;
  if (doLog) std::cerr << "08 m_Pos was set to " << m_Pos << std::endl;
      break;
    }
  }
}
...
void CPDF_SyntaxParser::ToNextLine() {
  ...
  while (GetNextChar(ch)) {
    ...
    if (ch == '\r') {
      ...
      if (ch != '\n') {
        --m_Pos;
  if (doLog) std::cerr << "09 m_Pos was set to " << m_Pos << std::endl;
      }
      break;
    }
  }
...
void CPDF_SyntaxParser::ToNextWord() {
  ...
  m_Pos--;
  if (doLog) std::cerr << "10 m_Pos was set to " << m_Pos << std::endl;
}
...
ByteString CPDF_SyntaxParser::PeekNextWord(bool* bIsNumber) {
  AutoRestorer<FX_FILESIZE> save_pos(&m_Pos);
  if (doLog) std::cerr << "15 m_Pos will be restored to " << m_Pos << std::endl;
...
std::unique_ptr<CPDF_Object> CPDF_SyntaxParser::GetObjectBodyInternal(
  ...
  if (doLog) std::cerr << "16 m_Pos will be restored to " << m_Pos << std::endl;
  ...
  if (word == "<<") {
    ...
    while (1) {
      ...
      if (inner_word == "endobj") {
        m_Pos = SavedPos;
  if (doLog) std::cerr << "11 m_Pos was set to " << m_Pos << std::endl;
        break;
      }
      if (inner_word[0] != '/')
        continue;
    ...
    AutoRestorer<FX_FILESIZE> pos_restorer(&m_Pos);
  if (doLog) std::cerr << "17 m_Pos will be restored to " << m_Pos << std::endl;
  ...
  if (word == ">>") {
    m_Pos = SavedObjPos;
  if (doLog) std::cerr << "12 m_Pos was set to " << m_Pos << std::endl;
  }
...
FX_FILESIZE CPDF_SyntaxParser::FindWordPos(const ByteStringView& word) {
  AutoRestorer<FX_FILESIZE> pos_restorer(&m_Pos);
  if (doLog) std::cerr << "18 m_Pos will be restored to " << m_Pos << std::endl;
...
std::unique_ptr<CPDF_Stream> CPDF_SyntaxParser::ReadStream(
  ...
  if (doLog) std::cerr << "13 m_Pos was set to " << m_Pos << std::endl;
  ...
  while (1) {
    ...
    if (byte == tag[offset]) {
      ...
      if (IsWholeWord(pos, limit, tag, false)) {
        m_Pos = pos;
  if (doLog) std::cerr << "14 m_Pos was set to " << m_Pos << std::endl;
        return true;
      }
    }
