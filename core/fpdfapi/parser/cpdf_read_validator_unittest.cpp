...
TEST(CPDF_ReadValidatorTest, UnavailableData) {
  ...
  EXPECT_FALSE(validator->ReadBlockWithOffset(read_buffer.data(), 5000,
                                              read_buffer.size()));
  ...
  EXPECT_TRUE(validator->ReadBlockWithOffset(read_buffer.data(), 5000,
                                             read_buffer.size()));
  EXPECT_FALSE(validator->read_error());
...
TEST(CPDF_ReadValidatorTest, UnavailableDataWithHints) {
  ...
  EXPECT_FALSE(validator->ReadBlockWithOffset(
...
TEST(CPDF_ReadValidatorTest, ReadError) {
  ...
  EXPECT_FALSE(validator->ReadBlockWithOffset(buffer.data(), 5000, 100));
...
TEST(CPDF_ReadValidatorTest, Session) {
  ...
  // Data is unavailable
  validator->ReadBlockWithOffset(test_data.data(), 0, 100);
