...
RetainPtr<CPDF_ReadValidator> MakeValidatorFromFile(
  ...
  ASSERT(PathService::GetTestFilePath(file_name, &file_path));
...
class MemoryAccess : public IFX_SeekableReadStream {
 public:
 ...
 private:
  MemoryAccess(const char* buffer, size_t size)
...
class TestLinearizedHeader : public CPDF_LinearizedHeader {
 public:
  ...
  static std::unique_ptr<CPDF_LinearizedHeader> MakeHeader(
    ...
    parser.InitParser(pdfium::MakeRetain<MemoryAccess>(inline_data.data(),
                                                       inline_data.size()),
                      0);
...
TEST_F(CPDF_HintTablesTest, PageAndGroupInfos) {
  ...
  EXPECT_EQ(1u, hint_tables->SharedGroupInfos()[0].m_nObjectsCount);
  ...
  EXPECT_EQ(1u, hint_tables->SharedGroupInfos()[1].m_nObjectsCount);
  ...
  EXPECT_EQ(1u, hint_tables->SharedGroupInfos()[2].m_nObjectsCount);
  ...
  EXPECT_EQ(1u, hint_tables->SharedGroupInfos()[3].m_nObjectsCount);
  ...
  EXPECT_EQ(1u, hint_tables->SharedGroupInfos()[4].m_nObjectsCount);
  ...
  EXPECT_EQ(1u, hint_tables->SharedGroupInfos()[5].m_nObjectsCount);
}

TEST_F(CPDF_HintTablesTest, FirstPageOffset) {
  // Test that valid hint table is loaded, and have correct {} offsets.
  ...
  // This hint table is exctracted from linearized file, generated by qpdf tool.
  ...
  // Check that hint table have correct firts page offset.
