...
fxcodec::CodecDecodeResult RunLengthDecode(
    pdfium::span<const uint8_t> src_span);

fxcodec::CodecDecodeResult A85Decode(pdfium::span<const uint8_t> src_span);

fxcodec::CodecDecodeResult HexDecode(pdfium::span<const uint8_t> src_span);
...
struct DataDecodeResult {
  DataDecodeResult();
  DataDecodeResult(std::unique_ptr<uint8_t, FxFreeDeleter> data,
                   uint32_t size,
                   ByteString image_encoding,
                   RetainPtr<const CPDF_Dictionary> image_params);
  DataDecodeResult(DataDecodeResult&& that) noexcept;
  DataDecodeResult& operator=(DataDecodeResult&& that) noexcept;
  ~DataDecodeResult();
