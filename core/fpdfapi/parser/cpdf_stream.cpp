...
void CPDF_Stream::InitStreamFromFile(RetainPtr<IFX_SeekableReadStream> pFile,
  ...
  data_.emplace<FileStream>(std::move(pFile), size);
  dict_ = std::move(pDict);
...
void CPDF_Stream::SetData(pdfium::span<const uint8_t> pData) {
  DataVector<uint8_t> data_copy;
  if (!pData.empty()) {
    data_copy.resize(pData.size());
...
void CPDF_Stream::TakeData(DataVector<uint8_t> data) {
  ...
  data_.emplace<DataVector<uint8_t>>(std::move(data));
  if (!dict_)
...
bool CPDF_Stream::ReadRawData(FX_FILESIZE offset,
                              ...
                              size_t size) const {
  CHECK(IsFileBased());
  return absl::get<FileStream>(data_).file->ReadBlockAtOffset(buf, offset,
                                                              size);
}
