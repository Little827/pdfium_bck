...
class TraversalContext {
 public:
  explicit TraversalContext(const CPDF_Dictionary* root)
      : object_queue_({pdfium::WrapRetain(root)}), seen_objects_({root}) {
    const uint32_t root_object_number = root->GetObjNum();
  ...
  const CPDF_Object* PopCurrentObject() {
    if (object_queue_.empty()) {
      return nullptr;
    }

    const CPDF_Object* current_object = object_queue_.front();
    object_queue_.pop();
    return current_object;
  }

  void PushNewObject(const CPDF_Object* parent_object,
                     const CPDF_Object* child_object) {
    CHECK(parent_object);
    CHECK(child_object);
    ...
    object_queue_.push(child_object);
  }
 ...
 private:
  ...
  std::queue<const CPDF_Object*> object_queue_;
...
std::map<uint32_t, int> GetObjectsReferenceMap(const CPDF_Document* document) {
  // Set of objects that referenced other objects.
  std::set<uint32_t> seen_ref_objects;
  // Map of object number to number of times referenced.
  std::map<uint32_t, int> referenced_objects;

  const CPDF_Parser* parser = document->GetParser();
  const CPDF_Dictionary* trailer = parser ? parser->GetTrailer() : nullptr;
  const CPDF_Dictionary* root = trailer ? trailer : document->GetRoot();
  const uint32_t root_object_number =
      trailer ? parser->GetTrailerObjectNumber() : root->GetObjNum();
  // If `root` is a trailer, then it may not have an object number, as many
  // trailers are inlined.
  if (root_object_number) {
    referenced_objects[root_object_number] = 1;
  }

  TraversalContext context(root);
  while (const CPDF_Object* current_object = context.PopCurrentObject()) {
    switch (current_object->GetType()) {
      case CPDF_Object::kArray: {
        CPDF_ArrayLocker locker(current_object->AsArray());
        for (const auto& it : locker) {
          context.PushNewObject(current_object, it.Get());
        }
        break;
      }
      case CPDF_Object::kDictionary: {
        ...
        for (const auto& it : locker) {
          context.PushNewObject(current_object, it.second.Get());
        }
        break;
      }
      case CPDF_Object::kReference: {
        ...
        if (ref_object->HasIndirectObjectHolder()) {
          ...
          referenced_object =
              document->GetIndirectObject(referenced_object_number);
        }
        // Unlike the other object types, CPDF_Reference can point at nullptr.
        if (referenced_object) {
          // Make sure this is not a self-reference / circular reference.
          const uint32_t ref_object_number =
              context.GetObjectNumber(ref_object);
          if (referenced_object_number != ref_object_number &&
              (!pdfium::Contains(seen_ref_objects, ref_object_number) ||
               !pdfium::Contains(seen_ref_objects, referenced_object_number))) {
            ++referenced_objects[referenced_object_number];
            seen_ref_objects.insert(ref_object_number);
          }
          context.PushNewObject(ref_object, referenced_object);
        }
        break;
      }
...
std::set<uint32_t> GetObjectsWithReferences(const CPDF_Document* document) {
  ...
  std::map<uint32_t, int> referenced_objects = GetObjectsReferenceMap(document);
  for (const auto& it : referenced_objects) {
