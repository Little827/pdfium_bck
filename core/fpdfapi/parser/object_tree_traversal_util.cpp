...
class ObjectTreeTraverser {
 public:
  ...
  void Traverse() {
    while (const CPDF_Object* current_object = PopCurrentObject()) {
      switch (current_object->GetType()) {
        case CPDF_Object::kArray: {
          CPDF_ArrayLocker locker(current_object->AsArray());
          for (const auto& it : locker) {
            PushNewObject(current_object, it.Get());
          }
          break;
        }
        case CPDF_Object::kDictionary: {
          ...
          for (const auto& it : locker) {
            PushNewObject(current_object, it.second.Get());
          }
          break;
        }
        case CPDF_Object::kReference: {
          ...
          if (referenced_object) {
            ...
            if (referenced_object_number != ref_object_number &&
                (!pdfium::Contains(seen_ref_objects_, ref_object_number) ||
                 !pdfium::Contains(seen_ref_objects_,
 ...
 private:
  ...
  std::set<uint32_t> seen_ref_objects_;
