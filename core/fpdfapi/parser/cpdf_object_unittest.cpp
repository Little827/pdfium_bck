...
class PDFObjectsTest : public testing::Test {
 public:
  void SetUp() override {
    ...
    // Stream object.
    static constexpr uint8_t kContents[] = "abcdefghijklmnopqrstuvwxyz";
    DataVector<uint8_t> buf(std::begin(kContents), std::end(kContents));
    ...
    auto stream_obj =
        pdfium::MakeRetain<CPDF_Stream>(std::move(buf), std::move(pNewDict));
    // Null Object.
...
TEST(PDFArrayTest, GetTypeAt) {
  ...
  }
  {
    ...
    for (size_t i = 0; i < 3; ++i) {
      ...
      DataVector<uint8_t> data(std::begin(kContents), std::end(kContents));
      stream_vals[i] = arr->AppendNew<CPDF_Stream>(std::move(data), vals[i]);
    }
    for (size_t i = 0; i < 3; ++i) {
      TestArrayAccessors(arr.Get(), i,           // Array and index.
                         "",                     // String value.
  ...
  }
  {
    ...
    DataVector<uint8_t> buf(std::begin(kData), std::end(kData));
    CPDF_Stream* stream_val =
        arr->InsertNewAt<CPDF_Stream>(13, std::move(buf), stream_dict);
    const char* const expected_str[] = {
...
TEST(PDFStreamTest, LengthInDictionaryOnCreate) {
  ...
  {
    DataVector<uint8_t> data(kBufSize);
    auto stream = pdfium::MakeRetain<CPDF_Stream>(
        std::move(data), pdfium::MakeRetain<CPDF_Dictionary>());
...
TEST(PDFObjectTest, CloneCheckLoop) {
  ...
  }
  {
    ...
    auto stream_obj = dict_obj->SetNewFor<CPDF_Stream>(
        "stream", std::move(empty_data), dict_obj);
    // Clone this object to see whether stack overflow will be triggered.
