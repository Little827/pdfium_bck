...
TEST(PDFArrayTest, GetTypeAt) {
  ...
  }
  {
    ...
    for (size_t i = 0; i < 3; ++i) {
      ...
      std::vector<uint8_t, FxAllocAllocator<uint8_t>> data(
          std::begin(kContents), std::end(kContents));
  ...
  }
  {
    ...
    std::vector<uint8_t, FxAllocAllocator<uint8_t>> buf(std::begin(kData),
                                                        std::end(kData));
    CPDF_Stream* stream_val =
...
TEST(PDFStreamTest, LengthInDictionaryOnCreate) {
  ...
  {
    std::vector<uint8_t, FxAllocAllocator<uint8_t>> data(kBufSize);
    auto stream = pdfium::MakeRetain<CPDF_Stream>(
...
TEST(PDFObjectTest, CloneCheckLoop) {
  ...
  }
  {
    ...
    std::vector<uint8_t, FxAllocAllocator<uint8_t>> empty_data;
