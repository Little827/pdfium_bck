...
#ifndef CORE_FPDFAPI_PARSER_CPDF_OBJ_STREAM_H_
#define CORE_FPDFAPI_PARSER_CPDF_OBJ_STREAM_H_
...
class CPDF_ObjStream {
 public:
  ...
  static std::unique_ptr<CPDF_ObjStream> Create(
      std::unique_ptr<CPDF_Stream> stream);

  virtual ~CPDF_ObjStream();
  ...
  std::vector<std::unique_ptr<CPDF_Object>> ParseAllObjects(
      CPDF_IndirectObjectHolder* pObjList) const;

 protected:
  explicit CPDF_ObjStream(std::unique_ptr<CPDF_Stream> obj_stream);

  void Init(std::unique_ptr<CPDF_Stream> stream);
...
#endif  // CORE_FPDFAPI_PARSER_CPDF_OBJ_STREAM_H_
