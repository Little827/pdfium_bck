...
CodecDecodeResult A85Decode(pdfium::span<const uint8_t> src_span) {
  if (src_span.empty()) {
...
CodecDecodeResult HexDecode(pdfium::span<const uint8_t> src_span) {
  if (src_span.empty()) {
    return {nullptr, 0u, 0u};
  }
...
CodecDecodeResult RunLengthDecode(pdfium::span<const uint8_t> src_span) {
  uint32_t dest_size = 0;
  size_t i = 0;
...
DataDecodeResult::DataDecodeResult() = default;

DataDecodeResult::DataDecodeResult(
...
DataDecodeResult::DataDecodeResult(DataDecodeResult&& that) noexcept = default;

DataDecodeResult& DataDecodeResult::operator=(
    DataDecodeResult&& that) noexcept = default;

DataDecodeResult::~DataDecodeResult() = default;

std::optional<DataDecodeResult> PDF_DataDecode(
    pdfium::span<const uint8_t> src_span,
  ...
  // below `result.data` and let it get destroyed first.
  ...
  for (size_t i = 0; i < nSize; ++i) {
    ...
    if (decoder == "FlateDecode" || decoder == "Fl") {
      ...
      CodecDecodeResult decode_result = A85Decode(last_span);
      new_buf = std::move(decode_result.data);
      new_size = decode_result.size;
      offset = decode_result.offset;
    } else if (decoder == "ASCIIHexDecode" || decoder == "AHx") {
      CodecDecodeResult decode_result = HexDecode(last_span);
      new_buf = std::move(decode_result.data);
      new_size = decode_result.size;
      offset = decode_result.offset;
      ...
      CodecDecodeResult decode_result = RunLengthDecode(last_span);
      new_buf = std::move(decode_result.data);
      new_size = decode_result.size;
      offset = decode_result.offset;
    } else {
