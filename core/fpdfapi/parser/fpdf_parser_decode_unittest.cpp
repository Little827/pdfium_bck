...
TEST(ParserDecodeTest, A85Decode) {
  ...
  for (const auto& test_case : kTestData) {
    CodecDecodeResult result = A85Decode(UNSAFE_BUFFERS(
        pdfium::make_span(test_case.input, test_case.input_size)));
    EXPECT_EQ(test_case.processed_size, result.offset)
...
TEST(ParserDecodeTest, HexDecode) {
  ...
  for (const auto& test_case : kTestData) {
    CodecDecodeResult result = HexDecode(UNSAFE_BUFFERS(
        pdfium::make_span(test_case.input, test_case.input_size)));
    EXPECT_EQ(test_case.processed_size, result.offset)
        << "for case " << test_case.input;
    ASSERT_EQ(test_case.expected_size, result.size);
