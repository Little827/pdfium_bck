...
CPDF_DataAvail::DocAvailStatus CPDF_DataAvail::CheckLinearizedData() {
  ...
  if (!m_bMainXRefLoadTried) {
    ...
    const FX_SAFE_FILESIZE xrefstm_offset =
        m_pDocument->GetParser()->GetTrailer()->GetIntegerFor("XRefStm");
    if (!xrefstm_offset.IsValid())
      return DataError;

    if (xrefstm_offset.ValueOrDie() == 0)
      return DataAvailable;

    data_size = m_dwFileLen;
    data_size -= xrefstm_offset.ValueOrDie();
    if (!data_size.IsValid())
      return DataError;

    if (!GetValidator()->CheckDataRangeAndRequestIfUnavailable(
            xrefstm_offset.ValueOrDie(), data_size.ValueOrDie()))
      return DataNotAvailable;
