...
TEST(cpdf_array, Find) {
  ...
  ASSERT_TRUE(maybe_found.has_value());
  EXPECT_EQ(2u, maybe_found.value());

  maybe_found = arr->Find(dict2.Get());
  EXPECT_FALSE(maybe_found.has_value());

  maybe_found = arr->FindNonNull(nullptr);
  EXPECT_FALSE(maybe_found.has_value());

  maybe_found = arr->FindNonNull(dict0.Get());
  ASSERT_TRUE(maybe_found.has_value());
  EXPECT_EQ(1u, maybe_found.value());

  maybe_found = arr->FindNonNull(dict1.Get());
  ASSERT_TRUE(maybe_found.has_value());
  EXPECT_EQ(2u, maybe_found.value());

  maybe_found = arr->FindNonNull(dict2.Get());
  EXPECT_FALSE(maybe_found.has_value());
}
...
TEST(cpdf_array, Contains) {
  ...
  EXPECT_FALSE(arr->ContainsNonNull(nullptr));
  EXPECT_TRUE(arr->ContainsNonNull(dict0.Get()));
  EXPECT_TRUE(arr->ContainsNonNull(dict1.Get()));
  EXPECT_FALSE(arr->ContainsNonNull(dict2.Get()));
}
