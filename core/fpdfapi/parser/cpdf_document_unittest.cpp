...
RetainPtr<CPDF_Dictionary> CreatePageTreeNode(RetainPtr<CPDF_Array> kids,
                                              CPDF_Document* pDoc,
                                              int count) {
  uint32_t kids_objnum = pDoc->AddIndirectObject(std::move(kids));
  RetainPtr<CPDF_Array> pArray =
      ToArray(pDoc->GetMutableIndirectObject(kids_objnum));

  auto pageNode = pDoc->NewIndirect<CPDF_Dictionary>();
  pageNode->SetNewFor<CPDF_Name>("Type", "Pages");
  pageNode->SetNewFor<CPDF_Reference>("Kids", pDoc, kids_objnum);
  pageNode->SetNewFor<CPDF_Number>("Count", count);
  for (size_t i = 0; i < pArray->size(); i++) {
    pArray->GetMutableDictAt(i)->SetNewFor<CPDF_Reference>(
...
TEST_F(DocumentTest, IsValidPageObject) {
  ...
  EXPECT_TRUE(CPDF_Document::IsValidPageObject(document.GetIndirectObject(
  ...
  EXPECT_FALSE(CPDF_Document::IsValidPageObject(document.GetIndirectObject(
...
TEST_F(DocumentTest, UseCachedPageObjNumIfHaveNotPagesDict) {
  ...
  {
    int first_page_obj_num = document.AddIndirectObject(CreateNumberedPage(0));
