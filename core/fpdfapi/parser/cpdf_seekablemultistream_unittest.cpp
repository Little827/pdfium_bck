...
TEST(CPDFSeekableMultiStreamTest, NoStreams) {
  ...
  auto fileread =
      fxcrt::MakeRetain<CPDF_SeekableMultiStream>(std::move(streams));
...
TEST(CXFAFileReadTest, EmptyStreams) {
  std::vector<RetainPtr<const CPDF_Stream>> streams;
  streams.push_back(fxcrt::MakeRetain<CPDF_Stream>());
  auto fileread =
      fxcrt::MakeRetain<CPDF_SeekableMultiStream>(std::move(streams));
