...
std::unique_ptr<CPDF_ObjStream> CPDF_ObjStream::Create(
  ...
  if (!IsObjectsStreamObject(stream))
    return nullptr;
  // The ctor of CPDF_ObjStream is protected. Use WrapUnique instead MakeUnique.
  return pdfium::WrapUnique(new CPDF_ObjStream(stream));
}

CPDF_ObjStream::CPDF_ObjStream(const CPDF_Stream* obj_stream) {
  DCHECK(obj_stream);
...
void CPDF_ObjStream::Init(const CPDF_Stream* stream) {
