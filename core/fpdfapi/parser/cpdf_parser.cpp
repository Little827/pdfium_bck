...
bool IsObjectInfoFree(const CPDF_Parser::ObjectInfo* info) {
  return !info ||
         absl::holds_alternative<CPDF_Parser::ObjectInfo::Free>(info->vary);
}

bool IsObjectInfoNormal(const CPDF_Parser::ObjectInfo* info) {
  return info &&
         absl::holds_alternative<CPDF_Parser::ObjectInfo::Normal>(info->vary);
}

bool IsObjectInfoCompressed(const CPDF_Parser::ObjectInfo* info) {
  return info && absl::holds_alternative<CPDF_Parser::ObjectInfo::Compressed>(
                     info->vary);
}

bool IsObjectInfoNull(const CPDF_Parser::ObjectInfo* info) {
  return info &&
         absl::holds_alternative<CPDF_Parser::ObjectInfo::Null>(info->vary);
}
...
void CPDF_Parser::ProcessCrossRefV5Entry(
  ...
  if (field_widths[0]) {
    required_type = GetObjectTypeCodeFromCrossRefStreamEntry(
