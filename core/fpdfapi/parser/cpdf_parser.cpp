  ...
  for (CPDF_SyntaxParser::WordResult result = m_pSyntax->GetNextWord();
    ...
    if (word == "(") {
      ...
      if (pTrailer) {
        ...
        // Grab the object number before potentially calling std::move(pTrailer)
        // below.
        const uint32_t trailer_object_number = pTrailer->GetObjNum();
        RetainPtr<CPDF_Dictionary> trailer_dict;
        if (stream_trailer) {
          trailer_dict = stream_trailer->GetMutableDict();
        } else {
          trailer_dict = ToDictionary(std::move(pTrailer));
        }
