...
CPDF_Parser::Error CPDF_Parser::LoadLinearizedCrossRefStream() {
  if (!m_MainXRefStreamOffset.IsValid())
    return FORMAT_ERROR;

  if (m_MainXRefStreamOffset.ValueOrDie() == 0) {
    return SUCCESS;
  }

  if (!LoadLinearizedAllCrossRefV4(m_MainXRefStreamOffset.ValueOrDie()) &&
      !LoadLinearizedAllCrossRefV5(m_MainXRefStreamOffset.ValueOrDie())) {
    m_LastXRefOffset = 0;
    return FORMAT_ERROR;
  }
