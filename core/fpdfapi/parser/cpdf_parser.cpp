  ...
  for (ByteString word = m_pSyntax->GetNextWord(&bIsNumber); !word.IsEmpty();
    ...
    if (bIsNumber) {
      numbers.insert(numbers.begin(),
                     std::make_pair(FXSYS_atoui(word.c_str()),
    ...
    if (word == "(") {
      ...
      const FX_FILESIZE obj_pos = numbers[1].second;
      const uint32_t obj_num = numbers[1].first;
      const uint32_t gen_num = numbers[0].first;
