...
bool CPDF_Parser::LoadCrossRefV5(FX_FILESIZE* pos, bool bMainXRef) {
  ...
  *pos = pDict->GetIntegerFor("Prev");
  ...
  std::vector<uint32_t> width_array;
  FX_SAFE_UINT32 dwAccWidth = 0;
  for (size_t i = 0; i < pArray->size(); ++i) {
    width_array.push_back(pArray->GetIntegerAt(i));
    dwAccWidth += width_array[i];
  }

  if (!dwAccWidth.IsValid() || width_array.size() < 3)
    return false;
    ...
    for (uint32_t i = 0; i < count; i++) {
      ...
      if (width_array[0]) {
        const uint32_t cross_ref_stream_obj_type =
            GetVarInt(entry_span.subspan(0, width_array[0]));
      ...
      if (existing_type == ObjectType::kNull) {
        uint32_t offset =
            GetVarInt(entry_span.subspan(width_array[0], width_array[1]));
