...
CPDF_Parser::ObjectType GetObjectTypeCodeFromCrossRefStreamEntry(
...
FX_FILESIZE CPDF_Parser::GetObjectPositionOrZero(uint32_t objnum) const {
  ...
  auto* norm = absl::get_if<ObjectInfo::Normal>(&info->vary);
  return norm ? norm->pos : 0;
}
...
bool CPDF_Parser::IsObjectFreeOrNull(uint32_t objnum) const {
  ...
  return absl::holds_alternative<ObjectInfo::Free>(info->vary) ||
         absl::holds_alternative<ObjectInfo::Compressed>(info->vary);
}

bool CPDF_Parser::IsObjectFree(uint32_t objnum) const {
  auto* info = GetObjectInfo(objnum);
  return info && absl::holds_alternative<ObjectInfo::Free>(info->vary);
}
...
bool CPDF_Parser::VerifyCrossRefV4() {
  for (const auto& it : m_CrossRefTable->objects_info()) {
    if (!absl::holds_alternative<ObjectInfo::Normal>(it.second.vary)) {
      continue;
    }
  ...
  for (CPDF_SyntaxParser::WordResult result = m_pSyntax->GetNextWord();
    ...
    if (word == "(") {
      ...
      if (obj_num < kMaxObjectNumber) {
        cross_ref_table->AddNormal(obj_num, gen_num, false, obj_pos);
...
void CPDF_Parser::ProcessCrossRefV5Entry(
  ...
  ObjectType type;
  if (field_widths[0]) {
    type = GetObjectTypeCodeFromCrossRefStreamEntry(
  ...
  bool is_null = info && absl::holds_alternative<ObjectInfo::Null>(info->vary);
  if (is_null) {
    ...
    if (pdfium::base::IsValueInRangeForNumericType<FX_FILESIZE>(offset))
      m_CrossRefTable->AddNormal(obj_num, 0, false, offset);
    return;
  }

  bool is_free = !info || absl::holds_alternative<ObjectInfo::Free>(info->vary);
  if (!overwrite_existing && !is_free) {
    return;
  }
...
RetainPtr<CPDF_Object> CPDF_Parser::ParseIndirectObject(uint32_t objnum) {
  ...
  auto* info = GetObjectInfo(objnum);
  if (info && absl::holds_alternative<ObjectInfo::Normal>(info->vary)) {
    FX_FILESIZE pos = GetObjectPositionOrZero(objnum);
    return pos > 0 ? ParseIndirectObjectAt(pos, objnum) : nullptr;
  }
  if (!info || !absl::holds_alternative<ObjectInfo::Compressed>(info->vary)) {
    return nullptr;
  }
