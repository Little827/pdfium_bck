...
pdfium::Optional<uint32_t> CPDF_ToUnicodeMap::StringToCode(ByteStringView str) {
  ...
  return static_cast<uint32_t>(code.ValueOrDefault(0u));
}
...
void CPDF_ToUnicodeMap::HandleBeginBFRange(CPDF_SimpleParser* pParser) {
  while (1) {
    ...
    if (low.IsEmpty() || low == "endbfrange")
      return;
    pdfium::Optional<uint32_t> lowcode = StringToCode(low);
    if (!lowcode)
      return;

    ByteStringView high = pParser->GetWord();
    pdfium::Optional<uint32_t> highcode = StringToCode(high);
    if (!highcode)
      return;

    // TODO(crbug.com/1021762): Find the correct behavior for parsing bfrange
    // when the higher bytes of |lowcode| and |highcode| cross boundaries.
    // Right now use |lowcode| and |highcode| as they are to prevent corrupting
    // existing CMap values.
