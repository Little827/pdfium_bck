...
pdfium::Optional<uint32_t> CPDF_ToUnicodeMap::StringToCode(ByteStringView str) {
  ...
  return pdfium::Optional<uint32_t>(result);
}
...
void CPDF_ToUnicodeMap::HandleBeginBFRange(CPDF_SimpleParser* pParser) {
  while (1) {
    ...
    pdfium::Optional<uint32_t> low_code = StringToCode(low);
    if (!low_code)
      return;

    ByteStringView high = pParser->GetWord();
    pdfium::Optional<uint32_t> high_code = StringToCode(high);
    if (!high_code)
      return;

    uint32_t lowcode = *low_code;
    uint32_t highcode = (lowcode & 0xffffff00) | (*high_code & 0xff);
