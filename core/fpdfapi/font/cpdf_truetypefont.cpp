...
void CPDF_TrueTypeFont::LoadGlyphMap() {
    ...
    for (uint32_t charcode = 0; charcode < 256; charcode++) {
      ...
      if (!name) {
        m_GlyphIndex[charcode] =
            m_pFontFile ? FXFT_Get_Char_Index(m_Font.GetFaceRec(), charcode)
                        : -1;
        continue;
      }
      ...
      if (bMSSymbol) {
        ...
        if (bMSUnicode) {
          ...
          if (!maccode) {
            ...
            m_GlyphIndex[charcode] =
                FXFT_Get_Char_Index(m_Font.GetFaceRec(), maccode);
          }
        }
      }
      ...
      if (strcmp(name, ".notdef") == 0) {
        m_GlyphIndex[charcode] = FXFT_Get_Char_Index(m_Font.GetFaceRec(), 32);
        continue;
      }
      ...
      if (!wsUnicode.IsEmpty()) {
        m_GlyphIndex[charcode] =
            FXFT_Get_Char_Index(m_Font.GetFaceRec(), wsUnicode[0]);
  ...
  if (FXFT_Select_Charmap(m_Font.GetFaceRec(), FXFT_ENCODING_UNICODE) == 0) {
    bool bFound = false;
