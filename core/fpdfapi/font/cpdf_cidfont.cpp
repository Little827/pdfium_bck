...
bool CPDF_CIDFont::Load() {
  ...
  if (GetFont()->GetFaceRec()) {
    if (m_bType1)
      ...
      FT_UseCIDCharmap(GetFont()->GetFaceRec(), m_pCMap->GetCoding());
  }
...
int CPDF_CIDFont::GetGlyphIndex(uint32_t unicode, bool* pVertGlyph) {
  ...
  if (!GetFont()->GetSubData()) {
    unsigned long length = 0;
    int error = FT_Load_Sfnt_Table(face, FT_MAKE_TAG('G', 'S', 'U', 'B'), 0,
                                   nullptr, &length);
    if (!error)
      GetFont()->SetSubData(FX_Alloc(uint8_t, length));
  }
  int error = FT_Load_Sfnt_Table(face, FT_MAKE_TAG('G', 'S', 'U', 'B'), 0,
                                 GetFont()->GetSubData(), nullptr);
  if (error || !GetFont()->GetSubData())
    return index;

  m_pTTGSUBTable = std::make_unique<CFX_CTTGSUBTable>(GetFont()->GetSubData());
  return GetVerticalGlyph(index, pVertGlyph);
}
...
int CPDF_CIDFont::GlyphFromCharCode(uint32_t charcode, bool* pVertGlyph) {
  ...
  if (!m_pStreamAcc) {
    ...
    if (m_pCMap->GetCoding() == CIDCODING_UNKNOWN ||
        !FXFT_Get_Face_Charmap(GetFont()->GetFaceRec())) {
      return cid;
    }
    if (FXFT_Get_Charmap_Encoding(FXFT_Get_Face_Charmap(
            GetFont()->GetFaceRec())) == FT_ENCODING_UNICODE) {
      WideString unicode_str = UnicodeFromCharCode(charcode);
      if (unicode_str.IsEmpty())
        return -1;
...
void CPDF_CIDFont::LoadSubstFont() {
  ...
  safeStemV *= 5;
  GetFont()->LoadSubst(m_BaseFontName, !m_bType1, m_Flags,
                       safeStemV.ValueOrDefault(FXFONT_FW_NORMAL),
