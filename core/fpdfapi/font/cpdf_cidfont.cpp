...
FX_RECT CPDF_CIDFont::GetCharBBox(uint32_t charcode) {
  ...
  if (face) {
    if (FXFT_Is_Face_Tricky(face)) {
      ...
      if (!err) {
        FXFT_Glyph glyph;
        err = FXFT_Get_Glyph(((FXFT_FaceRec*)face)->glyph, &glyph);
        if (!err) {
          FXFT_BBox cbox;
          ...
          int pixel_size_x = ((FXFT_FaceRec*)face)->size->metrics.x_ppem;
          int pixel_size_y = ((FXFT_FaceRec*)face)->size->metrics.y_ppem;
