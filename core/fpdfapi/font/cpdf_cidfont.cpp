...
bool CPDF_CIDFont::Load() {
  ...
  if (m_pFont->GetFaceRec()) {
    if (m_bType1)
      ...
      FT_UseCIDCharmap(m_pFont->GetFaceRec(), m_pCMap->GetCoding());
  }
...
int CPDF_CIDFont::GetGlyphIndex(uint32_t unicode, bool* pVertGlyph) {
  ...
  if (!m_pFont->GetSubData()) {
    unsigned long length = 0;
    int error = FT_Load_Sfnt_Table(face, FT_MAKE_TAG('G', 'S', 'U', 'B'), 0,
                                   nullptr, &length);
    if (!error)
      m_pFont->SetSubData(FX_Alloc(uint8_t, length));
  }
  int error = FT_Load_Sfnt_Table(face, FT_MAKE_TAG('G', 'S', 'U', 'B'), 0,
                                 m_pFont->GetSubData(), nullptr);
  if (error || !m_pFont->GetSubData())
    return index;

  m_pTTGSUBTable = std::make_unique<CFX_CTTGSUBTable>(m_pFont->GetSubData());
  return GetVerticalGlyph(index, pVertGlyph);
}
...
int CPDF_CIDFont::GlyphFromCharCode(uint32_t charcode, bool* pVertGlyph) {
  ...
  if (!m_pStreamAcc) {
    ...
    if (m_pCMap->GetCoding() == CIDCODING_UNKNOWN ||
        !FXFT_Get_Face_Charmap(m_pFont->GetFaceRec())) {
      return cid;
    }
    if (FXFT_Get_Charmap_Encoding(FXFT_Get_Face_Charmap(
            m_pFont->GetFaceRec())) == FT_ENCODING_UNICODE) {
      WideString unicode_str = UnicodeFromCharCode(charcode);
      if (unicode_str.IsEmpty())
        return -1;
...
void CPDF_CIDFont::LoadSubstFont() {
  ...
  safeStemV *= 5;
  m_pFont->LoadSubst(m_BaseFontName, !m_bType1, m_Flags,
