...
uint16_t CPDF_CIDFont::GetVertWidth(uint16_t cid) const {
  ...
  if (vertsize) {
    ...
    for (size_t i = 0; i < vertsize; i++) {
      ...
      if (IsMetricForCID(pEntry, cid))
        return static_cast<uint16_t>(pEntry[2]);
    }
  }
  return m_DefaultW1;
}

void CPDF_CIDFont::GetVertOrigin(uint16_t cid,
                                 uint16_t& vx,
                                 uint16_t& vy) const {
  size_t vertsize = m_VertMetrics.size() / 5;
  if (vertsize) {
    ...
    for (size_t i = 0; i < vertsize; i++) {
      const uint32_t* pEntry = pTable + (i * 5);
      if (IsMetricForCID(pEntry, cid)) {
        vx = static_cast<uint16_t>(pEntry[3]);
        vy = static_cast<uint16_t>(pEntry[4]);
        return;
      }
    }
  }
  ...
  vx = static_cast<uint16_t>(dwWidth) / 2;
  vy = m_DefaultVY;
}
