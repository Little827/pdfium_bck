...
TEST(cpdf_tounicodemap, HandleBeginBFRange) {
  ...
  RetainPtr<CFX_MemoryStream> pFile = pdfium::MakeRetain<CFX_MemoryStream>();
  pFile->WriteBlockAtOffset(bfrange_str.c_str(), 0, bfrange_str.length());

  auto stream = pdfium::MakeRetain<CPDF_Stream>();
  stream->InitStreamFromFile(pFile, pdfium::MakeRetain<CPDF_Dictionary>());
  ASSERT_TRUE(stream);
