...
TEST(cpdf_tounicodemap, StringToCode) {
  ...
  // Integer overflow
  EXPECT_FALSE(CPDF_ToUnicodeMap::StringToCode("<100000000>"));
  EXPECT_FALSE(CPDF_ToUnicodeMap::StringToCode("<1abcdFFFF>"));

  // Invalid string
  EXPECT_FALSE(CPDF_ToUnicodeMap::StringToCode(""));
  EXPECT_FALSE(CPDF_ToUnicodeMap::StringToCode("<>"));
  EXPECT_FALSE(CPDF_ToUnicodeMap::StringToCode("128"));
  EXPECT_FALSE(CPDF_ToUnicodeMap::StringToCode("<12"));
  EXPECT_FALSE(CPDF_ToUnicodeMap::StringToCode("12>"));
  EXPECT_FALSE(CPDF_ToUnicodeMap::StringToCode("<1-7>"));
  EXPECT_FALSE(CPDF_ToUnicodeMap::StringToCode("00AB"));
  EXPECT_FALSE(CPDF_ToUnicodeMap::StringToCode("<00NN>"));
}
