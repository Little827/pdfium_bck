...
TEST(cpdf_tounicodemap, InsertIntoMultimap) {
  {
    ...
    EXPECT_EQ(1u, map.GetUnicodeNumberByCharcodeForTesting(1u));
    EXPECT_EQ(1u, map.GetUnicodeNumberByCharcodeForTesting(2u));
  }
  {
    ...
    EXPECT_EQ(2u, map.GetUnicodeNumberByCharcodeForTesting(0u));
  }
  {
    ...
    CPDF_ToUnicodeMap map(stream.Get());
    EXPECT_EQ(0u, map.ReverseLookup(0x0041));
    EXPECT_EQ(1u, map.GetUnicodeNumberByCharcodeForTesting(0u));
  }
}
