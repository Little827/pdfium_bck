...
bool CFX_CTTGSUBTable::LoadGSUBTable(pdfium::span<const uint8_t> gsub) {
  ...
  size_t scriptlist_index = FXSYS_UINT16_GET_MSBFIRST(gsub.subspan(4, 2));
  size_t featurelist_index = FXSYS_UINT16_GET_MSBFIRST(gsub.subspan(6, 2));
  size_t lookuplist_index = FXSYS_UINT16_GET_MSBFIRST(gsub.subspan(8, 2));
...
CFX_CTTGSUBTable::ParseCoverage(const uint8_t* raw) {
  ...
  if (format == 1) {
    ...
    return std::make_unique<CoverageFormat>(std::move(glyph_array));
  }
  ...
  return std::make_unique<CoverageFormat>(std::move(range_records));
}
...
std::unique_ptr<CFX_CTTGSUBTable::SubTable> CFX_CTTGSUBTable::ParseSingleSubst(
  ...
  auto rec = std::make_unique<SubTable>();
