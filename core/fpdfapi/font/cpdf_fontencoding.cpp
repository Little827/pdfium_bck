...
constexpr size_t kEncodingNamesTableSize =
    CPDF_FontEncoding::kEncodingTableSize - 32;
constexpr size_t kPDFDocEncodingNamesTableSize =
    CPDF_FontEncoding::kEncodingTableSize - 24;
...
const char* PDF_CharNameFromPredefinedCharSet(int encoding, uint8_t charcode) {
  if (encoding == PDFFONT_ENCODING_PDFDOC) {
    static constexpr size_t kMinCharcode =
        CPDF_FontEncoding::kEncodingTableSize - kPDFDocEncodingNamesTableSize;
    if (charcode < kMinCharcode)
      return nullptr;

    charcode -= kMinCharcode;
  } else {
    static constexpr size_t kMinCharcode =
        CPDF_FontEncoding::kEncodingTableSize - kEncodingNamesTableSize;
    if (charcode < kMinCharcode)
      return nullptr;
