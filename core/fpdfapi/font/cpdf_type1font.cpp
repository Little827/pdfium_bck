...
bool CPDF_Type1Font::Load() {
  ...
  else if (CFX_FontMapper::IsSymbolicFont(m_Base14Font))
    m_Flags = FXFONT_SYMBOLIC;
  else
    m_Flags = FXFONT_NONSYMBOLIC;

  if (CFX_FontMapper::IsFixedFont(m_Base14Font)) {
    for (int i = 0; i < 256; i++)
      m_CharWidth[i] = 600;
  }
  if (m_Base14Font == CFX_FontMapper::StandardFont::kSymbol)
    m_BaseEncoding = PDFFONT_ENCODING_ADOBE_SYMBOL;
  else if (m_Base14Font == CFX_FontMapper::StandardFont::kDingbats)
...
#endif
  if (!IsEmbedded() && !CFX_FontMapper::IsSymbolicFont(m_Base14Font) &&
      m_Font.IsTTFont()) {
