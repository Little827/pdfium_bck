...
void CPDF_Type1Font::LoadGlyphMap() {
  ...
  CQuartz2D& quartz2d =
      static_cast<CApplePlatform*>(CFX_GEModule::Get()->GetPlatform())
          ->m_quartz2d;
  if (!GetFont()->GetPlatformFont()) {
    if (GetFont()->GetPsName() == "DFHeiStd-W5")
      bCoreText = false;

    pdfium::span<const uint8_t> span = GetFont()->GetFontSpan();
    GetFont()->SetPlatformFont(quartz2d.CreateFont(span.data(), span.size()));
    if (!GetFont()->GetPlatformFont())
      bCoreText = false;
  }
#endif
  if (!IsEmbedded() && !IsSymbolicFont() && GetFont()->IsTTFont()) {
    if (FT_UseTTCharmap(GetFont()->GetFaceRec(), 3, 0)) {
      bool bGotOne = false;
      for (uint32_t charcode = 0; charcode < 256; charcode++) {
        ...
        for (int j = 0; j < 4; j++) {
          ...
          m_GlyphIndex[charcode] =
              FT_Get_Char_Index(GetFont()->GetFaceRec(), unicode);
#if defined(OS_APPLE)
          CalcExtGID(charcode);
#endif
    ...
    FXFT_Select_Charmap(GetFont()->GetFaceRec(), FT_ENCODING_UNICODE);
    if (m_BaseEncoding == 0)
      m_BaseEncoding = PDFFONT_ENCODING_STANDARD;
    ...
    for (uint32_t charcode = 0; charcode < 256; charcode++) {
      ...
      m_GlyphIndex[charcode] = FT_Get_Char_Index(
          GetFont()->GetFaceRec(), m_Encoding.UnicodeFromCharCode(charcode));
#if defined(OS_APPLE)
      CalcExtGID(charcode);
#endif
      if (m_GlyphIndex[charcode] == 0 && strcmp(name, ".notdef") == 0) {
        ...
        m_GlyphIndex[charcode] =
            FT_Get_Char_Index(GetFont()->GetFaceRec(), 0x20);
#if defined(OS_APPLE)
        CalcExtGID(charcode);
#endif
      }
  ...
  FT_UseType1Charmap(GetFont()->GetFaceRec());
#if defined(OS_APPLE)
  if (bCoreText) {
    if (FontStyleIsSymbolic(m_Flags)) {
      for (uint32_t charcode = 0; charcode < 256; charcode++) {
        ...
        if (name) {
          m_Encoding.SetUnicode(charcode, PDF_UnicodeFromAdobeName(name));
          m_GlyphIndex[charcode] =
              FXFT_Get_Name_Index(GetFont()->GetFaceRec(), name);
          SetExtGID(name, charcode);
        } else {
          m_GlyphIndex[charcode] =
              FT_Get_Char_Index(GetFont()->GetFaceRec(), charcode);
          wchar_t unicode = 0;
          if (m_GlyphIndex[charcode]) {
            unicode =
          ...
          FT_Get_Glyph_Name(GetFont()->GetFaceRec(), m_GlyphIndex[charcode],
                            name_glyph, 256);
          name_glyph[255] = 0;
    ...
    bool bUnicode =
        FXFT_Select_Charmap(GetFont()->GetFaceRec(), FT_ENCODING_UNICODE) == 0;
    for (uint32_t charcode = 0; charcode < 256; charcode++) {
      const char* name =
          GetAdobeCharName(m_BaseEncoding, m_CharNames, charcode);
      if (!name)
      ...
      if (pStrUnicode &&
          FXFT_Get_Name_Index(GetFont()->GetFaceRec(), name) == 0) {
        name = pStrUnicode;
      }
      m_GlyphIndex[charcode] =
          FXFT_Get_Name_Index(GetFont()->GetFaceRec(), name);
      ...
      if (strcmp(name, ".notdef") != 0 && strcmp(name, "space") != 0) {
        ...
        m_Encoding.SetUnicode(charcode, 0x20);
        m_GlyphIndex[charcode] =
            bUnicode ? FT_Get_Char_Index(GetFont()->GetFaceRec(), 0x20)
                     : 0xffff;
        CalcExtGID(charcode);
      }
    }
  ...
  for (int charcode = 0; charcode < 256; charcode++) {
    ...
    m_Encoding.SetUnicode(charcode, PDF_UnicodeFromAdobeName(name));
    m_GlyphIndex[charcode] = FXFT_Get_Name_Index(GetFont()->GetFaceRec(), name);
    if (m_GlyphIndex[charcode] != 0)
      continue;
...
void CPDF_Type1Font::SetExtGID(const char* name, uint32_t charcode) {
  ...
  m_ExtGID[charcode] = CGFontGetGlyphWithGlyphName(
      (CGFontRef)GetFont()->GetPlatformFont(), name_ct);
  if (name_ct)
    CFRelease(name_ct);
}
...
void CPDF_Type1Font::CalcExtGID(uint32_t charcode) {
  char name_glyph[256];
  FT_Get_Glyph_Name(GetFont()->GetFaceRec(), m_GlyphIndex[charcode], name_glyph,
                    256);
  name_glyph[255] = 0;
  SetExtGID(name_glyph, charcode);
}
