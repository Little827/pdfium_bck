...
bool FindFamilyNameMatch(const ByteStringView& family_name,
                         const ByteString& subst_font_name) {
  Optional<size_t> result = subst_font_name.Find(family_name, 0);
  if (!result.has_value())
    return false;

  size_t next_index = result.value() + family_name.GetLength();
  // Rule out the case that |family_name| is a substring of |subst_font_name|
  // but the family names are actually different words. For example: "Univers"
  // and "Universal" are not a match because they have different family names,
  // but "Univers" and "Univers Bold" are a match.
  if (subst_font_name.IsValidIndex(next_index) &&
      FXSYS_IsLowerASCII(subst_font_name[next_index]))
    return false;
