...
static const CFX_FillRenderOptions kWindingFillOptions(FXFILL_WINDING);
...
TEST_F(CFX_WindowsRenderDeviceTest, SimpleClipTriangle) {
  ...
  path_data.ClosePath();
  EXPECT_TRUE(m_driver->SetClip_PathFill(&path_data, &kIdentityMatrix,
                                         kWindingFillOptions));
}
...
TEST_F(CFX_WindowsRenderDeviceTest, GargantuanClipRect) {
  ...
  EXPECT_FALSE(m_driver->SetClip_PathFill(&path_data, &kIdentityMatrix,
                                          kWindingFillOptions));
}
