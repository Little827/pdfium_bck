...
bool CGdiplusExt::DrawPath(HDC hDC,
  ...
  if (fill_options.aliased_path) {
    ...
    if (!bSmooth && IsWindingOrEvenOddFillType(fill_options.fill_type))
      bSmooth = true;
  ...
  CallFunc(GdipCreatePath2)(gp_points.data(), gp_types.data(), points.size(),
                            FillType2Gdip(fill_options.fill_type), &pGpPath);
  if (!pGpPath) {
    if (pMatrix)
  ...
  if (IsWindingOrEvenOddFillType(fill_options.fill_type)) {
    Gdiplus::GpBrush* pBrush = GdipCreateBrushImpl(fill_argb);
    CallFunc(GdipSetPathFillMode)(pGpPath,
      ...
      for (size_t i = 0; i < points.size(); ++i) {
        if (i == points.size() - 1 ||
          ...
          CallFunc(GdipCreatePath2)(
              &gp_points[iStart], &gp_types[iStart], i - iStart + 1,
              FillType2Gdip(fill_options.fill_type), &pSubPath);
          iStart = i + 1;
