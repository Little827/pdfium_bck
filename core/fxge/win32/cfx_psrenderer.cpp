...
CFX_PSRenderer::PSCompressResult::PSCompressResult(PSCompressResult&&) =
    default;

CFX_PSRenderer::PSCompressResult& CFX_PSRenderer::PSCompressResult::operator=(
    PSCompressResult&&) = default;
...
absl::optional<CFX_PSRenderer::PSCompressResult> CFX_PSRenderer::PSCompressData(
  ...
  if (m_Level.value() == RenderingLevel::kLevel3 ||
    ...
    DataVector<uint8_t> decode_result =
        m_pEncoderIface->pFlateEncodeFunc(src_span);
    if (decode_result.size() > 0 && decode_result.size() < src_span.size()) {
      PSCompressResult result;
      result.data = std::move(decode_result);
      result.filter = "/FlateDecode filter ";
      return result;
    }
  } else {
    DataVector<uint8_t> decode_result =
        m_pEncoderIface->pRunLengthEncodeFunc(src_span);
    if (decode_result.size() > 0 && decode_result.size() < src_span.size()) {
      PSCompressResult result;
      result.data = std::move(decode_result);
      result.filter = "/RunLengthDecode filter ";
      return result;
    }
  }
  return absl::nullopt;
