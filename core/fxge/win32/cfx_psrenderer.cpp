...
Optional<ByteString> GenerateType42SfntData(
  ...
  constexpr size_t kMaxCharsPerLine = 32;
  ...
  for (uint8_t datum : font_data) {
    ...
    output << buf[1];
    chars_per_line++;
    if (chars_per_line == kMaxCharsPerLine) {
      output << "\n";
      chars_per_line = 0;
    }
  }
...
ByteString GenerateType42FontDictionary(const ByteString& psname,
  ...
  size_t descendant_font_count = num_glyphs / glyphs_per_descendant_font;
  if (num_glyphs % glyphs_per_descendant_font)
  ...
  for (size_t i = 0; i < descendant_font_count; ++i) {
    ...
    output << "/Encoding 256 array\n";
    for (size_t j = 0; j < glyphs_per_descendant_font; ++j) {
      size_t pos = i * glyphs_per_descendant_font + j;
      if (pos >= num_glyphs)
        break;
    ...
    output << "/CharStrings 257 dict dup begin\n";
    output << "/.notdef 0 def\n";
    for (size_t j = 0; j < glyphs_per_descendant_font; ++j) {
      size_t pos = i * glyphs_per_descendant_font + j;
      if (pos >= num_glyphs)
