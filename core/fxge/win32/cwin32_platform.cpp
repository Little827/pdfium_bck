...
struct Variant {
  ...
  pdfium::span<const char> m_pVariantName;  // UTF-16LE terminator required.
};

constexpr Variant kVariantNames[] = {
    {"DFKai-SB", pdfium::make_span("\x19\x6A\x77\x69\xD4\x9A\x00\x00")},
};
...
void* CFX_Win32FontInfo::MapFont(int weight,
  ...
  for (const Variant& variant : kVariantNames) {
    ...
    size_t len = variant.m_pVariantName.size();
    WideString wsName = WideString::FromUTF16LE(
        {reinterpret_cast<const uint8_t*>(variant.m_pVariantName.data()), len});
    if (wsFace == wsName)
      return hFont;
  }
  ::DeleteObject(hFont);
