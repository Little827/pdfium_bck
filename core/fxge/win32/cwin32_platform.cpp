...
class CFX_Win32FontInfo final : public SystemFontInfoIface {
 ...
 private:
  bool IsSupportFont(const LOGFONTA* plf);
...
bool CFX_Win32FontInfo::IsSupportFont(const LOGFONTA* plf) {
  ...
  if (font_size != GDI_ERROR && font_size >= sizeof(uint32_t)) {
    ...
    ret = header == FXBSTR_ID('O', 'T', 'T', 'O') || header == 0x00010000 ||
          header == FXBSTR_ID('t', 't', 'c', 'f') ||
          header == FXBSTR_ID('t', 'r', 'u', 'e') || header == 0x00020000 ||
...
void CFX_Win32FontInfo::AddInstalledFont(const LOGFONTA* plf,
  ...
  if (!(font_type & TRUETYPE_FONTTYPE)) {
    if (!(font_type & DEVICE_FONTTYPE) || !IsSupportFont(plf))
      return;
  }
