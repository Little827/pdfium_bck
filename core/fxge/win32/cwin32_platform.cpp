...
constexpt Variant kVariantNames[] = {
...
void* CFX_Win32FontInfo::MapFont(int weight,
  ...
  for (iBaseFont = 0; iBaseFont < 12; iBaseFont++) {
    if (face == ByteStringView(kBase14Substs[iBaseFont].m_pName)) {
      face = kBase14Substs[iBaseFont].m_pWinName;
  ...
  for (size_t i = 0; i < pdfium::size(kVariantNames); ++i) {
    if (face != kVariantNames[i].m_pFaceName)
      continue;
