...
bool CPSOutput::WriteBlock(pdfium::span<const uint8_t> pBlock) {
  while (!pBlock.empty()) {
    uint8_t buffer[1026];
    size_t send_len = std::min<size_t>(pBlock.size(), 1024);
    *(reinterpret_cast<uint16_t*>(buffer)) = static_cast<uint16_t>(send_len);
    memcpy(buffer + 2, pBlock.data(), send_len);
    ...
    pBlock = pBlock.subspan(send_len);
  }
  return true;
}
