...
int CFX_Font::GetGlyphWidthImpl(uint32_t glyph_index,
  ...
  return m_Face->AdjustMMAndGetGlyphWidth(glyph_index, dest_width, weight,
                                          m_pSubstFont.get());
}
...
absl::optional<FX_RECT> CFX_Font::GetGlyphBBox(uint32_t glyph_index) {
  ...
  return m_Face->GetGlyphBBox(glyph_index);
}
