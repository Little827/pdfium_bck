...
RetainPtr<CFX_Face> LoadFileImp(FXFT_Library library,
...
bool CFX_Font::LoadFile(const RetainPtr<IFX_SeekableReadStream>& pFile,
  ...
  FXFT_Set_Pixel_Sizes(m_Face->GetRec(), 0, 64);
  return true;
}
...
bool CFX_Font::IsTTFont() const {
  return m_Face && FXFT_Is_Face_TT_OT(m_Face->GetRec()) == FXFT_FACE_FLAG_SFNT;
}
...
bool CFX_Font::GetGlyphBBox(uint32_t glyph_index, FX_RECT* pBBox) {
  ...
  if (FXFT_Is_Face_Tricky(m_Face->GetRec())) {
    int error = FXFT_Set_Char_Size(m_Face->GetRec(), 0, 1000 * 64, 72, 72);
    if (error)
      return false;

    error = FXFT_Load_Glyph(m_Face->GetRec(), glyph_index,
    ...
    FXFT_BBox cbox;
    FT_Glyph glyph;
    error = FXFT_Get_Glyph(m_Face->GetRec()->glyph, &glyph);
    if (error)
      return false;
    ...
    FT_Done_Glyph(glyph);
    return FXFT_Set_Pixel_Sizes(m_Face->GetRec(), 0, 64) == 0;
  }
  if (FXFT_Load_Glyph(
          m_Face->GetRec(), glyph_index,
          FXFT_LOAD_NO_SCALE | FXFT_LOAD_IGNORE_GLOBAL_ADVANCE_WIDTH)) {
...
bool CFX_Font::IsItalic() const {
  if (!m_Face)
    return false;
  if (FXFT_Is_Face_Italic(m_Face->GetRec()) == FXFT_STYLE_FLAG_ITALIC)
    return true;
...
bool CFX_Font::IsBold() const {
  return m_Face && FXFT_Is_Face_Bold(m_Face->GetRec()) == FXFT_STYLE_FLAG_BOLD;
}
...
ByteString CFX_Font::GetPsName() const {
  ...
  ByteString psName = FXFT_Get_Postscript_Name(m_Face->GetRec());
  if (psName.IsEmpty())
...
void CFX_Font::AdjustMMParams(int glyph_index,
  ...
  FXFT_Get_MM_Var(m_Face->GetRec(), &pMasters);
  if (!pMasters)
    return;
  ...
  if (dest_width == 0) {
    ...
    coords[1] = min_param;
    FXFT_Set_MM_Design_Coordinates(m_Face->GetRec(), 2, coords);
    FXFT_Load_Glyph(m_Face->GetRec(), glyph_index,
                    FXFT_LOAD_NO_SCALE | FXFT_LOAD_IGNORE_GLOBAL_ADVANCE_WIDTH);
...
CFX_PathData* CFX_Font::LoadGlyphPathImpl(uint32_t glyph_index,
  ...
  if (FXFT_Load_Glyph(m_Face->GetRec(), glyph_index, load_flags))
    return nullptr;
  if (m_pSubstFont && !m_pSubstFont->m_bFlagMM &&
    ...
    FXFT_Outline_Embolden(FXFT_Get_Glyph_Outline(m_Face->GetRec()), level);
  }
  ...
  FXFT_Outline_Decompose(FXFT_Get_Glyph_Outline(m_Face->GetRec()), &funcs,
                         &params);
