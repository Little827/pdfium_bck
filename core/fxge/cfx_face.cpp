...
int CFX_Face::GetAdjustedAscender() const {
  return NormalizeFontMetric(GetAscender(), GetUnitsPerEm());
}

int CFX_Face::GetAdjustedDescender() const {
  return NormalizeFontMetric(GetDescender(), GetUnitsPerEm());
}
...
int CFX_Face::GetGlyphWidth(uint32_t glyph_index,
  ...
  return NormalizeFontMetric(rec->glyph->metrics.horiAdvance, GetUnitsPerEm());
}
