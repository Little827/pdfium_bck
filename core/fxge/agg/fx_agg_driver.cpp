...
bool CFX_AggDeviceDriver::DrawPath(const CFX_PathData* pPathData,
  ...
  if (IsAlternateOrWindingFillMode(fill_options.fill_type) && fill_color) {
    ...
    rasterizer.filling_rule(
        GetAlternateOrWindingFillType(fill_options.fill_type));
    if (!RenderRasterizer(rasterizer, fill_color, fill_options.is_full_cover,
                          false)) {
      return false;
    }
  }
  ...
  if (fill_options.is_zero_area) {
    ...
    return RenderRasterizer(rasterizer, stroke_color,
                            fill_options.is_full_cover, m_bGroupKnockout);
  }
  CFX_Matrix matrix1;
  CFX_Matrix matrix2;
  if (pObject2Device) {
  ...
  return RenderRasterizer(rasterizer, stroke_color, fill_options.is_full_cover,
                          m_bGroupKnockout);
}
