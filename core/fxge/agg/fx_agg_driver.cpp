...
// Hard clips and returns a value based on `curr_pos`, attempting to recalculate
// the x and y values such that both x and y values are within [-kMaxPos,
// kMaxPos] boundaries, and the slope of the line connecting `prev_pos` and
...
CFX_PointF HardClipScaled(const CFX_PointF& prev_pos,
  ...
  DCHECK_EQ(prev_pos, HardClip(prev_pos));

  // Clamp the current position.
  CFX_PointF new_pos = HardClip(curr_pos);

  // Return if `curr_pos` is already within bounds.
  if (curr_pos == new_pos) {
    return new_pos;
  }

  // The x and y value with the smaller absolute value will need to be
  // recalculated.

  // This prevents divide by 0 errors when calculating the slope.
