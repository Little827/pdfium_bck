...
bool IsHorizontalLine(const CFX_PointF& a,
                      const CFX_PointF& b,
                      const CFX_PointF& c) {
  return a.x == b.x && b.x == c.x && (b.y - a.y) * (b.y - c.y) > 0;
}

bool IsVerticalLine(const CFX_PointF& a,
                    const CFX_PointF& b,
                    const CFX_PointF& c) {
  ...
  *setIdentity = false;
  ...
  for (size_t i = 0; i < m_Points.size(); i++) {
    ...
    if (m_Points[next].m_Type != FXPT_TYPE::BezierTo &&
      ...
      if (IsHorizontalLine(m_Points[i - 1].m_Point, m_Points[i].m_Point,
      ...
      } else if (IsVerticalLine(m_Points[i - 1].m_Point, m_Points[i].m_Point,
                                m_Points[next].m_Point)) {
        int pre = i;
