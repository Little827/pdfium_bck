...
std::array<uint8_t, 3> AdobeCMYK_to_sRGB1(uint8_t c,
  ...
  return std::array<uint8_t, 3>{static_cast<uint8_t>(fix_r),
                                static_cast<uint8_t>(fix_g),
...
std::array<float, 3> AdobeCMYK_to_sRGB(float c, float m, float y, float k) {
  ...
  std::array<uint8_t, 3> int_results = AdobeCMYK_to_sRGB1(c1, m1, y1, k1);
  ...
  for (size_t i = 0; i < int_results.size(); ++i) {
    results[i] = int_results[i] * kToFloat;
  }
  return results;
}
