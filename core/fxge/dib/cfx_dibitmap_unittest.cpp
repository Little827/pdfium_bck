...
TEST(CFX_DIBitmap, CalculatePitchAndSizeGood) {
  ...
  // Simple case with provided pitch matching width * boo.
  result =
      CFX_DIBitmap::CalculatePitchAndSize(100, 100, FXDIB_Format::kArgb, 400);
  ASSERT_TRUE(result.has_value());
  EXPECT_EQ(400u, result.value().pitch);
  EXPECT_EQ(40000u, result.value().size);

  // Simple case with provided pitch, where pitch exceeds width * bpp.
  result =
      CFX_DIBitmap::CalculatePitchAndSize(100, 100, FXDIB_Format::kArgb, 455);
  ASSERT_TRUE(result.has_value());
  EXPECT_EQ(455u, result.value().pitch);
  EXPECT_EQ(45500u, result.value().size);
}
...
TEST(CFX_DIBitmap, CalculatePitchAndSizeBad) {
  ...
  EXPECT_FALSE(
      CFX_DIBitmap::CalculatePitchAndSize(101, 100, FXDIB_Format::kArgb, 400));
