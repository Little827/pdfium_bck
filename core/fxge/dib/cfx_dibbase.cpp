...
RetainPtr<CFX_DIBitmap> CFX_DIBBase::ClipToInternal(
  ...
  if (GetBPP() == 1 && rect.left % 8 != 0) {
    ...
    for (int row = rect.top; row < rect.bottom; ++row) {
      auto src_span =
          pdfium::reinterpret_span<const uint32_t>(GetScanline(row));
      auto dst_span = pdfium::reinterpret_span<uint32_t>(
      ...
      uint32_t* dest_scan = dst_span.first(dword_count).data();
...
RetainPtr<CFX_DIBitmap> CFX_DIBBase::SwapXY(bool bXFlip, bool bYFlip) const {
  ...
  if (GetBPP() == 1) {
    ...
    for (int row = row_start; row < row_end; ++row) {
      ...
      if (nBytes == 4) {
        const uint32_t* src_scan =
            reinterpret_span<const uint32_t>(GetScanline(row))
                .subspan(col_start)
                .data();
        for (int col = col_start; col < col_end; ++col) {
          uint32_t* dest_scan32 = reinterpret_cast<uint32_t*>(dest_scan);
