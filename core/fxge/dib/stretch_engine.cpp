...
inline uint8_t PixelFromFixed(uint32_t fixed) {
  return static_cast<uint8_t>(fixed >> fxge::WeightTable::kFixedPointBits);
}
...
StretchEngine::StretchEngine(ScanlineComposerIface* pDestBitmap,
...
bool StretchEngine::Continue(PauseIndicatorIface* pPause) {
...
bool StretchEngine::StartStretchHorz() {
...
bool StretchEngine::ContinueStretchHorz(PauseIndicatorIface* pPause) {
  if (!m_DestWidth)
    return false;
    ...
    switch (m_TransMethod) {
      ...
      case TransformMethod::k1BppToManyBpp: {
        for (int col = m_DestClip.left; col < m_DestClip.right; ++col) {
          WeightTable::WeightVector* pWeights =
              m_WeightTable.GetPixelWeights(col);
          uint32_t dest_a = 0;
...
void StretchEngine::StretchVert() {
  if (m_DestHeight == 0)
    return;
  ...
  for (int row = m_DestClip.top; row < m_DestClip.bottom; ++row) {
    ...
    WeightTable::WeightVector* pWeights = table.GetPixelWeights(row);
    switch (m_TransMethod) {
      case TransformMethod::k1BppTo8Bpp:
      case TransformMethod::k1BppToManyBpp:
      case TransformMethod::k8BppTo8Bpp: {
