...
int GetPitch(int input) {
  return (input + 31) / 32 * 4;
}
...
CStretchEngine::CStretchEngine(ScanlineComposerIface* pDestBitmap,
  ...
  m_InterPitch = GetPitch(m_DestClip.Width() * m_DestBpp);
  m_ExtraMaskPitch = GetPitch(m_DestClip.Width() * 8);
  m_SrcPitch = GetPitch(m_SrcWidth * m_SrcBpp);
