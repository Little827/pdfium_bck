...
void CFX_BitmapComposer::ComposeScanlineV(
  ...
  if (!dest_span.empty()) {
    size_t dest_x_offset = Fx2DSizeOrDie(dest_x, Bpp);
    size_t dest_y_offset = Fx2DSizeOrDie(m_DestTop, dest_pitch);
    dest_span = dest_span.subspan(dest_x_offset).subspan(dest_y_offset);
    if (m_bFlipY) {
      size_t dest_flip_offset = Fx2DSizeOrDie(dest_pitch, m_DestHeight - 1);
