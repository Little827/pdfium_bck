...
bool CFX_DIBitmap::TransferWithUnequalFormats(
  ...
  ConvertBuffer(dest_format, dest_buf, m_Pitch, width, height, pSrcBitmap,
                src_left, src_top);
  return true;
}
...
bool CFX_DIBitmap::ConvertFormat(FXDIB_Format dest_format) {
  ...
  m_palette = ConvertBuffer(dest_format, {dest_buf.get(), dest_buf_size},
                            dest_pitch, m_Width, m_Height, holder, 0, 0);
