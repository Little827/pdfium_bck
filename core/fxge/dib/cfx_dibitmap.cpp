...
void CFX_DIBitmap::Clear(uint32_t color) {
  ...
  auto buffer_span = GetWritableBuffer();
  switch (GetFormat()) {
...
bool CFX_DIBitmap::TransferWithUnequalFormats(
  ...
  pdfium::span<uint8_t> dest_buf =
      GetWritableScanline(dest_top).subspan(offset.ValueOrDie());
...
void CFX_DIBitmap::TransferWithMultipleBPP(int dest_left,
  ...
  for (int row = 0; row < height; ++row) {
    ...
    fxcrt::spancpy(dest_span.subspan(dest_left * Bpp),
                   src_span.subspan(src_left * Bpp, width * Bpp));
  }
}
...
bool CFX_DIBitmap::ConvertFormat(FXDIB_Format dest_format) {
  ...
  if (dest_format == FXDIB_Format::kArgb && m_Format == FXDIB_Format::kRgb32) {
    ...
    for (int row = 0; row < m_Height; row++) {
      uint8_t* scanline = GetWritableScanline(row).subspan(3).data();
      for (int col = 0; col < m_Width; col++) {
        *scanline = 0xff;
        scanline += 4;
      }
