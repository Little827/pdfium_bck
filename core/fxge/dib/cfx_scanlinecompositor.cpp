...
void CFX_ScanlineCompositor::InitSourcePalette(FXDIB_Format src_format,
  ...
  const int pal_count = 1 << GetBppFromFormat(src_format);
  if (pSrcPalette) {
    if (bIsDestBpp8) {
      uint8_t* gray_pal = FX_Alloc(uint8_t, pal_count);
      m_SrcPalette.Set8BitPalette(gray_pal);
      if (bIsSrcCmyk) {
    ...
    uint32_t* pPalette = FX_Alloc(uint32_t, pal_count);
    m_SrcPalette.Set32BitPalette(pPalette);
    ...
    if (m_SrcFormat == FXDIB_1bppRgb) {
      ...
      if (m_DestFormat == FXDIB_Argb) {
        CompositeRow_8bppRgb2Argb_NoBlend_RgbByteOrder(
            dest_scan, src_scan, width, m_SrcPalette.Get32BitPalette(),
            clip_scan);
      } else {
...
void CFX_ScanlineCompositor::Palette::Set8BitPalette(uint8_t* ptr) {
  m_pData.reset(reinterpret_cast<uint32_t*>(ptr));
  m_Width = sizeof(uint8_t);
}

void CFX_ScanlineCompositor::Palette::Set32BitPalette(uint32_t* ptr) {
  m_pData.reset(ptr);
  m_Width = sizeof(uint32_t);
}

const uint8_t* CFX_ScanlineCompositor::Palette::Get8BitPalette() const {
  DCHECK(!m_pData || m_Width == sizeof(uint8_t));
  return reinterpret_cast<const uint8_t*>(m_pData.get());
}

const uint32_t* CFX_ScanlineCompositor::Palette::Get32BitPalette() const {
  DCHECK(!m_pData || m_Width == sizeof(uint32_t));
  return m_pData.get();
}
