...
bool InStretchBounds(int col, int row, const FX_RECT& clip_rect) {
  return col >= 0 && col <= clip_rect.Width() && row >= 0 &&
         row <= clip_rect.Height();
}

void AdjustCoords(int* col, int* row, const FX_RECT& clip_rect) {
  int& src_col = *col;
  int& src_row = *row;
  ...
  for (int row = 0; row < result_rect.Height(); row++) {
    ...
    for (int col = 0; col < result_rect.Width(); col++) {
      ...
      matrix_fix.Transform(col, row, &d.src_col_l, &d.src_row_l, &d.res_x,
                           &d.res_y);
      if (LIKELY(InStretchBounds(d.src_col_l, d.src_row_l, clip_rect))) {
        AdjustCoords(&d.src_col_l, &d.src_row_l, clip_rect);
        d.src_col_r = d.src_col_l + 1;
        d.src_row_r = d.src_row_l + 1;
        AdjustCoords(&d.src_col_r, &d.src_row_r, clip_rect);
...
void DoDownSampleLoop(const CFX_ImageTransformer::CalcData& cdata,
  ...
  for (int row = 0; row < result_rect.Height(); row++) {
    ...
    for (int col = 0; col < result_rect.Width(); col++) {
      ...
      if (LIKELY(InStretchBounds(d.src_col, d.src_row, clip_rect))) {
        AdjustCoords(&d.src_col, &d.src_row, clip_rect);
        func(d, dest);
      }
      dest += increment;
    }
