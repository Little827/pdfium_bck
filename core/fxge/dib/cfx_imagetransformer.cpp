...
bool CFX_ImageTransformer::Continue(PauseIndicatorIface* pPause) {
  ...
  switch (m_type) {
    case kNarrow:
      ContinueNarrow(pPause);
      break;
    case kSmall:
      break;
    case kNormal:
      ContinueNormal(pPause);
      break;
    default:
      NOTREACHED();
      break;
...
void CFX_ImageTransformer::ContinueNarrow(PauseIndicatorIface* pPause) {
...
void CFX_ImageTransformer::ContinueNormal(PauseIndicatorIface* pPause) {
  if (!m_Storer.GetBitmap())
    return;
