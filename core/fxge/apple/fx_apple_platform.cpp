...
bool CFX_AggDeviceDriver::DrawDeviceText(
  ...
  void* gGraphics = static_cast<CApplePlatform*>(pPlatform)->GetGrpahics();
  CGContextRef ctx = CGContextRef(pGraphics);
  if (!ctx)
    return false;
...
void CFX_Font::ReleasePlatformResource() {
  if (m_pPlatformFont) {
    auto* pPlatform = CFX_GEModule::Get()->GetPlatform();
    static_cast<CApplePlatform*>(pPlatform)->DestroyFont(m_pPlatformFont);
