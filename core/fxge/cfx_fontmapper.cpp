...
FixedUniniDataVector<uint8_t> CFX_FontMapper::RawBytesForIndex(size_t index) {
  CHECK_LT(index, m_FaceArray.size());

  void* font_handle = m_pFontInfo->MapFont(0, false, FX_Charset::kDefault, 0,
                                           GetFaceName(index));
  if (!font_handle)
    return FixedUniniDataVector<uint8_t>();
  ...
  FixedUniniDataVector<uint8_t> result(required_size);
  size_t actual_size =
...
RetainPtr<CFX_Face> CFX_FontMapper::GetCachedTTCFace(void* font_handle,
  ...
  if (!pFontDesc) {
    FixedUniniDataVector<uint8_t> font_data(ttc_size);
...
RetainPtr<CFX_Face> CFX_FontMapper::GetCachedFace(void* font_handle,
  ...
  if (!pFontDesc) {
    FixedUniniDataVector<uint8_t> font_data(data_size);
