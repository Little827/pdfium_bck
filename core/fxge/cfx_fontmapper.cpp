...
RetainPtr<CFX_Face> CFX_FontMapper::UseExternalSubst(
  ...
  ScopedFontDeleter scoped_font(m_pFontInfo.get(), font_handle);
  ByteString name;
  m_pFontInfo->GetFaceName(font_handle, &name);
  ...
  RetainPtr<CFX_Face> face =
      ttc_size ? GetCachedTTCFace(font_handle, ttc_size, font_size)
               : GetCachedFace(font_handle, name, weight, is_italic, font_size);
  if (!face)
    return nullptr;

  subst_font->m_Family = name;
...
RetainPtr<CFX_Face> CFX_FontMapper::FindSubstFont(const ByteString& name,
  ...
  return UseExternalSubst(font_handle, weight, is_italic, italic_angle, Charset,
                          subst_font);
}
