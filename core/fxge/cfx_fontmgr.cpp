...
RetainPtr<CFX_Face> CFX_FontMgr::AddCachedFace(
  ...
  RetainPtr<CFX_Face> face =
      CFX_Face::New(m_FTLibrary, {pData.get(), size}, face_index);
  if (!face)
    return nullptr;

  if (FXFT_Set_Pixel_Sizes(face->GetRec(), 64, 64) != 0)
    return nullptr;
...
RetainPtr<CFX_Face> CFX_FontMgr::GetFixedFace(pdfium::span<const uint8_t> span,
                                              int face_index) {
  InitFTLibrary();
  RetainPtr<CFX_Face> face = CFX_Face::New(m_FTLibrary, span, face_index);
  if (!face)
    return nullptr;
