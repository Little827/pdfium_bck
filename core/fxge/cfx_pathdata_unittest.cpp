...
TEST(CFX_PathData, BasicTest) {
  ...
  path.AppendPoint({0, 0}, FX_PathToType::kMove);
  path.AppendPoint({0, 1}, FX_PathToType::kLine);
  path.AppendPoint({1, 1}, FX_PathToType::kLine);
  path.AppendPoint({1, 0}, FX_PathToType::kLine);
...
TEST(CFX_PathData, Hexagon) {
  CFX_PathData path;
  path.AppendPoint({1, 0}, FX_PathToType::kMove);
  path.AppendPoint({2, 0}, FX_PathToType::kLine);
  path.AppendPoint({3, 1}, FX_PathToType::kLine);
  path.AppendPoint({2, 2}, FX_PathToType::kLine);
  path.AppendPoint({1, 2}, FX_PathToType::kLine);
  path.AppendPoint({0, 1}, FX_PathToType::kLine);
  ASSERT_EQ(6u, path.GetPoints().size());
  EXPECT_EQ(FX_PathToType::kLine, path.GetType(5));
...
TEST(CFX_PathData, ClosePath) {
  ...
  EXPECT_EQ(FX_PathToType::kLine, path.GetType(3));
  ...
  path.AppendPointAndClose({0, 0}, FX_PathToType::kLine);
  ASSERT_EQ(5u, path.GetPoints().size());
  EXPECT_EQ(FX_PathToType::kLine, path.GetType(3));
  EXPECT_TRUE(path.IsClosingFigure(3));
  EXPECT_EQ(FX_PathToType::kLine, path.GetType(4));
...
TEST(CFX_PathData, FivePointRect) {
  ...
  path.AppendPoint({0, 0}, FX_PathToType::kMove);
  path.AppendPoint({2, 0}, FX_PathToType::kLine);
  path.AppendPoint({2, 1}, FX_PathToType::kLine);
  path.AppendPoint({0, 1}, FX_PathToType::kLine);
  path.AppendPoint({0, 0}, FX_PathToType::kLine);
  ASSERT_EQ(5u, path.GetPoints().size());
  EXPECT_EQ(FX_PathToType::kLine, path.GetType(4));
...
TEST(CFX_PathData, NotRect) {
  ...
  path.AppendPoint({2, 1}, FX_PathToType::kLine);
  path.AppendPoint({0, 1}, FX_PathToType::kLine);
  path.AppendPoint({0, 0.1f}, FX_PathToType::kLine);
  EXPECT_FALSE(path.IsRect());
  Optional<CFX_FloatRect> rect = path.GetRect(nullptr);
  EXPECT_FALSE(rect.has_value());
  EXPECT_EQ(CFX_FloatRect(0, 0, 2, 1), path.GetBoundingBox());
  ...
  path.AppendPoint({2, 0}, FX_PathToType::kLine);
  path.AppendPoint({3, 1}, FX_PathToType::kLine);
  path.AppendPointAndClose({0, 1}, FX_PathToType::kLine);
  EXPECT_FALSE(path.IsRect());
  rect = path.GetRect(nullptr);
  EXPECT_FALSE(rect.has_value());
  ...
  path.AppendPoint({2, 0}, FX_PathToType::kLine);
  path.AppendPoint({2, 1}, FX_PathToType::kLine);
  path.AppendPointAndClose({0, 1}, FX_PathToType::kMove);
  EXPECT_FALSE(path.IsRect());
  rect = path.GetRect(nullptr);
  EXPECT_FALSE(rect.has_value());
  EXPECT_EQ(CFX_FloatRect(0, 0, 2, 1), path.GetBoundingBox());
  ...
  path.AppendPoint({0, 0}, FX_PathToType::kMove);
  path.AppendPoint({2, 0}, FX_PathToType::kLine);
  path.AppendPoint({3, 0}, FX_PathToType::kLine);
  path.AppendPointAndClose({0, 1}, FX_PathToType::kLine);
  EXPECT_FALSE(path.IsRect());
  rect = path.GetRect(nullptr);
  EXPECT_FALSE(rect.has_value());
...
TEST(CFX_PathData, Append) {
  CFX_PathData path;
  path.AppendPoint({5, 6}, FX_PathToType::kMove);
