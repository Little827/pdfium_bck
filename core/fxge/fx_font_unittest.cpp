...
TEST(FXFontTest, ReadFontNameFromMicrosoftEntries) {
  ...
  {
    // `folderFontInfo` has to be deallocated before the `fontMapper` or we run
    // into `UnownedPtr` issues with ASAN.
    CFX_FolderFontInfo folderFontInfo;
    folderFontInfo.AddPath(
        (testDataDir + PATH_SEPARATOR + "font_tests").c_str());
