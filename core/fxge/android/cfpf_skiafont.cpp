...
ByteString CFPF_SkiaFont::GetFamilyName() {
  if (!m_Face)
    return ByteString();
  return ByteString(FXFT_Get_Face_Family_Name(m_Face->GetRec()->GetRec()));
}

ByteString CFPF_SkiaFont::GetPsName() {
  if (!m_Face)
    return ByteString();
  return FXFT_Get_Postscript_Name(m_Face->GetRec());
}

int32_t CFPF_SkiaFont::GetGlyphIndex(wchar_t wUnicode) {
  if (!m_Face)
    return wUnicode;
  if (FXFT_Select_Charmap(m_Face->GetRec(), FXFT_ENCODING_UNICODE))
    return 0;
  return FXFT_Get_Char_Index(m_Face->GetRec(), wUnicode);
}
...
bool CFPF_SkiaFont::GetGlyphBBox(int32_t iGlyphIndex, FX_RECT& rtBBox) {
  ...
  if (FXFT_Is_Face_Tricky(m_Face->GetRec())) {
    if (FXFT_Set_Char_Size(m_Face->GetRec(), 0, 1000 * 64, 72, 72))
      return false;
    if (FXFT_Load_Glyph(m_Face->GetRec(), iGlyphIndex,
                        FXFT_LOAD_IGNORE_GLOBAL_ADVANCE_WIDTH)) {
      FXFT_Set_Pixel_Sizes(m_Face->GetRec(), 0, 64);
      return false;
    }
    FXFT_Glyph glyph;
    if (FXFT_Get_Glyph(m_Face->GetRec()->glyph, &glyph)) {
      FXFT_Set_Pixel_Sizes(m_Face->GetRec(), 0, 64);
      return false;
    }
    FXFT_BBox cbox;
...
uint32_t CFPF_SkiaFont::GetFontData(uint32_t dwTable,
  ...
  if (FXFT_Load_Sfnt_Table(m_Face->GetRec(), dwTable, 0, pBuffer, &ulSize))
    return 0;
