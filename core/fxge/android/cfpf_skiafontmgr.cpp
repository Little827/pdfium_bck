...
CFPF_SkiaFont* CFPF_SkiaFontMgr::CreateFont(const ByteStringView& bsFamilyname,
  ...
  auto familyIter = m_FamilyFonts.find(dwHash);
  if (familyIter != m_FamilyFonts.end())
    return familyIter->second.get();
  ...
  int32_t nMax = -1;
  int32_t nGlyphNum = 0;
  for (auto faceIter = m_FontFaces.rbegin(); faceIter != m_FontFaces.rend();
       ++faceIter) {
    const CFPF_SkiaPathFont* pFont = faceIter->get();
