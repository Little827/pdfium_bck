...
class CFX_FolderFontInfoTest : public ::testing::Test {
 public:
  ...
  void* FindFont(int weight,
                 ...
                 const char* family,
                 bool bMatchName) {
    return m_fontinfo.FindFont(weight, bItalic, charset, pitch_family, family,
                               bMatchName);
  }

  bool VerfiyFacename(void* pFont, const char* facename) {
    return static_cast<CFX_FolderFontInfo::FontFaceInfo*>(pFont)->m_FaceName ==
           facename;
  }
...
TEST_F(CFX_FolderFontInfoTest, TestFindFont) {
  ...
  EXPECT_TRUE(VerfiyFacename(pFont, kSymbol));
  ...
  ASSERT_TRUE(pFont != nullptr);
  EXPECT_TRUE(VerfiyFacename(pFont, kBookshelfSymbol7));
  ...
  ASSERT_TRUE(pFont != nullptr);
  EXPECT_TRUE(VerfiyFacename(pFont, kArial));
}
