...
class CFX_FolderFontInfoTest : public ::testing::Test {
 public:
  ...
  CFX_FolderFontInfo::FontFaceInfo* FindFont(int weight,
                                             ...
                                             const char* family,
                                             bool bMatchName) {
    return static_cast<CFX_FolderFontInfo::FontFaceInfo*>(font_info_.FindFont(
...
TEST_F(CFX_FolderFontInfoTest, TestFindFont) {
  // Find "Symbol" font
  CFX_FolderFontInfo::FontFaceInfo* font =
      FindFont(/*weight=*/0, /*bItalic=*/false, /*charset=*/2,
               /*pitch_family=*/2, kSymbol, /*bMatchName=*/true);
  ASSERT_TRUE(font);
  EXPECT_EQ(font->m_FaceName, kSymbol);
