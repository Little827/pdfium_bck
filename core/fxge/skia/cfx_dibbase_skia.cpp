...
void ReleaseRetainedHeldBySkImage(const void* /*pixels*/,
  ...
  RetainPtr<const CFX_DIBitmap> retained;
  retained.Unleak(reinterpret_cast<const CFX_DIBitmap*>(context));
}
...
sk_sp<SkImage> CreateSkiaImageFromDib(const CFX_DIBBase* source,
  ...
  RetainPtr<const CFX_DIBitmap> retained_bitmap = source->Realize();
  if (!retained_bitmap) {
    return nullptr;
  }
  CHECK(!retained_bitmap->GetBuffer().empty());

  // Convert retained pointer to a raw pointer, then "leak" to `SkImage`.
  const CFX_DIBitmap* bitmap = retained_bitmap.Leak();
