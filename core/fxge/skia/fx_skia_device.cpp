...
bool IsOpenPointOnLine(const CFX_Path::Point& begin_point,
  ...
  float slope1 = (maybe_middle.y - begin.y) / (maybe_middle.x - begin.x);
  float slope2 = (end.y - maybe_middle.y) / (end.x - maybe_middle.x);
...
SkPath BuildPath(const CFX_Path& path) {
  ...
  std::optional<size_t> last_used_line;
  for (size_t i = 0; i < points.size(); ++i) {
    ...
    if (point_type == CFX_Path::Point::Type::kMove) {
      ...
      last_used_line = std::nullopt;
    } else if (point_type == CFX_Path::Point::Type::kLine) {
      if (i > 0 && points[i - 1] == points[i]) {
        continue;
      }

      if (i > 0 && i < points.size() - 1 && last_used_line.has_value()) {
        if (IsOpenPointOnLine(points[last_used_line.value()], points[i],
                              points[i + 1])) {
          continue;
        }
      }
