...
class SkiaImageBuilder {
 public:
  ...
  // Constructs a builder backed by allocated 32-bit pixels.
  explicit SkiaImageBuilder(const SkImageInfo& info) : info_(info) {
    static_assert(sizeof(value_type) >= 4);
    DCHECK_LE(info_.bytesPerPixel(), 4);

    row_bytes_ = fxge::CalculatePitch32OrDie(
        /*bpp=*/8 * info_.bytesPerPixel(), info_.width());
    DCHECK_GE(row_bytes_, info_.minRowBytes());

    data_size_ = Fx2DSizeOrDie(row_bytes_ / 4, info_.height());
    data_ = AllocatedType(FX_Alloc(value_type, data_size_));
  }
  ...
  pdfium::span<value_type> allocated_pixels() {
    return pdfium::make_span(absl::get<AllocatedType>(data_).get(), data_size_);
  }
 ...
 private:
  ...
  using AllocatedType = std::unique_ptr<value_type, FxFreeDeleter>;
  ...
  // unowned `CFX_DIBBase` pointer or an owned `uint32_t` pointer.
  absl::variant<UnownedType, AllocatedType> data_;
  size_t data_size_;
};

// Creates a `SkiaImageBuilder` using a 1-bit-per-pixel palette.
SkiaImageBuilder CreateSkiaImageBuilderUsingSingleBitPalette(
  ...
  pdfium::span<SkPMColor> dst32_pixels(image_builder.allocated_pixels());
  ...
  switch (source->GetBPP()) {
    case 1: {
      ...
      pdfium::span<uint8_t> dst8_pixels =
          pdfium::as_writable_bytes(image_builder.allocated_pixels());
      for (int y = 0; y < height; ++y) {
    ...
    case 24: {
      ...
      pdfium::span<uint32_t> dst32_pixels = image_builder.allocated_pixels();
      for (int y = 0; y < height; ++y) {
        const uint8_t* srcRow = buffer + y * row_bytes;
        pdfium::span<uint32_t> dst_row = dst32_pixels.subspan(y * width);
        for (int x = 0; x < width; ++x) {
