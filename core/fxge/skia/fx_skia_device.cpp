...
class SkiaState {
 public:
  ...
  bool DrawText(int nChars,
    ...
    const size_t char_count = pdfium::base::checked_cast<size_t>(nChars);
    FX_SAFE_SIZE_T safe_count = m_charDetails.Count();
    safe_count += char_count;
    const size_t count = safe_count.ValueOrDie();
    m_charDetails.SetCount(count);
    if (hasRSX)
      m_rsxform.resize(count);

    const SkScalar flip = m_fontSize < 0 ? -1 : 1;
    const SkScalar vFlip = pFont->IsVertical() ? -1 : 1;
    for (size_t index = 0; index < char_count; ++index) {
      const TextCharPos& cp = pCharPos[index];
      size_t cur_index = index + count;
