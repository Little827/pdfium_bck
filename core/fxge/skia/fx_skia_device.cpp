...
// `palette_size`.
void Fill32BppDestStorageWithPalette(const RetainPtr<CFX_DIBBase>& source,
...
bool Upsample(const RetainPtr<CFX_DIBBase>& pSource,
  ...
  switch (pSource->GetBPP()) {
    case 1: {
      ...
      Fill32BppDestStorageWithPalette(pSource, new_palette,
                                      CFX_DIBBase::kPaletteSize, dst32_storage,
                                      &buffer);
    ...
    case 8:
      ...
      if (pSource->HasPalette()) {
        ...
        Fill32BppDestStorageWithPalette(pSource, src_palette, src_palette_size,
                                        dst32_storage, &buffer);
        rowBytes = width * sizeof(uint32_t);
        colorType = Get32BitSkColorType(bRgbByteOrder);
      }
      break;
