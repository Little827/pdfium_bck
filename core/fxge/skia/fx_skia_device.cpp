 ...
 public:
  ...
  void FlushText() {
    ...
    if (m_rsxform.count()) {
      ...
      for (int i = 0; i < m_glyphs.count(); ++i) {
        int gWidth = m_pFont->GetGlyphWidth(m_glyphs[i]);
        int pWidth = m_fontCharWidths[i];
        if (gWidth && pWidth && gWidth > pWidth) {
          float scale_x = (float)pWidth / gWidth;
...
bool CFX_SkiaDeviceDriver::DrawDeviceText(int nChars,
  ...
  if (useRSXform) {
    ...
    for (int index = 0; index < nChars; ++index) {
      const TextCharPos& cp = pCharPos[index];
      int gWidth = pFont->GetGlyphWidth(cp.m_GlyphIndex);
      int pWidth = cp.m_FontCharWidth;
      if (gWidth && pWidth && gWidth > pWidth) {
        float scaleX = (float)pWidth / gWidth;
