 ...
 public:
  ...
  bool DrawText(int nChars,
    ...
    if (Accumulator::kText != m_type) {
      m_italicAngle = pFont->GetItalicAngle();
  ...
  bool FontChanged(CFX_Font* pFont,
    ...
    int italicAngle = pFont->GetItalicAngle();
...
bool CFX_SkiaDeviceDriver::DrawDeviceText(int nChars,
  ...
  int italic_angle = pFont->GetItalicAngle();
  SkFont font;
  font.setTypeface(typeface);
  font.setHinting(SkFontHinting::kNone);
  font.setSize(SkTAbs(font_size));
  font.setSubpixel(true);
  font.setSkewX(tanf(italic_angle * FX_PI / 180.0));
