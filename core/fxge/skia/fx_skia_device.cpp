...
// from `palette`. `palette_size` sets the number of colors which can be used
// from `palette` since the actual size of `palette` can be larger than
// `palette_size`.
...
bool Upsample(const RetainPtr<CFX_DIBBase>& pSource,
  ...
  switch (pSource->GetBPP()) {
    case 1: {
      ...
      dst32_storage = Fill32BppDestStorageWithPalette(
          pSource, new_palette, CFX_DIBBase::kPaletteSize);
