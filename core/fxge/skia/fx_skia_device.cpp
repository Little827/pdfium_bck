...
class SkiaState {
 public:
  ...
  bool DrawText(int nChars,
    ...
    m_fontCharWidths.setCount(nChars + count);
    if (hasRSX)
    ...
    for (int index = 0; index < nChars; ++index) {
      ...
      m_fontCharWidths[index + count] = static_cast<int>(cp.m_FontCharWidth);
    ...
    if (m_rsxform.count()) {
      ...
      for (int i = 0; i < m_glyphs.count(); ++i) {
        uint32_t ftGlyphWidth =
            m_pFont ? m_pFont->GetGlyphWidth(m_glyphs[i]) : 0;
        uint32_t pdfGlyphWidth = m_fontCharWidths[i];
        if (ftGlyphWidth && pdfGlyphWidth && ftGlyphWidth > pdfGlyphWidth) {
          scale_x = static_cast<float>(pdfGlyphWidth) / ftGlyphWidth;
          font.setScaleX(scale_x);
        }
        sk_sp<SkTextBlob> blob = SkTextBlob::MakeFromText(
            &m_glyphs[i], (size_t)m_glyphs.bytes() / m_glyphs.count(), font,
            SkTextEncoding::kGlyphID);
        skCanvas->drawTextBlob(blob, m_positions[i].fX, m_positions[i].fY,
                               skPaint);
        font.setScaleX(1.0);
      }
    }
 ...
 private:
  ...
  SkTDArray<int>
      m_fontCharWidths;  // accumulator for glyphs' width defined in pdf
...
bool CFX_SkiaDeviceDriver::DrawDeviceText(int nChars,
  ...
  if (useRSXform) {
    ...
    for (int index = 0; index < nChars; ++index) {
      const TextCharPos& cp = pCharPos[index];
      uint32_t ftGlyphWidth = pFont ? pFont->GetGlyphWidth(cp.m_GlyphIndex) : 0;
      uint32_t pdfGlyphWidth = cp.m_FontCharWidth;
      if (ftGlyphWidth && pdfGlyphWidth && ftGlyphWidth > pdfGlyphWidth) {
        scale_x = static_cast<float>(pdfGlyphWidth) / ftGlyphWidth;
        font.setScaleX(scale_x);
      }
      auto blob = SkTextBlob::MakeFromText(&glyphs[index],
                                           (size_t)glyphs.bytes() / nChars,
                                           font, SkTextEncoding::kGlyphID);
      m_pCanvas->drawTextBlob(blob, positions[index].fX, positions[index].fY,
                              paint);
      font.setScaleX(1.0);
