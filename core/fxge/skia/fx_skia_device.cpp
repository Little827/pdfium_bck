...
class SkiaState {
 public:
  ...
  void FlushText() {
    ...
    if (!m_options.text_is_smooth)
      font.setEdging(SkFont::Edging::kAlias);
    if (m_options.text_use_lcd) {
      ASSERT(m_options.text_is_smooth);
  ...
  bool FontChanged(CFX_Font* pFont,
                   ...
                   float scaleX,
                   uint32_t color,
                   const CFX_RenderOptions& options) const {
...
bool CFX_SkiaDeviceDriver::DrawDeviceText(int nChars,
  ...
  if (!options.text_is_smooth)
    font.setEdging(SkFont::Edging::kAlias);
  if (options.text_use_lcd) {
    ASSERT(options.text_is_smooth);
    font.setEdging(SkFont::Edging::kSubpixelAntiAlias);
  }
