 ...
 public:
  ...
  bool DrawPath(const CFX_PathData* pPathData,
    ...
    if (Accumulator::kPath != m_type) {
      m_skPath.reset();
      m_fillFullCover = fill_options.fill_full_cover;
      m_fillPath =
          GetAlternateOrWindingFillMode(fill_options.fill_type) && fill_color;
      m_skPath.setFillType(
          GetAlternateOrWindingFillType(fill_options.fill_type));
      if (pDrawState)
  ...
  bool DrawChanged(const CFX_Matrix* pMatrix,
    ...
    return MatrixChanged(pMatrix) || StateChanged(pState, m_drawState) ||
           ...
           IsAlternateFillMode(fill_options.fill_type) ||
...
void CFX_SkiaDeviceDriver::SetClipMask(const FX_RECT& clipBox,
  ...
  SkPaint paint;
  paint.setAntiAlias(!m_FillFlags.no_path_smooth);
  canvas->drawPath(path, paint);
...
bool CFX_SkiaDeviceDriver::SetClip_PathFill(
    ...
    const CFX_FillRenderOptions&
        fill_options  // fill mode, WINDING or ALTERNATE
) {
  CFX_Matrix identity;
  const CFX_Matrix* deviceMatrix = pObject2Device ? pObject2Device : &identity;
  bool cached =
      m_pCache->SetClipFill(pPathData, deviceMatrix, fill_options.fill_type);

#ifdef _SKIA_SUPPORT_PATHS_
  m_FillFlags = fill_options;
  if (!m_pClipRgn) {
  ...
  skClipPath.setFillType(GetAlternateOrWindingFillType(fill_options.fill_type));
...
bool CFX_SkiaDeviceDriver::DrawPath(
    ...
    const CFX_FillRenderOptions&
        fill_options,  // fill mode, WINDING or ALTERNATE. 0 for not filled
    BlendMode blend_type) {
  ASSERT(GetAlternateOrWindingFillMode(fill_options.fill_type) !=
  ...
  SkPaint skPaint;
  skPaint.setAntiAlias(true);
  if (fill_options.fill_full_cover)
    skPaint.setBlendMode(SkBlendMode::kPlus);
  ...
  m_pCanvas->concat(skMatrix);
  bool do_stroke = true;
  if (GetAlternateOrWindingFillMode(fill_options.fill_type) && fill_color) {
    skPath.setFillType(GetAlternateOrWindingFillType(fill_options.fill_type));
    SkPath strokePath;
