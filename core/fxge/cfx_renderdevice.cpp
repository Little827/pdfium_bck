...
bool CFX_RenderDevice::DrawPathWithBlend(
  ...
  if (fill_options.fill_type != 0 && stroke_alpha == 0 &&
      !fill_options.is_stroke && !fill_options.is_text_mode) {
    ...
    if (pPathData->GetZeroAreaPath(pObject2Device,
      ...
      smooth_path.is_zero_area = true;
      smooth_path.is_path_aliased = fill_options.is_path_aliased;
...
bool CFX_RenderDevice::DrawNormalText(int nChars,
  ...
  if (fabs(char2device.a) + fabs(char2device.b) > 50 * 1.0f ||
    ...
    if (pFont->GetFaceRec()) {
      ...
      path_options.is_path_aliased = !is_text_smooth;
