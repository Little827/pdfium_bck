...
bool ShouldDrawDeviceText(const CFX_Font* pFont,
  ...
  if (options.GetFontOptions().is_cid)
    return false;
...
bool CFX_RenderDevice::DrawNormalText(int nChars,
                                      ...
                                      uint32_t fill_color,
                                      const CFX_RenderOptions& options) {
  const CFX_RenderOptions::TextOptions& text_options = options.GetTextOptions();
  CFX_RenderOptions::TextOptions native_text_options(text_options);
  // int nativetext_flags = text_flags; //TODO
  if (m_DeviceType != DeviceType::kDisplay) {
    if (!text_options
             .print_graphic_text /*(text_flags & FXTEXT_PRINTGRAPHICTEXT)*/) {
    ...
    if (FXARGB_A(fill_color) < 255)
      return false;
                  ...
                  .no_native_text /*(text_flags & FXTEXT_NO_NATIVETEXT)*/) {
    if (ShouldDrawDeviceText(pFont, options) &&
  ...
  if (fabs(char2device.a) + fabs(char2device.b) > 50 * 1.0f ||
      (m_DeviceType == DeviceType::kPrinter &&
       !text_options
            .print_image_text /*(text_flags & FXTEXT_PRINTIMAGETEXT)*/)) {
    if (pFont->GetFaceRec()) {
      int nPathFlags = text_options.anti_aliasing ? 0 : FXFILL_NOPATHSMOOTH;
  ...
  bool bNormal = false;
  if (text_options.anti_aliasing /*(text_flags & FXTEXT_NOSMOOTH) == 0*/) {
    if (m_DeviceType == DeviceType::kDisplay && m_bpp > 1) {
      if (!CFX_GEModule::Get()->GetFontMgr()->FTLibrarySupportsHinting()) {
        ...
        if (pFont->GetFaceRec())
          bClearType =
              text_options.use_lcd;  //!!(text_flags & FXTEXT_CLEARTYPE);
        bNormal = !bClearType;
      }
    }
  }
  ...
  for (const TextGlyphPos& glyph : glyphs) {
    ...
    bool bBGRStripe =
        text_options.bgr_stripe;  //!!(text_flags & FXTEXT_BGR_STRIPE);
    ncols /= 3;
