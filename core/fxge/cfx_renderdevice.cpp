...
bool CFX_RenderDevice::DrawPathWithBlend(
    ...
    const CFX_FillRenderOptions& fill_options,  // fill_mode
  ...
  if (fill_options.fill_type != 0 && stroke_alpha == 0 &&
    ...
    if (pPathData->GetZeroAreaPath(pObject2Device,
      ...
      if (fill_options.no_path_smooth)
        smooth_path.no_path_smooth = true;
...
bool CFX_RenderDevice::DrawCosmeticLine(
    ...
    uint32_t color,
    const CFX_FillRenderOptions& fill_options, /*int fill_mode,*/
    BlendMode blend_type) {
  ...
  return m_pDeviceDriver->DrawPath(&path, nullptr, &graph_state, 0, color,
                                   fill_options, /*fill_mode,*/ blend_type);
}
...
bool CFX_RenderDevice::DrawTextPath(
    int nChars,
    const TextCharPos* pCharPos,
    CFX_Font* pFont,
    float font_size,
    ...
    const CFX_FillRenderOptions& options /*int nFlag*/) {
    ...
    if (fill_color || stroke_color) {
      ...
      if (fill_color)
        fill_options.fill_type |=
            FXFILL_WINDING;                // fill_mode |= FXFILL_WINDING;
      fill_options.fill_text_mode = true;  // fill_mode |= FX_FILL_TEXT_MODE;
...
void CFX_RenderDevice::DrawBorder(const CFX_Matrix* pUser2Device,
  ...
  CFX_FillRenderOptions fill_options = CFX_FillRenderOptions(FXFILL_ALTERNATE);
  ...
  switch (nStyle) {
    ...
    case BorderStyle::DASH: {
      ...
      path.AppendPoint(CFX_PointF(fLeft + fHalfWidth, fBottom + fHalfWidth),
                       FXPT_TYPE::LineTo);
      CFX_FillRenderOptions winding_options =
    ...
    case BorderStyle::INSET: {
      ...
      DrawPath(&path_left_top, pUser2Device, &gsd,
               crLeftTop.ToFXColor(nTransparency), 0, fill_options);
      ...
      DrawPath(&path_right_bottom, pUser2Device, &gsd,
               crRightBottom.ToFXColor(nTransparency), 0, fill_options);
