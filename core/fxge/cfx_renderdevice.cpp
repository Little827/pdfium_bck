...
bool CFX_RenderDevice::SetClip_Rect(const FX_RECT& rect) {
  ...
  if (!SetClip_PathFill(&path, nullptr, CFX_FillRenderOptions(FXFILL_WINDING)))
    return false;
...
bool CFX_RenderDevice::DrawPathWithBlend(
  ...
  if (fill_options.fill_type != 0 && stroke_alpha == 0 &&
    ...
    if (pPathData->GetZeroAreaPath(pObject2Device,
      ...
      smooth_path.no_path_smooth = fill_options.no_path_smooth;
...
bool CFX_RenderDevice::DrawNormalText(int nChars,
  ...
  if (fabs(char2device.a) + fabs(char2device.b) > 50 * 1.0f ||
    ...
    if (pFont->GetFaceRec()) {
      ...
      nPathFlags.no_path_smooth = !!(text_flags & FXTEXT_NOSMOOTH);
...
void CFX_RenderDevice::DrawBorder(const CFX_Matrix* pUser2Device,
  ...
  const CFX_FillRenderOptions alternate_fill_options(FXFILL_ALTERNATE);
  ...
  switch (nStyle) {
    ...
    case BorderStyle::INSET: {
      ...
      DrawPath(&path_left_top, pUser2Device, &gsd,
               crLeftTop.ToFXColor(nTransparency), 0, alternate_fill_options);
