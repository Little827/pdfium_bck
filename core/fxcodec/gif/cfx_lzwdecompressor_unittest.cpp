...
TEST(CFX_LZWDecompressor, CreateBadParams) {
  EXPECT_FALSE(CFX_LZWDecompressor::Create(0x10, 0x02));
  EXPECT_FALSE(CFX_LZWDecompressor::Create(0x04, 0x0F));
  EXPECT_FALSE(CFX_LZWDecompressor::Create(0x02, 0x02));
}

TEST(CFX_LZWDecompressor, ExtractData) {
  static constexpr uint8_t kPaletteExp = 0x1;
  static constexpr uint8_t kCodeExp = 0x2;
  auto decompressor = CFX_LZWDecompressor::Create(kPaletteExp, kCodeExp);
...
TEST(CFX_LZWDecompressor, DecodeBadParams) {
  static constexpr uint8_t kPaletteExp = 0x0;
  static constexpr uint8_t kCodeExp = 0x2;
  auto decompressor = CFX_LZWDecompressor::Create(kPaletteExp, kCodeExp);
  ASSERT_TRUE(decompressor);

  uint8_t image_data[10];
  static constexpr uint32_t kImageSize = FX_ArraySize(image_data);
  ...
  EXPECT_EQ(
      ...
      decompressor->Decode(nullptr, kImageSize, output_data, &output_size));
  EXPECT_EQ(CFX_GifDecodeStatus::Error,
            decompressor->Decode(image_data, 0, output_data, &output_size));
  EXPECT_EQ(
      CFX_GifDecodeStatus::Error,
      decompressor->Decode(image_data, kImageSize, nullptr, &output_size));
  EXPECT_EQ(CFX_GifDecodeStatus::Error,
            decompressor->Decode(image_data, kImageSize, output_data, nullptr));

  output_size = 0;
  EXPECT_EQ(
      CFX_GifDecodeStatus::InsufficientDestSize,
      decompressor->Decode(image_data, kImageSize, output_data, &output_size));
}
...
TEST(CFX_LZWDecompressor, Decode1x1SingleColour) {
  ...
  static constexpr uint8_t kImageData[] = {0x44, 0x01};
  static constexpr uint32_t kImageSize = FX_ArraySize(kImageData);

  static constexpr uint8_t kExpectedData[] = {0x00};
  ...
  EXPECT_EQ(
      ...
      decompressor->Decode(kImageData, kImageSize, output_data, &output_size));

  EXPECT_EQ(FX_ArraySize(output_data), output_size);
  EXPECT_EQ(0, memcmp(kExpectedData, output_data, sizeof(kExpectedData)));
}
...
TEST(CFX_LZWDecompressor, HandleColourCodeOutOfPalette) {
  // Image uses 10 colours, so |kPaletteExp| should be 2, 2^(3+1) = 16 colours.
  static constexpr uint8_t kPaletteExp = 0x2;
  static constexpr uint8_t kCodeExp = 0x4;
  auto decompressor = CFX_LZWDecompressor::Create(kPaletteExp, kCodeExp);
  ASSERT_TRUE(decompressor);
