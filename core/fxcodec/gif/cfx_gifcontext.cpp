  ...
  while (true) {
    switch (decode_status_) {
      ...
      case GIF_D_STATUS_SIG: {
        uint8_t signature;
        if (!ReadAllOrNone(pdfium::span_from_ref(signature))) {
      ...
      case GIF_D_STATUS_EXT: {
        uint8_t extension;
        if (!ReadAllOrNone(pdfium::span_from_ref(extension))) {
          return GifDecoder::Status::kUnfinished;
        }
        switch (extension) {
          case GIF_BLOCK_CE:
      ...
      case GIF_D_STATUS_IMG_DATA: {
        ...
        if (!ReadAllOrNone(pdfium::span_from_ref(img_data_size))) {
          return GifDecoder::Status::kUnfinished;
        }
...
GifDecoder::Status CFX_GifContext::ReadGifSignature() {
  ...
  auto header_span = pdfium::as_writable_bytes(pdfium::span_from_ref(header));
  if (!ReadAllOrNone(header_span)) {
    return GifDecoder::Status::kUnfinished;
  }
...
GifDecoder::Status CFX_GifContext::ReadLogicalScreenDescriptor() {
  ...
  auto lsd_span = pdfium::as_writable_bytes(pdfium::span_from_ref(lsd));
  size_t read_marker = input_buffer_->GetPosition();
  if (!ReadAllOrNone(lsd_span)) {
    return GifDecoder::Status::kUnfinished;
  }
...
GifDecoder::Status CFX_GifContext::DecodeExtension() {
  ...
  switch (decode_status_) {
    ...
    case GIF_D_STATUS_EXT_PTE: {
      ...
      auto pte_span = pdfium::as_writable_bytes(pdfium::span_from_ref(gif_pte));
      if (!ReadAllOrNone(pte_span)) {
        return GifDecoder::Status::kUnfinished;
      }
    ...
    case GIF_D_STATUS_EXT_GCE: {
      ...
      auto gce_span = pdfium::as_writable_bytes(pdfium::span_from_ref(gif_gce));
      if (!ReadAllOrNone(gce_span)) {
        return GifDecoder::Status::kUnfinished;
      }
...
GifDecoder::Status CFX_GifContext::DecodeImageInfo() {
  ...
  auto info_span = pdfium::as_writable_bytes(pdfium::span_from_ref(img_info));
  if (!ReadAllOrNone(info_span)) {
    return GifDecoder::Status::kUnfinished;
  }
