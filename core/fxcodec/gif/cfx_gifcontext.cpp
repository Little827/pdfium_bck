...
CFX_GifDecodeStatus CFX_GifContext::ReadLogicalScreenDescriptor() {
  ...
  width_ = FXWORD_GetLsbFirst(lsd.width);
  height_ = FXWORD_GetLsbFirst(lsd.height);
  ...
  switch (decode_status_) {
    ...
    case GIF_D_STATUS_EXT_GCE: {
      ...
      graphic_control_extension_->delay_time =
          FXWORD_GetLsbFirst(gif_gce.delay_time);
...
CFX_GifDecodeStatus CFX_GifContext::DecodeImageInfo() {
  ...
  gif_image->image_info.left = FXWORD_GetLsbFirst(img_info.left);
  gif_image->image_info.top = FXWORD_GetLsbFirst(img_info.top);
  gif_image->image_info.width = FXWORD_GetLsbFirst(img_info.width);
  gif_image->image_info.height = FXWORD_GetLsbFirst(img_info.height);
