...
CJPX_Decoder::CJPX_Decoder(bool bUseColorSpace, bool bColorSpaceIndexed)
    : m_bUseColorSpace(bUseColorSpace),
      m_bColorSpaceIndexed(bColorSpaceIndexed) {}
...
bool CJPX_Decoder::Init(pdfium::span<const uint8_t> src_data) {
  ...
  if (m_bUseColorSpace && m_bColorSpaceIndexed)
  ...
  m_Image = pTempImage;
#ifndef USE_SYSTEM_LIBOPENJPEG2
  m_Image->pdfium_use_colorspace = m_bUseColorSpace;
#endif
  return true;
}
