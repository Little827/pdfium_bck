...
class CCodec_JpegDecoder final : public CCodec_ScanlineDecoder {
 ...
 private:
  ...
  bool HasKnownBadOfficeJetHeader() const;

  // Patch up the in-memory JPEG header for known bad JPEGs.
  void PatchUpKnownBadOfficeJetHeader();
  ...
  static const int kKnownBadOfficeJetHeaderByteOffsetEnd = 164;
...
bool CCodec_JpegDecoder::InitDecode(bool bAcceptKnownBadHeader) {
  ...
  bool bHasKnownBadHeader = false;
  if (setjmp(m_JmpBuf) == -1) {
    bHasKnownBadHeader = bAcceptKnownBadHeader && HasKnownBadOfficeJetHeader();
    jpeg_destroy_decompress(&m_cinfo);
    ...
    PatchUpKnownBadOfficeJetHeader();
...
bool CCodec_JpegDecoder::HasKnownBadOfficeJetHeader() const {
  ...
  if (m_SrcSpan.size() <= kKnownBadOfficeJetHeaderByteOffsetEnd)
    return false;

  const uint8_t* pHeaderDimensions =
      &m_SrcSpan[kKnownBadOfficeJetHeaderByteOffsetEnd - 3];
  uint8_t nExpectedWidthByte1 = (m_OrigWidth >> 8) & 0xff;
  uint8_t nExpectedWidthByte2 = m_OrigWidth & 0xff;
  return pHeaderDimensions[0] == nExpectedWidthByte1 &&
         pHeaderDimensions[1] == nExpectedWidthByte2 &&
         pHeaderDimensions[2] == 0xff && pHeaderDimensions[3] == 0xff;
}

void CCodec_JpegDecoder::PatchUpKnownBadOfficeJetHeader() {
  ASSERT(m_SrcSpan.size() > kKnownBadOfficeJetHeaderByteOffsetEnd);
  uint8_t* pData = GetWritableSrcData();
  pData[kKnownBadOfficeJetHeaderByteOffsetEnd - 1] = (m_OrigHeight >> 8) & 0xff;
  pData[kKnownBadOfficeJetHeaderByteOffsetEnd] = m_OrigHeight & 0xff;
}

void CCodec_JpegDecoder::PatchUpFooter() {
  uint8_t* pData = GetWritableSrcData();
...
bool CCodec_JpegModule::JpegEncode(const RetainPtr<CFX_DIBBase>& pSource,
  ...
  *dest_buf = FX_TryAlloc(uint8_t, dest_buf_length);
  ...
  while (cinfo.next_scanline < cinfo.image_height) {
    uint8_t* src_scan = pSource->GetScanline(cinfo.next_scanline);
    if (nComponents > 1) {
