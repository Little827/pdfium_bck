...
class CCodec_JpegDecoder final : public CCodec_ScanlineDecoder {
 ...
 private:
  ...
  static const int kKnownBadOfficeJetHeaderByteOffsetStart = 163;
...
bool CCodec_JpegDecoder::HasKnownBadOfficeJetHeader() const {
  ...
  if (m_SrcSpan.size() <= kKnownBadOfficeJetHeaderByteOffsetStart + 3)
    return false;

  const uint8_t* pHeaderDimensions =
      &m_SrcSpan[kKnownBadOfficeJetHeaderByteOffsetStart];
...
void CCodec_JpegDecoder::PatchUpKnownBadOfficeJetHeader() {
  ASSERT(m_SrcSpan.size() > kKnownBadOfficeJetHeaderByteOffsetStart + 1);
  uint8_t* pData = GetWritableSrcData();
  pData[kKnownBadOfficeJetHeaderByteOffsetStart] = (m_OrigHeight >> 8) & 0xff;
  pData[kKnownBadOfficeJetHeaderByteOffsetStart + 1] = m_OrigHeight & 0xff;
}
