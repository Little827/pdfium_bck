...
bool CCodec_ProgressiveDecoder::PngDetectImageTypeInBuffer(
  ...
  while (pPngModule->Input(m_pPngContext.get(), m_pCodecMemory, pAttribute)) {
    ...
    if (!m_pFile->ReadBlockWithOffset(m_pCodecMemory->GetBuffer(), m_offSet,
                                      input_size)) {
...
FXCODEC_STATUS CCodec_ProgressiveDecoder::PngContinueDecode() {
  ...
  while (true) {
    ...
    bool bResult = m_pFile->ReadBlockWithOffset(m_pCodecMemory->GetBuffer(),
                                                m_offSet, input_size);
    if (!bResult) {
      m_pDeviceBitmap = nullptr;
      m_pFile = nullptr;
...
bool CCodec_ProgressiveDecoder::ReadMoreData(
  ...
  if (!m_pFile->ReadBlockWithOffset(m_pCodecMemory->GetBuffer() + dwUnconsumed,
