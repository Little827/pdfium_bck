...
bool CJPX_Decoder::Decode(pdfium::span<uint8_t> dest_buf,
  ...
  CHECK_LE(color_channel_count, m_Image->numcomps);
  uint32_t channel_count = color_channel_count;
  ...
  // Initialize `channel_bufs` and `adjust_comps` to store components
  // information from the JPX image. They will contain more information besides
  // the color channel data if `m_Image->numcomps` > `channel_count`. Currently
  // only the color channel data is used for rendering.
