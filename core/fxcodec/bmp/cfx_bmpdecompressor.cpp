...
void CFX_BmpDecompressor::ReadScanlineAndAdvanceRowNumber() {
  uint32_t row_num = img_tb_flag_ ? row_num_ : (height_ - 1 - row_num_);
  ++row_num_;
  context_->m_pDelegate->BmpReadScanline(row_num, out_row_buffer_);
}
...
int32_t CFX_BmpDecompressor::DecodeRGB() {
  ...
  while (row_num_ < height_) {
    ...
    ReadScanlineAndAdvanceRowNumber();
  }
