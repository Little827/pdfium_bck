...
constexpr int kOptConstant9[] = {12, 9, 7};
constexpr int kOptConstant10[] = {0x07, 0x0f, 0x07};
constexpr int kOptConstant11[] = {0x1f, 0x1f, 0x0f};
constexpr int kOptConstant12[] = {0x0f, 0x07, 0x03};
...
std::unique_ptr<CJBig2_Image> CJBig2_GRDProc::DecodeArithTemplateUnopt(
  ...
  for (uint32_t h = 0; h < GBH; h++) {
    ...
    if (LTP) {
      ...
      uint32_t line1 = GBREG->GetPixel(1 + (UNOPT % 2), h - 2);
      line1 |= GBREG->GetPixel((UNOPT % 2), h - 2) << 1;
      if (UNOPT == 1)
        line1 |= GBREG->GetPixel(0, h - 2) << 2;
      uint32_t line2 = GBREG->GetPixel(2 - (UNOPT / 2), h - 1);
      line2 |= GBREG->GetPixel(1 - (UNOPT / 2), h - 1) << 1;
      if (UNOPT < 2)
      ...
      for (uint32_t w = 0; w < GBW; w++) {
        ...
        if (USESKIP && SKIP->GetPixel(w, h)) {
          ...
          CONTEXT |= GBREG->GetPixel(w + GBAT[0], h + GBAT[1]) << (4 - UNOPT);
          CONTEXT |= line2 << (5 - UNOPT);
        ...
        line1 = ((line1 << 1) | GBREG->GetPixel(w + 2 + (UNOPT % 2), h - 2)) &
                kOptConstant10[UNOPT];
        line2 = ((line2 << 1) | GBREG->GetPixel(w + 3 - (UNOPT / 2), h - 1)) &
