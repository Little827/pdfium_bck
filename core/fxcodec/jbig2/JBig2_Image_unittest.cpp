...
TEST(fxcodec, JBig2EmptyImage) {
  ...
  // Bad dimensions and bad offsets give an emtpy imge.
...
TEST(fxcodec, JBig2SubImage) {
  ...
  // Empty subimage.
  auto sub1 = img32->SubImage(0, 0, 0, 0);
  EXPECT_EQ(sub1->width(), 0);
  EXPECT_EQ(sub1->height(), 0);

  // Full sub-image.
  auto sub2 = img32->SubImage(0, 0, 32, 5);
  EXPECT_EQ(sub2->width(), 32);
  EXPECT_EQ(sub2->height(), 5);
  CheckImageEq(img32.get(), sub2.get(), __LINE__);

  sub2 = img37->SubImage(0, 0, 32, 5);
  EXPECT_EQ(sub2->width(), 32);
  EXPECT_EQ(sub2->height(), 5);
  CheckImageEq(img32.get(), sub2.get(), __LINE__);

  // Actual bit manipulations.
  auto sub3 = img32->SubImage(2, 0, 30, 5);
  CheckImageEq(expected20.get(), sub3.get(), __LINE__);

  auto sub4 = img37->SubImage(2, 2, 30, 5);
  CheckImageEq(expected22.get(), sub4.get(), __LINE__);

  // Aligned Sub-image including cruft in stride beyond width.
  auto sub5 = img37->SubImage(32, 0, 32, 5);
  CheckImageEq(expected_zeros.get(), sub5.get(), __LINE__);

  // Sub-image waaaaay beyond width.
  auto sub6 = img37->SubImage(2000, 0, 32, 5);
  CheckImageEq(expected_zeros.get(), sub6.get(), __LINE__);

  // Sub-image waaaaay beyond height.
  auto sub7 = img37->SubImage(0, 2000, 32, 5);
  CheckImageEq(expected_zeros.get(), sub7.get(), __LINE__);

  // Sub-image with negative x offset.
  auto sub8 = img37->SubImage(-1, 0, 32, 5);
  CheckImageEq(expected_zeros.get(), sub8.get(), __LINE__);

  // Sub-image with negative y offset.
  auto sub9 = img37->SubImage(0, -1, 32, 5);
  CheckImageEq(expected_zeros.get(), sub9.get(), __LINE__);

  // Sub-image with negative width.
  auto sub10 = img37->SubImage(-1, 0, 32, 5);
  CheckImageEq(expected_zeros.get(), sub10.get(), __LINE__);

  // Sub-image with negative height.
  auto sub11 = img37->SubImage(0, -1, 32, 5);
  CheckImageEq(expected_zeros.get(), sub11.get(), __LINE__);
}
