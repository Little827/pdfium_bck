...
std::unique_ptr<CJBig2_Image> CJBig2_GRRDProc::DecodeTemplate0Unopt(
  ...
  for (uint32_t h = 0; h < GRH; h++) {
    if (TPGRON) {
      ...
      LTP = LTP ^ pArithDecoder->Decode(UNSAFE_TODO(&grContexts[0x0010]));
    }
    uint32_t lines[5];
    ...
    if (!LTP) {
      for (uint32_t w = 0; w < GRW; w++) {
        ...
        int bVal = pArithDecoder->Decode(UNSAFE_TODO(&grContexts[CONTEXT]));
      ...
      for (uint32_t w = 0; w < GRW; w++) {
        ...
        if (!(TPGRON && (bVal == GRREFERENCE->GetPixel(w - 1, h - 1)) &&
          ...
          bVal = pArithDecoder->Decode(UNSAFE_TODO(&grContexts[CONTEXT]));
        }
        DecodeTemplate0UnoptSetPixel(GRREG.get(), lines, w, h, bVal);
      }
    }
...
std::unique_ptr<CJBig2_Image> CJBig2_GRRDProc::DecodeTemplate1Unopt(
  ...
  for (uint32_t h = 0; h < GRH; h++) {
    if (TPGRON) {
      ...
      LTP = LTP ^ pArithDecoder->Decode(UNSAFE_TODO(&grContexts[0x0008]));
    }
    if (!LTP) {
