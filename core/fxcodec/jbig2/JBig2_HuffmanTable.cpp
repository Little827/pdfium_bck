...
// Zero dummy to preserve numbering.
constexpr JBig2TableLine kTableLine0[] = {{0, 0, 0}};
...
constexpr HuffmanTable kHuffmanTables[16] = {
    {false, kTableLine0, FX_ArraySize(kTableLine0)},
...
CJBig2_HuffmanTable::CJBig2_HuffmanTable(size_t idx) {
  ...
  HTOOB = table.HTOOB;
  NTEMP = table.size;
  m_bOK = ParseFromStandardTable(table.lines);
  ASSERT(m_bOK);
}
