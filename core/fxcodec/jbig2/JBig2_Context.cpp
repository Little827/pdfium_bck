...
JBig2_Result CJBig2_Context::DecodeSequential(PauseIndicatorIface* pPause) {
  ...
  while (m_pStream->getByteLeft() >= JBIG2_MIN_SEGMENT_SIZE) {
    ...
    if (m_ProcessingStatus == FXCODEC_STATUS::kDECODE_TOBECONTINUE) {
      m_ProcessingStatus = FXCODEC_STATUS::kDECODE_TOBECONTINUE;
      m_PauseStep = 2;
...
bool CJBig2_Context::Continue(PauseIndicatorIface* pPause) {
  m_ProcessingStatus = FXCODEC_STATUS::kDECODE_READY;
  JBig2_Result nRet = JBig2_Result::kSuccess;
  if (m_PauseStep == 5) {
    m_ProcessingStatus = FXCODEC_STATUS::kDECODE_FINISH;
    return true;
  }
...
JBig2_Result CJBig2_Context::ParseSegmentData(CJBig2_Segment* pSegment,
  ...
  while (m_ProcessingStatus == FXCODEC_STATUS::kDECODE_TOBECONTINUE &&
