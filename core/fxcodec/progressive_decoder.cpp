...
FXCODEC_STATUS ProgressiveDecoder::BmpStartDecode(
  ...
  m_status = FXCODEC_STATUS::kDECODE_TOBECONTINUE;
  return m_status;
}
...
FXCODEC_STATUS ProgressiveDecoder::BmpContinueDecode() {
  ...
  while (read_res == BmpDecoder::Status::kContinue) {
    FXCODEC_STATUS error_status = FXCODEC_STATUS::kDECODE_FINISH;
...
FXCODEC_STATUS ProgressiveDecoder::GifContinueDecode() {
  ...
  if (readRes == GifDecoder::Status::kSuccess) {
    m_pDeviceBitmap = nullptr;
    m_pFile = nullptr;
    m_status = FXCODEC_STATUS::kDECODE_FINISH;
    return m_status;
  }
...
FXCODEC_STATUS ProgressiveDecoder::LoadImageInfo(
  ...
  switch (m_status) {
    case FXCODEC_STATUS::kFRAME_READY:
    case FXCODEC_STATUS::kFRAME_TOBECONTINUE:
    case FXCODEC_STATUS::kDECODE_READY:
    case FXCODEC_STATUS::kDECODE_TOBECONTINUE:
      return FXCODEC_STATUS::kERROR;
    default:
      break;
  }
  ...
  if (imageType != FXCODEC_IMAGE_UNKNOWN &&
    ...
    m_status = FXCODEC_STATUS::kFRAME_READY;
    return m_status;
  }
...
void ProgressiveDecoder::SetClipBox(FX_RECT* clip) {
  if (m_status != FXCODEC_STATUS::kFRAME_READY)
    return;
...
std::pair<FXCODEC_STATUS, size_t> ProgressiveDecoder::GetFrames() {
  if (!(m_status == FXCODEC_STATUS::kFRAME_READY ||
        m_status == FXCODEC_STATUS::kFRAME_TOBECONTINUE)) {
...
#endif  // PDF_ENABLE_XFA_TIFF
      m_FrameNumber = 1;
      m_status = FXCODEC_STATUS::kDECODE_READY;
      return {m_status, 1};
#ifdef PDF_ENABLE_XFA_GIF
...
FXCODEC_STATUS ProgressiveDecoder::StartDecode(
    ...
    int size_x,
    int size_y) {
  if (m_status != FXCODEC_STATUS::kDECODE_READY)
    return FXCODEC_STATUS::kERROR;
...
FXCODEC_STATUS ProgressiveDecoder::ContinueDecode() {
  if (m_status != FXCODEC_STATUS::kDECODE_TOBECONTINUE)
    return FXCODEC_STATUS::kERROR;
