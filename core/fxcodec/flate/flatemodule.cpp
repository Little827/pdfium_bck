...
void CLZWDecoder::AddCode(uint32_t prefix_code, uint8_t append_char) {
  ...
  codes_.writable_span()[current_code_++] = (prefix_code << 16) | append_char;
...
void CLZWDecoder::DecodeString(uint32_t code) {
  while (true) {
    ...
    uint32_t data = codes_.span()[index];
    if (stack_len_ >= decode_stack_.size())
      return;

    decode_stack_.writable_span()[stack_len_++] = static_cast<uint8_t>(data);
    code = data >> 16;
  }
  if (stack_len_ >= decode_stack_.size())
    return;

  decode_stack_.writable_span()[stack_len_++] = static_cast<uint8_t>(code);
}
...
bool CLZWDecoder::Decode() {
  ...
  while (true) {
    ...
    if (code - 258 >= current_code_) {
      if (stack_len_ < decode_stack_.size())
        decode_stack_.writable_span()[stack_len_++] = last_char;
    ...
    for (uint32_t i = 0; i < stack_len_; i++)
      dest_buf_.get()[dest_byte_pos_ + i] =
          decode_stack_.span()[stack_len_ - i - 1];
    dest_byte_pos_ += stack_len_;
    last_char = decode_stack_.span()[stack_len_ - 1];
