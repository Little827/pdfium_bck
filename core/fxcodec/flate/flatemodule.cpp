...
class CLZWDecoder {
 ...
 private:
  ...
  size_t src_bit_pos_ = 0;
  size_t dest_byte_pos_ = 0;
...
void CLZWDecoder::AddCode(uint32_t prefix_code, uint8_t append_char) {
  if (current_code_ + m_nEarly == 4094)
    return;

  codes_[current_code_++] = (prefix_code << 16) | append_char;
  if (current_code_ + m_nEarly == 512 - 258)
    code_len_ = 10;
  else if (current_code_ + m_nEarly == 1024 - 258)
    code_len_ = 11;
  else if (current_code_ + m_nEarly == 2048 - 258)
    code_len_ = 12;
}
