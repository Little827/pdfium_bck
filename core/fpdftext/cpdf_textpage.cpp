...
CPDF_TextPage::StreamObject::StreamObject() = default;

CPDF_TextPage::StreamObject::StreamObject(const StreamObject& that) = default;
...
void CPDF_TextPage::ProcessObject() {
  ...
  for (const auto& obj : m_StreamObjects)
...
void CPDF_TextPage::ProcessTextObject(
  ...
  size_t count = m_StreamObjects.size();
  StreamObject new_obj;
  new_obj.m_pTextObj = pTextObj;
  new_obj.m_formMatrix = formMatrix;
  if (count == 0) {
    m_StreamObjects.push_back(new_obj);
    return;
  }
  if (IsSameAsPreTextObject(pTextObj, pObjList, ObjPos))
    return;

  StreamObject prev_obj = m_StreamObjects[count - 1];
  ...
  if (fabs(this_pos.y - prev_pos.y) > threshold * 2) {
    for (size_t i = 0; i < count; ++i)
      ProcessTextObject(m_StreamObjects[i]);
    m_StreamObjects.clear();
    m_StreamObjects.push_back(new_obj);
    return;
  }
  ...
  for (size_t i = count; i > 0; --i) {
    StreamObject prev_text_obj = m_StreamObjects[i - 1];
    ...
    if (this_pos.x >= new_prev_pos.x) {
      m_StreamObjects.insert(m_StreamObjects.begin() + i, new_obj);
      return;
    }
  }
  m_StreamObjects.insert(m_StreamObjects.begin(), new_obj);
}
...
void CPDF_TextPage::ProcessMarkedContent(const StreamObject& obj) {
...
void CPDF_TextPage::ProcessTextObject(const StreamObject& obj) {
