...
float CalculateBaseSpace(const CPDF_TextObject* pTextObj,
  ...
  if (!pTextObj->m_GraphicStates.m_TextState.GetCharSpace() || nItems < 3) {
    return 0.0f;
  }

  bool bAllChar = true;
  float spacing = matrix.TransformDistance(
      pTextObj->m_GraphicStates.m_TextState.GetCharSpace());
  float baseSpace = spacing;
  for (size_t i = 0; i < nItems; ++i) {
    CPDF_TextObject::Item item = pTextObj->GetItemInfo(i);
    if (item.m_CharCode == 0xffffffff) {
      float fontsize_h = pTextObj->m_GraphicStates.m_TextState.GetFontSizeH();
...
void CPDF_TextPage::ProcessTextObject(const TransformedTextObject& obj) {
  ...
  for (size_t i = 0; i < nItems; ++i) {
    ...
    float charSpace = pTextObj->m_GraphicStates.m_TextState.GetCharSpace();
    if (charSpace > 0.001)
