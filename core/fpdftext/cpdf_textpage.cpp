...
bool IsControlChar(const CPDF_TextPage::PageCharInfo& char_info) {
...
CPDF_TextPage::CharInfoBase::CharInfoBase() = default;

CPDF_TextPage::CharInfoBase::CharInfoBase(const CharInfoBase&) = default;
...
CPDF_TextPage::PageCharInfo::PageCharInfo() = default;

CPDF_TextPage::PageCharInfo::PageCharInfo(const PageCharInfo&) = default;
...
void CPDF_TextPage::Init() {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    const PageCharInfo& charinfo = m_CharList[i];
...
std::vector<CFX_FloatRect> CPDF_TextPage::GetRectArray(int start,
  ...
  while (nCount--) {
    const PageCharInfo& info_curchar = m_CharList[curPos++];
...
WideString CPDF_TextPage::GetTextByPredicate(
    const std::function<bool(const PageCharInfo&)>& predicate) const {
  if (!m_bIsParsed)
    return WideString();
...
WideString CPDF_TextPage::GetTextByRect(const CFX_FloatRect& rect) const {
  return GetTextByPredicate([&rect](const PageCharInfo& charinfo) {
...
WideString CPDF_TextPage::GetTextByObject(
  ...
  return GetTextByPredicate([pTextObj](const PageCharInfo& charinfo) {
...
void CPDF_TextPage::GetCharInfo(size_t index, CharInfo* info) const {
  ...
  const PageCharInfo& charinfo = m_CharList[index];
...
void CPDF_TextPage::AppendGeneratedCharacter(wchar_t unicode,
  ...
  Optional<PageCharInfo> pGenerateChar = GenerateCharInfo(unicode);
  if (!pGenerateChar)
    return;
...
void CPDF_TextPage::AddCharInfoByLRDirection(wchar_t wChar,
                                             const PageCharInfo& info) {
  PageCharInfo info2 = info;
...
void CPDF_TextPage::FindPreviousTextObject() {
  const PageCharInfo* pPrevCharInfo = GetPrevCharInfo();
  if (!pPrevCharInfo)
    return;
...
void CPDF_TextPage::ProcessTextObject(PDFTEXT_Obj Obj) {
  ...
  if (m_pPreTextObj) {
    ...
    switch (result) {
      ...
      case GenerateCharacter::kSpace: {
        Optional<PageCharInfo> pGenerateChar =
      ...
      case GenerateCharacter::kHyphen:
        ...
        PageCharInfo* charinfo = &m_TempCharList.back();
        m_TempTextBuf.Delete(m_TempTextBuf.GetLength() - 1, 1);
  ...
  for (size_t i = 0; i < nItems; ++i) {
    ...
    for (int n = pdfium::CollectionSize<int>(m_TempCharList);
      ...
      const PageCharInfo& charinfo1 = m_TempCharList[n - 1];
...
const CPDF_TextPage::PageCharInfo* CPDF_TextPage::GetPrevCharInfo() const {
  if (!m_TempCharList.empty())
...
Optional<CPDF_TextPage::PageCharInfo> CPDF_TextPage::GenerateCharInfo(
    wchar_t unicode) {
  const PageCharInfo* pPrevCharInfo = GetPrevCharInfo();
  if (!pPrevCharInfo)
    return {};
