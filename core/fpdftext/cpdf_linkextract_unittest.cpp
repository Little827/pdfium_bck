...
TEST(CPDF_LinkExtractTest, CheckMailLink) {
  ...
  constexpr std::pair<const wchar_t*, const wchar_t*> kValidStrings[] = {
  ...
  for (const auto& it : kValidStrings) {
    WideString text_str(it.first);
    WideString expected_str(L"mailto:");
    expected_str += it.second;
    EXPECT_TRUE(extractor.CheckMailLink(&text_str)) << it.first;
