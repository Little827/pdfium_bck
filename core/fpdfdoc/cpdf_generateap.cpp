...
ByteString GetFontSetString(IPVT_FontMap* pFontMap,
  ...
  if (pFontMap) {
    const ByteString sFontAlias = pFontMap->GetPDFFontAlias(nFontIndex);
...
bool GenerateStrikeOutAP(CPDF_Document* pDoc, CPDF_Dictionary* pAnnotDict) {
  ...
  if (pArray) {
    ...
    for (size_t i = 0; i < nQuadPointCount; ++i) {
      ...
      const float fY = (rect.top + rect.bottom) / 2;
...
void CPDF_GenerateAP::GenerateFormAP(CPDF_Document* pDoc,
  ...
  const CFX_Color crText = fpdfdoc::CFXColorFromString(DA);
  ...
  std::ostringstream sAppStream;
  const ByteString sBG = GenerateColorAP(crBG, PaintOperation::kFill);
  if (sBG.GetLength() > 0) {
    sAppStream << "q\n"
  ...
  const ByteString sBorderStream =
  ...
  switch (type) {
    case CPDF_GenerateAP::kTextField: {
      ...
      const WideString swValue = pV ? pV->GetUnicodeText() : WideString();
      ...
      const ByteString sBody = GenerateEditAP(&map, vt.GetIterator(), ptOffset,
                                              !bCharArray, subWord);
    ...
    case CPDF_GenerateAP::kComboBox: {
      ...
      if (sButton.GetLength() > 0 && !rcButton.IsEmpty()) {
        ...
        sAppStream << "Q\n";
        const ByteString sButtonBorder = GenerateBorderAP(
