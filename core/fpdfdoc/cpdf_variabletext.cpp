...
constexpr uint8_t gFontSizeSteps[] = {4,  6,  8,   9,   10,  12,  14, 18, 20,
...
bool CPDF_VariableText::Iterator::NextLine() {
  ...
  if (m_CurPos.nLineIndex <
      pdfium::CollectionSize<int32_t>(pSection->line_array()) - 1) {
...
bool CPDF_VariableText::Iterator::GetWord(CPVT_Word& word) const {
  ...
  CPVT_Section* pSection = m_pVT->m_SectionArray[m_CurPos.nSecIndex].get();
  if (!pdfium::IndexInBounds(pSection->line_array(), m_CurPos.nLineIndex) ||
      !pdfium::IndexInBounds(pSection->word_array(), m_CurPos.nWordIndex)) {
    return false;
  }

  const CPVT_WordInfo* pWord =
      pSection->word_array()[m_CurPos.nWordIndex].get();
  word.Word = pWord->Word;
  word.nCharset = pWord->nCharset;
  word.fWidth = m_pVT->GetWordWidth(*pWord);
  word.ptWord =
      m_pVT->InToOut(CFX_PointF(pWord->fWordX + pSection->rect().left,
                                pWord->fWordY + pSection->rect().top));
...
bool CPDF_VariableText::Iterator::GetLine(CPVT_Line& line) const {
  ...
  const CPVT_Section::Line* pLine =
      pSection->line_array()[m_CurPos.nLineIndex].get();
  line.ptLine = m_pVT->InToOut(
      CFX_PointF(pLine->m_LineInfo.fLineX + pSection->rect().left,
                 pLine->m_LineInfo.fLineY + pSection->rect().top));
...
CPVT_WordPlace CPDF_VariableText::InsertSection(const CPVT_WordPlace& place) {
  ...
  if (pdfium::IndexInBounds(m_SectionArray, NewPlace.nSecIndex)) {
    ...
    for (int32_t w = wordplace.nWordIndex + 1;
         w < pdfium::CollectionSize<int32_t>(pSection->word_array()); ++w) {
      NewPlace.nWordIndex++;
      pNewSection->AddWord(NewPlace, *pSection->word_array()[w]);
    }
  }
...
void CPDF_VariableText::SetText(const WideString& swText) {
  ...
  if (!m_SectionArray.empty())
    m_SectionArray.front()->set_rect(CPVT_FloatRect());
...
int32_t CPDF_VariableText::WordPlaceToWordIndex(
  ...
  for (i = 0, sz = pdfium::CollectionSize<int32_t>(m_SectionArray);
    ...
    nIndex += pdfium::CollectionSize<int32_t>(pSection->word_array());
    if (i != sz - 1)
...
CPVT_WordPlace CPDF_VariableText::SearchWordPlace(
  ...
  while (nLeft <= nRight) {
    ...
    if (IsFloatBigger(pt.y, pSection->rect().top))
      bUp = false;
    if (IsFloatBigger(pSection->rect().bottom, pt.y))
      bDown = false;
    if (IsFloatSmaller(pt.y, pSection->rect().top)) {
      nRight = nMid - 1;
      nMid = (nLeft + nRight) / 2;
      continue;
    }
    if (IsFloatBigger(pt.y, pSection->rect().bottom)) {
      nLeft = nMid + 1;
      nMid = (nLeft + nRight) / 2;
      continue;
    }
    place = pSection->SearchWordPlace(
        CFX_PointF(pt.x - pSection->rect().left, pt.y - pSection->rect().top));
...
CPVT_WordPlace CPDF_VariableText::GetUpWordPlace(
  ...
  if (temp.nLineIndex-- > 0) {
    return pSection->SearchWordPlace(pt.x - pSection->rect().left, temp);
  }
  if (temp.nSecIndex-- > 0) {
    if (pdfium::IndexInBounds(m_SectionArray, temp.nSecIndex)) {
      CPVT_Section* pLastSection = m_SectionArray[temp.nSecIndex].get();
      temp.nLineIndex =
          pdfium::CollectionSize<int32_t>(pLastSection->line_array()) - 1;
      return pLastSection->SearchWordPlace(pt.x - pLastSection->rect().left,
                                           temp);
    }
  }
  return place;
...
CPVT_WordPlace CPDF_VariableText::GetLineEndPlace(
  ...
  CPVT_Section* pSection = m_SectionArray[place.nSecIndex].get();
  if (!pdfium::IndexInBounds(pSection->line_array(), place.nLineIndex))
    return place;

  return pSection->line_array()[place.nLineIndex]->GetEndWordPlace();
}
...
int32_t CPDF_VariableText::GetTotalWords() const {
  ...
  for (const auto& pSection : m_SectionArray) {
    nTotal +=
        pdfium::CollectionSize<int32_t>(pSection->word_array()) + kReturnLength;
  }
...
CPVT_WordPlace CPDF_VariableText::AddSection(const CPVT_WordPlace& place) {
  ...
  pSection->set_rect(CPVT_FloatRect());
  pSection->set_place_index(nSecIndex);
...
bool CPDF_VariableText::ClearEmptySection(const CPVT_WordPlace& place) {
  ...
  if (!m_SectionArray[place.nSecIndex]->word_array().empty())
    return false;
...
void CPDF_VariableText::LinkLatterSection(const CPVT_WordPlace& place) {
  ...
  if (pdfium::IndexInBounds(m_SectionArray, oldplace.nSecIndex)) {
    ...
    for (const auto& pWord : pNextSection->word_array()) {
      oldplace.nWordIndex++;
...
CPVT_FloatRect CPDF_VariableText::RearrangeSections(
  ...
  for (int32_t s = 0, sz = pdfium::CollectionSize<int32_t>(m_SectionArray);
    ...
    pSection->set_place(place);
    CPVT_FloatRect rcSec = pSection->rect();
    if (s >= PlaceRange.BeginPos.nSecIndex) {
      if (s <= PlaceRange.EndPos.nSecIndex) {
        ...
        float fOldHeight = pSection->rect().bottom - pSection->rect().top;
        rcSec.top = fPosY;
        rcSec.bottom = fPosY + fOldHeight;
      }
      pSection->set_rect(rcSec);
