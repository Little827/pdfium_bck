...
int32_t CPDF_InteractiveForm::FindFieldInCalculationOrder(
  ...
  absl::optional<size_t> maybe_find = pArray->FindNonNull(pField->GetDict());
  if (!maybe_find.has_value())
    return -1;

  return pdfium::base::checked_cast<int32_t>(maybe_find.value());
}
