...
TEST_F(CPDFAnnotListTest, CreatePopupAnnotFromUnicode) {
  const std::vector<uint8_t> kContents = {0xFE, 0xFF, 0x00, 'A',  0x00,
  ...
  // Note that surrogate pairs are not decoded before encoding to UTF-8.
...
TEST_F(CPDFAnnotListTest, CreatePopupAnnotFromEmptyUnicode) {
  const std::vector<uint8_t> kContents = {0xFE, 0xFF};
  AddTextAnnotation(ByteStringView(kContents));
