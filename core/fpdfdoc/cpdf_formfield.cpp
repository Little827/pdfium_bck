...
bool CPDF_FormField::SetItemSelection(int index,
  ...
  if (bSelected)
    SetItemSelectionSelect(index, opt_value);
  else
    SetItemSelectionUnselect(index, opt_value);
...
void CPDF_FormField::SetItemSelectionSelect(int index,
...
void CPDF_FormField::SetItemSelectionUnselect(int index,
                                              const WideString& opt_value) {
  const CPDF_Object* pValue = FPDF_GetFieldAttr(m_pDict.Get(), "V");
  if (!pValue)
    return;
