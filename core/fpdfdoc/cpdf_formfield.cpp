...
bool CPDF_FormField::UseSelectedIndicesObject() const {
  ...
  size_t selected_indices_size = 0;
  if (pSelectedIndicesObject->IsArray())
    selected_indices_size = pSelectedIndicesObject->AsArray()->size();
  ...
  if (pSelectedIndicesObject->IsArray()) {
    ...
    for (const auto& pObj : locker) {
      ...
      if (values.find(wsOptValue) == values.end())
        return false;

      values[wsOptValue]--;
      if (values[wsOptValue] == 0)
        values.erase(wsOptValue);
    }

    return values.empty();
  }

  // |pSelectedIndicesObject| must be a number here.
  int index = pSelectedIndicesObject->GetInteger();
  if (index < 0 || index >= num_options)
    return false;

  return values.find(GetOptionValue(index)) != values.end();
}
