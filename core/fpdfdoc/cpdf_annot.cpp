...
CFX_FloatRect CPDF_Annot::BoundingRectFromQuadPoints(
  ...
  for (size_t i = 1; i < QuadPointCount(pArray); ++i) {
    CFX_FloatRect rect = RectFromQuadPointsArray(pArray, i);
    ret.left = std::min(ret.left, rect.left);
    ret.bottom = std::min(ret.bottom, rect.bottom);
    ret.right = std::max(ret.right, rect.right);
    ret.top = std::max(ret.top, rect.top);
  }
  return ret;
}
