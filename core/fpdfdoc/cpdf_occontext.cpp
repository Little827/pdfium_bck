...
bool CPDF_OCContext::CheckObjectVisible(CPDF_PageObject* pObj) {
  for (size_t i = 0; i < pObj->m_ContentMark.CountItems(); ++i) {
    CPDF_ContentMarkItem* pItem = pObj->m_ContentMark.GetItem(i);
    if (pItem->GetName() == "OC" &&
        pItem->GetParamType() == CPDF_ContentMarkItem::PropertiesDict &&
        !CheckOCGVisible(pItem->GetParam())) {
      return false;
    }
  }
  return true;
