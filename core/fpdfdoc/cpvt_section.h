...
class CPVT_Section final {
 public:
  ...
  void set_place(const CPVT_WordPlace& place) { m_SecPlace = place; }
  void set_place_index(int32_t index) { m_SecPlace.nSecIndex = index; }
  const CPVT_FloatRect& rect() const { return m_Rect; }
  void set_rect(const CPVT_FloatRect& rect) { m_Rect = rect; }
  const std::vector<std::unique_ptr<Line>>& line_array() const {
    return m_LineArray;
  }
  const std::vector<std::unique_ptr<CPVT_WordInfo>>& word_array() const {
    return m_WordArray;
  }
