...
void DumpStructureElementAttributes(FPDF_STRUCTELEMENT_ATTR attr, int indent) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (!FPDF_StructElement_Attr_GetName(attr, i, attr_name, sizeof(attr_name),
                                         &attr_len)) {
      printf("%*s FPDF_StructElement_Attr_GetName failed for %d\n", indent, "",
             i);
      continue;
    }
    std::wstring attr_name_wstring = ConvertToWString(
        reinterpret_cast<unsigned short*>(attr_name), attr_len);
