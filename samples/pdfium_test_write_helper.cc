...
bool WriteBitmapToFile(ScopedFPDFBitmap bitmap,
                       const char* filename,
                       const char* filetype) {
...
void WriteImages(FPDF_PAGE page, const char* pdf_name, int page_num) {
  for (int i = 0; i < FPDFPage_CountObjects(page); ++i) {
    ...
    if (!WriteBitmapToFile(std::move(bitmap), filename, "image")) {
      fprintf(stderr,
              "Failed to convert image object #%d, on page #%d to png.\n",
              i + 1, page_num + 1);
    }
...
void WriteThumbnail(FPDF_PAGE page, const char* pdf_name, int page_num) {
  ...
  if (!WriteBitmapToFile(std::move(thumb_bitmap), filename, "thumbnail")) {
