...
struct FPDF_FORMFILLINFO_PDFiumTest : public FPDF_FORMFILLINFO {
  ...
  // to get loaded twice.
  std::map<int, UniqueFPDFPage> loaded_pages;
...
FPDF_PAGE GetPageForIndex(FPDF_FORMFILLINFO* param,
  ...
  UniqueFPDFPage page(FPDF_LoadPage(doc, index));
  if (!page)
    return nullptr;
...
bool RenderPage(const std::string& name,
  ...
  UniqueFPDFTextPage text_page(FPDFText_LoadPage(page));
  double scale = 1.0;
  if (!options.scale_factor_as_string.empty())
...
void RenderPdf(const std::string& name,
  ...
  UniqueFPDFAvail pdf_avail(FPDFAvail_Create(&file_avail, &file_access));

  // The document must outlive |form_callbacks.loaded_pages|.
  UniqueFPDFDocument doc;
