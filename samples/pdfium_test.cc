...
void ShowConfig() {
  ...
  maybe_comma = ",";
#endif
  // TODO(crbug.com/pdfium/11)  With Skia vs. AGG determined at runtime, what
  // to show here?  One of { PDF_ENABLE_SKIA | PDF_ENABLE_SKIA_PATHS } will be
  // enabled, but that no longer means that Skia is used for rendering.  Does
  // that matter for how this config output is used?
  printf("%s\n", config.c_str());
}
