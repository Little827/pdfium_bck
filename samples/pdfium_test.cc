...
bool RenderPage(const std::string& name,
  ...
  if (bitmap) {
    ...
    if (options.render_oneshot) {
      ...
      // Client programs will be setting these values when calling render.
      // Setting default colors for sample rendering. Used only when
      // |options.forced_color| is true.
