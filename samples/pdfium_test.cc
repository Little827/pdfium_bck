...
int ExampleDocGetFilePath(IPDF_JSPLATFORM*, void* filePath, int length) {
  static const char kPath[] = "myfile.pdf";
  const int required = static_cast<int>(sizeof(kPath));
  if (filePath && length >= required)
    memcpy(filePath, kPath, required);
  return required;
}
...
void ExampleDocSubmitForm(IPDF_JSPLATFORM*,
                          ...
                          int length,
                          FPDF_WIDESTRING url) {
  printf("Doc Submit Form: url=%ls, length=%d\n",
...
int ExampleFieldBrowse(IPDF_JSPLATFORM*, void* filePath, int length) {
