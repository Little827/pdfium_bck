...
class BitmapPageRenderer : public PageRenderer {
 ...
 private:
  void ClearBitmap() {
    bool alpha = (FPDFBitmap_GetFormat(bitmap()) == FPDFBitmap_BGRA);
...
class ScopedGdiObject final {
 public:
  ...
  void Reset(HGDIOBJ object) {
    if (object_) {
      BOOL success = DeleteObject(object_);
      DCHECK(success);
    }
    object_ = object;
  }
...
class GdiDisplayPageRenderer : public BitmapPageRenderer {
 public:
  ...
  void Finish(FPDF_FORMHANDLE /*form*/) override {
    ...
    DCHECK_EQ(width() * sizeof(uint32_t), stride);
