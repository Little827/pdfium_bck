 ...
 public:
  ...
  bool Start() override {
    ...
    FPDF_RenderPageBitmap(bitmap(), page(), /*start_x=*/0, /*start_y=*/0,
                          /*width=*/width(), /*height=*/height(), /*rotate=*/0,
                          /*flags=*/flags());
    return true;
  }
};
 ...
 public:
  ...
  bool Start() override {
    ...
    if (FPDF_RenderPageBitmapWithColorScheme_Start(
            bitmap(), page(), /*start_x=*/0, /*start_y=*/0, /*width=*/width(),
            /*height=*/height(), /*rotate=*/0, /*flags=*/flags(), color_scheme_,
...
#else
  {
    ...
    if (options.render_oneshot) {
      renderer = std::make_unique<OneShotBitmapPageRenderer>(
          page, width, height, flags, idler, writer);
    } else {
      ...
      renderer = std::make_unique<ProgressiveBitmapPageRenderer>(
          page, width, height, flags, idler, writer,
          options.forced_color ? &color_scheme : nullptr);
    }
  }
