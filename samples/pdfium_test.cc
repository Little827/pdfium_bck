...
class PageRenderer {
 public:
  ...
  // Writes rendered output to a file, return `false` on failure.
...
class BitmapPageRenderer : public PageRenderer {
 public:
  ...
  bool Start() override {
    bitmap_.reset(FPDFBitmap_Create(width(), height(), alpha_));
    if (!HasOutput())
      return false;

    FPDF_DWORD fill_color = alpha_ ? 0x00000000 : 0xFFFFFFFF;
    FPDFBitmap_FillRect(bitmap(), 0, 0, width(), height(), fill_color);
    return true;
  }

  void Finish(FPDF_FORMHANDLE form) override {
    FPDF_FFLDraw(form, bitmap(), page(), /*start_x=*/0, /*start_y=*/0, width(),
                 height(), /*rotate=*/0, flags());
  }
  ...
  bool Write(const std::string& name, int page_index, bool md5) override {
    ...
    std::string image_file_name =
        writer_(name.c_str(),
                /*num=*/page_index, buffer, /*stride=*/stride,
...
class OneShotBitmapPageRenderer : public BitmapPageRenderer {
 public:
  using BitmapPageRenderer::BitmapPageRenderer;
...
class ProgressiveBitmapPageRenderer : public BitmapPageRenderer {
 public:
  ProgressiveBitmapPageRenderer(FPDF_PAGE page,
      ...
      : BitmapPageRenderer(page, width, height, flags, alpha, writer),
  ...
  bool Start() override {
    ...
    if (FPDF_RenderPageBitmapWithColorScheme_Start(
            bitmap(), page(), /*start_x=*/0, /*start_y=*/0, width(), height(),
            /*rotate=*/0, flags(), color_scheme_,
  ...
  bool Continue() override {
    if (to_be_continued_ && FPDF_RenderPage_Continue(page(), &pause_) !=
                                FPDF_RENDER_TOBECONTINUED) {
      to_be_continued_ = false;
    }
 ...
 public:
  ...
  void Finish(FPDF_FORMHANDLE form) override {
    FPDF_FFLRecord(form, reinterpret_cast<FPDF_RECORDER>(recorder_.get()),
                   page(), /*start_x=*/0, /*start_y=*/0, width(), height(),
                   /*rotate=*/0,
                   /*flags=*/0);
  }
...
bool ProcessPage(const std::string& name,
  ...
  FPDF_BOOL alpha = FPDFPage_HasTransparency(page);

  std::unique_ptr<PageRenderer> renderer;
  if (options.output_format == OutputFormat::kSkp) {
    renderer =
        std::make_unique<SkPicturePageRenderer>(page, width, height, flags);
  } else {
    ...
    if (options.render_oneshot) {
      renderer = std::make_unique<OneShotBitmapPageRenderer>(
          page, width, height, flags, alpha, writer);
    } else {
      renderer = std::make_unique<ProgressiveBitmapPageRenderer>(
          page, width, height, flags, alpha, writer, nullptr);
    }
  }
  ...
  if (renderer->Start()) {
    ...
    switch (options.output_format) {
      ...
      default:
        renderer->Write(name, page_index, options.md5);
        break;
    }
  } else {
