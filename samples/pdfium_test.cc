...
int main(int argc, const char* argv[]) {
  ...
  if (options.time > -1) {
    // This must be static to avoid explicit capture, so the lambda can be
    // converted to a function ptr.
    static time_t time_ret = static_cast<time_t>(options.time);
    FSDK_SetTimeFunction([](time_t* tloc) -> time_t {
      if (tloc)
        *tloc = time_ret;
      return time_ret;
    });
