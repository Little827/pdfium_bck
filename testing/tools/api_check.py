...
fpdf_view_c_api_test.c is alphabetical within each file.
...
def _CheckSorted(functions):
  ...
  for i in range(0, len(functions) - 1):
    if functions[i] > functions[i+1]:
      print ('Make sure %s and %s are in alphabetical order within the file.'
  ...
  with open(api_test_path) as f:
    ...
    for line in contents:
      if (file_regex.match(line)):
        assert _CheckSorted(functions_in_file)
        functions_in_file = []
