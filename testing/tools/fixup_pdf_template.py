...
script replaces {{name}}-style variables in the input with calculated results
  ...
  {{embedroboto x}} - expands to a set of objects (x, 0), (x+1, 0), (x+2, 0),
                      and (x+3, 0) that contain an embedded version of the
                      Roboto-Regular font. This version has been reduced to the
                      first 256 characters of the font to save on space.
"""
...
'''
  ...
  ROBOTO_FONT_FILE = 'Roboto-Regular.reduced.sfd'
...
'''
  ...
  def replace_embed_roboto_tag(self, x):
    ...
    encoding_str = self.ROBOTO_ENCODING_BLOB % (encoding_x)
    (line, delta) = self.insert_font_obj(line, delta, encoding_x, encoding_str)
