    ...
    for page in itertools.count():
      ...
      if os.path.exists(expected_path):
        ...
        if compare_error:
          ...
          for unexpected_path in path_templates.GetUnexpectedPaths(page):
...
class _PathTemplates:
  ...
  def _GetExpectedPaths(self, page):
  ...
  def GetExpectedPath(self, page, default_to_base=True):
    ...
    for expected_path in self._GetExpectedPaths(page):
      if os.path.exists(expected_path):
        return expected_path
      last_not_found_expected_path = expected_path
    return last_not_found_expected_path if default_to_base else None

  def GetUnexpectedPaths(self, page):
    """Returns the (unexpected) paths after the most specific expected path."""
    return list(filter(os.path.exists, self._GetExpectedPaths(page)))[1:]
