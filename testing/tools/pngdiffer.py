...
class PathMode:
  ...
  Attributes:
    DEFAULT:   Used for default expected paths in the format of
               ...
               exists.
    SKIA:      Used only when Skia or SkiaPaths is enabled, for paths in the
               format of 'NAME_expected_skia(_OSNAME)?.pdf.#.png'.
               Such path only exists when the expected result of Skia o
               SkiaPaths is different from that of AGG.
    SKIAPATHS: Used only when SkiaPaths is enabled, indicating path in the
               format of 'NAME_expected_skiapaths(_OSNAME)?.pdf.#.png'.
               Such path only exists when the expected results of using AGG,
  ...
  order of its matching paths' decrementing probability.
  """
...
class PathTemplates(object):
  ...
  def __init__(self, input_filename, source_dir, working_dir, os_name,
               max_path_mode):
    assert max_path_mode >= 0 and max_path_mode <= PathMode.SKIAPATHS, (
