...
class _PathMode:
  """_PathMode indicates the available expected results' paths.
...
class PNGDiffer():
  ...
  def __init__(self, finder, features, reverse_byte_order):
    ...
    if 'SKIA' in features:
      self.max_path_mode = _PathMode.SKIA
    else:
      self.max_path_mode = _PathMode.DEFAULT
...
class _PathTemplates:
  ...
  def __init__(self, input_filename, source_dir, working_dir, os_name,
               max_path_mode):
    assert _PathMode.DEFAULT <= max_path_mode <= _PathMode.SKIA, (
    ...
    for mode in range(max_path_mode, _PathMode.DEFAULT - 1, -1):
  ...
  def _GetExpectedTemplateByPathMode(self, mode, os_name=None):
    ...
    if mode == _PathMode.DEFAULT:
      pass
    elif mode == _PathMode.SKIA:
      expected_str += '_skia'
    else:
      assert False, f'Unexpected _PathMode: {mode}.'

    if os_name:
      expected_str += '_' + self.os_name
    return os.path.join(self.source_dir,
                        self.input_root + expected_str + _ACTUAL_TEMPLATE)
  ...
  def GetExpectedPath(self, page, default_to_base=True):
    ...
    for template in self.expected_templates:
      expected_path = template % page
      if os.path.exists(expected_path):
        return expected_path
    return expected_path if default_to_base else None
