...
class PNGDiffer():
  ...
  def _ImageDiffError(self, expected_path, actual_path):
    if os.path.exists(expected_path):
      ...
      # Return error = 1 for missing expected file.
      error = 1
    return error
  ...
  def HasDifferences(self, input_filename, source_dir, working_dir):
    ...
    for page in itertools.count():
      ...
      if error:
        ...
        new_error = self._ImageDiffError(platform_expected_path, actual_path)
        # Update error code. No need to overwrite the previous error code if
        # |platform_expected_path| doesnt exist.
        if not new_error or new_error != 1:
          error = new_error
        if error and not self.skia_enabled:
          print "FAILURE: " + input_filename + "; " + str(error)
          return True

      # When Skia or SkiaPaths is enabled, check against Skia expected results.
      if error:
        error = self._ImageDiffError(skia_expected_path, actual_path)
        if error:
          new_error = self._ImageDiffError(skia_platform_expected_path,
                                           actual_path)
          if not new_error or new_error != 1:
            error = new_error
          if error and not self.skia_enabled:
      ...
      if error:
        ...
        if error:
          new_error = self._ImageDiffError(skiapaths_platform_expected_path,
                                           actual_path)
          if not new_error or new_error != 1:
            error = new_error
          if error:
