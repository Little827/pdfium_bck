...
class PNGDiffer():
  ...
  def __init__(self, finder, reverse_byte_order):
    ...
    self.skia_enabled = common.GetBooleanGnArg('pdf_use_skia', finder.build_dir)
    #if common.GetBooleanGnArg('pdf_use_skia',
    #                         finder.build_dir) or common.GetBooleanGnArg(
    #                            'pdf_use_skia_paths', finder.build_dir):
  ...
  def HasDifferences(self, input_filename, source_dir, working_dir):
    ...
    for page in itertools.count():
      ...
      if error and self._ImageDiffError(skiapaths_expected_path, actual_path):
        if self._ImageDiffError(skiapaths_platform_expected_path, actual_path):
          print "FAILURE: " + input_filename + "; " + str(error)
          return True
