...
class PathTemplates(object):
  ...
  def _GetExpectedTemplateByPathMode(self, mode, platform=None):
  ...
  def GetExpectedPathByPathMode(self, page, mode, platform=None):
    return self._GetExpectedTemplateByPathMode(mode, platform) % page

  def GetExpectedPaths(self, page):
    expected_paths = []
    for temp in self.expected_templates:
      expected_paths.append(temp % page)
