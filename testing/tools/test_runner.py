...
class TestRunner:
    ...
    if image_paths and self.test_type not in TEXT_TESTS:
      for img_path, _ in image_paths:
        ...
        skia_success = self.GetSkiaGoldTester().UploadTestResultToSkiaGold(
            test_name, img_path)
        if not skia_success:
          self.skia_gold_failures.append(input_path)
  ...
  def _PrintSummary(self):
    ...
    number_skia_gold_failures = len(self.skia_gold_failures)
    number_skia_gold_successes = number_test_cases - len(self.skia_gold_failures)
    ...
    print '  Skia Gold Successes: %d' % number_skia_gold_successes
    print '  Skia Gold Failures: %d' % number_skia_gold_failures
    print
