...
def SendtoSkiaGoldOuter(this, img_path_input_filename):
...
class TestRunner(object):
  ...
  def GetSkiaGoldTester(self, process_name=None):
    if not self.skia_tester:
      self.skia_tester = skia_gold.SkiaGoldTester(
          source_type=self.test_type, skia_gold_args=self.options, process_name=process_name)
  ...
  def RunSkia(self, img_path, process_name=None):
    ...
    skia_success = skia_tester.UploadTestResultToSkiaGold(
        test_name, img_path, process_name)
    sys.stdout.flush()
        ...
        for worker_result in worker_results:
          ...
          if self.test_type not in TEXT_TESTS:
            ...
            if image_paths:
              ...
              img_paths_input_filenames.extend(path_filename_tuples)

        if img_paths_input_filenames and self.test_type not in TEXT_TESTS:
          gold_worker_func = functools.partial(SendtoSkiaGoldOuter, self)

          skia_gold.clear_gold_output_dir(self.options.gold_output_dir)
          gold_results = pool.imap(gold_worker_func, img_paths_input_filenames)
  ...
  def _PrintSummary(self):
    ...
    print()
    print('Test cases executed:', number_test_cases)
    ...
    if self.test_type not in TEXT_TESTS:
      ...
      print()
      print('Skia Gold Test cases executed:', number_total_gold_tests)
      ...
      if self.skia_gold_failures and self.GetSkiaGoldTester().IsTryjobRun():
        # SkiaGoldSession GetTriageLinks() expects a test name to be passed in
        cl_triage_link = self.skia_tester.GetTriageLink(
            self.skia_gold_failures[0])
        if cl_triage_link is not None:
          print('  Triage link for CL:', cl_triage_link)
          self.skia_tester.WriteCLTriageLink(cl_triage_link)
        else:
          omission_reason = self.skia_tester.GetTriageLinkOmissionReason(
              self.skia_gold_failures[0])
          print('  Could not retrieve triage link:', omission_reason)
    print()
