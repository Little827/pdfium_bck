...
class KeyboardInterruptError(Exception):
  """Custom exception to wrap `KeyboardInterrupt` exceptions."""
...
class TestRunner:
  ...
  def Run(self):
    ...
    # Collect test cases.
    walk_from_dir = self.per_process_state.finder.TestingDir(relative_test_dir)
      ...
      for file_dir, _, filename_list in os.walk(walk_from_dir):
        for input_filename in filename_list:
          if input_file_re.match(input_filename):
            ...
            if self.per_process_state.test_suppressor.IsExecutionSuppressed(
                input_path):
...
class _PerProcessState:
  ...
  def __init__(self, config):
    ...
    self.finder = config.NewFinder()
    self.fixup_path = self.finder.ScriptPath('fixup_pdf_template.py')
    self.text_diff_path = self.finder.ScriptPath('text_diff.py')
    self.font_dir = os.path.join(self.finder.TestingDir(), 'resources', 'fonts')
    self.third_party_font_dir = self.finder.ThirdPartyFontsDir()

    self.source_dir = self.finder.TestingDir()

    self.pdfium_test_path = config.GetPdfiumTestPath(self.finder)
    self.working_dir = self.finder.WorkingDir(
        os.path.join('testing', self.test_dir))

    self.test_suppressor = suppressor.Suppressor(
        self.finder, self.features, self.options.disable_javascript,
        self.options.disable_xfa)
    self.image_differ = pngdiffer.PNGDiffer(self.finder, self.features,
