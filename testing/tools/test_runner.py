...
class TestRunner:
  ...
  def GetSkiaGoldTester(self):
    if not self.skia_tester:
      self.skia_tester = skia_gold.SkiaGoldTester(
          source_type=self.test_type, skia_gold_args=self.options)
  ...
  def _PrintSummary(self):
    ...
    if self.test_type not in TEXT_TESTS:
      ...
      print 'Skia Gold Test cases executed: %d' % number_total_gold_tests
      print '  Skia Gold Successes: %d' % number_gold_successes
      print '  Skia Gold Surprises: %d' % number_gold_surprises
      print '  Skia Gold Failures: %d' % number_gold_failures
      if self.skia_gold_failures and self.skia_tester.IsTryjobRun():
        # SkiaGoldSession GetTriageLinks() expects a test name to be passed in
        _, cl_triage_link = self.skia_tester.GetTriageLinks(
            self.skia_gold_failures[0])
        print '  Triage link for CL: {}'.format(cl_triage_link)
