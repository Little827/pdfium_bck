...
class SkiaGoldTester(object):
  ...
  def UploadTestResultToSkiaGold(self, image_name, image_path):
    ...
    if status == status_codes.SUCCESS:
      ...
      print('Gold authentication failed with output %s', error)
    elif status == status_codes.INIT_FAILURE:
      print('Gold initialization failed with output %s', error)
    elif status == status_codes.COMPARISON_FAILURE_REMOTE:
      print('Remote comparison failed. See outputted triage links.')
    elif status == status_codes.COMPARISON_FAILURE_LOCAL:
      print('Local comparison failed. Local diff files:')
...
def _OutputLocalDiffFiles(gold_session, image_name):
  ...
  print('Generated image for %s: %s', image_name, given_file or
                failure_message)
  print('Closest image for %s: %s', image_name, closest_file or
                failure_message)
  print('Diff image for %s: %s', image_name, diff_file or
                failure_message)
