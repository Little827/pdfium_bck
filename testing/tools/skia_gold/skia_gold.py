...
class SkiaGoldTester(object):
  ...
  def __init__(self, source_type, skia_gold_args, process_name=None):
    ...
    if process_name:
      self._output_dir = os.path.join(skia_gold_args.gold_output_dir, process_name)
  ...
  def GetTriageLink(self, name):
    gold_session = self.GetSkiaGoldSessionManager()\
        .GetSkiaGoldSession(self._keys, corpus=self._source_type,
                            bucket=GS_BUCKET)
    return gold_session.GetTriageLinks(name)[1]

  def GetTriageLinkOmissionReason(self, name):
    gold_session = self.GetSkiaGoldSessionManager()\
        .GetSkiaGoldSession(self._keys, corpus=self._source_type,
                            bucket=GS_BUCKET)
    return gold_session.GetTriageLinkOmissionReason(name)

  def UploadTestResultToSkiaGold(self, image_name, image_path, multiprocessing_name=None):
    ...
    if status == status_codes.SUCCESS:
      ...
      logging.error(
          'Remote comparison failed for {}. See outputted triage links.')
...
def _OutputLocalDiffFiles(gold_session, image_name):
  ...
  logging.error('Generated image for {image_name}: {file_name}'.format(
      image_name=image_name, file_name = given_file or failure_message))
  logging.error('Closest image for {image_name}: {file_name}'.format(
      image_name=image_name, file_name = closest_file or failure_message))
  logging.error('Diff image for {image_name}: {file_name}'.format(
      image_name=image_name, file_name = diff_file or failure_message))
