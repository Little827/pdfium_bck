...
# in pdfium-g so one need not tryto generate these locally for the various
# platforms.
#
# Given a "red bot" result, the first step is to identify the failing
# "stdout" test log for the failing test step on that bot (pixel or corpus).
#
# The next step is to identify the suffix the platform appends to the
# expected test resutls, for example for the win_xfa_skia bot, this
# is skia_win.
#
...
# directory (because I've been too lazy to try and parse out where they
# belong). Move them by hand into the target directories.
#
set -e
if [ $# -ne 2 ] ; then
    echo "Usage: golden_fetch.sh <image_suffix> <log_url>, e.g."
    echo -n "golden_fetch.sh skia_win "
