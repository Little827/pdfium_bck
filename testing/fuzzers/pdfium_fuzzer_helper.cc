...
void PDFiumFuzzerHelper::RenderPdf(const uint8_t* data, size_t len) {
  if (len < 8)
    return;

  int render_flags = GetInteger(data);
  int form_flags = GetInteger(data + 4);
  data += 8;
  len -= 8;
  ...
  TestLoader loader({reinterpret_cast<const char*>(data), len});
