...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  FXDIB_Format src_format = kFormat[data[30] % 15];
  ...
  if (src_bitmap->IsAlphaMask()) {
    dest_bitmap->CompositeMask(
        dest_left, dest_top, width, height, src_bitmap, argb, src_left, src_top,
        blend_mode, (is_clip ? clip_rgn.get() : nullptr), is_rgb_byte_order);
  } else {
    dest_bitmap->CompositeBitmap(
        dest_left, dest_top, width, height, src_bitmap, src_left, src_top,
        blend_mode, (is_clip ? clip_rgn.get() : nullptr), is_rgb_byte_order);
  }
  return 0;
}
