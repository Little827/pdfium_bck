...
std::string GenXfaScriptFuncName(FuzzedDataProvider* data_provider) {
  [[clang::no_destroy]] static const std::vector<std::string> xfa_script_funcs =
      {
          "Abs",          "Avg",         "Ceil",        "Count",
          "Floor",        "Max",         "Min",         "Mod",
          "Round",        "Sum",         "Date",        "Date2Num",
          "DateFmt",      "IsoDate2Num", "IsoTime2Num", "LocalDateFmt",
          "LocalTimeFmt", "Num2Date",    "Num2GMTime",  "Num2Time",
          "Time",         "Time2Num",    "TimeFmt",     "Apr",
          "Cterm",        "FV",          "Ipmt",        "NPV",
          "Pmt",          "PPmt",        "PV",          "Rate",
          "Term",         "Choose",      "Exists",      "HasValue",
          "Oneof",        "Within",      "If",          "Eval",
          "Translate",    "Ref",         "UnitType",    "UnitValue",
          "At",           "Concat",      "Decode",      "Encode",
          "Format",       "Left",        "Len",         "Lower",
          "Ltrim",        "Parse",       "Replace",     "Right",
          "Rtrim",        "Space",       "Str",         "Stuff",
          "Substr",       "Uuid",        "Upper",       "WordNum",
          "Get",          "Post",        "Put",
      };

  size_t elemSelector = data_provider->ConsumeIntegralInRange<size_t>(
      0, xfa_script_funcs.size() - 1);
  return xfa_script_funcs[elemSelector];
}

// Possible arguments to a XFA script function
std::string GenXfaScriptParam(FuzzedDataProvider* data_provider) {
  [[clang::no_destroy]] static const std::vector<std::string> xfa_func_params =
      {
          "&amp;|",
          "HTML",
          "123124",
...
std::string GenXfaTag(FuzzedDataProvider* data_provider) {
  [[clang::no_destroy]] static const std::vector<std::string> xfa_elem_tags = {
      "template",
      "validate",
      "subform",
      "config",
