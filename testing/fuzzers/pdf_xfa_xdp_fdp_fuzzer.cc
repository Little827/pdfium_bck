...
std::string GenQuickBody(Tag* t2, FuzzedDataProvider* data_provider) {
  std::string tag_content = data_provider->ConsumeRandomLengthString();

  return t2->tag_start + tag_content + t2->tag_end;
}
...
std::string GenXDPPdfFile(FuzzedDataProvider* data_provider) {
  ...
  // The three XFA tags
  pdf_objects.push_back(
      CreateStreamObject(6, GenQuickBody(&tag1, data_provider)));
  pdf_objects.push_back(
      CreateStreamObject(7, GenQuickBody(&tag2, data_provider)));
  pdf_objects.push_back(
      CreateStreamObject(8, GenQuickBody(&tag3, data_provider)));
