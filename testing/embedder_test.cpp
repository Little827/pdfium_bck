...
bool EmbedderTest::CreateEmptyDocument() {
  ...
  form_handle_ = SetupFormFillEnvironment(document_, kEnableJavaScript);
  return true;
}

bool EmbedderTest::OpenDocument(const std::string& filename) {
  return OpenDocumentWithOptions(filename, nullptr, kDefaultLinearize,
                                 kEnableJavaScript);
}

bool EmbedderTest::OpenDocumentLinearized(const std::string& filename) {
  return OpenDocumentWithOptions(filename, nullptr, kMustLinearize,
                                 kEnableJavaScript);
}

bool EmbedderTest::OpenDocumentWithPassword(const std::string& filename,
                                            const char* password) {
  return OpenDocumentWithOptions(filename, password, kDefaultLinearize,
                                 kEnableJavaScript);
}
  ...
  *avail = FPDFAvail_Create(network_simulator->GetFileAvail(),
  ...
  if (FPDFAvail_IsLinearized(*avail) == PDF_LINEARIZED) {
    ...
    *document = FPDFAvail_GetDocument(*avail, password);
    ...
    if (linearize_option == kMustLinearize)
      return false;
...
FPDF_FORMHANDLE EmbedderTest::SetupFormFillEnvironment(
  ...
  if (javascript_option == kEnableJavaScript)
...
FPDF_DOCUMENT EmbedderTest::OpenSavedDocument(const char* password) {
  ...
  EXPECT_TRUE(OpenDocumentHelper(password, kDefaultLinearize, kEnableJavaScript,
                                 saved_fake_file_access_.get(),
