...
float PercentageDifferent(const Image& baseline,
                          const Image& actual,
                          uint16_t max_pixel_per_channel_delta) {
  int w = std::min(baseline.w(), actual.w());
  int h = std::min(baseline.h(), actual.h());
  ...
  for (int y = 0; y < h; ++y) {
    for (int x = 0; x < w; ++x) {
      const UnpackedPixel baseline_pixel(baseline.pixel_at(x, y));
      const UnpackedPixel actual_pixel(actual.pixel_at(x, y));

      if (ChannelDelta(baseline_pixel.red, actual_pixel.red) >
...
int main(int argc, const char* argv[]) {
  ...
  uint16_t max_pixel_per_channel_delta = 0;
