...
GFGAS_GEGraphics::GFGAS_GEGraphics(CFX_RenderDevice* renderDevice)
    : m_renderDevice(renderDevice) {
  DCHECK(m_renderDevice);
}

GFGAS_GEGraphics::~GFGAS_GEGraphics() = default;

void GFGAS_GEGraphics::SaveGraphState() {
  m_renderDevice->SaveState();
  m_infoStack.push_back(std::make_unique<TInfo>(m_info));
}

void GFGAS_GEGraphics::RestoreGraphState() {
...
void GFGAS_GEGraphics::SetLineCap(CFX_GraphStateData::LineCap lineCap) {
  m_info.graphState.m_LineCap = lineCap;
}

void GFGAS_GEGraphics::SetLineDash(float dashPhase,
...
void GFGAS_GEGraphics::SetSolidLineDash() {
  m_info.graphState.m_DashArray.clear();
}

void GFGAS_GEGraphics::SetLineWidth(float lineWidth) {
  m_info.graphState.m_LineWidth = lineWidth;
}

void GFGAS_GEGraphics::EnableActOnDash() {
  m_info.isActOnDash = true;
}

void GFGAS_GEGraphics::SetStrokeColor(const CFGAS_GEColor& color) {
  m_info.strokeColor = color;
}

void GFGAS_GEGraphics::SetFillColor(const CFGAS_GEColor& color) {
  m_info.fillColor = color;
}

void GFGAS_GEGraphics::StrokePath(CFGAS_GEPath* path,
                                  const CFX_Matrix* matrix) {
  if (path)
    RenderDeviceStrokePath(path, matrix);
}

void GFGAS_GEGraphics::FillPath(CFGAS_GEPath* path,
...
void GFGAS_GEGraphics::ConcatMatrix(const CFX_Matrix* matrix) {
  if (matrix)
    m_info.CTM.Concat(*matrix);
}

const CFX_Matrix* GFGAS_GEGraphics::GetMatrix() const {
  return &m_info.CTM;
}

CFX_RectF GFGAS_GEGraphics::GetClipRect() const {
  FX_RECT r = m_renderDevice->GetClipBox();
  return CFX_RectF(r.left, r.top, r.Width(), r.Height());
}

void GFGAS_GEGraphics::SetClipRect(const CFX_RectF& rect) {
...
CFX_RenderDevice* GFGAS_GEGraphics::GetRenderDevice() {
  return m_renderDevice;
}

void GFGAS_GEGraphics::RenderDeviceStrokePath(const CFGAS_GEPath* path,
                                              const CFX_Matrix* matrix) {
...
void GFGAS_GEGraphics::RenderDeviceFillPath(
...
void GFGAS_GEGraphics::FillPathWithPattern(
    const CFGAS_GEPath* path,
...
void GFGAS_GEGraphics::FillPathWithShading(
    const CFGAS_GEPath* path,
    const CFX_FillRenderOptions& fill_options,
    const CFX_Matrix& matrix) {
  RetainPtr<CFX_DIBitmap> bitmap = m_renderDevice->GetBitmap();
...
void GFGAS_GEGraphics::SetDIBitsWithMatrix(const RetainPtr<CFX_DIBBase>& source,
                                           const CFX_Matrix& matrix) {
...
GFGAS_GEGraphics::TInfo::TInfo() = default;

GFGAS_GEGraphics::TInfo::TInfo(const TInfo& info)
...
GFGAS_GEGraphics::TInfo& GFGAS_GEGraphics::TInfo::operator=(
    const TInfo& other) {
