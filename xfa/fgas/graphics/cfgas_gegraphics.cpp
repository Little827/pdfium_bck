...
CFXA_GEGraphics::CFXA_GEGraphics(CFX_RenderDevice* renderDevice)
    : m_renderDevice(renderDevice) {
  DCHECK(m_renderDevice);
}

CFXA_GEGraphics::~CFXA_GEGraphics() = default;

void CFXA_GEGraphics::SaveGraphState() {
  m_renderDevice->SaveState();
  m_infoStack.push_back(std::make_unique<TInfo>(m_info));
}

void CFXA_GEGraphics::RestoreGraphState() {
...
void CFXA_GEGraphics::SetLineCap(CFX_GraphStateData::LineCap lineCap) {
  m_info.graphState.m_LineCap = lineCap;
}

void CFXA_GEGraphics::SetLineDash(float dashPhase,
...
void CFXA_GEGraphics::SetSolidLineDash() {
  m_info.graphState.m_DashArray.clear();
}

void CFXA_GEGraphics::SetLineWidth(float lineWidth) {
  m_info.graphState.m_LineWidth = lineWidth;
}

void CFXA_GEGraphics::EnableActOnDash() {
  m_info.isActOnDash = true;
}

void CFXA_GEGraphics::SetStrokeColor(const CFGAS_GEColor& color) {
  m_info.strokeColor = color;
}

void CFXA_GEGraphics::SetFillColor(const CFGAS_GEColor& color) {
  m_info.fillColor = color;
}

void CFXA_GEGraphics::StrokePath(CFGAS_GEPath* path, const CFX_Matrix* matrix) {
  if (path)
    RenderDeviceStrokePath(path, matrix);
}

void CFXA_GEGraphics::FillPath(CFGAS_GEPath* path,
...
void CFXA_GEGraphics::ConcatMatrix(const CFX_Matrix* matrix) {
  if (matrix)
    m_info.CTM.Concat(*matrix);
}

const CFX_Matrix* CFXA_GEGraphics::GetMatrix() const {
  return &m_info.CTM;
}

CFX_RectF CFXA_GEGraphics::GetClipRect() const {
  FX_RECT r = m_renderDevice->GetClipBox();
  return CFX_RectF(r.left, r.top, r.Width(), r.Height());
}

void CFXA_GEGraphics::SetClipRect(const CFX_RectF& rect) {
...
CFX_RenderDevice* CFXA_GEGraphics::GetRenderDevice() {
  return m_renderDevice;
}

void CFXA_GEGraphics::RenderDeviceStrokePath(const CFGAS_GEPath* path,
                                             const CFX_Matrix* matrix) {
...
void CFXA_GEGraphics::RenderDeviceFillPath(
...
void CFXA_GEGraphics::FillPathWithPattern(
    const CFGAS_GEPath* path,
...
void CFXA_GEGraphics::FillPathWithShading(
    const CFGAS_GEPath* path,
    const CFX_FillRenderOptions& fill_options,
    const CFX_Matrix& matrix) {
  RetainPtr<CFX_DIBitmap> bitmap = m_renderDevice->GetBitmap();
...
void CFXA_GEGraphics::SetDIBitsWithMatrix(const RetainPtr<CFX_DIBBase>& source,
                                          const CFX_Matrix& matrix) {
...
CFXA_GEGraphics::TInfo::TInfo() = default;

CFXA_GEGraphics::TInfo::TInfo(const TInfo& info)
...
CFXA_GEGraphics::TInfo& CFXA_GEGraphics::TInfo::operator=(const TInfo& other) {
