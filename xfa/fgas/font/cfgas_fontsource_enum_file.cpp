...
CFFAS_FontSourceEnumFile::CFFAS_FontSourceEnumFile()
    : m_FolderPaths(std::begin(kFontFolders), std::end(kFontFolders)) {}

CFFAS_FontSourceEnumFile::~CFFAS_FontSourceEnumFile() = default;

ByteString CFFAS_FontSourceEnumFile::GetNextFile() {
...
void CFFAS_FontSourceEnumFile::GetNext() {
  m_wsNext = WideString::FromUTF8(GetNextFile().AsStringView());
}

bool CFFAS_FontSourceEnumFile::HasNext() const {
  return !m_wsNext.IsEmpty();
}

RetainPtr<IFX_SeekableStream> CFFAS_FontSourceEnumFile::GetStream() const {
  ASSERT(HasNext());
