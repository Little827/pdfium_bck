...
Optional<int32_t> CFGAS_GEFont::GetCharWidth(wchar_t wUnicode) {
  ...
  // TODO(tsepez): might have wacky flag set in high byte?
  int32_t width = m_pFont->GetGlyphWidth(glyph);
  if (width < 0) {
    m_CharWidthMap[wUnicode] = pdfium::nullopt;
    return pdfium::nullopt;
  }
  // TODO(tsepez): high bytes won't be returned next time around.
  m_CharWidthMap[wUnicode] = static_cast<uint16_t>(width);
  return width;
}
