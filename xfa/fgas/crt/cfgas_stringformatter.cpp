...
const LocaleDateTimeSubcategoryWithHash kLocaleDateTimeSubcategoryData[] = {
...
const LocaleNumberSubcategoryWithHash kLocaleNumSubcategoryData[] = {
...
CFGAS_StringFormatter::LocaleCategory CFGAS_StringFormatter::GetCategory()
    const {
  LocaleCategory eCategory = LocaleCategory::kUnknown;
  size_t ccf = 0;
  ...
  while (ccf < m_spPattern.size()) {
    if (m_spPattern[ccf] == '\'') {
      ...
      if (dwHash == FX_LOCALECATEGORY_DateTimeHash)
        return LocaleCategory::kDateTime;
      if (dwHash == FX_LOCALECATEGORY_TextHash)
        return LocaleCategory::kText;
      if (dwHash == FX_LOCALECATEGORY_NumHash)
        return LocaleCategory::kNum;
      if (dwHash == FX_LOCALECATEGORY_ZeroHash)
        return LocaleCategory::kZero;
      if (dwHash == FX_LOCALECATEGORY_NullHash)
        return LocaleCategory::kNull;
      if (dwHash == FX_LOCALECATEGORY_DateHash) {
        if (eCategory == LocaleCategory::kTime)
          return LocaleCategory::kDateTime;
        eCategory = LocaleCategory::kDate;
      } else if (dwHash == FX_LOCALECATEGORY_TimeHash) {
        if (eCategory == LocaleCategory::kDate)
          return LocaleCategory::kDateTime;
        eCategory = LocaleCategory::kTime;
      }
  ...
  while (ccf < m_spPattern.size()) {
    if (m_spPattern[ccf] == '\'') {
      ...
      if (!(iFindCategory & 1) && wsCategory.EqualsASCII("date")) {
        ...
        iFindCategory = 3;
        eCategory = LocaleCategory::kDateTime;
      } else {
        continue;
      }
      while (ccf < m_spPattern.size()) {
      ...
      if (!wsTempPattern.IsEmpty()) {
        ...
        else if (eCategory == LocaleCategory::kDate)
