...
WideString GetLiteralText(pdfium::span<const wchar_t> pStrPattern,
  ...
  (*iPattern)++;
  int32_t iQuote = 1;
  while (*iPattern < pdfium::CollectionSize<int32_t>(pStrPattern)) {
    if (pStrPattern[*iPattern] == '\'') {
      iQuote++;
      if ((*iPattern + 1 >= pdfium::CollectionSize<int32_t>(pStrPattern)) ||
      ...
      (*iPattern)++;
               ...
               (*iPattern + 1 < pdfium::CollectionSize<int32_t>(pStrPattern)) &&
               pStrPattern[*iPattern + 1] == 'u') {
      int32_t iKeyValue = 0;
      *iPattern += 2;
      for (int32_t i = 0;
           *iPattern < pdfium::CollectionSize<int32_t>(pStrPattern) && i < 4;
           ++i) {
...
bool GetNumericDotIndex(const WideString& wsNum,
  ...
  for (int32_t ccf = 0; ccf < pdfium::CollectionSize<int32_t>(spNum); ++ccf) {
    if (spNum[ccf] == '\'') {
      GetLiteralText(spNum, &ccf);
      continue;
    }
    if (ccf + iLenDot <= pdfium::CollectionSize<int32_t>(spNum) &&
  ...
  *iDotIndex = result.value_or(pdfium::CollectionSize<int32_t>(spNum));
...
bool ParseLocaleDate(const WideString& wsDate,
  ...
  while (*cc < len && ccf < pdfium::CollectionSize<int32_t>(spDatePattern)) {
    ...
    while (ccf < pdfium::CollectionSize<int32_t>(spDatePattern) &&
           spDatePattern[ccf] == symbol[0]) {
      symbol += spDatePattern[ccf++];
    }
...
bool ParseLocaleTime(const WideString& wsTime,
  ...
  while (*cc < len && ccf < pdfium::CollectionSize<int32_t>(spTimePattern)) {
    ...
    while (ccf < pdfium::CollectionSize<int32_t>(spTimePattern) &&
           spTimePattern[ccf] == symbol[0])
      symbol += spTimePattern[ccf++];
...
WideString TimeFormat(const WideString& wsTimePattern,
  ...
  WideStringView wsTimeSymbols(gs_wsTimeSymbols);
  while (ccf < pdfium::CollectionSize<int32_t>(spTimePattern)) {
    if (wsTimePattern[ccf] == '\'') {
      wsResult += GetLiteralText(spTimePattern, &ccf);
      ccf++;
      continue;
    }
    if (!wsTimeSymbols.Contains(wsTimePattern[ccf])) {
      wsResult += wsTimePattern[ccf++];
      continue;
    }

    WideString symbol;
    symbol.Reserve(4);
    symbol += wsTimePattern[ccf++];
    while (ccf < pdfium::CollectionSize<int32_t>(spTimePattern) &&
           wsTimePattern[ccf] == symbol[0])
      symbol += wsTimePattern[ccf++];
...
FX_LOCALECATEGORY CFGAS_FormatString::GetCategory(
  ...
  while (ccf < pdfium::CollectionSize<int32_t>(spPattern)) {
    if (spPattern[ccf] == '\'') {
      ...
      ccf++;
      while (true) {
        if (ccf == pdfium::CollectionSize<int32_t>(spPattern))
          return eCategory;
...
LocaleIface* CFGAS_FormatString::GetNumericFormat(
  ...
  *dwStyle = 0;
  ...
  while (ccf < pdfium::CollectionSize<int32_t>(spPattern)) {
    ...
    if (!bFindDot && ccf < pdfium::CollectionSize<int32_t>(spPattern) &&
...
bool CFGAS_FormatString::ParseText(const WideString& wsSrcText,
  ...
  while (iPattern < pdfium::CollectionSize<int32_t>(spTextFormat) &&
         iText < iLenText) {
  ...
  return iPattern == pdfium::CollectionSize<int32_t>(spTextFormat) &&
         iText == iLenText;
}
...
bool CFGAS_FormatString::ParseNum(const WideString& wsSrcNum,
  ...
  if (!bReverseParse) {
    ...
    for (ccf = dot_index_f + 1;
         cc < len && ccf < pdfium::CollectionSize<int32_t>(spNumFormat);
         ++ccf) {
      switch (spNumFormat[ccf]) {
        case '\'': {
        ...
        case 'c':
        case 'C':
          if (ccf + 1 < pdfium::CollectionSize<int32_t>(spNumFormat) &&
        ...
        case '8': {
          while (ccf + 1 < pdfium::CollectionSize<int32_t>(spNumFormat) &&
                 spNumFormat[ccf + 1] == '8')
            ccf++;
  ...
  while (iPattern < iLenPattern && iText < iLenText) {
    if (pStrPattern[iPattern] == '\'') {
      WideString wsLiteral =
          GetLiteralText({pStrPattern, iLenPattern}, &iPattern);
      int32_t iLiteralLen = wsLiteral.GetLength();
      if (iText + iLiteralLen > iLenText ||
          wcsncmp(pStrText + iText, wsLiteral.c_str(), iLiteralLen)) {
        return false;
...
bool CFGAS_FormatString::FormatText(const WideString& wsSrcText,
  ...
  while (iPattern < iLenPattern) {
    switch (pStrPattern[iPattern]) {
      case '\'': {
        *wsOutput += GetLiteralText({pStrPattern, iLenPattern}, &iPattern);
        iPattern++;
        break;
      }
      case 'A':
...
bool CFGAS_FormatString::FormatStrNum(WideStringView wsInputNum,
  ...
  if (dwNumStyle & FX_NUMSTYLE_Exponent) {
    ...
    for (int ccf = 0; ccf < dot_index_f; ++ccf) {
      switch (strf[ccf]) {
        case '\'':
          GetLiteralText({strf, dot_index_f}, &ccf);
          break;
        case '9':
        case 'z':
        case 'Z':
  ...
  for (int ccf = dot_index_f - 1; ccf >= 0; --ccf) {
    switch (strf[ccf]) {
      ...
      case ')':
        ...
        break;
      case '\'':
        *wsOutput = GetLiteralTextReverse({strf, lenf}, &ccf) + *wsOutput;
        break;
      default:
  ...
  for (int ccf = dot_index_f + 1; ccf < lenf; ++ccf) {
    switch (strf[ccf]) {
      case '\'':
        *wsOutput += GetLiteralText({strf, lenf}, &ccf);
        break;
      case '9':
        if (cc < len) {
          if (!FXSYS_IsDecimalDigit(str[cc]))
