...
class CXFA_FFField : public CXFA_FFWidget, public IFWL_WidgetDelegate {
 public:
  ...
  void RenderWidget(GFGAS_GEGraphics* pGS,
 ...
 protected:
  ...
  void LayoutCaption();
  void RenderCaption(GFGAS_GEGraphics* pGS, CFX_Matrix* pMatrix);

  int32_t CalculateOverride();
  int32_t CalculateNode(CXFA_Node* pNode);
  bool ProcessCommittedData();
  void DrawHighlight(GFGAS_GEGraphics* pGS,
                     CFX_Matrix* pMatrix,
                     HighlightOption highlight,
                     ShapeOption shape);
  void DrawFocus(GFGAS_GEGraphics* pGS, CFX_Matrix* pMatrix);
