...
bool CXFA_FFField::OnMouseEnter() {
  ...
  SendFWLMessageToForm(std::move(ms));
  return true;
}
...
bool CXFA_FFField::OnMouseWheel(uint32_t dwFlags,
  ...
  auto ms = pdfium::MakeUnique<CFWL_MessageMouseWheel>(nullptr,
                                                       m_pNormalWidget.get());
  ms->m_dwFlags = dwFlags;
  ms->m_pos = FWLToClient(point);
...
bool CXFA_FFField::OnKillFocus(CXFA_FFWidget* pNewWidget) {
  if (m_pNormalWidget) {
    SendFWLMessageToForm(pdfium::MakeUnique<CFWL_MessageKillFocus>(
...
bool CXFA_FFField::OnKeyDown(uint32_t dwKeyCode, uint32_t dwFlags) {
  ...
  auto ms = pdfium::MakeUnique<CFWL_MessageKey>(nullptr, m_pNormalWidget.get());
...
void CXFA_FFField::SendFWLMessageToForm(
