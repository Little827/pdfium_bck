...
bool CXFA_FFField::OnLButtonDown(Mask<FWL_KeyFlag> dwFlags,
                                 const CFX_PointF& point) {
  SetButtonDown(true);
  CFWL_MessageMouse msg(GetNormalWidget(),
...
bool CXFA_FFField::OnLButtonUp(Mask<FWL_KeyFlag> dwFlags,
                               const CFX_PointF& point) {
  if (!GetNormalWidget())
    return false;
  if (!IsButtonDown())
...
bool CXFA_FFField::OnLButtonDblClk(Mask<FWL_KeyFlag> dwFlags,
                                   const CFX_PointF& point) {
  if (!GetNormalWidget())
    return false;
...
bool CXFA_FFField::OnMouseMove(Mask<FWL_KeyFlag> dwFlags,
                               const CFX_PointF& point) {
  if (!GetNormalWidget())
    return false;
...
bool CXFA_FFField::OnMouseWheel(Mask<FWL_KeyFlag> dwFlags,
                                const CFX_PointF& point,
...
bool CXFA_FFField::OnRButtonDown(Mask<FWL_KeyFlag> dwFlags,
                                 const CFX_PointF& point) {
  SetButtonDown(true);
...
bool CXFA_FFField::OnRButtonUp(Mask<FWL_KeyFlag> dwFlags,
                               const CFX_PointF& point) {
  if (!GetNormalWidget())
    return false;
  if (!IsButtonDown())
...
bool CXFA_FFField::OnRButtonDblClk(Mask<FWL_KeyFlag> dwFlags,
                                   const CFX_PointF& point) {
  if (!GetNormalWidget())
    return false;
...
bool CXFA_FFField::OnChar(uint32_t dwChar, Mask<FWL_KeyFlag> dwFlags) {
  if (!GetDoc()->GetXFADoc()->IsInteractive())
    return false;
