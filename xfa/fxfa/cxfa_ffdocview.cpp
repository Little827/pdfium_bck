...
XFA_EventError CXFA_FFDocView::ExecEventActivityByDeepFirst(
  ...
  if (bRecursive) {
    for (CXFA_Node* pNode = pFormNode->GetFirstContainerChild(); pNode;
      ...
      if (elementType != XFA_Element::Variables &&
        ...
        XFA_EventError iCurrentRet = ExecEventActivityByDeepFirst(
            pNode, eEventType, bIsFormReady, bRecursive);
        if (iCurrentRet == XFA_EventError::kError ||
            iRet == XFA_EventError::kNotExist) {
          iRet = iCurrentRet;
        }
      }
    }
  ...
  XFA_EventError iFinalRet = XFA_ProcessEvent(this, pFormNode, &eParam);
  if (iFinalRet == XFA_EventError::kError ||
      iRet == XFA_EventError::kNotExist) {
    iRet = iFinalRet;
  }
  return iRet;
