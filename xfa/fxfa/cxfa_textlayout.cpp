...
float CXFA_TextLayout::DoLayout(int32_t iBlockIndex,
  ...
  for (int32_t i = iLineIndex; i < iCount; ++i) {
    ...
    if (i == iLineIndex && fLineHeight - fContentAreaHeight > kHeightTolerance)
      return 0;
    ...
    if (iBlockCount >= iBlockIndex + 1) {
      m_Blocks[iBlockIndex].iIndex = iLineIndex;
      m_Blocks[iBlockIndex].iLength = i - iLineIndex;
    } else {
    ...
    if (pdfium::CollectionSize<int32_t>(m_pLoader->blockHeights) >
            iBlockIndex &&
        (m_pLoader->blockHeights[iBlockIndex].iBlockIndex == iBlockIndex)) {
      m_pLoader->blockHeights[iBlockIndex].fHeight = fCalcHeight;
    } else {
      m_pLoader->blockHeights.emplace_back(iBlockIndex, fCalcHeight);
    }
  }
  return fCalcHeight;
}
