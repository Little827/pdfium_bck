...
float CXFA_TextLayout::DoSplitLayout(size_t szBlockIndex,
  ...
  std::vector<CXFA_BlockHeight>& block_heights = m_pLoader->blockHeights;
  ...
  if (!m_Blocks.empty()) {
    ...
    if (!block_heights.empty()) {
      for (size_t i = 0; i < szBlockIndex; ++i)
        fLinePos -= block_heights[i].fHeight;
    }
  }

  const std::vector<float>& line_heights = m_pLoader->lineHeights;
  if (szLineIndex >= line_heights.size())
    return fCalcHeight;

  if (line_heights[szLineIndex] - fCalcHeight > kHeightTolerance)
    return 0;

  for (size_t i = szLineIndex; i < line_heights.size(); ++i) {
    float fLineHeight = line_heights[i];
    ...
    if (szBlockIndex < block_heights.size() &&
        block_heights[szBlockIndex].szBlockIndex == szBlockIndex) {
      block_heights[szBlockIndex].fHeight = fCalcHeight;
    } else {
      block_heights.push_back({szBlockIndex, fCalcHeight});
    }
    return fCalcHeight;
  }
  return fCalcHeight;
...
bool CXFA_TextLayout::LayoutInternal(size_t szBlockIndex) {
  ...
  if (szBlockIndex == m_pLoader->blockHeights.size()) {
    ...
    if (szBlockIndex < m_Blocks.size() - 1)
      m_pLoader->iTotalLines = m_Blocks[szBlockIndex].szLength;
