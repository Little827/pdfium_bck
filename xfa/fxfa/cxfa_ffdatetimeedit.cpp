...
void CXFA_FFDateTimeEdit::UpdateWidgetProperty() {
  CFWL_DateTimePicker* pWidget = GetPickerWidget();
  if (!pWidget)
    return;
...
bool CXFA_FFDateTimeEdit::UpdateFWLData() {
  ...
  auto* normalWidget = GetPickerWidget();
...
void CXFA_FFDateTimeEdit::OnSelectChanged(CFWL_Widget* pWidget,
  ...
  auto* pDateTime = GetPickerWidget();
...
Optional<WideString> CXFA_FFDateTimeEdit::Copy() {
  std::cerr << (void*)this << " CXFA_FFDateTimeEdit::Copy() classid "
            << (int)m_pNormalWidget.get()->GetClassID() << std::endl;
