...
bool ExistContainerKeep(CXFA_Node* pCurNode, bool bPreFind) {
  ...
  if (pKeep) {
    ...
    if (previous.has_value()) {
      if (previous.value() == XFA_AttributeValue::ContentArea ||
          previous.value() == XFA_AttributeValue::PageArea) {
        return true;
      }
    }
  }
  ...
  if (!next.has_value())
    return false;
  if (next.value() == XFA_AttributeValue::ContentArea ||
      next.value() == XFA_AttributeValue::PageArea) {
    return true;
  }
  return false;
}
