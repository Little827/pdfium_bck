...
bool CXFA_ViewLayoutProcessor::InitLayoutPage(CXFA_Node* pFormNode) {
  ...
  int32_t iCount = 0;
  for (pPageArea = m_pPageSetTemplateNode->GetFirstChild(); pPageArea;
  ...
  pPageArea = m_pPageSetTemplateNode->GetChild<CXFA_Node>(
      0, XFA_Element::PageArea, false);
  if (!pPageArea) {
    pPageArea = pDocument->CreateNode(m_pPageSetTemplateNode->GetPacketType(),
                                      XFA_Element::PageArea);
    if (!pPageArea)
      return false;

    m_pPageSetTemplateNode->InsertChildAndNotify(pPageArea, nullptr);
...
CXFA_ViewLayoutProcessor::CreateViewRecord(CXFA_Node* pPageNode,
  ...
  if (!HasCurrentViewRecord()) {
    ...
    if (pPageSet == m_pPageSetTemplateNode) {
...
CXFA_ViewLayoutProcessor::ExecuteBreakBeforeOrAfter(const CXFA_Node* pCurNode,
  ...
  switch (eType) {
    ...
    case XFA_Element::BreakAfter: {
      ...
      CXFA_Node* pTarget =
          ResolveBreakTarget(m_pPageSetTemplateNode, true, &wsTarget);
...
bool CXFA_ViewLayoutProcessor::BreakOverflow(const CXFA_Node* pOverflowNode,
  ...
  if (pOverflowNode->GetElementType() == XFA_Element::Break) {
    ...
    if (!wsOverflowTarget.IsEmpty() && bCreatePage && !m_bCreateOverFlowPage) {
      CXFA_Node* pTarget =
          ResolveBreakTarget(m_pPageSetTemplateNode, true, &wsOverflowTarget);
      if (pTarget) {
...
CXFA_Node* CXFA_ViewLayoutProcessor::GetNextAvailPageArea(
    ...
    bool bQuery) {
  if (!m_pCurPageArea) {
    FindPageAreaFromPageSet(m_pPageSetTemplateNode, nullptr, pTargetPageArea,
...
void CXFA_ViewLayoutProcessor::InitPageSetMap() {
  ...
  CXFA_NodeIterator sIterator(m_pPageSetTemplateNode);
...
void CXFA_ViewLayoutProcessor::SaveLayoutItemChildren(
  ...
  CXFA_Document* pDocument = m_pPageSetTemplateNode->GetDocument();
...
void CXFA_ViewLayoutProcessor::SyncLayoutData() {
  ...
  CXFA_FFNotify* pNotify = m_pPageSetTemplateNode->GetDocument()->GetNotify();
  int32_t nPageIdx = -1;
  for (CXFA_ViewLayoutItem* pRootLayoutItem = GetRootLayoutItem();
       pRootLayoutItem;
       pRootLayoutItem = ToViewLayoutItem(pRootLayoutItem->GetNextSibling())) {
