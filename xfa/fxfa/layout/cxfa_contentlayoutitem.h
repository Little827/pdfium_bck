...
class CXFA_ContentLayoutItem : public CXFA_LayoutItem {
 public:
  class WidgetIface {
   public:
    virtual ~WidgetIface() = default;
    virtual void SetLayoutItem(CXFA_ContentLayoutItem* pItem) = 0;
  };
  ...
  WidgetIface* GetFFWidget() { return m_pFFWidget.get(); }
 ...
 private:
  CXFA_ContentLayoutItem(CXFA_Node* pNode,
                         std::unique_ptr<WidgetIface> pFFWidget);
  ...
  std::unique_ptr<WidgetIface> const m_pFFWidget;
};
