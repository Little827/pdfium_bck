...
void CXFA_ItemLayoutProcessor::DoLayoutPageArea(
  ...
  for (Stage nCurChildNodeStage = GotoNextContainerNode(
    ...
    if (!pBeforeItem)
      ...
      pPageAreaLayoutItem->InsertBefore(pProcessItem, pBeforeItem);
  ...
  while (pLayoutItem) {
    ...
    if (!pBeforeItem)
      ...
      pPageAreaLayoutItem->InsertBefore(pLayoutItem, pBeforeItem);
...
CXFA_ItemLayoutProcessor::DoLayoutFlowedContainer(
  ...
  while (m_nCurChildNodeStage != Stage::kDone) {
    ...
    if (pLastChild) {
      ...
      if (pLastChild->GetParent())
        pLastChild->GetParent()->RemoveChild(pLastChild);
