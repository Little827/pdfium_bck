...
bool CXFA_ItemLayoutProcessor::ProcessKeepForSplit(
    ...
    CXFA_ItemLayoutProcessor::Result eRetValue,
    ...
    bool* bForceEndPage,
    CXFA_ItemLayoutProcessor::Result* result) {
  if (!pChildProcessor)
    return false;
  ...
  if (JudgePutNextPage(m_pLayoutItem, childSize.height, &keepLayoutItems)) {
    ...
    *bForceEndPage = true;
    *result = CXFA_ItemLayoutProcessor::Result::kPageFullBreak;
    return true;
  }
...
CXFA_ItemLayoutProcessor::DoLayoutFlowedContainer(
  ...
  if (m_pCurChildPreprocessor) {
    m_pCurChildPreprocessor->m_ePreProcessRs =
        CXFA_ItemLayoutProcessor::Result::kDone;
  }
  CFX_SizeF containerSize = CalculateContainerSpecifiedSize(
      GetFormNode(), &bContainerWidthAutoSize, &bContainerHeightAutoSize);
    ...
    while (m_pCurChildNode) {
      ...
      switch (m_nCurChildNodeStage) {
        ...
        case Stage::kContainer: {
          ...
          CXFA_ItemLayoutProcessor::Result rs = InsertFlowedItem(
          ...
          switch (rs) {
            case CXFA_ItemLayoutProcessor::Result::kManualBreak:
              bIsManualBreak = true;
              FALLTHROUGH;
            case CXFA_ItemLayoutProcessor::Result::kPageFullBreak:
              bForceEndPage = true;
              FALLTHROUGH;
            case CXFA_ItemLayoutProcessor::Result::kRowFullBreak:
              goto SuspendAndCreateNewRow;
            case CXFA_ItemLayoutProcessor::Result::kDone:
            default:
              fContentCurRowY +=
                  pProcessor->InsertPendingItems(m_pCurChildNode);
              pProcessor.reset();
  ...
  return bRetValue ? CXFA_ItemLayoutProcessor::Result::kDone
                   : (bIsManualBreak
                          ? CXFA_ItemLayoutProcessor::Result::kManualBreak
                          : CXFA_ItemLayoutProcessor::Result::kPageFullBreak);
}
    ...
    case XFA_Element::SubformSet: {
      ...
      switch (eLayoutStrategy) {
        ...
        default:
          ...
          return CXFA_ItemLayoutProcessor::Result::kDone;
...
CXFA_ItemLayoutProcessor::Result CXFA_ItemLayoutProcessor::InsertFlowedItem(
  ...
  if ((eFlowStrategy != XFA_AttributeValue::Rl_tb &&
      ...
      (eFlowStrategy == XFA_AttributeValue::Rl_tb &&
       uHAlign > *uCurHAlignState)) {
    return CXFA_ItemLayoutProcessor::Result::kRowFullBreak;
  }
  ...
  *uCurHAlignState = uHAlign;
  ...
  CXFA_ItemLayoutProcessor::Result eRetValue =
      CXFA_ItemLayoutProcessor::Result::kDone;
  if (!bNewRow ||
      pProcessor->m_ePreProcessRs == CXFA_ItemLayoutProcessor::Result::kDone) {
  ...
  if (!bTakeSpace ||
    ...
    if (!bTakeSpace || eRetValue == CXFA_ItemLayoutProcessor::Result::kDone) {
    ...
    if (eRetValue == CXFA_ItemLayoutProcessor::Result::kPageFullBreak) {
      if (pProcessor->m_bUseInheriated) {
        if (pTrailerLayoutItem) {
          pProcessor->AddTrailerBeforeSplit(childSize.height,
                                            pTrailerLayoutItem, false);
  ...
  CXFA_ItemLayoutProcessor::Result eResult;
  ...
  *bForceEndPage = true;
  ...
  if (fSplitPos > kXFALayoutPrecision) {
    ...
    if (eLayout == XFA_AttributeValue::Tb &&
        eRetValue == CXFA_ItemLayoutProcessor::Result::kDone) {
      pProcessor->ProcessUnUseOverFlow(pOverflowLeaderNode,
      ...
      *bAddedItemInRow = true;
      ...
      return CXFA_ItemLayoutProcessor::Result::kPageFullBreak;
    }

    CXFA_Node* pTempLeaderNode = nullptr;
    CXFA_Node* pTempTrailerNode = nullptr;
    if (m_pPageMgr && !pProcessor->m_bUseInheriated &&
        eRetValue != CXFA_ItemLayoutProcessor::Result::kPageFullBreak) {
  ...
  if (*fContentCurRowY <= kXFALayoutPrecision) {
    ...
    *bAddedItemInRow = true;
    ...
    if (eRetValue == CXFA_ItemLayoutProcessor::Result::kDone)
      *bForceEndPage = false;
