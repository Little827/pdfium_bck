...
void XFA_DrawImage(CXFA_Graphics* pGS,
  ...
  CFX_RectF rtFit(rtImage.TopLeft(),
                  XFA_UnitPx2Pt(static_cast<float>(pDIBitmap->GetWidth()),
                                static_cast<float>(dpi.width)),
                  XFA_UnitPx2Pt(static_cast<float>(pDIBitmap->GetHeight()),
                                static_cast<float>(dpi.height)));
  switch (iAspect) {
