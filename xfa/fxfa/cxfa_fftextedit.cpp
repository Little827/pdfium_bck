...
bool CXFA_FFTextEdit::OnSetFocus(CXFA_FFWidget* pOldWidget) {
  GetLayoutItem()->ClearStatusBit(XFA_WidgetStatus_TextEditValueChanged);
  if (!IsFocused()) {
    GetLayoutItem()->SetStatusBit(XFA_WidgetStatus_Focused);
    UpdateFWLData();
    InvalidateRect();
...
bool CXFA_FFTextEdit::IsDataChanged() {
  return GetLayoutItem()->TestStatusBit(XFA_WidgetStatus_TextEditValueChanged);
}
...
void CXFA_FFTextEdit::OnTextWillChange(CFWL_Widget* pWidget,
  ...
  GetLayoutItem()->SetStatusBit(XFA_WidgetStatus_TextEditValueChanged);
