...
CFGAS_StringFormatter::LocaleCategory ValueCategory(
    CFGAS_StringFormatter::LocaleCategory eCategory,
    uint32_t dwValueType) {
  if (eCategory != CFGAS_StringFormatter::LocaleCategory::kUnknown)
    return eCategory;
  ...
  switch (dwValueType) {
    ...
    case XFA_VT_DECIMAL:
    case XFA_VT_FLOAT:
      return CFGAS_StringFormatter::LocaleCategory::kNum;
    case XFA_VT_TEXT:
      return CFGAS_StringFormatter::LocaleCategory::kText;
    case XFA_VT_DATE:
      return CFGAS_StringFormatter::LocaleCategory::kDate;
    case XFA_VT_TIME:
      return CFGAS_StringFormatter::LocaleCategory::kTime;
    case XFA_VT_DATETIME:
      return CFGAS_StringFormatter::LocaleCategory::kDateTime;
  }
  return CFGAS_StringFormatter::LocaleCategory::kUnknown;
}
...
bool CXFA_LocaleValue::ValidateValue(const WideString& wsValue,
  ...
  for (; !bRet && i < wsPatterns.size(); i++) {
    ...
    switch (ValueCategory(pFormat->GetCategory(), m_dwType)) {
      case CFGAS_StringFormatter::LocaleCategory::kNull:
        bRet = pFormat->ParseNull(wsValue);
        if (!bRet)
          bRet = wsValue.IsEmpty();
        break;
      case CFGAS_StringFormatter::LocaleCategory::kZero:
        bRet = pFormat->ParseZero(wsValue);
        if (!bRet)
          bRet = wsValue.EqualsASCII("0");
        break;
      case CFGAS_StringFormatter::LocaleCategory::kNum: {
        WideString fNum;
      ...
      case CFGAS_StringFormatter::LocaleCategory::kText:
        bRet = pFormat->ParseText(wsValue, &wsOutput);
        wsOutput.clear();
        if (!bRet)
          bRet = pFormat->FormatText(wsValue, &wsOutput);
        break;
      case CFGAS_StringFormatter::LocaleCategory::kDate: {
        CFX_DateTime dt;
      ...
      case CFGAS_StringFormatter::LocaleCategory::kTime: {
        CFX_DateTime dt;
        bRet = pFormat->ParseDateTime(
      ...
      case CFGAS_StringFormatter::LocaleCategory::kDateTime: {
        CFX_DateTime dt;
        bRet = pFormat->ParseDateTime(
...
bool CXFA_LocaleValue::FormatSinglePattern(WideString& wsResult,
  ...
  if (!bRet && (eCategory != CFGAS_StringFormatter::LocaleCategory::kNum ||
