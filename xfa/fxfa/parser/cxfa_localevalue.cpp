...
bool CXFA_LocaleValue::ValidateCanonicalDate(const WideString& wsDate,
  ...
  nStart = nIndex;
  while (spDate[nIndex] != '\0' && nIndex - nStart < wCountM &&
         nIndex < spDate.size()) {
    if (!FXSYS_IsDecimalDigit(spDate[nIndex]))
      return false;
  ...
  nStart = nIndex;
  while (spDate[nIndex] != '\0' && nIndex - nStart < wCountD &&
         nIndex < spDate.size()) {
    if (!FXSYS_IsDecimalDigit(spDate[nIndex]))
      return false;
...
bool CXFA_LocaleValue::ValidateCanonicalTime(const WideString& wsTime) {
  ...
  nStart = nIndex;
  while (nIndex - nStart < wCountM && nIndex < spTime.size() &&
         spTime[nIndex]) {
    if (!FXSYS_IsDecimalDigit(spTime[nIndex]))
      return false;
  ...
  nStart = nIndex;
  while (nIndex - nStart < wCountS && nIndex < spTime.size() &&
         spTime[nIndex]) {
    if (!FXSYS_IsDecimalDigit(spTime[nIndex]))
      return false;
  ...
  if (pos.has_value() && pos.value() != 0) {
    ...
    nIndex++;
    nStart = nIndex;
    while (nIndex - nStart < wCountF && nIndex < spTime.size() &&
           spTime[nIndex]) {
      if (!FXSYS_IsDecimalDigit(spTime[nIndex]))
        return false;
  ...
  if (nIndex < spTime.size()) {
    if (spTime[nIndex] == 'Z') {
      ...
      nIndex++;
      nStart = nIndex;
      while (nIndex - nStart < wCountH && nIndex < spTime.size() &&
             spTime[nIndex]) {
        if (!FXSYS_IsDecimalDigit(spTime[nIndex]))
          return false;
