...
class CXFA_Rectangle : public CXFA_Box {
 ...
 protected:
  ...
  void Stroke(const std::vector<CXFA_Stroke*>& strokes,
              ...
              CFX_RectF rtWidget,
              const CFX_Matrix& matrix);
  void StrokeEmbossed(GFGAS_GEGraphics* pGS,
                      CFX_RectF rt,
                      float fThickness,
                      const CFX_Matrix& matrix);
  void StrokeLowered(GFGAS_GEGraphics* pGS,
                     CFX_RectF rt,
                     float fThickness,
                     const CFX_Matrix& matrix);
  void StrokeRaised(GFGAS_GEGraphics* pGS,
                    CFX_RectF rt,
                    float fThickness,
                    const CFX_Matrix& matrix);
  void StrokeEtched(GFGAS_GEGraphics* pGS,
                    CFX_RectF rt,
                    float fThickness,
                    const CFX_Matrix& matrix);
  void StrokeRect(GFGAS_GEGraphics* pGraphic,
                  const CFX_RectF& rt,
                  float fLineWidth,
                  const CFX_Matrix& matrix,
                  FX_ARGB argbTopLeft,
