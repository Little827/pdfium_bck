...
bool CXFA_DocumentParser::Parse(const RetainPtr<IFX_SeekableStream>& pStream,
  ...
  if (!xml_doc_ && !xml_doc_->GetRoot())
    return false;

  m_pRootNode = ParseAsXDPPacket(xml_doc_->GetRoot(), ePacketID);
  return !!m_pRootNode;
}
...
std::unique_ptr<CFX_XMLDocument> CXFA_DocumentParser::LoadXML(
  ...
  if (!doc->GetInstruction())
    doc->SetInstruction(pdfium::MakeUnique<CFX_XMLInstruction>(L"xml"));
...
CXFA_Node* CXFA_DocumentParser::ParseAsXDPPacket_Data(
  ...
  if (MatchNodeName(pXMLDocumentNode, L"data", packet->uri, packet->flags)) {
    ...
    pDataXMLNode = pDataElement.get();
    xml_doc_->AddNode(std::move(pDataElement));
  }
  if (!pDataXMLNode)
    return nullptr;
