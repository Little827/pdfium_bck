...
void RegenerateFormFile_Changed(CXFA_Node* pNode,
  ...
  if (!wsChildren.IsEmpty() || !wsAttrs.IsEmpty() ||
    ...
    WideString wsName;
    SaveAttribute(pNode, XFA_Attribute::Name, WideString(L"name"), true,
                  wsName);
    buf << "<";
    buf << wsElement;
    buf << wsName;
...
void XFA_DataExporter_DealWithDataGroupNode(CXFA_Node* pDataNode) {
  ...
  if (iChildNum > 0) {
    if (pElement->HasAttribute(WideString(L"xfa:dataNode")))
      pElement->RemoveAttribute(WideString(L"xfa:dataNode"));
    return;
  }
  pElement->SetAttribute(WideString(L"xfa:dataNode"), WideString(L"dataGroup"));
}
