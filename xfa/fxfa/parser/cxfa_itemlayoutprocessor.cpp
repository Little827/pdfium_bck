        ...
        if (pParaNode && pLayoutChild->m_pFirstChild) {
          ...
          if (fOffHeight > 0) {
            for (CXFA_LayoutItem* pInnerIter = pLayoutChild->m_pFirstChild;
                ...
                auto* pInnerLayoutChild =
                    static_cast<CXFA_ContentLayoutItem*>(pInnerIter);
...
void CXFA_ItemLayoutProcessor::SplitLayoutItem(
  ...
  if (pSecondParent) {
    ...
    if (fCurTopMargin > 0 && pLayoutItem->m_pFirstChild) {
      ...
      auto* pParentItem =
          static_cast<CXFA_ContentLayoutItem*>(pSecondParent->m_pParent);
      while (pParentItem) {
        pParentItem->m_sSize.height += fCurTopMargin;
        pParentItem =
            static_cast<CXFA_ContentLayoutItem*>(pParentItem->m_pParent);
      }
    }
  } else {
  ...
  while (m_nCurChildNodeStage != XFA_ItemLayoutProcessorStages::Done) {
    ...
    if (pLayoutChild) {
      ...
      if (static_cast<CXFA_ContentLayoutItem*>(m_pLayoutItem->m_pFirstChild) ==
        ...
        for (auto* pLayoutNext = static_cast<CXFA_ContentLayoutItem*>(
                 m_pLayoutItem->m_pFirstChild);
             pLayoutNext; pLayoutNext = static_cast<CXFA_ContentLayoutItem*>(
                              pLayoutNext->m_pNextSibling)) {
...
void CXFA_ItemLayoutProcessor::AddLeaderAfterSplit(
  ...
  for (auto* pChildItem =
           static_cast<CXFA_ContentLayoutItem*>(m_pLayoutItem->m_pFirstChild);
       pChildItem; pChildItem = static_cast<CXFA_ContentLayoutItem*>(
