...
bool CXFA_NodeHelper::CreateNode(const WideString& wsName,
  ...
  WideString wsActualName = wsName;
  bool bIsClassName = false;
  bool bResult = false;
  if (wsActualName[0] == '!') {
    wsActualName = wsActualName.Right(wsActualName.GetLength() - 1);
  ...
  if (bIsClassName) {
    XFA_Element eType = XFA_GetElementByName(wsActualName);
    ...
    for (int32_t i = 0; i < m_iCreateCount; ++i) {
      ...
      if (pNewNode) {
        pNewNode->JSObject()->SetAttribute(XFA_Attribute::Name,
                                           wsActualName.AsStringView(), false);
