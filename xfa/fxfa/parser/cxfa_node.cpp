...
CXFA_Node* CXFA_Node::GetContainerNode() {
  ...
  if (eType == XFA_Element::Field) {
    CXFA_Node* pFieldNode = this;
    if (pFieldNode && pFieldNode->IsChoiceListMultiSelect())
      return nullptr;
  ...
  switch (GetUIType()) {
    ...
    case XFA_Element::ExclGroup: {
      ...
      while (pNextChild) {
        ...
        CXFA_Node* pNode = pChild;
        bool done = false;
        if (wsValue.IsEmpty()) {
          CXFA_Value* defValue = pNode->GetDefaultValueIfExists();
          if (defValue) {
            wsValue = defValue->GetChildValueContent();
            SetValue(XFA_VALUEPICTURE_Raw, wsValue);
            pNode->SetValue(XFA_VALUEPICTURE_Raw, wsValue);
            done = true;
          }
        }
        if (!done) {
          ...
          pNode->SetValue(XFA_VALUEPICTURE_Raw, itemText);
        }
