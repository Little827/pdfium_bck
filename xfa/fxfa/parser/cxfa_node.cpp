...
bool CXFA_Node::HasButtonRollover() const {
  const CXFA_Items* pItems = GetChild<CXFA_Items>(0, XFA_Element::Items, false);
  if (!pItems)
    return false;
...
std::pair<XFA_Element, int32_t> CXFA_Node::GetMaxChars() const {
  if (auto* pNode = GetChild<CXFA_Value>(0, XFA_Element::Value, false)) {
...
int32_t CXFA_Node::GetFracDigits() const {
  const CXFA_Value* pNode = GetChild<CXFA_Value>(0, XFA_Element::Value, false);
  if (!pNode)
    return -1;

  const CXFA_Decimal* pChild =
