...
Optional<XFA_SCRIPTATTRIBUTEINFO> XFA_GetScriptAttributeByName(
  ...
  while (element != XFA_Element::Unknown) {
    ...
    auto* it = std::lower_bound(std::begin(g_ElementAttributeTable),
                                std::end(g_ElementAttributeTable), compound_key,
                                [](const ElementAttributeRecord& arg,
                                   std::pair<XFA_Element, XFA_Attribute> key) {
                                  return std::make_pair(arg.element,
                                                        arg.attribute) < key;
                                });
