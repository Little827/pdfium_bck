...
class CFDE_TextEditEngineTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    const wchar_t font_family[] = L"Arimo Bold";
#else
    const wchar_t font_family[] = L"Arial Bold";
#endif
    font_ = CFGAS_GEFont::LoadFont(font_family, 0, FX_CodePage::kDefANSI);
    ASSERT_TRUE(font_);
...
TEST_F(CFDE_TextEditEngineTest, Insert) {
  ...
  engine()->Insert(5, L" World");
  EXPECT_TRUE(delegate->text_is_full);
  EXPECT_STREQ(L"Hello Worl", engine()->GetText().c_str());
