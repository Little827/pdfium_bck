...
WordBreakProperty FX_GetWordBreakProperty(wchar_t wcCodePoint) {
  if (static_cast<size_t>(wcCodePoint) >= FX_ArraySize(kCodePointProperties))
    return static_cast<WordBreakProperty>(kWordBreakValueNone);

  uint8_t dwProperty = kCodePointProperties[wcCodePoint >> 1];
