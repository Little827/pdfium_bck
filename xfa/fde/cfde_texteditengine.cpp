...
size_t CFDE_TextEditEngine::CountCharsExceedingSize(const WideString& text,
  ...
  while (low < high) {
    ...
    if ((limit_horizontal_area_ && text_rect.width > available_width_) ||
      ...
      // The current text is too large, search in the left half
      high = mid;
    } else {
      // The current text is within limits, search in the right half
      low = mid + 1;
    }
  }

  size_t chars_exceeding_size = num_to_check - low;
