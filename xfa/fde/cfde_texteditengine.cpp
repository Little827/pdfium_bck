...
void CFDE_TextEditEngine::SelectAll() {
  ...
  selection_ = Selection{.start_idx = 0, .count = text_length_};
}
...
void CFDE_TextEditEngine::SetSelection(size_t start_idx, size_t count) {
  ...
  selection_ = Selection{.start_idx = start_idx, .count = count};
}
...
void CFDE_TextEditEngine::ReplaceSelectedText(const WideString& requested_rep) {
  ...
  if (delegate_) {
    ...
    rep = change.text;
    selection_.value().start_idx = change.selection_start;
    selection_.value().count = change.selection_end - change.selection_start;
  }
