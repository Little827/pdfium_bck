...
void CFWL_ScrollBar::DrawTrack(CXFA_Graphics* pGraphics,
  ...
  param.m_rect = bLower ? m_rtMinTrack : m_rtMaxTrack;
...
void CFWL_ScrollBar::DrawArrowBtn(CXFA_Graphics* pGraphics,
  ...
  param.m_pGraphics = pGraphics;
  param.m_matrix.Concat(*pMatrix);
  param.m_rect = bMinBtn ? m_rtMinBtn : m_rtMaxBtn;
  if (param.m_rect.height > 0 && param.m_rect.width > 0)
    pTheme->DrawBackground(param);
}
