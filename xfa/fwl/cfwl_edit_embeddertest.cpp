...
class CFWLEditEmbedderTest : public XFAJSEmbedderTest {
 protected:
  ...
  void TearDown() override {
    ...
    // TODO(crbug.com/pdfium/11):  Remove this `form_handle()` check once Skia
    // is supported.  Tests which exit early won't even load a page, so should
    // not try to unload in such cases.
    if (form_handle())
      UnloadPage(page());
