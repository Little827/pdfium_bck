...
CFWL_ListItem* CFWL_ListBox::GetItemAtPoint(const CFX_PointF& point) {
  CFX_PointF pos = point - m_ConentRect.TopLeft();
  float fPosX = 0.0f;
  if (m_pHorzScrollBar)
...
bool CFWL_ListBox::ScrollToVisible(CFWL_ListItem* pItem) {
  ...
  rtItem.Offset(0, -fPosY + m_ConentRect.top);
  if (rtItem.top < m_ConentRect.top) {
    fPosY += rtItem.top - m_ConentRect.top;
    bScroll = true;
  } else if (rtItem.bottom() > m_ConentRect.bottom()) {
    fPosY += rtItem.bottom() - m_ConentRect.bottom();
    bScroll = true;
  }
  if (!bScroll)
    return false;
...
void CFWL_ListBox::DrawItems(CXFA_Graphics* pGraphics,
  ...
  for (int32_t i = 0; i < iCount; i++) {
    ...
    CFX_RectF rtItem = pItem->GetRect();
    rtItem.Offset(m_ConentRect.left - fPosX, m_ConentRect.top - fPosY);
    if (rtItem.bottom() < m_ConentRect.top)
      continue;
    if (rtItem.top >= m_ConentRect.bottom())
      break;
...
CFX_SizeF CFWL_ListBox::CalcSize(bool bAutoSize) {
  ...
  if (!m_pOuter) {
    ...
    m_ConentRect.Deflate(pUIMargin.left, pUIMargin.top, pUIMargin.width,
                         pUIMargin.height);
  }
  ...
  if (bShowVertScr) {
    ...
    szRange.width = 0;
    szRange.height = std::max(fs.height - m_ConentRect.height, m_fItemHeight);
