...
void CFWL_DateTimePicker::GetPopupPos(float fMinHeight,
  ...
  if (rtAnchor.bottom() + point.y > 0.0f) {
    *pPopupRect = CFX_RectF(rtAnchor.left, rtAnchor.top - pPopupRect->height,
                            pPopupRect->width, pPopupRect->height);
  } else {
    *pPopupRect = CFX_RectF(rtAnchor.left, rtAnchor.bottom(), pPopupRect->width,
                            pPopupRect->height);
  }
