...
void CFWL_WidgetMgr::InsertWidget(CFWL_Widget* pParent, CFWL_Widget* pChild) {
  Item* pParentItem = GetOrCreateWidgetMgrItem(pParent);
  if (!pParentItem->GetParent())
    GetWidgetMgrItem(nullptr)->AppendChildItem(pParentItem);

  pParentItem->AppendChildItem(GetOrCreateWidgetMgrItem(pChild));
}
...
void CFWL_WidgetMgr::SetParent(CFWL_Widget* pParent, CFWL_Widget* pChild) {
  ...
  pParentItem->AppendChildItem(pItem);
}
...
CFWL_WidgetMgr::Item* CFWL_WidgetMgr::GetOrCreateWidgetMgrItem(
    CFWL_Widget* pWidget) {
  Item* pItem = GetWidgetMgrItem(pWidget);
  if (!pItem) {
    auto pOwnedItem = std::make_unique<Item>(pWidget);
    pItem = pOwnedItem.get();
...
void CFWL_WidgetMgr::Item::AppendChildItem(Item* pItem) {
  if (!pItem)
    return;

  pItem->RemoveSelfIfParented();
  AppendLastChild(pItem);
}
