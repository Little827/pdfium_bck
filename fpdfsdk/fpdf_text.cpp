...
FPDF_EXPORT int FPDF_CALLCONV FPDFText_GetText(FPDF_TEXTPAGE page,
  ...
  // terminator.
  CHECK_LT(char_count, std::numeric_limits<int>::max() - 1);
  ...
  while (str_span.size() % kBytesPerCharacter != 0) {
    str_span = str_span.first(str_span.size() - 1);
  }
...
FPDF_EXPORT int FPDF_CALLCONV FPDFText_GetBoundedText(FPDF_TEXTPAGE text_page,
  ...
  // describes the number of values buffer can hold excluding a
  // terminating NUL.
  CHECK_LT(buflen, std::numeric_limits<int>::max() - 1);
  const auto buffer_span =
      UNSAFE_BUFFERS(pdfium::make_span(buffer, buflen + 1));
