...
FPDFText_GetFillColor(FPDF_TEXTPAGE text_page,
  ...
  CPDF_TextPage* textpage = GetTextPageForValidIndex(text_page, index);
  if (!textpage || color == nullptr)
    return false;

  FPDF_CHAR_INFO charinfo;
  textpage->GetCharInfo(index, &charinfo);
  *color = static_cast<unsigned int>(charinfo.m_pTextObj->GetFillColorRef());
  if (is_rendered != nullptr) {
...
FPDFText_GetStrokeColor(FPDF_TEXTPAGE text_page,
  ...
  FPDF_CHAR_INFO charinfo;
  textpage->GetCharInfo(index, &charinfo);
  *color = static_cast<unsigned int>(charinfo.m_pTextObj->GetStrokeColorRef());
  if (is_rendered != nullptr) {
    TextRenderingMode rendering_mode =
        charinfo.m_pTextObj->GetTextRenderingMode();
    switch (rendering_mode) {
