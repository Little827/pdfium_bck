...
FPDFAnnot_SetAP(FPDF_ANNOTATION annot,
  ...
  // If value is null, we're in remove mode, so intermediary dicts will not be
  // created.
  bool bRemoving = !value;
  ...
  if (bRemoving && appearanceMode == FPDF_ANNOT_APPEARANCEMODE_NORMAL) {
  ...
  if (bRemoving) {
    ...
    std::unique_ptr<CPDF_Stream> pNewApStream =
        pdfium::MakeUnique<CPDF_Stream>();
