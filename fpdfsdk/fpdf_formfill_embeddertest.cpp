...
constexpr int kCharCtrlA = 0x01;
constexpr int kCharTab = 0x09;
constexpr int kCharReturn = 0x0d;
constexpr int kCharSpace = 0x20;
...
TEST_F(FPDFFormFillEmbedderTest, CheckReadOnlyInCheckbox) {
...

  {
    ...
    EXPECT_TRUE(FORM_OnChar(form_handle(), page, kCharReturn, 0));
    EXPECT_TRUE(FPDFAnnot_IsChecked(form_handle(), annot));

    EXPECT_TRUE(FORM_OnChar(form_handle(), page, kCharSpace, 0));
    EXPECT_TRUE(FPDFAnnot_IsChecked(form_handle(), annot));
...
TEST_F(FPDFFormFillComboBoxFormEmbedderTest,
  ...
  // Tab to the next control.
  EXPECT_TRUE(FORM_OnChar(form_handle(), page(), kCharTab, 0));

  // Shift-tab to the previous control.
  EXPECT_TRUE(
      FORM_OnChar(form_handle(), page(), kCharTab, FWL_EVENTFLAG_ShiftKey));
...
#endif
  FORM_OnChar(form_handle(), page(), kCharCtrlA, kCorrectModifier);
  CheckSelection(L"AB");
...
#endif
  FORM_OnChar(form_handle(), page(), kCharCtrlA, kWrongModifier);
  CheckSelection(L"");
}
...
TEST_F(FPDFFormFillActionUriTest, ButtonActionInvokeTest) {
  ...
  ASSERT_FALSE(FORM_OnChar(form_handle(), page(), kCharReturn, 0));
}
