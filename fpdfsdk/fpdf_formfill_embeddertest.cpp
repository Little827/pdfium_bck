...
TEST_F(FPDFFormFillEmbedderTest, GetFocusedAnnotation) {
  ...
  ASSERT_FALSE(FORM_GetFocusedAnnot(form_handle(), &annot, &page_index));
  EXPECT_FALSE(annot);
  EXPECT_EQ(-1, page_index);

  ASSERT_FALSE(FORM_GetFocusedAnnot(nullptr, &annot, &page_index));
  ASSERT_FALSE(FORM_GetFocusedAnnot(form_handle(), nullptr, &page_index));
  ASSERT_FALSE(FORM_GetFocusedAnnot(form_handle(), &annot, nullptr));

  const CFX_PointF right_bottom_annot_point(410.0, 210.0);
  constexpr int expected_annot_index = 3;
  ...
  for (size_t i = 0; i < pages.size(); ++i) {
    ...
    ASSERT_TRUE(FORM_GetFocusedAnnot(form_handle(), &annot, &page_index));
    ASSERT_TRUE(annot);

    EXPECT_EQ(expected_annot_index, FPDFPage_GetAnnotIndex(pages[i], annot));
...
TEST_F(FPDFFormFillEmbedderTest, SetFocusedAnnotation) {
  ...
  ASSERT_FALSE(FORM_SetFocusedAnnot(nullptr, pages[0], annot));
  ASSERT_FALSE(FORM_SetFocusedAnnot(form_handle(), nullptr, annot));
  ASSERT_FALSE(FORM_SetFocusedAnnot(form_handle(), pages[0], nullptr));

  constexpr int expected_annot_index = 2;

  for (size_t i = 0; i < pages.size(); ++i) {
    // Setting focus on an annotation on page i.
    ScopedFPDFAnnotation focused_annot(
        FPDFPage_GetAnnot(pages[i], expected_annot_index));
