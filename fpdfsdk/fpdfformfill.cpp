...
FPDF_SetFormFieldHighlightColor(FPDF_FORMHANDLE hHandle,
  ...
  if (fieldType == FPDF_FORMFIELD_UNKNOWN) {
    interForm->SetAllHighlightColors(color);
  } else if (fieldType > static_cast<int>(FormFieldType::Unknown) &&
             static_cast<size_t>(fieldType) < kFormFieldTypeCount) {
    interForm->SetHighlightColor(color, static_cast<FormFieldType>(fieldType));
  }
}
