...
TEST_F(FPDFAnnotEmbeddertest, GetAndSetQuadPoints) {
...

  {
    ...
    ASSERT_EQ(1, FPDFAnnot_CountAttachmentPoints(annot));
...

  {
    ...
    ASSERT_EQ(2, FPDFAnnot_CountAttachmentPoints(annot));
...

  {
    ...
    EXPECT_FALSE(FPDFAnnot_GetAttachmentPoints(annot, -1, &quadpoints));
    EXPECT_FALSE(FPDFAnnot_SetAttachmentPoints(annot, -2, &quadpoints));
  }
...

  {
    ...
    EXPECT_EQ(0, FPDFAnnot_CountAttachmentPoints(squareAnnot));
...
TEST_F(FPDFAnnotEmbeddertest, GetAndSetBadQuadPoints) {
  // Open file with annotation with malformed quadpoints (no multiples of 8)
  ASSERT_TRUE(OpenDocument("annotation_highlight_bad_quadpoints.pdf"));
  FPDF_PAGE page = LoadPage(0);
  ASSERT_TRUE(page);
  EXPECT_EQ(1, FPDFPage_GetAnnotCount(page));
  ...
  {
    // quadpoints have 10 coords
    FPDF_ANNOTATION annot = FPDFPage_GetAnnot(page, 0);
    EXPECT_EQ(1, FPDFAnnot_CountAttachmentPoints(annot));
    ASSERT_TRUE(FPDFAnnot_SetAttachmentPoints(annot, 1, &new_quadpoints));
    EXPECT_EQ(2, FPDFAnnot_CountAttachmentPoints(annot));
    ASSERT_TRUE(FPDFAnnot_GetAttachmentPoints(annot, 1, &quadpoints));
    EXPECT_NEAR(new_quadpoints.x1, quadpoints.x1, 0.001f);
    EXPECT_NEAR(new_quadpoints.y4, quadpoints.y4, 0.001f);
    FPDFPage_CloseAnnot(annot);
