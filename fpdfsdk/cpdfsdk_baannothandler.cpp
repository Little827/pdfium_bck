...
CPDFSDK_BAAnnotHandler::CPDFSDK_BAAnnotHandler() {
  for (const auto& annot_type : GetFocusableAnnotSubtypes()) {
    if (static_cast<CPDF_Annot::Subtype>(annot_type) !=
        CPDF_Annot::Subtype::WIDGET) {
      focusable_annot_types_.push_back(
          static_cast<CPDF_Annot::Subtype>(annot_type));
    }
  }
}
