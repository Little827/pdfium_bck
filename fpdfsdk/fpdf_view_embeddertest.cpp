...
TEST_F(FPDFViewEmbedderTest, DeviceCoordinatesToPageCoordinates) {
  ...
  EXPECT_NEAR(9.5625, page_x, tolerance);
  EXPECT_NEAR(775.5, page_y, tolerance);

  // Rotate 90 degrees clockwise
  rotate = 1;
  EXPECT_TRUE(FPDF_DeviceToPage(page, start_x, start_y, size_x, size_y, rotate,
                                device_x, device_y, &page_x, &page_y));
  EXPECT_NEAR(12.75, page_x, tolerance);
  EXPECT_NEAR(12.375, page_y, tolerance);

  // Rotate 180 degrees
  rotate = 2;
  EXPECT_TRUE(FPDF_DeviceToPage(page, start_x, start_y, size_x, size_y, rotate,
                                device_x, device_y, &page_x, &page_y));
  EXPECT_NEAR(602.4374, page_x, tolerance);
  EXPECT_NEAR(16.5, page_y, tolerance);

  // Rotate 90 degrees counter-clockwise
  rotate = 3;
  EXPECT_TRUE(FPDF_DeviceToPage(page, start_x, start_y, size_x, size_y, rotate,
                                device_x, device_y, &page_x, &page_y));
  EXPECT_NEAR(599.25, page_x, tolerance);
  EXPECT_NEAR(779.625, page_y, tolerance);
  ...
  EXPECT_TRUE(FPDF_DeviceToPage(page, start_x, start_y, size_x, size_y, rotate,
                                device_x, device_y, &page_x, &page_y));
  EXPECT_NEAR(device_x, page_x, tolerance);
  EXPECT_NEAR(device_y, page_y, tolerance);
