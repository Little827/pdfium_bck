...
TEST_F(FPDFViewEmbedderTest, DeviceCoordinatesToPageCoordinatesInPDF) {
  ...
  // Allowed Roatation values are 0, 1, 2, 3 passing invalid rotation value
  EXPECT_FALSE(FPDF_DeviceToPage(page, start_x, start_y, size_x, size_y, 4,
                                 device_x, device_y, &page_x, &page_y));
  // Invalid left and top pixel position of display area in device coordinates
  EXPECT_FALSE(FPDF_DeviceToPage(page, -10, -10, size_x, size_y, rotate,
                                 device_x, device_y, &page_x, &page_y));
  // Invalid horizonal and vertical size (in pixels) for display the page
  EXPECT_FALSE(FPDF_DeviceToPage(page, start_x, start_y, -10, -10, rotate,
                                 device_x, device_y, &page_x, &page_y));
  // Invalid page
...
TEST_F(FPDFViewEmbedderTest, PageCoordinatesToDeviceCoordinateInPDF) {
  ...
  // Allowed Roatation values are 0, 1, 2, 3 passing invalid rotation value
  EXPECT_FALSE(FPDF_PageToDevice(page, start_x, start_y, size_x, size_y, 4,
                                 device_x, device_y, &device_x, &device_y));
  // Invalid left and top pixel position of display area in device coordinates
  EXPECT_FALSE(FPDF_PageToDevice(page, -10, -10, size_x, size_y, rotate,
                                 device_x, device_y, &device_x, &device_y));
  // Invalid horizonal and vertical size (in pixels) for display the page
  EXPECT_FALSE(FPDF_PageToDevice(page, start_x, start_y, -10, -10, rotate,
                                 device_x, device_y, &device_x, &device_y));
  // Invalid page
