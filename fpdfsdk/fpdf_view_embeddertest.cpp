...
TEST_F(FPDFViewEmbedderTest, DeviceCoordinatesToPageCoordinates) {
  ...
  // Device corordinates to be converted
  int device_x = 10;
  int device_y = 10;
  ...
  // FPDF_DeviceToPage() convertes the roatation into legal ratation by taking
  ...
  EXPECT_EQ(device_x, std::round(page_x));
  EXPECT_EQ(device_y, std::round(page_y));
...
TEST_F(FPDFViewEmbedderTest, PageCoordinatesToDeviceCoordinates) {
  ...
  // Page coordinates to be converted
  double page_x = 9.0;
  double page_y = 775.0;
  ...
  // FPDF_PageToDevice() convertes the roatation into legal ratation by taking
  // modulo by 4. A value of 4 is expected to be converted into 0 (normal
  // rotation)
  rotate = 4;
  EXPECT_TRUE(FPDF_PageToDevice(page, start_x, start_y, size_x, size_y, rotate,
