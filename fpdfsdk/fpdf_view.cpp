...
struct InitParameters {
  // Cannot use ByteString, as PartitionAlloc may not be initialized yet.
  std::vector<std::unique_ptr<char, FxFreeDeleter>> paths;
...
FPDF_InitParamAppendFontPath(FPDF_INIT_PARAMS params, const char* font_path) {
  ...
  init_parameters->paths.push_back(
      std::unique_ptr<char, FxFreeDeleter>(strdup(font_path)));
}
