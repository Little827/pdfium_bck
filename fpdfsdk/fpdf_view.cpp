...
FPDF_GetNamedDestByName(FPDF_DOCUMENT document, FPDF_BYTESTRING name) {
  ...
  CPDF_Array* dest_array = CPDF_NameTree::LookupNamedDest(pDoc, dest_name);
  if (!dest_array)
    return nullptr;
  return FPDFDestFromCPDFArray(dest_array);
}
...
FPDF_EXPORT FPDF_DEST FPDF_CALLCONV FPDF_GetNamedDest(FPDF_DOCUMENT document,
  ...
  size_t name_tree_count = 0;
  auto name_tree = CPDF_NameTree::Create(pDoc, "Dests");
  if (name_tree)
    name_tree_count = name_tree->GetCount();
