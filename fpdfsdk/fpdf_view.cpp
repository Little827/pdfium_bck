...
#ifdef PDF_ENABLE_XFA
  return static_cast<CPDFXFA_Extension*>(pDoc->GetExtension())
...
FPDF_EXPORT int FPDF_CALLCONV FPDF_GetPageSizeByIndex(FPDF_DOCUMENT document,
  ...
  if (page_index < 0 || page_index >= count)
    return false;
  auto* pExtension = static_cast<CPDFXFA_Extension*>(pDoc->GetExtension());
  RetainPtr<CPDFXFA_Page> pPage = pExtension->GetXFAPage(page_index);
  if (!pPage)
    return false;
