...
FPDF_EXPORT FPDF_PAGE FPDF_CALLCONV FPDF_LoadPage(FPDF_DOCUMENT document,
  ...
  auto* pContext = static_cast<CPDFXFA_Context*>(pDoc->GetExtension());
  if (pContext)
    return FPDFPageFromCPDFPageHandle(pContext->GetXFAPage(page_index).Leak());
  ...
  return FPDFPageFromCPDFPageHandle(pExtension.Leak());
#endif  // PDF_ENABLE_XFA
}

FPDF_EXPORT double FPDF_CALLCONV FPDF_GetPageWidth(FPDF_PAGE page) {
  CPDF_Page::Handle* pPage = CPDFPageHandleFromFPDFPage(page);
#ifdef PDF_ENABLE_XFA
...
FPDF_EXPORT void FPDF_CALLCONV FPDF_ClosePage(FPDF_PAGE page) {
  ...
  CPDFSDK_PageView* pPageView =
      static_cast<CPDFSDK_PageView*>(pPage->GetPDFPage()->GetView());
  if (pPageView) {
