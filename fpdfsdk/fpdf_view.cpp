...
FPDF_EXPORT void FPDF_CALLCONV FPDF_RenderPageBitmap(FPDF_BITMAP bitmap,
...
#ifdef _SKIA_SUPPORT_
  bool convert_format = pBitmap->GetFormat() == FXDIB_Format::kRgb;
#endif
  ...
  if (CFX_DefaultRenderDevice::SkiaIsDefaultRenderer()) {
    ...
    if (convert_format && !pBitmap->ConvertFormat(FXDIB_Format::kRgb))
      return;
  }
#endif
}
...
FPDF_RenderPageBitmapWithMatrix(FPDF_BITMAP bitmap,
  ...
  CPDFSDK_RenderPage(pContext, pPage, transform_matrix, clip_rect, flags,
                     /*color_scheme=*/nullptr);
#ifdef _SKIA_SUPPORT_
  if (CFX_DefaultRenderDevice::SkiaIsDefaultRenderer() && convert_format &&
      !pBitmap->ConvertFormat(FXDIB_Format::kRgb)) {
    return;
  }
#endif
}
