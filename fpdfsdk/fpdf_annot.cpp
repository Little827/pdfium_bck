...
bool SetQuadPointsAtIndex(CPDF_Array* array,
  ...
  // Fix existing quadpoint coords if not a multiple of 8
  size_t coordsToRemove = coordsCount % 8;
  for (size_t i = 0; i < coordsToRemove; ++i)
    array->RemoveAt(coordsCount - i - 1);
...
FPDFAnnot_SetAttachmentPoints(FPDF_ANNOTATION annot,
  ...
  if (!SetQuadPointsAtIndex(pQuadPoints, quad_index, quad_points))
    return false;
