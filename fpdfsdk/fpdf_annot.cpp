...
FPDFAnnot_SetAP(FPDF_ANNOTATION annot,
  ...
  if (value) {
    ...
    // Transparency values are specified in range [0.0f, 1.0f]. We are
    // multiplying them by 255 to avoid floating point precision issues during
    // comparison. We are strictly checking for value < 255 and not <= 255 so
    // that size of Pdf does not unnecessarily bloat up by creating a new
    // dictionary in case of solid color.
    if (pAnnotDict->KeyExist("CA") &&
        (static_cast<int>(std::ceil(pAnnotDict->GetNumberFor("CA") * 255)) <
         255)) {
