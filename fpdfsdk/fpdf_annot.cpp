...
RetainPtr<CPDF_Dictionary> SetExtGStateInResourceDict(
  ...
  float fOpacity =
      pAnnotDict->KeyExist("CA") ? pAnnotDict->GetNumberFor("CA") : 1.0f;
...
FPDFAnnot_SetAP(FPDF_ANNOTATION annot,
  ...
  if (value) {
    ...
    // Transparency values are specified in range 0 - 1.0f. We are multiplying
    // them by 255 to avoid floating point precision issues during comparison.
