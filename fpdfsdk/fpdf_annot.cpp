...
FPDFAnnot_SetAP(FPDF_ANNOTATION annot,
  ...
  // mode.
  if (value) {
    // Check for the presence of bounding rect on annotation.
    // This rect will be used to set appearance's bounding box later.
    ...
    if (!pAnnotContext || !pAnnotContext->GetPage())
      return false;
    ...
    ByteString encodedAPStream =
        PDF_EncodeText(WideStringFromFPDFWideString(value));
    pNormalStream->SetData(encodedAPStream.raw_span());
