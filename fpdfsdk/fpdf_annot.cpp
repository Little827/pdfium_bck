...
FPDFAnnot_GetContents(FPDF_FORMHANDLE hHandle,
  ...
  if (!pFormFillEnv)
    return 0;
  CFFL_InteractiveFormFiller* pIntFormFiller =
  ...
  if (!pPageView)
    return 0;
  CPDFSDK_Annot* cpdfsdk_annot = pPageView->GetAnnotByDict(pAnnotDict);

  return Utf16EncodeMaybeCopyAndReturnLength(
      pIntFormFiller->GetText(cpdfsdk_annot), buffer, buflen);
}
