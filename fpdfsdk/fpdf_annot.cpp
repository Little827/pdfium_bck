...
CPDFSDK_Widget* GetRadioOrCheckBoxWidget(FPDF_FORMHANDLE hHandle,
...
FPDF_EXPORT FPDF_BOOL FPDF_CALLCONV FPDFAnnot_IsChecked(FPDF_FORMHANDLE hHandle,
                                                        FPDF_ANNOTATION annot) {
  CPDFSDK_Widget* pWidget = GetRadioOrCheckBoxWidget(hHandle, annot);
...
FPDFAnnot_GetFormControlIndex(FPDF_FORMHANDLE hHandle, FPDF_ANNOTATION annot) {
  ...
  CPDF_FormField* pFormField = pPDFForm->GetFieldByDict(pAnnotDict);
  CPDF_FormControl* pFormControl = pPDFForm->GetControlByDict(pAnnotDict);
  return pFormField && pFormControl ? pFormField->GetControlIndex(pFormControl)
                                    : -1;
}

FPDF_EXPORT unsigned long FPDF_CALLCONV
FPDFAnnot_GetFormFieldLabel(FPDF_FORMHANDLE hHandle,
                            FPDF_ANNOTATION annot,
                            FPDF_WCHAR* buffer,
                            unsigned long buflen) {
  CPDFSDK_Widget* pWidget = GetRadioOrCheckBoxWidget(hHandle, annot);
  ...
  return Utf16EncodeMaybeCopyAndReturnLength(pWidget->GetControlLabel(), buffer,
                                             buflen);
}
