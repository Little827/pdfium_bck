...
TEST_F(FPDFTextEmbedderTest, GetMatrix) {
  ...
  constexpr FS_MATRIX kExpectedMatrix[] = {
  ...
  constexpr size_t kExpectedCount = FX_ArraySize(kExpectedMatrix);
...

  {
...

    {
      ...
      EXPECT_TRUE(check_unsigned_shorts(kExpectedText, buffer,
                                        FX_ArraySize(kExpectedText)));
    }
    ...
    for (size_t i = 0; i < kExpectedCount; ++i) {
      ...
      EXPECT_FLOAT_EQ(kExpectedMatrix[i].a, matrix.a) << i;
      EXPECT_FLOAT_EQ(kExpectedMatrix[i].b, matrix.b) << i;
      EXPECT_FLOAT_EQ(kExpectedMatrix[i].c, matrix.c) << i;
      EXPECT_FLOAT_EQ(kExpectedMatrix[i].d, matrix.d) << i;
      EXPECT_FLOAT_EQ(kExpectedMatrix[i].e, matrix.e) << i;
      EXPECT_FLOAT_EQ(kExpectedMatrix[i].f, matrix.f) << i;
    }
