...
TEST_F(FPDFTextEmbedderTest, TextVertical) {
  ...
  double top = 0.0;
  EXPECT_TRUE(FPDFText_GetCharBox(textpage, 4, &left, &right, &bottom, &top));
  EXPECT_NEAR(5.596, left, 0.001);
  EXPECT_NEAR(14.692, right, 0.001);
  EXPECT_NEAR(110.276, bottom, 0.001);
  EXPECT_NEAR(121.424, top, 0.001);
  ...
  EXPECT_NEAR(4.888, left, 0.001);
  EXPECT_NEAR(15.484, right, 0.001);
  EXPECT_NEAR(98.372, bottom, 0.001);
  EXPECT_NEAR(109.244, top, 0.001);
