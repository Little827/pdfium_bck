...
TEST_F(FPDFTextEmbedderTest, TextVertical) {
  ...
  EXPECT_NEAR(111.440, y, 0.001);

  EXPECT_TRUE(FPDFText_GetCharOrigin(textpage, 5, &x, &y));
  EXPECT_NEAR(4, x, 0.001);
  EXPECT_NEAR(99.440, y, 0.001);
  ...
  double top = 0.0;
  EXPECT_TRUE(FPDFText_GetCharBox(textpage, 4, &left, &right, &bottom, &top));
  EXPECT_NEAR(5.644, left, 0.001);
  EXPECT_NEAR(14.776, right, 0.001);
  EXPECT_NEAR(110.420, bottom, 0.001);
  EXPECT_NEAR(121.292, top, 0.001);

  EXPECT_TRUE(
      FPDFText_GetLooseCharBox(textpage, 4, &left, &right, &bottom, &top));
  EXPECT_NEAR(4, left, 0.001);
  EXPECT_NEAR(16, right, 0.001);
  EXPECT_NEAR(109.758, bottom, 0.001);
  EXPECT_NEAR(121.758, top, 0.001);
  EXPECT_NEAR(12.000, top - bottom, 0.001);

  EXPECT_TRUE(FPDFText_GetCharBox(textpage, 5, &left, &right, &bottom, &top));
  EXPECT_NEAR(4.564, left, 0.001);
  EXPECT_NEAR(15.484, right, 0.001);
  EXPECT_NEAR(98.420, bottom, 0.001);
  EXPECT_NEAR(109.292, top, 0.001);

  EXPECT_TRUE(
      FPDFText_GetLooseCharBox(textpage, 5, &left, &right, &bottom, &top));
  EXPECT_NEAR(4, left, 0.001);
  EXPECT_NEAR(16, right, 0.001);
  EXPECT_NEAR(97.758, bottom, 0.001);
  EXPECT_NEAR(109.758, top, 0.001);
  EXPECT_NEAR(12.000, top - bottom, 0.001);
