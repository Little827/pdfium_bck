...
TEST_F(FPDFTextEmbedderTest, Text) {
  ...
  EXPECT_NEAR(41.012, left, 0.001);
  EXPECT_NEAR(46.304, right, 0.001);
  ...
  EXPECT_FLOAT_EQ(47.348713f, rect.bottom);
  EXPECT_FLOAT_EQ(59.348713f, rect.top);
  ...
  EXPECT_NEAR(20.768, left, 0.001);
  EXPECT_NEAR(135.040, right, 0.001);
  EXPECT_NEAR(96.592, bottom, 0.001);
  EXPECT_NEAR(111.792, top, 0.001);
  ...
  EXPECT_EQ(
      8, FPDFText_GetBoundedText(textpage, 41.0, 56.0, 82.0, 48.0, nullptr, 0));
  ...
  memset(buffer, 0xbd, sizeof(buffer));
  EXPECT_EQ(9, FPDFText_GetBoundedText(textpage, 41.0, 56.0, 82.0, 48.0, buffer,
                                       128));
  EXPECT_TRUE(check_unsigned_shorts(kHelloGoodbyeText + 4, buffer, 8));
...
TEST_F(FPDFTextEmbedderTest, WebLinks) {
  ...
  EXPECT_NEAR(50.780, left, 0.001);
  EXPECT_NEAR(187.988, right, 0.001);
  EXPECT_NEAR(97.528, bottom, 0.001);
  EXPECT_NEAR(108.844, top, 0.001);
...
TEST_F(FPDFTextEmbedderTest, CharBox) {
  ...
  constexpr double kExpectedCharWidth = 8.292;
  constexpr double kExpectedCharHeight = 6.740;
