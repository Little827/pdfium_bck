...
TEST_F(FPDFTextEmbedderTest, TextVertical) {
  ...
  double top = 0.0;
  EXPECT_TRUE(FPDFText_GetCharBox(textpage, 4, &left, &right, &bottom, &top));
  EXPECT_NEAR(5.308, left, 0.001);
  EXPECT_NEAR(15.388, right, 0.001);
  EXPECT_NEAR(110, bottom, 0.001);
  EXPECT_NEAR(120.956, top, 0.001);
  ...
  EXPECT_NEAR(4.516, left, 0.001);
  EXPECT_NEAR(15.532, right, 0.001);
  EXPECT_NEAR(98.468, bottom, 0.001);
  EXPECT_NEAR(108.668, top, 0.001);
