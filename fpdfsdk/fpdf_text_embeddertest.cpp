...
TEST_F(FPDFTextEmbedderTest, IsInvalidUnicode) {
...

  {
    ...
    EXPECT_EQ(static_cast<unsigned int>('X'),
              FPDFText_GetUnicode(textpage.get(), 0));
    EXPECT_EQ(0, FPDFText_IsInvalidUnicode(textpage.get(), 0));
    EXPECT_EQ(static_cast<unsigned int>(' '),
              FPDFText_GetUnicode(textpage.get(), 1));
    EXPECT_EQ(0, FPDFText_IsInvalidUnicode(textpage.get(), 1));

    EXPECT_EQ(31u, FPDFText_GetUnicode(textpage.get(), 2));
    EXPECT_EQ(1, FPDFText_IsInvalidUnicode(textpage.get(), 2));

    EXPECT_EQ(-1, FPDFText_IsInvalidUnicode(textpage.get(), -1));
    EXPECT_EQ(-1,
              FPDFText_IsInvalidUnicode(textpage.get(), kExpectedCharCount));
    EXPECT_EQ(-1, FPDFText_IsInvalidUnicode(nullptr, 0));
  }
