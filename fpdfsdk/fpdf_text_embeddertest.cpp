...
TEST_F(FPDFTextEmbedderTest, TextVertical) {
  ...
  double bottom = 0.0;
  double top = 0.0;
  EXPECT_TRUE(FPDFText_GetCharBox(textpage, 1, &left, &right, &bottom, &top));
  EXPECT_NEAR(7.420, left, 0.001);
  EXPECT_NEAR(11.956, right, 0.001);
  EXPECT_NEAR(171.604, bottom, 0.001);
  EXPECT_NEAR(177.124, top, 0.001);
  ...
  EXPECT_TRUE(FPDFText_GetCharBox(textpage, 2, &left, &right, &bottom, &top));
  EXPECT_NEAR(9.844, left, 0.001);
  EXPECT_NEAR(13.492, right, 0.001);
  EXPECT_NEAR(160.684, bottom, 0.001);
  EXPECT_NEAR(169.204, top, 0.001);
