...
bool CPDFSDK_InterForm::IsNeedHighLight(FormFieldType fieldType) {
  if (fieldType == FormFieldType::Unknown)
    return false;
  if (m_NeedsHighlight.end() == m_NeedsHighlight.find(fieldType)) {
#ifdef PDF_ENABLE_XFA
    if (IsFormFieldTypeXFA(fieldType)) {
      if (m_NeedsHighlight.end() != m_NeedsHighlight.find(FormFieldType::XFA))
        return m_NeedsHighlight[FormFieldType::XFA];
    }
#endif  // PDF_ENABLE_XFA
    return false;
  }
  return m_NeedsHighlight[fieldType];
}
...
FX_COLORREF CPDFSDK_InterForm::GetHighlightColor(FormFieldType fieldType) {
  if (fieldType == FormFieldType::Unknown)
    return m_HighlightColor.begin()->second;
  if (m_HighlightColor.end() == m_HighlightColor.find(fieldType)) {
#ifdef PDF_ENABLE_XFA
    if (IsFormFieldTypeXFA(fieldType)) {
      if (m_HighlightColor.end() != m_HighlightColor.find(FormFieldType::XFA))
        return m_HighlightColor[FormFieldType::XFA];
    }
#endif  // PDF_ENABLE_XFA
    return FXSYS_RGB(255, 255, 255);
  }
  return m_HighlightColor[fieldType];
}
