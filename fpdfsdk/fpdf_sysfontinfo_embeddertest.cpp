...
TEST_F(FPDFSysFontInfoEmbedderTest, DefaultTTFMap) {
  static const int kAllowedCharsets[] = {
      FXFONT_ANSI_CHARSET,        FXFONT_DEFAULT_CHARSET, FXFONT_SYMBOL_CHARSET,
      FXFONT_SHIFTJIS_CHARSET,    FXFONT_HANGEUL_CHARSET, FXFONT_GB2312_CHARSET,
  ...
  while (cfmap->charset != -1 && cfmap->fontname) {
    ...
    EXPECT_FALSE(pdfium::ContainsValue(seen_charsets, cfmap->charset))
        << " for " << cfmap->charset;
    seen_charsets.insert(cfmap->charset);
