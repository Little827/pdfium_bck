...
TEST_F(FPDFEditEmbedderTest, EmbedNotoFont) {
  EXPECT_TRUE(CreateEmptyDocument());
  ScopedFPDFPage page(FPDFPage_New(document(), 0, 400, 400));
  std::string font_data;
  std::string font_path;
  ASSERT_TRUE(PathService::GetTestFilePath(
      "fonts/third_party/NotoSans/NotoSansSC-Regular.subset.otf", &font_path));
  ReadFileToString(font_path, &font_data);
  ASSERT_TRUE(font_data.size());

  ScopedFPDFFont font(FPDFText_LoadFont(
      document(), reinterpret_cast<const uint8_t*>(font_data.data()),
      font_data.size(), FPDF_FONT_TRUETYPE, /*cid=*/true));
...
// TODO: add windows and macOS results.
#if defined(_SKIA_SUPPORT_) || defined(_SKIA_SUPPORT_PATHS_)
#if defined(OS_WIN)
  const char kChecksum[] = "xxx1";
#elif defined(OS_APPLE)
  const char kChecksum[] = "xxx2";
#else
  const char kChecksum[] = "5bb65e15fc0a685934cd5006dec08a76";
#endif  // defined(OS_WIN)
#else
#if defined(OS_WIN)
  const char kChecksum[] = "xxx3";
#elif defined(OS_APPLE)
  const char kChecksum[] = "xxx4";
#else
...
#endif  // defined(_SKIA_SUPPORT_) || defined(_SKIA_SUPPORT_PATHS_)
  {
    ...
    WriteBitmapToPng(
        page_bitmap.get(),
        "/usr/local/google/home/huiyingst/Downloads/bitmap/noto_test.png");
