...
TEST_F(FPDFEditEmbeddertest, AddMarkedText) {
  ...
  // - blob "BlobKey": "\x01\x02\x03\0BlobValue1\0\0\0BlobValue2"
  ...
  EXPECT_EQ(FPDF_OBJECT_STRING,
            FPDFPageObjMark_GetParamValueTypeByKey(mark, "BlobKey"));
  out_buffer_len = 0;
  EXPECT_TRUE(FPDFPageObjMark_GetParamBlobValueByKey(mark, "BlobKey", buffer,
                                                     256, &out_buffer_len));
