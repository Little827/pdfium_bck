...
TEST_F(FPDFEditEmbeddertest, AddMarkedText) {
  ...
  EXPECT_EQ(0, FPDFPageObjMark_CountParams(mark));
  std::unique_ptr<unsigned short, pdfium::FreeDeleter> int_param_key =
      GetFPDFWideString(L"TestMarkParamKey");
  int param_index = FPDFPageObjMark_AddIntParam(mark, int_param_key.get(), 42);
