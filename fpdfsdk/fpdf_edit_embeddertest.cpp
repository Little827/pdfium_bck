...
struct FPDFEditMoveEmbedderTestCase {
  ...
  std::vector<int> resulting_order;  // relevant if `expected_result` is true
  std::string name;
};
...
TEST_F(FPDFEditMoveEmbedderTest, MAYBE_MovePagesTest) {
  ...
  for (const FPDFEditMoveEmbedderTestCase& tc : kTestCases) {
    ...
    if (tc.expected_result) {
      ...
      expected_hashes.reserve(5);
      for (size_t i = 0; i < expected_hashes.size(); ++i) {
        expected_hashes[i] = orig_hashes[tc.resulting_order[i]];
      }
      EXPECT_THAT(new_hashes, testing::ContainerEq(expected_hashes));
    } else {
      // Check that pages are unchanged.
      EXPECT_THAT(HashesForDocument(), testing::ContainerEq(orig_hashes));
    }
