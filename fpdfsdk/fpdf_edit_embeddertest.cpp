...
TEST_F(FPDFEditEmbeddertest, AddPaths) {
  ...
  {
    UniqueFPDFBitmap page_bitmap = RenderPageWithFlags(page, nullptr, 0);
...
TEST_F(FPDFEditEmbeddertest, EditOverExistingContent) {
  ...
  {
    UniqueFPDFBitmap new_bitmap = RenderSavedPage(saved_page);
...
TEST_F(FPDFEditEmbeddertest, GraphicsData) {
  // New page
  UniqueFPDFPage page(FPDFPage_New(CreateNewDocument(), 0, 612, 792));
...
TEST_F(FPDFEditEmbeddertest, AddTrueTypeFontText) {
  ...
  UniqueFPDFBitmap page_bitmap2 = RenderPageWithFlags(page, nullptr, 0);
#if _FX_PLATFORM_ == _FX_PLATFORM_APPLE_
...
TEST_F(FPDFEditEmbeddertest, SaveAndRender) {
  const char md5[] = "3c20472b0552c0c22b88ab1ed8c6202b";
  {
    ...
    UniqueFPDFBitmap page_bitmap = RenderLoadedPage(page);
    CompareBitmap(page_bitmap.get(), 612, 792, md5);
...
TEST_F(FPDFEditEmbeddertest, ExtractJBigImageBitmap) {
  ...
  ASSERT_EQ(FPDF_PAGEOBJ_IMAGE, FPDFPageObj_GetType(obj));
  {
    UniqueFPDFBitmap bitmap(FPDFImageObj_GetBitmap(obj));
    ASSERT_TRUE(bitmap);
