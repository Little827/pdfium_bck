...
// Platform specific original bitmaps
...

  {
    ...
    // Note that the new text render mode is set.
    // TODO(crbug.com/pdfium/1470): Fix FPDFTextObj_SetTextRenderMode() to work
    // on saved documents. The expected render mode should be
    // FPDF_TEXTRENDERMODE_STROKE and the expected bitmap hash should be
    // |md5_stroke|.
    EXPECT_TRUE(FPDF_SaveAsCopy(document(), this, 0));
    ASSERT_TRUE(OpenSavedDocument());
    FPDF_PAGE saved_page = LoadSavedPage(0);
