...
TEST_F(FPDFEditEmbeddertest, MaintainMarkedObjects) {
  ...
  EXPECT_EQ(8u, prime_count);
  EXPECT_EQ(3u, square_count);
  EXPECT_EQ(9u, greater_than_ten_count);
  EXPECT_EQ(1u, bounds_count);

  EXPECT_TRUE(FPDFPage_GenerateContent(page));
  OpenPDFFileForWrite("../../RemoveMarkedObjectsPrime.pdf");
  EXPECT_TRUE(FPDF_SaveAsCopy(document(), this, 0));
  ...
  EXPECT_EQ(18, FPDFPage_CountObjects(saved_page));
...
TEST_F(FPDFEditEmbeddertest, AddMarkedText) {
  ...
  // Add an int and a string parameter.
  EXPECT_EQ(0, FPDFPageObjMark_CountParams(mark));
