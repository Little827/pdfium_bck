...
FPDFPageObj_HasTransparency(FPDF_PAGEOBJECT page_object) {
  ...
  if (pPageObj->m_GraphicStates.m_GeneralState.GetBlendType() !=
      BlendMode::kNormal) {
    return true;
  }

  if (pPageObj->m_GraphicStates.m_GeneralState.GetSoftMask()) {
    return true;
  }

  if (pPageObj->m_GraphicStates.m_GeneralState.GetFillAlpha() != 1.0f) {
    return true;
  }

  if (pPageObj->IsPath() &&
      pPageObj->m_GraphicStates.m_GeneralState.GetStrokeAlpha() != 1.0f) {
    return true;
  }
...
FPDFPageObj_SetBlendMode(FPDF_PAGEOBJECT page_object,
  ...
  pPageObj->m_GraphicStates.m_GeneralState.SetBlendMode(blend_mode);
  pPageObj->SetDirty(true);
}
...
FPDF_BOOL FPDFPageObj_SetFillColor(FPDF_PAGEOBJECT page_object,
  ...
  pPageObj->m_GraphicStates.m_GeneralState.SetFillAlpha(A / 255.f);
  pPageObj->m_GraphicStates.m_ColorState.SetFillColor(
...
FPDFPageObj_GetFillColor(FPDF_PAGEOBJECT page_object,
  ...
  if (!pPageObj->m_GraphicStates.m_ColorState.HasRef()) {
    return false;
  }

  FX_COLORREF fill_color =
      pPageObj->m_GraphicStates.m_ColorState.GetFillColorRef();
...
FPDFPageObj_SetStrokeColor(FPDF_PAGEOBJECT page_object,
  ...
  std::vector<float> rgb = {R / 255.f, G / 255.f, B / 255.f};
  pPageObj->m_GraphicStates.m_GeneralState.SetStrokeAlpha(A / 255.f);
  pPageObj->m_GraphicStates.m_ColorState.SetStrokeColor(
      CPDF_ColorSpace::GetStockCS(CPDF_ColorSpace::Family::kDeviceRGB),
      std::move(rgb));
  pPageObj->SetDirty(true);
  return true;
...
FPDFPageObj_GetStrokeColor(FPDF_PAGEOBJECT page_object,
  ...
  FX_COLORREF stroke_color =
      pPageObj->m_GraphicStates.m_ColorState.GetStrokeColorRef();
  *R = FXSYS_GetRValue(stroke_color);
  *G = FXSYS_GetGValue(stroke_color);
  *B = FXSYS_GetBValue(stroke_color);
  *A = FXSYS_GetUnsignedAlpha(
      pPageObj->m_GraphicStates.m_GeneralState.GetStrokeAlpha());
  return true;
}
...
FPDFPageObj_SetStrokeWidth(FPDF_PAGEOBJECT page_object, float width) {
  ...
  pPageObj->m_GraphicStates.m_GraphState.SetLineWidth(width);
  pPageObj->SetDirty(true);
  return true;
}
...
FPDFPageObj_GetStrokeWidth(FPDF_PAGEOBJECT page_object, float* width) {
  ...
  *width = pPageObj->m_GraphicStates.m_GraphState.GetLineWidth();
  return true;
}
...
FPDFPageObj_GetLineJoin(FPDF_PAGEOBJECT page_object) {
  auto* pPageObj = CPDFPageObjectFromFPDFPageObject(page_object);
  return pPageObj ? static_cast<int>(
                        pPageObj->m_GraphicStates.m_GraphState.GetLineJoin())
                  : -1;
}
...
FPDFPageObj_SetLineJoin(FPDF_PAGEOBJECT page_object, int line_join) {
  ...
  pPageObj->m_GraphicStates.m_GraphState.SetLineJoin(
...
FPDFPageObj_GetLineCap(FPDF_PAGEOBJECT page_object) {
  auto* pPageObj = CPDFPageObjectFromFPDFPageObject(page_object);
  return pPageObj ? static_cast<int>(
                        pPageObj->m_GraphicStates.m_GraphState.GetLineCap())
                  : -1;
}
...
FPDFPageObj_SetLineCap(FPDF_PAGEOBJECT page_object, int line_cap) {
  ...
  pPageObj->m_GraphicStates.m_GraphState.SetLineCap(
...
FPDFPageObj_GetDashPhase(FPDF_PAGEOBJECT page_object, float* phase) {
  ...
  *phase = pPageObj->m_GraphicStates.m_GraphState.GetLineDashPhase();
  return true;
}
...
FPDFPageObj_SetDashPhase(FPDF_PAGEOBJECT page_object, float phase) {
  ...
  pPageObj->m_GraphicStates.m_GraphState.SetLineDashPhase(phase);
  pPageObj->SetDirty(true);
  return true;
}
...
FPDFPageObj_GetDashCount(FPDF_PAGEOBJECT page_object) {
  auto* pPageObj = CPDFPageObjectFromFPDFPageObject(page_object);
  return pPageObj
             ? pdfium::base::checked_cast<int>(
                   pPageObj->m_GraphicStates.m_GraphState.GetLineDashSize())
             : -1;
}
...
FPDFPageObj_GetDashArray(FPDF_PAGEOBJECT page_object,
  ...
  auto dash_vector = pPageObj->m_GraphicStates.m_GraphState.GetLineDashArray();
...
FPDFPageObj_SetDashArray(FPDF_PAGEOBJECT page_object,
  ...
  pPageObj->m_GraphicStates.m_GraphState.SetLineDash(dashes, phase, 1.0f);
