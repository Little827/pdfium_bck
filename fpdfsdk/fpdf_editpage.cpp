...
FPDFPageObjMark_GetParamBlobValue(FPDF_PAGEOBJECTMARK mark,
  ...
  pdfium::span<const char> value_span = value.AsStringView().span();
  fxcrt::try_spancpy(result_span, value_span);
  *out_buflen = pdfium::checked_cast<unsigned long>(value_span.size());
  return true;
}
