...
GetMarkParamPair(FPDF_PAGEOBJECTMARK mark, unsigned long index) {
  if (!mark)
    return nullptr;
  ...
  for (unsigned long i = 0; i < index && it != pParams->end(); ++i) {
    ++it;
  }
...
FPDFPageObjMark_GetParamKey(FPDF_PAGEOBJECTMARK mark,
                            ...
                            void* buffer,
                            unsigned long buflen) {
  auto param_pair = GetMarkParamPair(mark, index);
  if (!param_pair)
    return 0;
