...
TEST_F(FPDFPPOEmbeddertest, ImportNPages) {
  ...
  // At this point the document has two pages already.  The following test
  // adds one more page to the document.
  EXPECT_TRUE(
      FPDF_ImportNPagesToOne(output_doc, document(), "2-3", 0, &nupLayout));
  EXPECT_EQ(3, FPDF_GetPageCount(output_doc));
  EXPECT_TRUE(
      FPDF_ImportNPagesToOne(output_doc, document(), "2-4,3-5", 0, &nupLayout));
  // At this point the document has three pages already.  The following test
  // adds three more pages to the document.
