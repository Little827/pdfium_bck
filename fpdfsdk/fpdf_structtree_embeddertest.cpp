...
TEST_F(FPDFStructTreeEmbedderTest, GetActualText) {
...

  {
    ...
    gchild_element = FPDF_StructElement_GetChildAtIndex(child_element, 0);
    ASSERT_TRUE(gchild_element);
    ASSERT_EQ(24U, FPDF_StructElement_GetActualText(gchild_element, nullptr, 0));
...
TEST_F(FPDFStructTreeEmbedderTest, GetObjType) {
...

  {
    ...
    // test nullptr inputs
    unsigned short buffer[12];
    ASSERT_EQ(0U, FPDF_StructElement_GetObjType(nullptr, buffer, sizeof(buffer)));
    ...
    ASSERT_EQ(22U, FPDF_StructElement_GetObjType(element, buffer, sizeof(buffer)));
...
TEST_F(FPDFStructTreeEmbedderTest, GetParent) {
...

  {
    ...
    FPDF_STRUCTELEMENT child =
        FPDF_StructElement_GetChildAtIndex(parent, 0);
    ASSERT_TRUE(child);
...
TEST_F(FPDFStructTreeEmbedderTest, GetAttributes) {
...

  {
    ...
    FPDF_STRUCTELEMENT table = FPDF_StructElement_GetChildAtIndex(document, 0);
    ASSERT_TRUE(table);
    ASSERT_EQ(1, FPDF_StructElement_GetAttributeCount(table));
    ...
    ASSERT_EQ(nullptr, FPDF_StructElement_GetAttributeAtIndex(table, 1));

    FPDF_STRUCTELEMENT_ATTR attr = FPDF_StructElement_GetAttributeAtIndex(table, 0);
    ASSERT_TRUE(attr);

    ASSERT_EQ(3, FPDF_StructElement_Attr_GetCount(attr));
    ASSERT_FALSE(FPDF_StructElement_Attr_GetName(attr, 0, nullptr, 0U, nullptr));
    unsigned short buffer[18];
    unsigned long out_len = -1;
    memset(buffer, 0, sizeof(buffer));
    // Deliberately pass in a small buffer size to make sure |buffer| remains
    // untouched.
    ASSERT_TRUE(FPDF_StructElement_Attr_GetName(attr, 0, buffer, 1, &out_len));
    for (size_t i = 0; i < pdfium::size(buffer); ++i)
      EXPECT_EQ(0U, buffer[i]);
    EXPECT_EQ(4U, out_len);

    ASSERT_TRUE(FPDF_StructElement_Attr_GetName(attr, 0, buffer, sizeof(buffer), &out_len));
    EXPECT_EQ(4U, out_len);
    EXPECT_EQ(L"O", GetPlatformWString(buffer));
    EXPECT_EQ(FPDF_OBJECT_NAME, FPDF_StructElement_Attr_GetType(attr, GetPlatformString(buffer).c_str()));

    unsigned short str_val[12];
    memset(str_val, 0, sizeof(str_val));
    ASSERT_TRUE(FPDF_StructElement_Attr_GetStringValue(attr, GetPlatformString(buffer).c_str(), str_val, sizeof(str_val), &out_len));
    EXPECT_EQ(12U, out_len);
    EXPECT_EQ(L"Table", GetPlatformWString(str_val));

    memset(buffer, 0, sizeof(buffer));
    ASSERT_TRUE(FPDF_StructElement_Attr_GetName(attr, 1, buffer, sizeof(buffer), &out_len));
    EXPECT_EQ(18U, out_len);
    EXPECT_EQ(L"RowCount", GetPlatformWString(buffer));
    EXPECT_EQ(FPDF_OBJECT_NUMBER, FPDF_StructElement_Attr_GetType(attr, GetPlatformString(buffer).c_str()));
    int int_val;
    ASSERT_TRUE(FPDF_StructElement_Attr_GetIntValue(attr, GetPlatformString(buffer).c_str(), &int_val));
    EXPECT_EQ(2, int_val);
    float num_val;
    ASSERT_TRUE(FPDF_StructElement_Attr_GetNumberValue(attr, GetPlatformString(buffer).c_str(), &num_val));
    EXPECT_EQ(2.0, num_val);
  }
