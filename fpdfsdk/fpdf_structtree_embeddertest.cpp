...
TEST_F(FPDFStructTreeEmbedderTest, GetAttributes) {
...

  {
...

    {
      ...
      EXPECT_EQ("ColSpan", std::string(buffer));
      EXPECT_EQ(FPDF_OBJECT_NUMBER,
                FPDF_StructElement_Attr_GetType(attr, buffer));
      int int_val;
      ASSERT_TRUE(FPDF_StructElement_Attr_GetIntValue(attr, buffer, &int_val));
      EXPECT_EQ(2, int_val);
      float num_val;
      ASSERT_TRUE(
...

    {
      ...
      FPDF_STRUCTELEMENT_ATTR attr =
          FPDF_StructElement_GetAttributeAtIndex(tr, 0);
      char buffer[16] = {};
      unsigned long out_len = -1;
      ASSERT_TRUE(attr);
      ASSERT_TRUE(FPDF_StructElement_Attr_GetName(attr, 0, buffer,
