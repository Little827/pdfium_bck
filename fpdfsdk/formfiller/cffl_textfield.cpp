...
void CFFL_TextField::RestoreState(CPDFSDK_PageView* pPageView) {
  CPWL_Edit* pWnd = GetOrCreateEdit(pPageView);
  if (!pWnd)
    return;
...
CPWL_Edit* CFFL_TextField::GetEdit(CPDFSDK_PageView* pPageView) {
  return static_cast<CPWL_Edit*>(GetPWLWindow(pPageView));
}

CPWL_Edit* CFFL_TextField::GetOrCreateEdit(CPDFSDK_PageView* pPageView) {
  return static_cast<CPWL_Edit*>(GetOrCreatePWLWindow(pPageView));
}
