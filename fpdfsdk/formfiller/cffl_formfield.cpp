...
bool CFFL_FormField::OnLButtonDown(CPDFSDK_PageView* pPageView,
  ...
  CPWL_Wnd* pWnd = GetOrCreatePWLWindow(pPageView);
  if (!pWnd)
    return false;
...
void CFFL_FormField::SetFocusForAnnot(CPDFSDK_Annot* pAnnot, uint32_t nFlag) {
  ...
  if (CPWL_Wnd* pWnd = GetOrCreatePWLWindow(pPageView))
    pWnd->SetFocus();
...
CPWL_Wnd* CFFL_FormField::GetPWLWindow(CPDFSDK_PageView* pPageView) {
  DCHECK(pPageView);
  auto it = m_Maps.find(pPageView);
  return it != m_Maps.end() ? it->second.get() : nullptr;
}

CPWL_Wnd* CFFL_FormField::GetOrCreatePWLWindow(CPDFSDK_PageView* pPageView) {
  CPWL_Wnd* pWnd = GetPWLWindow(pPageView);
  if (!pWnd) {
  ...
  if (pPrivateData->ValueAgeEquals(m_pWidget->GetValueAge()))
    return RestorePWLWindow(pPageView);
...
CPWL_Wnd* CFFL_FormField::RestorePWLWindow(CPDFSDK_PageView* pPageView) {
  return GetOrCreatePWLWindow(pPageView);
}
