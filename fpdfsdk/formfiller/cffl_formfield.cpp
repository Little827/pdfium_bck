...
void CFFL_FormField::SetFocusForAnnot(CPDFSDK_Annot* pAnnot,
  ...
  CPDFSDK_PageView* pPageView =
      m_pFormFiller->GetDelegate()->GetOrCreatePageView(pWidget->GetPage());
  CPWL_Wnd* pWnd = CreateOrUpdatePWLWindow(pPageView);
  if (pWnd)
    pWnd->SetFocus();
...
void CFFL_FormField::KillFocusForAnnot(Mask<FWL_EVENTFLAG> nFlag) {
  ...
  CPDFSDK_PageView* pPageView =
      m_pFormFiller->GetDelegate()->GetPageView(m_pWidget->GetPage());
...
CPWL_Wnd::CreateParams CFFL_FormField::GetCreateParam() {
  ...
  cp.pTimerHandler.Reset(m_pFormFiller->GetDelegate()->GetTimerHandler());
  cp.pSystemHandler = m_pFormFiller->GetDelegate()->GetSysHandler();
  return cp;
}
...
CPDFSDK_PageView* CFFL_FormField::GetCurPageView() {
  return m_pFormFiller->GetDelegate()->GetOrCreatePageView(
      m_pWidget->GetPage());
}
...
bool CFFL_FormField::CommitData(const CPDFSDK_PageView* pPageView,
  ...
  m_pFormFiller->OnCalculate(&pObserved, pPageView, nFlag);
  if (!pObserved)
    return false;

  m_pFormFiller->OnFormat(&pObserved, pPageView, nFlag);
  if (!pObserved)
    return false;
...
void CFFL_FormField::SetChangeMark() {
  m_pFormFiller->GetDelegate()->OnChange();
}
...
void CFFL_FormField::InvalidateRect(const FX_RECT& rect) {
  m_pFormFiller->GetDelegate()->Invalidate(m_pWidget->GetPage(), rect);
}
