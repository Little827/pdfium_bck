...
CPWL_Wnd* CFFL_FormField::CreateOrUpdatePWLWindow(CPDFSDK_PageView* pPageView) {
  ...
  return ResetOrRestorePWLWindow(
      pPageView, pPrivateData->ValueAgeEquals(m_pWidget->GetValueAge()));
}
...
bool CFFL_FormField::CommitData(CPDFSDK_PageView* pPageView, uint32_t nFlag) {
  ...
  if (!pInteractiveFormFiller->OnKeyStrokeCommit(&pObserved, pPageView,
    ...
    if (!pObserved)
      return false;
    ResetOrRestorePWLWindow(pPageView, false);
    return true;
  }
  if (!pObserved)
    return false;
...
CPWL_Wnd* CFFL_FormField::ResetOrRestorePWLWindow(CPDFSDK_PageView* pPageView,
                                                  bool bRestoreValue) {
  return bRestoreValue ? RestorePWLWindow(pPageiew) : ResetPWLWindow(pPageView);
}
...
// virtual
CPWL_Wnd* RestorePWLWindow(CPDFSDK_PageView* pPageView) {
  return GetPWLWindow(pPageView);
}
