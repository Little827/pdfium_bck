...
void CFFL_ComboBox::RestoreState(CPDFSDK_PageView* pPageView) {
  CPWL_ComboBox* pComboBox = GetOrCreateComboBox(pPageView);
  if (!pComboBox)
    return;
...
CPWL_ComboBox* CFFL_ComboBox::GetOrCreateComboBox(CPDFSDK_PageView* pPageView) {
  return static_cast<CPWL_ComboBox*>(GetOrCreatePWLWindow(pPageView));
}
