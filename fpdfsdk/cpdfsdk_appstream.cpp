...
ByteString GetPushButtonAppStream(const CFX_FloatRect& rcBBox,
  ...
  if (IsFloatZero(fFontSize)) {
    ...
    pEdit->SetFontSize_(fFontSize);
    pEdit->Paint();
  }
  pEdit->Initialize();
  pEdit->SetText_(sLabel);
  pEdit->Paint();
...
void CPDFSDK_AppStream::SetAsComboBox(Optional<WideString> sValue) {
  ...
  if (sValue.has_value()) {
    pEdit->SetText_(sValue.value());
    pEdit->Paint();
  } else {
    int32_t nCurSel = pField->GetSelectedIndex(0);
    if (nCurSel < 0) {
      pEdit->SetText_(pField->GetValue());
      pEdit->Paint();
    } else {
      pEdit->SetText_(pField->GetOptionLabel(nCurSel));
      pEdit->Paint();
    }
  }
...
void CPDFSDK_AppStream::SetAsListBox() {
  ...
  float fFontSize = widget_->GetFontSize();
  pEdit->SetFontSize_(IsFloatZero(fFontSize) ? 12.0f : fFontSize);
  pEdit->Paint();
  pEdit->Initialize();
  ...
  for (int32_t i = nTop; i < nCount; ++i) {
    ...
    pEdit->SetText_(pField->GetOptionLabel(i));
    pEdit->Paint();
...
void CPDFSDK_AppStream::SetAsTextField(Optional<WideString> sValue) {
  ...
  pEdit->Initialize();
  pEdit->SetText_(sValue.value_or(pField->GetValue()));
  pEdit->Paint();
