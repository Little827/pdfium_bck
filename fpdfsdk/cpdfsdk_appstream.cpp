...
ByteString GetColorAppStreamFalse(const CFX_Color& color) {
...
ByteString GetColorAppStreamTrue(const CFX_Color& color) {
  fxcrt::ostringstream sColorStream;
  switch (color.nColorType) {
    case CFX_Color::Type::kTransparent:
      break;
...
ByteString GetAppStream_Check(const CFX_FloatRect& rcBBox,
  ...
  fxcrt::ostringstream sAP;
  {
    ...
    sAP << GetColorAppStreamTrue(crText) << GetAP_Check(rcBBox) << kFillOperator
        << "\n";
  }
...
ByteString GetAppStream_Circle(const CFX_FloatRect& rcBBox,
  ...
  {
    AutoClosedQCommand q(&sAP);
    sAP << GetColorAppStreamTrue(crText) << GetAP_Circle(rcBBox)
        << kFillOperator << "\n";
  }
  return ByteString(sAP);
}
...
ByteString GetAppStream_Cross(const CFX_FloatRect& rcBBox,
  ...
  {
    AutoClosedQCommand q(&sAP);
    sAP << GetColorAppStreamFalse(crText) << GetAP_Cross(rcBBox)
        << kStrokeOperator << "\n";
  }
  return ByteString(sAP);
}
...
ByteString GetAppStream_Diamond(const CFX_FloatRect& rcBBox,
  ...
  fxcrt::ostringstream sAP;
  {
    ...
    sAP << "1 " << kSetLineWidthOperator << "\n"
        << GetColorAppStreamTrue(crText) << GetAP_Diamond(rcBBox)
        << kFillOperator << "\n";
  }
  return ByteString(sAP);
}
...
ByteString GetAppStream_Square(const CFX_FloatRect& rcBBox,
  ...
  {
    AutoClosedQCommand q(&sAP);
    sAP << GetColorAppStreamTrue(crText) << GetAP_Square(rcBBox)
        << kFillOperator << "\n";
  }
  return ByteString(sAP);
}
...
ByteString GetAppStream_Star(const CFX_FloatRect& rcBBox,
  ...
  {
    AutoClosedQCommand q(&sAP);
    sAP << GetColorAppStreamTrue(crText) << GetAP_Star(rcBBox) << kFillOperator
        << "\n";
  }
  return ByteString(sAP);
}
...
ByteString GetCircleFillAppStream(const CFX_FloatRect& rect,
  ...
  ByteString sColor = GetColorAppStreamTrue(color);
  ...
  if (fWidth > 0.0f) {
    ...
    switch (nStyle) {
      ...
      case BorderStyle::kUnderline: {
        sColor = GetColorAppStreamFalse(color);
        if (sColor.GetLength() > 0) {
      ...
      case BorderStyle::kBeveled: {
        ...
        sColor = GetColorAppStreamFalse(crLeftTop);
        if (sColor.GetLength() > 0) {
          AutoClosedQCommand q2(&sAppStream);
          sAppStream << fHalfWidth << " " << kSetLineWidthOperator << "\n"
        ...
        sColor = GetColorAppStreamFalse(crRightBottom);
        if (sColor.GetLength() > 0) {
          AutoClosedQCommand q2(&sAppStream);
          sAppStream << fHalfWidth << " " << kSetLineWidthOperator << "\n"
                     << sColor
...
ByteString GetPushButtonAppStream(const CFX_FloatRect& rcBBox,
  ...
  if (!rcLabel.IsEmpty()) {
    ...
    if (sEdit.GetLength() > 0) {
      ...
      sTemp << GetColorAppStreamTrue(crText) << sEdit;
    }
  }
...
ByteString GetBorderAppStreamInternal(const CFX_FloatRect& rect,
  ...
  if (fWidth > 0.0f) {
    ...
    switch (nStyle) {
      default:
      case BorderStyle::kSolid:
        sColor = GetColorAppStreamTrue(color);
        if (sColor.GetLength() > 0) {
          sAppStream << sColor;
      ...
      case BorderStyle::kBeveled:
      case BorderStyle::kInset:
        sColor = GetColorAppStreamTrue(crLeftTop);
        if (sColor.GetLength() > 0) {
          sAppStream << sColor;
        ...
        sColor = GetColorAppStreamTrue(crRightBottom);
        if (sColor.GetLength() > 0) {
          sAppStream << sColor;
          sAppStream << fRight - fHalfWidth << " " << fTop - fHalfWidth << " "
                     << kMoveToOperator << "\n";
...
ByteString GetDropButtonAppStream(const CFX_FloatRect& rcBBox) {
  ...
  {
    AutoClosedQCommand q(&sAppStream);
    sAppStream << GetColorAppStreamTrue(
...
void CPDFSDK_AppStream::SetAsComboBox(absl::optional<WideString> sValue) {
  ...
  if (sEdit.GetLength() > 0) {
    ...
    sBody << GetColorAppStreamTrue(crText) << sEdit;
  }
  ...
  for (int32_t i = nTop; i < nCount; ++i) {
    ...
    if (bSelected) {
          ...
          CFX_FloatRect(rcClient.left, fy - fItemHeight, rcClient.right, fy);
      {
        ...
        sList << GetColorAppStreamTrue(CFX_Color(
      ...
      sList << GetColorAppStreamTrue(CFX_Color(CFX_Color::Type::kGray, 1))
            << GetEditAppStream(pEdit.get(), CFX_PointF(0.0f, fy), true, 0);
    } else {
      CFX_Color crText = widget_->GetTextPWLColor();

      AutoClosedCommand bt(&sList, kTextBeginOperator, kTextEndOperator);
      sList << GetColorAppStreamTrue(crText)
            << GetEditAppStream(pEdit.get(), CFX_PointF(0.0f, fy), true, 0);
    }
...
void CPDFSDK_AppStream::SetAsTextField(absl::optional<WideString> sValue) {
  ...
  if (bCharArray) {
    switch (widget_->GetBorderStyle()) {
      case BorderStyle::kSolid: {
        ByteString sColor =
            GetColorAppStreamFalse(widget_->GetBorderPWLColor());
        if (sColor.GetLength() > 0) {
