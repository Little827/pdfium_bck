...
ByteString GetAP_Check(const CFX_FloatRect& crBBox) {
  ...
  std::array<PointRow, 8> kPointTable = {{
  ...
  WriteMove(csAP, kPointTable[0][0]);

  for (size_t i = 0; i < kPointTable.size(); ++i) {
    size_t nNext = i < kPointTable.size() - 1 ? i + 1 : 0;
    const CFX_PointF& pt_next = kPointTable[nNext][0];
    float px1 = kPointTable[i][1].x - kPointTable[i][0].x;
    float py1 = kPointTable[i][1].y - kPointTable[i][0].y;
    float px2 = kPointTable[i][2].x - pt_next.x;
    float py2 = kPointTable[i][2].y - pt_next.y;
    WriteBezierCurve(
        csAP,
        {kPointTable[i][0].x + px1 * FXSYS_BEZIER,
         kPointTable[i][0].y + py1 * FXSYS_BEZIER},
