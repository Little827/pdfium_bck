...
TEST_F(FPDFPPOEmbeddertest, NupRenderImage) {
  ...
  UniqueFPDFDocument output_doc_3up(
  ...
  for (int i = 0; i < kPageCount; ++i) {
    UniqueFPDFPage page(FPDF_LoadPage(output_doc_3up.get(), i));
    ASSERT_TRUE(page);
    UniqueFPDFBitmap bitmap(RenderPageWithFlags(page.get(), nullptr, 0));
...
TEST_F(FPDFPPOEmbeddertest, BUG_750568) {
  ...
  for (size_t i = 0; i < 4; ++i) {
    ...
    UniqueFPDFBitmap bitmap = RenderPageWithFlags(page, nullptr, 0);
    ASSERT_EQ(200, FPDFBitmap_GetWidth(bitmap.get()));
    ASSERT_EQ(200, FPDFBitmap_GetHeight(bitmap.get()));
    ASSERT_EQ(800, FPDFBitmap_GetStride(bitmap.get()));
...
TEST_F(FPDFPPOEmbeddertest, ImportWithZeroLengthStream) {
  ...
  UniqueFPDFBitmap new_bitmap = RenderPageWithFlags(new_page, nullptr, 0);
