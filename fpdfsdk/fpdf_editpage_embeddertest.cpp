...
TEST_F(FPDFEditPageEmbedderTest, DashingArrayAndPhase) {
...

  {
    ...
    // modify phase
    EXPECT_TRUE(FPDFPageObj_SetDashPhase(path, 0.0f));
...

  {
    ...
    float set_array[] = {1.f, 2.f, 3.f};
    EXPECT_TRUE(FPDFPageObj_SetDashArray(path, set_array, 3, 5.f));

    phase = -1123.5f;
    EXPECT_TRUE(FPDFPageObj_GetDashPhase(path, &phase));
    EXPECT_FLOAT_EQ(5.f, phase);
