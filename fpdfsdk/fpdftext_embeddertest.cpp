...
// Testing that 'soft hyphens' (0x0002) are replacing hard hyphens when the
...
TEST_F(FPDFTextEmbeddertest, bug_1029) {
  ...
  // This text should 'METADATA table. When the split has committed, it noti'
  // followed by a 'soft hyphen' and then 'fi'. The original text has an fi
  // ligature, but that is broken up into two character when we process the PDF.
