...
class NupState {
 public:
  explicit NupState(FPDF_NUP_LAYOUT nupLayout);
...
NupState::NupState(FPDF_NUP_LAYOUT nupLayout)
    : m_numPagesOnXAxis(nupLayout.numPagesOnXAxis),
      ...
      m_inPages(0),
      m_outPages(0),
      m_numPagesPerSheet(nupLayout.numPagesOnXAxis * nupLayout.numPagesOnYAxis),
...
bool CPDF_PageOrganizer::ExportNPagesToOne(
  ...
  if (!NupState::IsPossible(nupLayout.numPagesOnXAxis *
                            nupLayout.numPagesOnYAxis))
    return false;

  SetDestPageSize(nupLayout.destPageWidth, nupLayout.destPageHeight);

  if (nupLayout.numPagesOnXAxis * nupLayout.numPagesOnYAxis == 1)
