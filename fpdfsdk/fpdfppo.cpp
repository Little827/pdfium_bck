...
class NupState {
 public:
  NupState(const double destPageWidth,
...
void NupState::CalculatePageEdit(const size_t subXPos,
                                 const size_t subYPos,
                                 const float inPageWidth,
                                 const float inPageHeight,
...
bool CPDF_PageOrganizer::ExportNPagesToOne(
    ...
    const unsigned int rows,
    const unsigned int columns) {
  FX_SAFE_SIZE_T safe_numPagesPerSheet = rows * columns;
