...
class NupState {
 public:
  ...
  // result stored in out paramemter |ret|.
  void CalculateNewPagePosition(float inWidth,
                                float inHeight,
                                NupPageSettings* ret);

 private:
  // Helper function to get the subX, subY pair based on |subPageIndex|.
  // The space of output page is evenly divided into slots along x and y axis.
  //  subX and subY is 0-based index indicates which allocated slot it is.
  std::pair<size_t, size_t> ConvertPageOrder(size_t subPageIndex) const;
  // Helper function to get sub page origin and scale.
  void CalculatePageEdit(const size_t subx,
                         const size_t suby,
                         const float inPageWidth,
                         const float inPageHeight,
...
std::pair<size_t, size_t> NupState::ConvertPageOrder(
    size_t subPageIndex) const {
  size_t subX = subPageIndex % m_numPagesOnXAxis;
  size_t subY = subPageIndex / m_numPagesOnXAxis;
...
void NupState::CalculateNewPagePosition(const float inWidth,
  ...
  size_t subX;
  size_t subY;
  std::tie(subX, subY) = ConvertPageOrder(m_subPageIndex);
...
void CPDF_PageOrganizer::AddSubPage(CPDF_Dictionary* pPageDict,
  ...
  ByteString xobjectName = ByteString::Format("X%d", pState->m_xobjectNum);
  pState->m_xobjs[xobjectName] = MakeXObject(pPageDict, m_pDestPDFDoc.Get());
...
bool CPDF_PageOrganizer::ExportNPagesToOne(
  ...
  for (size_t outerPage = 0; outerPage < pageNums.size();
    ...
    for (size_t innerPage = outerPage; innerPage < innerPageMax; ++innerPage) {
      ...
      AddSubPage(pSrcPageDict, pgEdit.subPageStartPoint, pgEdit.scale, &state,
                 &content);
    }

    // Finish up the current page.
    FinishPage(pCurPageDict, content, &state);
    ++curpage;
  }
