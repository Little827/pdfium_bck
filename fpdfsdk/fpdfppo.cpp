...
class NupState {
 ...
 private:
  ...
  Axis m_first = Axis::X;
  Position m_xStart = Position::LEFT;
  Position m_yStart = Position::TOP;
  Position m_xAlign = Position::CENTER;
  Position m_yAlign = Position::CENTER;
...
std::pair<size_t, size_t> NupState::ConvertPageOrder(size_t subPage) const {
  ...
  if (m_xStart == Position::RIGHT)
    subX = m_numPagesOnXAxis - subX - 1;
  if (m_yStart == Position::TOP)
...
void NupState::CalculatePageEdit(size_t subXPos,
  ...
  float subHeight = pageEdit->subPageRect.height * xScale;
  if (xScale > yScale) {
    pageEdit->point.x +=
        CalculateCenterShift(m_xAlign, m_subPageWidth - subWidth);
  } else {
    pageEdit->point.y +=
        CalculateCenterShift(m_yAlign, m_subPageHeight - subHeight);
  }
}
...
FPDF_ImportNPagesToOne(FPDF_DOCUMENT dest_doc,
  ...
  return pageOrg.PDFDocInit() &&
         pageOrg.ExportNPagesToOne(pageArray, index, nup_layout);
}
