...
bool NupState::CreateNewPage(float inWidth,
  ...
  if (!isSourceDocLandscape && inWidth > inHeight) {
    pageEdit.subPageRect.width = inHeight;
    pageEdit.subPageRect.height = inWidth;
  }
  size_t subX;
  size_t subY;
...
void CPDF_PageOrganizer::AddSubPage(CPDF_Dictionary* pPageDict,
  ...
  CFX_Matrix matrix;
  // If page object has a rotate field, we need to handle that.
  CPDF_Object* pRotate = PageDictGetInheritableTag(pPageDict, "Rotate");
  int pageRotate = pRotate ? (abs(pRotate->GetInteger()) / 90) % 4 : 0;
  switch (pageRotate) {
    case 0:
      break;
    case 1:
      matrix.Rotate(pageRotate * FX_PI / 2);
      matrix.Translate(m_pageSize.width, 0.0);
      break;
    case 2:
      matrix.Rotate(pageRotate * FX_PI / 2);
      matrix.Translate(m_pageSize.width, m_pageSize.height);
      break;
    case 3:
      matrix.Rotate(pageRotate * FX_PI / 2);
      matrix.Translate(0.0, m_pageSize.height);
      break;
  }
...
bool CPDF_PageOrganizer::ExportNPagesToOne(
  ...
  for (size_t i = 0; i < pageNums.size(); ++i) {
    ...
    AddSubPage(pSrcPageDict, CFX_PointF(pgEdit.point.x, pgEdit.point.y),
               pgEdit.scale, rotate, nupState.isLandscape(), &content, nullptr);
    ++inPages;
  }
