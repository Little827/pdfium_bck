...
// The output page is evenly divided along the X axis and Y axis based on the
// input numPagesOnXAxis and numPagesOnYAxis.
class NupState {
 public:
  ...
  // Calculate sub page origin and scale.
  void CalculateNewPagePosition(const float inWidth,
                                const float inHeight,
                                NupPageSettings* ret);

 private:
  // Helper function to get the subX, subY pair.
  ...
  const float m_subPageWidth;
  const float m_subPageHeight;
...
std::pair<size_t, size_t> NupState::ConvertPageOrder(size_t subPage) const {
  size_t subX = subPage % m_numPagesOnXAxis;
  size_t subY = subPage / m_numPagesOnXAxis;
...
FPDF_ImportNPagesToOne(FPDF_DOCUMENT src_doc,
  ...
  if (output_width <= 0 || output_height <= 0 || numPagesOnXAxis <= 0 ||
      numPagesOnYAxis <= 0)
    return nullptr;
  ...
  if (!pageOrg.PDFDocInit() ||
      !pageOrg.ExportNPagesToOne(pageArray, output_width, output_height,
                                 numPagesOnXAxis, numPagesOnYAxis)) {
    return nullptr;
  }
