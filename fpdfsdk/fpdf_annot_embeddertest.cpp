...
TEST_F(FPDFAnnotEmbedderTest, FocusableAnnotSubtypes) {
  ...
  // TODO(crbug.com/pdfium/1511): Handle focused annotation correctly when list
  // of focusable annotation type changes.
  ...
  ASSERT_TRUE(FPDFAnnot_SetFocusableSubtypes(form_handle(), subtypes.data(),
                                             subtypes.size()));
  ASSERT_EQ(1, FPDFAnnot_GetFocusableSubtypesCount(form_handle()));
  ASSERT_TRUE(
