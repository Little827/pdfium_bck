...
TEST_F(FPDFAnnotEmbedderTest, GetFormControlCountRadioButton) {
  // Open a file with checkbox and radio button widget annotations and load its
  // first page.
  ASSERT_TRUE(OpenDocument("click_form.pdf"));
  FPDF_PAGE page = LoadPage(0);
  ASSERT_TRUE(page);
...
TEST_F(FPDFAnnotEmbedderTest, GetFormFieldLabelRadioButton) {
  // Open a file with checkbox and radio button widget annotations and load its
  // first page.
  ASSERT_TRUE(OpenDocument("click_form.pdf"));
  FPDF_PAGE page = LoadPage(0);
...

  {
    // Checks for bad annot.
    EXPECT_EQ(0u,
              FPDFAnnot_GetFormFieldLabel(form_handle(), /*annot=*/nullptr,
    ...
    // Checks for bad form handle.
    EXPECT_EQ(0u,
              FPDFAnnot_GetFormFieldLabel(/*hHandle=*/nullptr, annot.get(),
                                          /*buffer=*/nullptr, /*buflen=*/0));

    unsigned long length_bytes =
        FPDFAnnot_GetFormFieldLabel(form_handle(), annot.get(),
                                    /*buffer=*/nullptr, /*buflen=*/0);
    ASSERT_EQ(14u, length_bytes);
    std::vector<FPDF_WCHAR> buf = GetFPDFWideStringBuffer(length_bytes);
    EXPECT_EQ(14u, FPDFAnnot_GetFormFieldLabel(form_handle(), annot.get(),
                                               buf.data(), length_bytes));
...
TEST_F(FPDFAnnotEmbedderTest, GetFormFieldLabelCheckBox) {
  // Open a file with checkbox and radio button widget annotations and load its
  // first page.
  ASSERT_TRUE(OpenDocument("click_form.pdf"));
  FPDF_PAGE page = LoadPage(0);
...

  {
    ...
    ASSERT_EQ(8u, length_bytes);
    std::vector<FPDF_WCHAR> buf = GetFPDFWideStringBuffer(length_bytes);
    EXPECT_EQ(8u, FPDFAnnot_GetFormFieldLabel(form_handle(), annot.get(),
                                              buf.data(), length_bytes));
...
TEST_F(FPDFAnnotEmbedderTest, GetFormFieldLabelInvalidAnnotation) {
  // Open a file with ink annotations and load its first page.
  ASSERT_TRUE(OpenDocument("annotation_ink_multiple.pdf"));
  FPDF_PAGE page = LoadPage(0);
  ASSERT_TRUE(page);
