...
TEST_F(FPDFAnnotEmbedderTest, GetFontSizeInvalidAnnotationTypes) {
  ...
  {
    // Annotations that do not have variable text and will return 0.
    ScopedFPDFAnnotation annot(FPDFPage_GetAnnot(page, 0));
    ASSERT_TRUE(annot);

    EXPECT_EQ(0.0, FPDFAnnot_GetFontSize(form_handle(), annot.get()));
...
TEST_F(FPDFAnnotEmbedderTest, GetFontSizeInvalidArguments) {
...

  {
    ...
    // Check bad form handle / annot.
    EXPECT_EQ(0.0, FPDFAnnot_GetFontSize(nullptr, annot.get()));
    EXPECT_EQ(0.0, FPDFAnnot_GetFontSize(form_handle(), nullptr));
    EXPECT_EQ(0.0, FPDFAnnot_GetFontSize(nullptr, nullptr));
  }
