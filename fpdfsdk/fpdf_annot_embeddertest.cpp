...
TEST_F(FPDFAnnotEmbedderTest, SetFocusableAnnotSubtypes) {
  ...
  EXPECT_TRUE(count == 1);
  constexpr FPDF_ANNOTATION_SUBTYPE kNoFocusableSubtypes[] = {};

  EXPECT_TRUE(
      FPDFAnnot_SetFocusableSubtypes(form_handle(), kNoFocusableSubtypes, 0));

  count = FPDFAnnot_GetFocusableSubtypesCount(form_handle());
  EXPECT_TRUE(count == 0);
  ...
  count = FPDFAnnot_GetFocusableSubtypesCount(form_handle());
  EXPECT_EQ(static_cast<int>(kSubtypeCount), count);
...
TEST_F(FPDFAnnotEmbedderTest, GetFocusableAnnotSubtypes) {
  ...
  EXPECT_FALSE(FPDFAnnot_GetFocusableSubtypes(form_handle(),
                                              set_subtypes.data(), count - 1));

  EXPECT_TRUE(FPDFAnnot_GetFocusableSubtypes(form_handle(), set_subtypes.data(),
                                             count));

  for (int i = 0; i < count; ++i) {
    EXPECT_EQ(kFocusableSubtypes[i], set_subtypes[i]);
  }
