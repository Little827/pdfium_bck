...
TEST_F(FPDFAnnotEmbedderTest, GetOptionCountValidAnnotations) {
...
TEST_F(FPDFAnnotEmbedderTest, GetOptionCountInvalidAnnotations) {
...

  {
    ...
    EXPECT_EQ(-1, FPDFAnnot_GetOptionCount(annot.get()));
...
TEST_F(FPDFAnnotEmbedderTest, GetOptionLabelValidAnnotations) {
  // Open a file with combobox widget annotations and load its first page.
  ASSERT_TRUE(OpenDocument("combobox_form.pdf"));
  FPDF_PAGE page = LoadPage(0);
  ASSERT_TRUE(page);

  {
    ...
    int index = 0;
    unsigned long len =
        FPDFAnnot_GetOptionLabel(annot.get(), index, nullptr, 0);
    std::vector<char> buf(len);
    EXPECT_EQ(8u,
              FPDFAnnot_GetOptionLabel(annot.get(), index, buf.data(), len));
    ...
    EXPECT_EQ(-1u, FPDFAnnot_GetOptionLabel(annot.get(), -1, nullptr, 0));
    EXPECT_EQ(-1u, FPDFAnnot_GetOptionLabel(annot.get(), 26, nullptr, 0));
  }
...
TEST_F(FPDFAnnotEmbedderTest, GetOptionLabelInvalidAnnotations) {
...

  {
    ...
    EXPECT_EQ(-1u, FPDFAnnot_GetOptionLabel(annot.get(), 0, nullptr, 0));
