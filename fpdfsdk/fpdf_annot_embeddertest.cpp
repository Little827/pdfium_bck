...
TEST_F(FPDFAnnotEmbedderTest, SetFocusableAnnotSubtypes) {
  ...
  constexpr unsigned int kSubtypeCount = FX_ArraySize(kFocusableSubtypes);
  ...
  EXPECT_EQ((int)kSubtypeCount, count);
...
TEST_F(FPDFAnnotEmbedderTest, GetFocusableAnnotSubtypesCount) {
  ...
  EXPECT_EQ(-1, FPDFAnnot_GetFocusableSubtypesCount(NULL));
