...
TEST_F(FPDFAnnotEmbeddertest, RenderAnnotWithOnlyRolloverAP) {
  ...
  // still display.
  UniqueFPDFBitmap bitmap = RenderLoadedPageWithFlags(page, FPDF_ANNOT);
...
TEST_F(FPDFAnnotEmbeddertest, ExtractHighlightLongContent) {
  ...
  {
    UniqueFPDFAnnotation annot(FPDFPage_GetAnnot(page, 0));
    ASSERT_TRUE(annot);
...
TEST_F(FPDFAnnotEmbeddertest, ExtractInkMultiple) {
...

  {
    ...
    UniqueFPDFAnnotation annot(FPDFPage_GetAnnot(page, 2));
    ASSERT_TRUE(annot);
...
TEST_F(FPDFAnnotEmbeddertest, AddFirstTextAnnotation) {
...

  {
    ...
    UniqueFPDFAnnotation annot(FPDFPage_CreateAnnot(page, FPDF_ANNOT_TEXT));
    ASSERT_TRUE(annot);
...
TEST_F(FPDFAnnotEmbeddertest, AddAndSaveUnderlineAnnotation) {
...

  {
    ...
    // its quadpoints.
    UniqueFPDFAnnotation new_annot(FPDFPage_GetAnnot(page, 1));
...
TEST_F(FPDFAnnotEmbeddertest, RemoveAnnotation) {
  ...
  {
    UniqueFPDFAnnotation annot(FPDFPage_GetAnnot(page, 1));
    ASSERT_TRUE(FPDFAnnot_GetRect(annot.get(), &rect));
  ...
  {
    UniqueFPDFAnnotation annot(FPDFPage_GetAnnot(new_page, 0));
    ASSERT_TRUE(FPDFAnnot_GetRect(annot.get(), &rect));
    EXPECT_NEAR(86.1971f, rect.left, 0.001f);
  }

  {
    UniqueFPDFAnnotation annot(FPDFPage_GetAnnot(new_page, 1));
    ASSERT_TRUE(FPDFAnnot_GetRect(annot.get(), &rect));
    EXPECT_NEAR(351.8204f, rect.left, 0.001f);
  }
...
TEST_F(FPDFAnnotEmbeddertest, AddAndModifyPath) {
...

  {
    ...
    UniqueFPDFAnnotation annot(FPDFPage_CreateAnnot(page, FPDF_ANNOT_STAMP));
    ASSERT_TRUE(annot);
    rect.left = 200.f;
    rect.bottom = 400.f;
    rect.right = 500.f;
...
TEST_F(FPDFAnnotEmbeddertest, GetSetStringValue) {
  ...
  {
    UniqueFPDFAnnotation new_annot(FPDFPage_GetAnnot(page, 0));
