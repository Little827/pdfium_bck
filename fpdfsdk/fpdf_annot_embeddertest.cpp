...
static const char kAnnotationStampWithApOriginalMD5[] =
...
TEST_F(FPDFAnnotEmbedderTest, MAYBE_AddAndModifyPath) {
  ...
  {
    ScopedFPDFBitmap bitmap = RenderLoadedPageWithFlags(page, FPDF_ANNOT);
    CompareBitmap(bitmap.get(), 595, 842, kAnnotationStampWithApOriginalMD5);
  }
...
TEST_F(FPDFAnnotEmbedderTest, ModifyAnnotationFlags) {
...

  {
    ...
    {
      ScopedFPDFBitmap bitmap = RenderLoadedPageWithFlags(page, FPDF_ANNOT);
      CompareBitmap(bitmap.get(), 612, 792, fpdfsdk::kBlankPage612By792MD5);
    }
