...
TEST_F(PDFAnnotTest, SetAP) {
  ...
  ScopedFPDFWideString ap_stream = GetFPDFWideString(stream_data);

  // Create a new ink annotation.
  ScopedFPDFAnnotation annot(FPDFPage_CreateAnnot(page.get(), FPDF_ANNOT_INK));
  ASSERT_TRUE(annot);

  // FPDFAnnot_SetAP() fails if bounding rect is not yet set on the annotation.
