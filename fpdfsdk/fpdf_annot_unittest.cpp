...
TEST_F(PDFAnnotTest, InkListAPIValidations) {
  ...
  static constexpr unsigned long kFirstStrokePointCount =
      FX_ArraySize(kFirstInkStroke);

  static constexpr FS_POINTF kSecondInkStroke[] = {
      {70.0f, 90.0f}, {71.0f, 91.0f}, {72.0f, 92.0f}};
  static constexpr unsigned long kSecondStrokePointCount =
  ...
  static constexpr FS_POINTF kThirdInkStroke[] = {{60.0f, 90.0f},
                                                  ...
                                                  {63.0f, 93.0f},
                                                  {64.0f, 94.0f}};
  static constexpr unsigned long kThirdStrokePointCount =
      FX_ArraySize(kThirdInkStroke);

  // Negative test: |annot| is passed as nullptr.
  EXPECT_EQ(-1, FPDFAnnot_AddInkStroke(nullptr, kFirstStrokePointCount,
                                       &kFirstInkStroke[0]));
  ...
  EXPECT_EQ(
      -1, FPDFAnnot_AddInkStroke(highlight_annot.get(), kFirstStrokePointCount,
                                 &kFirstInkStroke[0]));

  // Negative test: passing |point_count| as  0.
  EXPECT_EQ(-1,
            FPDFAnnot_AddInkStroke(ink_annot.get(), 0, &kFirstInkStroke[0]));

  // Negative test: passing |points| array as nullptr.
  EXPECT_EQ(-1, FPDFAnnot_AddInkStroke(ink_annot.get(), kFirstStrokePointCount,
                                       nullptr));
  ...
  // created.
  EXPECT_EQ(0, FPDFAnnot_AddInkStroke(ink_annot.get(), kFirstStrokePointCount,
                                      &kFirstInkStroke[0]));

  CPDF_Array* inklist = annot_dict->GetArrayFor("InkList");
  ASSERT_TRUE(inklist);
  EXPECT_EQ(1, static_cast<int64_t>(inklist->size()));
  EXPECT_EQ(kFirstStrokePointCount * 2,
  ...
  EXPECT_EQ(1, FPDFAnnot_AddInkStroke(ink_annot.get(), kSecondStrokePointCount,
                                      &kSecondInkStroke[0]));
  EXPECT_EQ(2, static_cast<int64_t>(inklist->size()));
  EXPECT_EQ(kSecondStrokePointCount * 2,
  ...
  EXPECT_EQ(2,
            FPDFAnnot_AddInkStroke(ink_annot.get(), kThirdStrokePointCount - 1,
                                   &kThirdInkStroke[0]));
  EXPECT_EQ(3, static_cast<int64_t>(inklist->size()));
  EXPECT_EQ((kThirdStrokePointCount - 1) * 2,
