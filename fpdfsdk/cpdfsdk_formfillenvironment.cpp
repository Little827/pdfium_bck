...
void CPDFSDK_FormFillEnvironment::SendSetFocusAnnot(
    ObservedPtr<CPDFSDK_Annot>* pAnnot) {
  if (!m_pInfo || m_pInfo->version < 2 || !m_pInfo->FFI_SetFocusAnnot)
    return;
  ...
  CPDF_Page* cpdf_page =
      CPDFPageFromFPDFPage(FPDFPageFromIPDFPage((*pAnnot)->GetPDFPage()));
  if (!cpdf_page)
    return;
