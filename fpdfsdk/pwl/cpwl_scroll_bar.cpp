...
void CPWL_ScrollBar::CreateButtons(const CreateParams& cp,
  ...
  if (!m_pMinButton) {
    m_pMinButton = new CPWL_SBButton(
        pAttachedData ? pAttachedData->Clone() : nullptr, m_sbType, PSBT_MIN);
    m_pMinButton->Create(scp);
  }

  if (!m_pMaxButton) {
    m_pMaxButton = new CPWL_SBButton(
        pAttachedData ? pAttachedData->Clone() : nullptr, m_sbType, PSBT_MAX);
    m_pMaxButton->Create(scp);
  }

  if (!m_pPosButton) {
    m_pPosButton = new CPWL_SBButton(
        pAttachedData ? pAttachedData->Clone() : nullptr, m_sbType, PSBT_POS);
    ObservedPtr thisObserved(this);
...
void CPWL_ScrollBar::CreateChildWnd(
    const CreateParams& cp,
    std::unique_ptr<PrivateData> pAttachedData) {
  CreateButtons(cp, std::move(pAttachedData));
}
