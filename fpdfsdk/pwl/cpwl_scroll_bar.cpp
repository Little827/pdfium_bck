...
void CPWL_ScrollBar::CreateButtons(const CreateParams& cp) {
  ...
  if (!m_pMinButton) {
    m_pMinButton = new CPWL_SBButton(nullptr, m_sbType, PSBT_MIN);  // FIXME
    m_pMinButton->Create(scp);
  }

  if (!m_pMaxButton) {
    m_pMaxButton = new CPWL_SBButton(nullptr, m_sbType, PSBT_MAX);  // FIXME
    m_pMaxButton->Create(scp);
  }

  if (!m_pPosButton) {
    m_pPosButton = new CPWL_SBButton(nullptr, m_sbType, PSBT_POS);  // FIXME
    ObservedPtr thisObserved(this);
