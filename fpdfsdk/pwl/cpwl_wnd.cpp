...
void CPWL_Wnd::Create(const CreateParams& cp) {
  ...
  PrivateData* pData = GetAttachedData();
  CreateParams ccp = m_CreationParams;
  ccp.dwFlags &= 0xFFFF0000L;  // remove sub styles
  CreateScrollBar(ccp, pData ? pData->Clone() : nullptr);
  CreateChildWnd(ccp, pData ? pData->Clone() : nullptr);
...
void CPWL_Wnd::CreateScrollBar(const CreateParams& cp,
  ...
  CreateVScrollBar(cp, std::move(pAttachedData));
}
...
void CPWL_Wnd::CreateVScrollBar(const CreateParams& cp,
  ...
  // TODO(tsepez): leaks?
  m_pVScrollBar = new CPWL_ScrollBar(std::move(pAttachedData), SBT_VSCROLL);
