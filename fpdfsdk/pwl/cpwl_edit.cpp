...
void CPWL_Edit::DrawThisAppearance(CFX_RenderDevice* pDevice,
  ...
  if (draw_border) {
    ...
    if (border_style == BorderStyle::SOLID) {
      ...
      ASSERT(border_style == BorderStyle::DASH);
      gsd.m_LineWidth = GetBorderWidth();
      gsd.m_DashArray = {GetBorderDash().nDash, GetBorderDash().nGap};
      gsd.m_DashPhase = GetBorderDash().nPhase;
    }

    float width = (rcClient.right - rcClient.left) / nCharArray;
    CFX_PathData path;
    for (int32_t i = 0; i < nCharArray - 1; i++) {
      float x = rcClient.left + width * (i + 1);
      path.AppendPoint(CFX_PointF(x, rcClient.bottom), FXPT_TYPE::MoveTo);
      path.AppendPoint(CFX_PointF(x, rcClient.top), FXPT_TYPE::LineTo);
    }
