...
void CPWL_EditImpl::ReplaceSelection(const WideString& text) {
  ClearSelection(/*bRedoEnabled =*/false);
  InsertText(text, FX_CHARSET_Default, /*bRedoEnabled =*/false);
...
bool CPWL_EditImpl::ClearSelection(bool bRedoEnabled) {
  return Clear(true, true, bRedoEnabled);
}

bool CPWL_EditImpl::InsertText(const WideString& sText,
                               int32_t charset,
                               bool bRedoEnabled) {
  return InsertText(sText, charset, true, true, bRedoEnabled);
}
...
bool CPWL_EditImpl::Clear(bool bAddUndo, bool bPaint, bool bRedoEnabled) {
  if (!m_pVT->IsValid() || m_SelState.IsEmpty())
    return false;

  CPVT_WordRange range = m_SelState.ConvertToWordRange();
  if (bAddUndo && m_bEnableUndo) {
    AddEditUndoItem(pdfium::MakeUnique<CFXEU_Clear>(
        this, range, GetSelectedText(), bRedoEnabled));
  }
...
bool CPWL_EditImpl::InsertText(const WideString& sText,
  ...
  if (bAddUndo && m_bEnableUndo) {
    AddEditUndoItem(pdfium::MakeUnique<CFXEU_InsertText>(
        this, m_wpOldCaret, m_wpCaret, sText, charset, bRedoEnabled));
  }
  if (bPaint)
    PaintInsertText(m_wpOldCaret, m_wpCaret);
