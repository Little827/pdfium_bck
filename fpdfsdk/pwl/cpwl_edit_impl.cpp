...
void CPWL_EditImpl_Undo::Undo() {
  m_bWorking = true;
  int nUndoRemain = 1;
  bool bIsInWrapper = false;
  while (CanUndo() && nUndoRemain > 0) {
    int tempUndoRemain = m_UndoItemStack[m_nCurUndoPos - 1]->Undo();
    // Undo() only returns a positive integer when the undo item is the end of a
    // wrapper(for example, the end of a ReplaceSelection action). We need to
    // make sure that when processing undo items inside a wrapper, no more
    // wrapper shows up inside the existing one.
    ASSERT(!bIsInWrapper || tempUndoRemain == 0);

    m_nCurUndoPos--;
    if (tempUndoRemain > 0) {
      // Mark the beginning of a wrapper and set the number of undo tasks to be
      // excuted
      bIsInWrapper = true;
      nUndoRemain = tempUndoRemain;
    } else {
      nUndoRemain--;
    }
  }
...
void CPWL_EditImpl_Undo::Redo() {
  ...
  while (CanRedo() && nRedoRemain > 0) {
    int tempRedoRemain = m_UndoItemStack[m_nCurUndoPos]->Redo();
    ASSERT(!bIsInWrapper || tempRedoRemain == 0);
    m_nCurUndoPos++;
    if (tempRedoRemain > 0) {
      bIsInWrapper = true;
      nRedoRemain = tempRedoRemain;
    } else {
      nRedoRemain--;
    }
  }
...
CFXEU_ReplaceSel::CFXEU_ReplaceSel(CPWL_EditImpl* pEdit, bool bIsEnd)
...
int CFXEU_ReplaceSel::Redo() {
  m_pEdit->SelectNone();
  if (!IsEnd())
    return 3;  // Redo ClearSelection, InsertText and ReplaceSel's end wrapper
  return 0;
}

int CFXEU_ReplaceSel::Undo() {
  m_pEdit->SelectNone();
  if (IsEnd())
    return 3;  // Undo InsertText, ClearSelection and ReplaceSel's beginning
               // wrapper
  return 0;
}
...
void CPWL_EditImpl::ReplaceSelection(const WideString& text) {
  AddEditUndoItem(pdfium::MakeUnique<CFXEU_ReplaceSel>(this, false));
  ClearSelection();
  InsertText(text, FX_CHARSET_Default);
  AddEditUndoItem(pdfium::MakeUnique<CFXEU_ReplaceSel>(this, true));
}
