...
FPDF_SetFormFieldHighlightColor(FPDF_FORMHANDLE hHandle,
                                int fieldType,
                                unsigned long color) {
  CPDFSDK_InteractiveForm* pInterForm = FormHandleToInteractiveForm(hHandle);
  if (!pInterForm)
    return;
  ...
  if (cast_input.value() == FormFieldType::kUnknown) {
    pInterForm->SetAllHighlightColors(color);
  } else {
    pInterForm->SetHighlightColor(color, cast_input.value());
  }
}

FPDF_EXPORT void FPDF_CALLCONV
FPDF_SetFormFieldHighlightAlpha(FPDF_FORMHANDLE hHandle, unsigned char alpha) {
  if (CPDFSDK_InteractiveForm* pInteractiveForm =
          FormHandleToInteractiveForm(hHandle))
    pInteractiveForm->SetHighlightAlpha(alpha);
}
...
FPDF_RemoveFormFieldHighlight(FPDF_FORMHANDLE hHandle) {
  if (CPDFSDK_InteractiveForm* pInteractiveForm =
          FormHandleToInteractiveForm(hHandle))
    pInteractiveForm->RemoveAllHighLights();
}
