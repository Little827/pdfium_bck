...
FPDFDOC_InitFormFillEnvironment(FPDF_DOCUMENT document,
  ...
  auto* pExtension = static_cast<CPDFXFA_Extension*>(pDocument->GetExtension());
  if (pExtension->GetFormFillEnv())
    return pExtension->GetFormFillEnv();
#endif
...
#ifdef PDF_ENABLE_XFA
  pExtension->SetFormFillEnv(pFormFillEnv.get());
#endif  // PDF_ENABLE_XFA
...
FPDFDOC_ExitFormFillEnvironment(FPDF_FORMHANDLE hHandle) {
  ...
  if (pExtension)
    pExtension->SetFormFillEnv(nullptr);
#endif  // PDF_ENABLE_XFA
  delete pFormFillEnv;
}
