...
TEST_F(FPDFSignatureEmbedderTest, GetContents) {
  ...
  const char expected_contents[] = {0x30, 0x80, 0x06, 0x09, 0x2A, 0x86, 0x48,
                                    0x86, 0xF7, 0x0D, 0x01, 0x07, 0x02, 0xA0,
                                    0x80, 0x30, 0x80, 0x02, 0x01, 0x01};
  ASSERT_EQ(sizeof(expected_contents), size);
  std::vector<char> contents(size);
  ASSERT_EQ(size,
            FPDFSignatureObj_GetContents(signature, contents.data(), size));
  ASSERT_EQ(0, memcmp(expected_contents, contents.data(), size));
