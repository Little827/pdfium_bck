...
TEST_F(FPDFSignatureEmbedderTest, GetContents) {
  ...
  const char kExpectedContents[] = {0x30, 0x80, 0x06, 0x09, 0x2A, 0x86, 0x48,
  ...
  ASSERT_STREQ(kExpectedContents, contents.data());
  ...
  ASSERT_EQ(sizeof(kExpectedContents), size);
  ASSERT_EQ(std::string("x"), std::string(contents.data()));
}
