...
TEST_F(FPDFSignatureEmbedderTest, GetByteRange) {
  ...
  const std::vector<unsigned long> kExpectedByteRange{0, 10, 30, 10};
  ASSERT_EQ(kExpectedByteRange.size(), size);
  std::vector<unsigned long> byte_range(size);
  ASSERT_EQ(size,
  ...
  size = FPDFSignatureObj_GetByteRange(signature, byte_range.data(),
                                       byte_range.size());
  ASSERT_EQ(kExpectedByteRange.size(), size);
  EXPECT_EQ(0U, byte_range[0]);
  EXPECT_EQ(1U, byte_range[1]);
}
