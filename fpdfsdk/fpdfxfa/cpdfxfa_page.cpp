...
CPDFSDK_Annot* CPDFXFA_Page::GetLastXFAAnnot(CPDFSDK_PageView* page_view,
                                             bool last) const {
  CXFA_FFPageView* xfa_page_view = GetXFAPageView();
  if (!xfa_page_view)
    return nullptr;
  std::unique_ptr<IXFA_WidgetIterator> widget_iterator =
      xfa_page_view->CreateTraverseWidgetIterator(XFA_WidgetStatus_Visible |
                                                  XFA_WidgetStatus_Viewable |
                                                  XFA_WidgetStatus_Focused);
  if (last)
    return page_view->GetAnnotByXFAWidget(widget_iterator->MoveToLast());
  return page_view->GetAnnotByXFAWidget(widget_iterator->MoveToFirst());
}
