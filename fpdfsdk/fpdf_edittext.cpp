...
FPDF_EXPORT int FPDF_CALLCONV FPDFFont_GetWeight(FPDF_FONT font) {
  auto* pFont = CPDFFontFromFPDFFont(font);
  return pFont ? pFont->GetFontWeight() : 0;
}

FPDF_EXPORT int FPDF_CALLCONV FPDFFont_GetItalicAngle(FPDF_FONT font) {
  auto* pFont = CPDFFontFromFPDFFont(font);
  return pFont ? pFont->GetItalicAngle() : 0;
}
...
FPDF_EXPORT float FPDF_CALLCONV FPDFFont_GetAscent(FPDF_FONT font,
                                                   float font_size) {
  auto* pFont = CPDFFontFromFPDFFont(font);
  return pFont ? pFont->GetTypeAscent() * font_size / 1000.f : 0.0f;
}

FPDF_EXPORT float FPDF_CALLCONV FPDFFont_GetDescent(FPDF_FONT font,
                                                    float font_size) {
  auto* pFont = CPDFFontFromFPDFFont(font);
  return pFont ? pFont->GetTypeDescent() * font_size / 1000.f : 0.0f;
}
...
FPDFFont_GetGlyphPath(FPDF_FONT font, uint32_t glyph, float font_size) {
  ...
  CFX_Font* pCfxFont = GetFont(pFont, pos[0].m_FallbackFontPosition);
  if (!pCfxFont)
    return nullptr;
