...
FPDFTextObj_GetRenderedBitmap(FPDF_DOCUMENT document,
  ...
  const FX_RECT rect = text->GetRect().GetOuterRect();
  if (rect.IsEmpty())
    return nullptr;
  ...
  if (optional_page)
    optional_page->SetRenderContext(std::move(pOwnedContext));
