      ...
      with open(output_path, 'wb') as outfile:
        for line in infile:
          ...
          if state == ParserState.OUTSIDE_CLASS:
            ...
            match = re.match(r'^\s+(explicit\s)*' + class_name + '\(', line)
            if match:
              saved += line
              print_line = False
...
def main():
  ...
  parser.add_option('-v', default=False, action="store_true", dest="verbose")
  options, args = parser.parse_args()
  for input_file_path in args:
    temp_file_path = input_file_path + '.tmp'
    process_file(input_file_path, temp_file_path, options.verbose)
