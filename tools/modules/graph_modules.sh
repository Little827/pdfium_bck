...
function crunch {
  ...
  gn desc $BUILD_DIR $1 deps | grep -v test | grep -v matches | sed "s|\\(.*\\)|  \"$1\" -> \"\\1\"|"
  echo '  edge [color=red,constraint=false]'
  gn desc $BUILD_DIR $1 allow_circular_includes_from | grep -v test | grep -v matches | grep -v 'how to display' | sed "s|\\(.*\\)|  \"\\1\" -> \"$1\"|"
}
