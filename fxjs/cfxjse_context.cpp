...
bool CFXJSE_Context::ExecuteScript(const char* szScript,
                                   ...
                                   bool doLog) {
  // if (doLog) std::cerr << " CFXJSE_Context::ExecuteScript lpNewThisObject=" << lpNewThisObject->ToString() << std::endl;
  // if (doLog) std::cerr << szScript << std::endl;
  CFXJSE_ScopeUtil_IsolateHandleContext scope(this);
  ...
  if (!lpNewThisObject) {
    if (doLog) std::cerr << "======== global ==============" << std::endl;
  ...
  if (doLog) std::cerr << "======== closure ==============" << std::endl;
