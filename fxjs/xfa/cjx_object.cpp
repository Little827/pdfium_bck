...
#define XFA_MAPDATABLOCK_SIZE(capacity) (sizeof(XFA_MAPDATABLOCK) + (capacity))
...
void CJX_Object::SetMapModuleBuffer(
  ...
  if (!pBuffer) {
    pBuffer = reinterpret_cast<XFA_MAPDATABLOCK*>(
        FX_Alloc(uint8_t, XFA_MAPDATABLOCK_SIZE(iBytes)));
    ...
    pBuffer = reinterpret_cast<XFA_MAPDATABLOCK*>(
        FX_Realloc(uint8_t, pBuffer, XFA_MAPDATABLOCK_SIZE(iBytes)));
  ...
  for (const auto& pair : pSrcModuleData->m_BufferMap) {
    ...
    if (!pDstBuffer) {
      pDstBuffer = (XFA_MAPDATABLOCK*)FX_Alloc(
          uint8_t, XFA_MAPDATABLOCK_SIZE(pSrcBuffer->iBytes));
      ...
      pDstBuffer = (XFA_MAPDATABLOCK*)FX_Realloc(
          uint8_t, pDstBuffer, XFA_MAPDATABLOCK_SIZE(pSrcBuffer->iBytes));
