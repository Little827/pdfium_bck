...
class MapValue {
 public:
  enum class Type { kInteger, kString, kMeasurement };

  MapValue(Type type) : type_(type) {}
  virtual ~MapValue() = default;

  virtual std::unique_ptr<MapValue> Clone() const = 0;
...
class IntegerMapValue : public MapValue {
 public:
  IntegerMapValue(int32_t integer)
      : MapValue(Type::kInteger), integer_(integer) {}
  ~IntegerMapValue() override = default;

  // MapValue:
  std::unique_ptr<MapValue> Clone() const override {
    return std::make_unique<IntegerMapValue>(integer_);
  }

  int32_t GetInteger() const { return integer_; }

 private:
  int32_t integer_;
};

class StringMapValue : public MapValue {
 public:
  StringMapValue(const WideString& wsString)
      : MapValue(Type::kString), string_(wsString) {}
  ~StringMapValue() override = default;

  // MapValue:
  std::unique_ptr<MapValue> Clone() const override {
    return std::make_unique<StringMapValue>(string_);
  }

  WideString GetString() const { return string_; }

 private:
  WideString string_;
};

class MeasurementMapValue : public MapValue {
 public:
  MeasurementMapValue(const CXFA_Measurement& measurement)
      : MapValue(Type::kMeasurement), measurement_(measurement) {}
  ~MeasurementMapValue() override = default;

  // MapValue:
  std::unique_ptr<MapValue> Clone() const override {
    return std::make_unique<MeasurementMapValue>(measurement_);
  }

  CXFA_Measurement GetMeasurement() const { return measurement_; }

 private:
  CXFA_Measurement measurement_;
};

IntegerMapValue* ToIntegerMapValue(MapValue* ptr) {
  return ptr && ptr->GetType() == MapValue::Type::kInteger
             ? static_cast<IntegerMapValue*>(ptr)
             : nullptr;
}

StringMapValue* ToStringMapValue(MapValue* ptr) {
  return ptr && ptr->GetType() == MapValue::Type::kString
             ? static_cast<StringMapValue*>(ptr)
             : nullptr;
}

MeasurementMapValue* ToMeasurementMapValue(MapValue* ptr) {
  return ptr && ptr->GetType() == MapValue::Type::kMeasurement
             ? static_cast<MeasurementMapValue*>(ptr)
             : nullptr;
}
...
class CXFA_MapModule {
 public:
  ...
  void SetInteger(uint32_t key, int32_t value) {
    m_Map[key] = std::make_unique<IntegerMapValue>(value);
  }

  void SetString(uint32_t key, const WideString& wsString) {
    m_Map[key] = std::make_unique<StringMapValue>(wsString);
  }

  void SetMeasurement(uint32_t key, const CXFA_Measurement& measurement) {
    m_Map[key] = std::make_unique<MeasurementMapValue>(measurement);
  }

  Optional<int32_t> GetInteger(uint32_t key) const {
    auto it = m_Map.find(key);
    if (it == m_Map.end())
      return pdfium::nullopt;
    IntegerMapValue* value = ToIntegerMapValue(it->second.get());
    if (!value)
      return pdfium::nullopt;
  ...
  Optional<WideString> GetString(uint32_t key) const {
    ...
    if (it == m_Map.end())
      return pdfium::nullopt;
    StringMapValue* value = ToStringMapValue(it->second.get());
    if (!value)
      return pdfium::nullopt;
  ...
  Optional<CXFA_Measurement> GetMeasurement(uint32_t key) const {
    ...
    if (it == m_Map.end())
      return pdfium::nullopt;
    MeasurementMapValue* value = ToMeasurementMapValue(it->second.get());
    if (!value)
      return pdfium::nullopt;
    return value->GetMeasurement();
  }

  bool HasKey(uint32_t key) const { return pdfium::Contains(m_Map, key); }

  void RemoveKey(uint32_t key) { m_Map.erase(key); }

  void MergeDataFrom(const CXFA_MapModule* pSrc) {
    for (const auto& pair : pSrc->m_Map)
      m_Map[pair.first] = pair.second->Clone();
  }

 private:
  // keyed by result of GetMapKey_*().
  std::map<uint32_t, std::unique_ptr<MapValue>> m_Map;
};
...
void CJX_Object::SetMeasure(XFA_Attribute eAttr,
  ...
  CreateMapModule()->SetMeasurement(key, mValue);
...
Optional<CXFA_Measurement> CJX_Object::TryMeasure(XFA_Attribute eAttr,
                                                  bool bUseDefault) const {
  uint32_t key = GetMapKey_Element(GetXFAObject()->GetElementType(), eAttr);
  Optional<CXFA_Measurement> result = GetMapModuleMeasurement(key);
  if (result.has_value())
    return result.value();
  if (!bUseDefault)
    return pdfium::nullopt;
...
void CJX_Object::SetMapModuleValue(uint32_t key, int32_t value) {
  CreateMapModule()->SetInteger(key, value);
}
...
Optional<int32_t> CJX_Object::GetMapModuleValue(uint32_t key) const {
  ...
  for (const CXFA_Node* pNode = GetXFANode(); pNode;
    ...
    if (pModule) {
      Optional<int32_t> result = pModule->GetInteger(key);
      if (result.has_value())
        return result;
    }
...
Optional<WideString> CJX_Object::GetMapModuleString(uint32_t key) const {
  ...
  for (const CXFA_Node* pNode = GetXFANode(); pNode;
    ...
    CXFA_MapModule* pModule = pNode->JSObject()->GetMapModule();
    if (pModule) {
      Optional<WideString> result = pModule->GetString(key);
      if (result.has_value())
        return result;
    }
    if (pNode->GetPacketType() == XFA_PacketType::Datasets)
...
Optional<CXFA_Measurement> CJX_Object::GetMapModuleMeasurement(
  ...
  for (const CXFA_Node* pNode = GetXFANode(); pNode;
    ...
    CXFA_MapModule* pModule = pNode->JSObject()->GetMapModule();
    if (pModule) {
      Optional<CXFA_Measurement> result = pModule->GetMeasurement(key);
      if (result.has_value())
        return result;
    }
    if (pNode->GetPacketType() == XFA_PacketType::Datasets)
