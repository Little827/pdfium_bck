...
bool SimpleValueCompare(v8::Isolate* pIsolate,
  ...
  if (fxv8::IsString(firstValue)) {
    const ByteString bsFirst = ValueToUTF8String(pIsolate, firstValue);
    const ByteString bsSecond = ValueToUTF8String(pIsolate, secondValue);
...
std::vector<v8::Global<v8::Value>> UnfoldArgs(
    const v8::FunctionCallbackInfo<v8::Value>& info) {
  std::vector<v8::Global<v8::Value>> results;
  ...
  for (int32_t i = 1; i < info.Length(); ++i) {
    ...
    if (fxv8::IsArray(arg)) {
      ...
      for (uint32_t j = 2; j < iLength; j++) {
        ...
        if (!fxv8::IsObject(jsValue)) {
          results.emplace_back(pIsolate, fxv8::NewUndefinedHelper(pIsolate));
          ...
          ByteString bsName =
              fxv8::ReentrantToByteStringHelper(pIsolate, propertyValue);
          results.emplace_back(pIsolate, fxv8::ReentrantGetObjectPropertyHelper(
                                             pIsolate, jsValue.As<v8::Object>(),
                                             bsName.AsStringView()));
        }
      }
    } else if (fxv8::IsObject(arg)) {
      results.emplace_back(
          pIsolate, GetObjectDefaultValue(pIsolate, arg.As<v8::Object>()));
    } else {
      results.emplace_back(pIsolate, arg);
    }
  }
  return results;
}
...
void CFXJSE_FormCalcContext::Oneof(
  ...
  v8::Local<v8::Value> argOne = GetSimpleValue(info, 0);
  for (const auto& iter : UnfoldArgs(info)) {
    auto value = v8::Local<v8::Value>::New(info.GetIsolate(), iter);
