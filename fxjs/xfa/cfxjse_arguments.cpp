...
bool CFXJSE_Arguments::GetBoolean(int32_t index) const {
  return CFX_V8::ToBooleanHelper(m_pInfo->GetIsolate(), (*m_pInfo)[index]);
}

int32_t CFXJSE_Arguments::GetInt32(int32_t index) const {
  return CFX_V8::ToInt32Helper(m_pInfo->GetIsolate(), (*m_pInfo)[index]);
}

float CFXJSE_Arguments::GetFloat(int32_t index) const {
  return static_cast<float>(
      CFX_V8::ToDoubleHelper(m_pInfo->GetIsolate(), (*m_pInfo)[index]));
}

ByteString CFXJSE_Arguments::GetUTF8String(int32_t index) const {
  return CFX_V8::ToByteStringHelper(m_pInfo->GetIsolate(), (*m_pInfo)[index]);
}
