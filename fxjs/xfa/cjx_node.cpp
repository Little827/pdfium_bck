...
const XFA_ExecEventParaInfo* GetEventParaInfoByName(
  ...
  auto cmpFunc = [](const XFA_ExecEventParaInfo& iter, const uint16_t& hash) {
    return iter.m_uHash < hash;
  };

  auto* result = std::lower_bound(std::begin(gs_eventParaInfos),
                                  std::end(gs_eventParaInfos), uHash, cmpFunc);
