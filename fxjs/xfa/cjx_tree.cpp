...
void CJX_Tree::nodes(CFXJSE_Value* pValue,
  ...
  CFXJSE_Engine* pScriptContext = GetDocument()->GetScriptContext();
  auto* pNodeList = static_cast<CXFA_AttachNodeList*>(
      pScriptContext->AddToCacheList(std::make_unique<CXFA_AttachNodeList>(
          nullptr, GetDocument(), GetXFANode())));
...
void CJX_Tree::ResolveNodeList(CFXJSE_Value* pValue,
  ...
  auto* pNodeList =
      static_cast<CXFA_ArrayNodeList*>(pScriptContext->AddToCacheList(
          std::make_unique<CXFA_ArrayNodeList>(nullptr, GetDocument())));
  if (resolveNodeRS.dwFlags == XFA_ResolveNode_RSType_Nodes) {
