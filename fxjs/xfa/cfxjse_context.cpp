  ...
  if (hNewThis.IsEmpty()) {
    ...
    if (v8::Script::Compile(hContext, hScriptString).ToLocal(&hScript)) {
      ...
      if (hScript->Run(hContext).ToLocal(&hValue)) {
        CHECK(!trycatch.HasCaught());
        return ExecutionResult(true, GetIsolate(), hValue);
      }
    }
    return ExecutionResult(false, GetIsolate(),
...
CFXJSE_Context::ExecutionResult::ExecutionResult(bool sts,
    ...
    : status(sts), value(std::make_unique<CFXJSE_Value>(isolate, val)) {}
