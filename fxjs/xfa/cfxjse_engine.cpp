...
std::unique_ptr<CFXJSE_Value> CFXJSE_Engine::RunScript(
  ...
  v8::Local<v8::Value> result =
      m_JsContext->ExecuteScript(btScript.c_str(), pThisBinding);
  if (result.IsEmpty())
    return nullptr;

  return std::make_unique<CFXJSE_Value>(GetIsolate(), result);
}
...
bool CFXJSE_Engine::RunVariablesScript(CXFA_Node* pScriptNode) {
  ...
  CFXJSE_ScopeUtil_IsolateHandle scope(GetIsolate());
  v8::Local<v8::Value> result = pVariablesContext->ExecuteScript(
      btScript.AsStringView(), v8::Local<v8::Object>());
  return fxv8::ReentrantToBooleanHelper(GetIsolate(), result);
}
