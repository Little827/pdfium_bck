...
void CFXJSE_Engine::GlobalPropertySetter(v8::Isolate* pIsolate,
                                         ...
                                         v8::Local<v8::Value> pValue) {
  CXFA_Object* pOrginalNode = ToObject(pIsolate, pObject);
  CXFA_Document* pDoc = pOrginalNode->GetDocument();
  CFXJSE_Engine* pScriptContext = pDoc->GetScriptContext();
  CXFA_Node* pRefNode = ToNode(pScriptContext->GetThisObject());
  if (pOrginalNode->IsThisProxy())
    pRefNode = ToNode(pScriptContext->GetVariablesThis(pOrginalNode));
  ...
  if (pOrginalNode->IsThisProxy() && fxv8::IsUndefined(pValue)) {
