...
Optional<v8::Local<v8::Value>> CFXJSE_Engine::RunScript(
  ...
  return m_JsContext->ExecuteScript(btScript.c_str(), pThisBinding);
}

v8::Local<v8::Value> CFXJSE_Engine::QueryNodeByFlag(CXFA_Node* refNode,
  ...
  Optional<CFXJSE_Engine::ResolveResult> maybeResult =
      ResolveObjects(refNode, propname, dwFlag, nullptr);
...
v8::Local<v8::Value> CFXJSE_Engine::GlobalPropertyGetter(
  ...
  v8::Local<v8::Value> result = lpScriptContext->QueryNodeByFlag(
      pRefNode, wsPropName.AsStringView(),
      XFA_RESOLVENODE_Children | XFA_RESOLVENODE_Properties |
          XFA_RESOLVENODE_Attributes);
  if (!result.IsEmpty())
    return result;

  result = lpScriptContext->QueryNodeByFlag(
      pRefNode, wsPropName.AsStringView(),
      XFA_RESOLVENODE_Parent | XFA_RESOLVENODE_Siblings);
  if (!result.IsEmpty())
    return result;
  ...
  CXFA_Node* pScriptNode =
      ToNode(lpScriptContext->GetVariablesScript(pOriginalObject));
  if (pScriptNode) {
    v8::Local<v8::Value> newvalue =
        lpScriptContext->QueryVariableValue(pScriptNode, szPropName);
    if (!newvalue.IsEmpty())
      return newvalue;
  }
...
v8::Local<v8::Value> CFXJSE_Engine::NormalPropertyGetter(
  ...
  v8::Local<v8::Value> pReturnValue = lpScriptContext->QueryNodeByFlag(
      ToNode(pObject), wsPropName.AsStringView(),
      XFA_RESOLVENODE_Children | XFA_RESOLVENODE_Properties |
          XFA_RESOLVENODE_Attributes);
  if (!pReturnValue.IsEmpty())
    return pReturnValue;
  ...
  if (pObject == lpScriptContext->GetThisObject() ||
    ...
    pReturnValue = lpScriptContext->QueryNodeByFlag(
        ToNode(pObject), wsPropName.AsStringView(),
        XFA_RESOLVENODE_Parent | XFA_RESOLVENODE_Siblings);
    if (!pReturnValue.IsEmpty())
      return pReturnValue;
...
void CFXJSE_Engine::NormalPropertySetter(v8::Isolate* pIsolate,
  ...
  CXFA_Node* pScriptNode =
      ToNode(lpScriptContext->GetVariablesScript(pOriginalObject));
  if (pScriptNode)
    lpScriptContext->UpdateVariableValue(pScriptNode, szPropName, pValue);
}
...
CFXJSE_Context* CFXJSE_Engine::GetVariablesContext(
    CXFA_Node* pScriptNode) const {
  ...
  if (it == m_mapVariableToContext.end())
    return nullptr;
...
bool CFXJSE_Engine::RunVariablesScript(CXFA_Node* pScriptNode) {
  ...
  return pVariablesContext
      ->ExecuteScript(btScript.c_str(), v8::Local<v8::Object>())
      .has_value();
}

v8::Local<v8::Value> CFXJSE_Engine::QueryVariableValue(
    CXFA_Node* pScriptNode,
    ByteStringView szPropName) const {
  CFXJSE_Context* pVariableContext = GetVariablesContext(pScriptNode);
  if (!pVariableContext)
    return v8::Local<v8::Value>();
...
void CFXJSE_Engine::UpdateVariableValue(CXFA_Node* pScriptNode,
                                        ByteStringView szPropName,
                                        v8::Local<v8::Value> pValue) {
  CFXJSE_Context* pVariableContext = GetVariablesContext(pScriptNode);
  if (!pVariableContext)
