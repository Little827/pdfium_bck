...
class CFXJSEngineEmbedderTest : public JSEmbedderTest {
 public:
  Optional<CFXJS_Error> ExecuteInCurrentContext(const WideString& script) {
    auto* current_engine =
...
TEST_F(CFXJSEngineEmbedderTest, Getters) {
  ...
  Optional<CFXJS_Error> err = ExecuteInCurrentContext(WideString(kScript1));
  EXPECT_FALSE(err);
...
TEST_F(CFXJSEngineEmbedderTest, MultipleEngines) {
  ...
  v8::Context::Scope context_scope(GetV8Context());
  Optional<CFXJS_Error> err = ExecuteInCurrentContext(WideString(kScript0));
  EXPECT_FALSE(err);
  ...
  }
  {
    ...
    Optional<CFXJS_Error> err = ExecuteInCurrentContext(WideString(kScript2));
    EXPECT_FALSE(err);
