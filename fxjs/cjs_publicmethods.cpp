...
CJS_Result CJS_PublicMethods::AFPercent_Format(
  ...
  size_t szNewSize;
  {
    ...
    size_t szBufferSize = iDec + 4;  // Negative sign, decimal, NUL and 1 extra.
    double dValueCopy = fabs(dValue);
    while (dValueCopy > 1) {
      dValueCopy /= 10;
      ++szBufferSize;
    }
    // Make sure there will always be a NUL terminator.
    pdfium::span<char> span = strValue.GetBuffer(szBufferSize);
    span[szBufferSize - 1] = '\0';

    // Figure out the format to use with FXSYS_snprintf(). |format| is small
    // because |iDec| is limited in size.
    ASSERT(iDec >= 0);
    ASSERT(iDec <= kDecLimit);
    char format[sizeof("%.512f")];
    FXSYS_snprintf(format, sizeof(format), "%%.%df", iDec);

    // Write into |strValue|.
    FXSYS_snprintf(span.data(), szBufferSize - 1, format, dValue);
