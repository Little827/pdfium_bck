...
class GCedTreeNodeEmbedderTest : public GCedEmbedderTest {
 public:
  ...
  void TearDown() override {
    s_root = nullptr;  // Can't (yet) outlive heap.
    heap_.reset();
...
TEST_F(GCedTreeNodeEmbedderTest, OneRefence) {
  s_root = cppgc::MakeGarbageCollected<ObservableGCedTreeNodeForTest>(heap());
...
TEST_F(GCedTreeNodeEmbedderTest, NoReferences) {
  ObservedPtr<ObservableGCedTreeNodeForTest> watcher(
      cppgc::MakeGarbageCollected<ObservableGCedTreeNodeForTest>(heap()));
  ForceGCAndPump();
