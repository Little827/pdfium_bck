...
class CFXGC_Platform : public cppgc::Platform {
 public:
...
std::unique_ptr<cppgc::Heap, FXGCHeapDeleter> FXGC_CreateHeap() {
  ++g_platform_ref_count;
  auto heap = cppgc::Heap::Create(std::make_shared<CFXGC_Platform>());
  return std::unique_ptr<cppgc::Heap, FXGCHeapDeleter>(heap.release());
}
