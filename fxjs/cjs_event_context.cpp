...
CJS_EventContext::CJS_EventContext(CJS_Runtime* pRuntime)
    : m_pRuntime(pRuntime),
      m_pEventHandler(pdfium::MakeUnique<CJS_EventRecorder>()) {
  ASSERT(pRuntime);
}
...
CJS_Field* CJS_EventContext::SourceField() {
  ...
  auto* pFormFillEnv = m_pEventHandler->GetFormFillEnvironment();
  if (!pFormFillEnv)
  ...
  pJSField->AttachField(pJSDocument, m_pEventHandler->SourceName());
  return pJSField;
}
...
CJS_Field* CJS_EventContext::TargetField() {
  ...
  auto* pJSField =
      static_cast<CJS_Field*>(CFXJS_Engine::GetObjectPrivate(pFieldObj));
  pJSField->AttachField(pJSDocument, m_pEventHandler->TargetName());
  return pJSField;
}
