...
class TestDelegate : public CFX_GlobalData::Delegate {
 public:
  bool StoreBuffer(const uint8_t* pBuffer, size_t nLength) override {
    last_buffer_ = std::vector<uint8_t>(pBuffer, pBuffer + nLength);
    return true;
  }
  bool LoadBuffer(uint8_t** pBuffer, size_t* nLength) override {
    *pBuffer = last_buffer_.data();
    *nLength = last_buffer_.size();
    return true;
  }
