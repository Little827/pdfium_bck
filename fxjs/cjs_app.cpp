...
CJS_Result CJS_App::response(CJS_Runtime* pRuntime,
  ...
  constexpr int kMaxInputBytes = 2048;
  constexpr int kMaxInputWideChars = kMaxInputBytes / 2;
  // Add 1 char for a NUL terminator.
  FixedZeroedDataVector<uint16_t> buffer(kMaxInputWideChars + 1);
  int byte_length = pRuntime->GetFormFillEnv()->JS_appResponse(
      ...
      pdfium::as_writable_bytes(
          buffer.writable_span().first(kMaxInputWideChars)));

  if (byte_length < 0 || byte_length > kMaxInputBytes)
