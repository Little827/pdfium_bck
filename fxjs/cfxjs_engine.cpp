...
static v8::Local<v8::ObjectTemplate> GetGlobalObjectTemplate(
  ...
  for (uint32_t i = 1; i <= pIsolateData->MaxObjDefinitionID(); ++i) {
...
uint32_t FXJS_PerIsolateData::MaxObjDefinitionID() const {
...
CFXJS_ObjDefinition* FXJS_PerIsolateData::ObjDefinitionForID(
    uint32_t id) const {
  return id > 0 && id <= MaxObjDefinitionID() ? m_ObjectDefnArray[id - 1].get()
                                              : nullptr;
}
...
void CFXJS_Engine::InitializeEngine() {
  ...
  FXJS_PerIsolateData* pIsolateData = FXJS_PerIsolateData::Get(GetIsolate());
  uint32_t maxID = pIsolateData->MaxObjDefinitionID();
